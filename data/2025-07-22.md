...
Motivation: 研究如何优化算法策略以更好地拟合3D图像数据，尤其是多晶体和泡沫等材料的模型。

Method: 比较线性/非线性规划、随机优化（交叉熵法）和梯度下降等方法，生成Voronoi、Laguerre和GBPDs模型。

Result: 通过真实数据集评估拟合质量，揭示了模型复杂度、优化方法复杂度和近似质量之间的权衡。
...
Method: 改进扩散后验采样方法，引入噪声污染似然近似，并在多元地质场景中进行条件建模（使用井数据和地震数据）。

Result: 显著提升统计稳健性、后验概率密度采样效果，并降低计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 人工智能（尤其是深度学习）在解决复杂任务（如自动驾驶）中表现出色，但需要更高效的场景理解方法。

Method: 使用BDD100k数据集，提出多种模型并比较不同主干网络作为编码器的效果。

Result: 结果表明，选择合适的主干网络显著提升了语义分割性能，优化了准确率、平均IoU和损失函数。

Conclusion: 研究证实了主干网络选择对语义分割的重要性，为场景理解提供了更优解决方案。
...
Method: 提出CLIPTTA，利用与CLIP预训练目标一致的软对比损失进行梯度更新，并通过理论分析证明其避免崩溃的能力。扩展到开放集设置时，使用异常对比暴露（OCE）损失改进OOD检测。

Result: 在75个数据集上的实验表明，CLIPTTA优于基于熵的方法，并在多种分布偏移下表现稳定，优于现有TTA方法。

Conclusion: CLIPTTA通过对齐预训练目标的对比损失，显著提升了视觉语言模型在分布偏移下的适应能力，尤其在开放集设置中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有GCD方法在处理未标记数据时容易受到背景区域干扰，导致特征提取不理想。

Method: AF由两部分组成：标记重要性度量（TIME）和多尺度标记自适应剪枝（TAP），通过剪除非信息性标记来优化注意力。
...
Method: 利用MLLM构建提示，评估幻觉视觉元素并生成‘幻觉分数’（HS）；同时发现某些深度特征距离与HS强相关，提出将其作为可微奖励函数优化GSR模型。

Result: HS与人类评估高度一致，并为超分辨率模型的图像指标提供了补充性见解；深度特征距离与HS的相关性显著。

Conclusion: 通过HS和深度特征距离优化GSR模型，可以有效缓解幻觉问题，提升生成图像的质量与保真度。
...
<details>
  <summary>Details</summary>
Motivation: B型超声中组织运动的准确跟踪因噪声和低对比度而困难，需要一种通用工具来量化组织动力学。

Method: 结合深度学习和光流技术，提供图形界面生成高质量训练数据，并采用新型光流滤波减少噪声。

Result: DUSTrack在准确性上优于零样本点跟踪器，与专业方法相当，适用于多种解剖结构和运动模式。

Conclusion: DUSTrack是一个开源、通用的工具，适用于临床和生物力学研究中的组织运动量化。
...
Method: 整合ConceptNet和语言模型的结构化常识先验与CLIP的视觉证据，采用能量推理循环迭代优化预测。

Result: 在多对象、无标签设置中，CRAFT提高了准确性和可解释性。

Conclusion: CRAFT为稳健和可信赖的场景理解提供了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射（3DGS）视频的数据量大且传输复杂，传统流媒体方法难以应对，因此需要新的解决方案。
...
Result: 在9个任务（如识别、定位）上，IRGPT优于更大规模的模型，达到最优性能。
...
<details>
  <summary>Details</summary>
Motivation: 点云配准中高质量对应关系的识别因局部与全局特征融合的冗余性和复杂空间关系而极具挑战性。Gestalt原则为分析局部与全局关系提供了优势。

Method: 提出GPI-Net，利用Gestalt原则促进局部与全局信息的互补交互，包括正交整合策略、Gestalt特征注意力块（GFA）和双路径多粒度并行交互聚合块（DMG）。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射视频流在学术界和工业界成为热点，但仍面临分块、质量评估和比特率适应等挑战。

Method: 提出自适应分块技术、质量评估框架和基于元学习的自适应比特率算法。

Result: 实验表明，所提方法显著优于现有技术。

Conclusion: 本文解决了3D高斯溅射视频流的关键挑战，为未来研究奠定了基础。
...
Result: 在Bench2Drive基准测试中表现优异，驾驶分数和成功率领先，单目视觉输入下仍达最佳性能。消融实验显示显著提升。

Conclusion: GEMINUS通过混合专家框架实现了自适应和鲁棒的自动驾驶性能，优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统可视化图像传播中，元数据（如源代码、交互功能）易丢失，现有嵌入方法对图像篡改（如裁剪、编辑）脆弱。
...
<details>
  <summary>Details</summary>
Motivation: 动态和感知混淆环境中的视觉地点识别（VPR）是长期定位的核心挑战，现有方法多忽视时间连贯性。

Method: 提出OptiCorNet框架，结合轻量级1D卷积编码器和可学习差分时间算子（DSD），通过LSTM精炼和四元组损失优化序列级嵌入。
...
Method: 设计结构共享特征编码器（SSFE）提取跨模态特征，并引入双通道控制门增强相关特征；提出渐进检索增强生成器（PRAG）分层融合参考先验信息。

Result: 在多数据集和真实场景中验证了方法在生成细粒度点云和处理稀疏数据及未见类别时的有效性。

Conclusion: 该方法通过检索增强和分层融合机制，显著提升了点云补全的质量和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 病理全切片图像的高分辨率和长上下文长度对多模态大语言模型提出了挑战，现有方法在资源消耗和生成能力上存在不足。

Method: TCP-LLaVA引入可训练的压缩令牌，通过模态压缩模块聚合视觉和文本信息，仅将压缩后的令牌输入语言模型。

Result: 在十种TCGA肿瘤亚型的实验中，TCP-LLaVA在视觉问答准确性上优于现有方法，同时大幅减少资源消耗。

Conclusion: TCP-LLaVA通过令牌压缩有效解决了病理全切片图像处理中的计算和生成问题，为未来研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法计算复杂且难以实时应用，前馈方法通过深度学习实现了快速且通用的3D重建与视图合成。

Method: 分类讨论了点云、3D高斯泼溅（3DGS）、神经辐射场（NeRF）等表示架构，并分析了关键任务如无姿态重建、动态3D重建等。

Result: 总结了常用数据集和评估协议，强调了前馈方法在数字人、SLAM等领域的应用潜力。

Conclusion: 前馈方法有望推动3D视觉领域的发展，但仍需解决开放性研究挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在多视角行人检测中引入噪声且精度低，依赖昂贵的3D标注且泛化能力差。

Method: 采用深度一致性建模和超像素高斯分布方法，实现多视角深度一致性和精确点云生成。

Result: 显著减少噪声，优于现有方法，首次在挑战性场景中实现行人重建和多视角分割。
...
Method: 开发ArtiMuse模型，结合评分与专家级理解能力，并构建ArtiMuse-10K数据集，包含10,000张专业标注图像。

Result: ArtiMuse模型具备更强的感知和泛化能力，ArtiMuse-10K数据集支持多维度美学分析。
...
Method: 利用基础模型将视频帧的退化感知语义上下文嵌入自然语言，学习近似知识以在推理时无需基础模型。

Result: 在多个基准测试中取得了最先进的性能，包括新提出的时间变化复合退化基准。

Conclusion: 提出的框架在无需预知退化信息的情况下实现了高性能，并呼吁标准化全能视频修复基准。
...
<details>
  <summary>Details</summary>
Motivation: 听障人士与普通人沟通困难，疫情期间视频会议需求增加，听障人士更倾向于用手语而非打字。

Method: 利用包含2000多个单词级ASL视频的大规模数据集，开发浏览器扩展实现手语到字幕的自动翻译。

Result: 通过计算机视觉技术消除沟通障碍，提升听障人士在视频会议中的交流体验。

Conclusion: 该浏览器扩展有望成为听障人士与普通人沟通的有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统卫星图像变化检测因数据传输和处理延迟而无法实时应用，需将整个流程移至卫星上。

Method: 使用深度神经网络，包含三个子模块：图像压缩、轻量级配准和高效变化检测模型。

Result: 在低功耗硬件上实现0.7 Mpixel/s的吞吐量，F1分数表现优异。

Conclusion: 该框架首次将卫星上变化检测任务整合为端到端解决方案，性能显著。
...
Result: 在KASVIR数据集上微调后，模型在挑战赛指标上表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D高斯泼溅（3DGS）在重建高质量新视图时容易过拟合训练视图的高频细节，传统基于傅里叶变换的频率正则化方法参数调优困难且偏向有害高频学习。
...
<details>
  <summary>Details</summary>
Motivation: 解决情感认知科学中外部情感刺激与人类内部体验关系的建模问题，尤其是ANN在个体差异建模上的不足。

Method: 采用新型感知边界采样方法生成ANN决策边界上的模糊面部表情刺激，构建varEmotion数据集，并通过大规模人类行为实验验证。

Result: ANN模糊刺激引发人类感知不确定性，通过微调ANN实现了与人类群体及个体感知模式的对齐。
...
<details>
  <summary>Details</summary>
Motivation: 业余摄影师常因疏忽或经验不足在照片中留下杂乱元素，影响照片情感表达。

Method: 系统通过算法评估对象对照片美学的贡献，提供交互式杂乱识别和去除工具，结合生成对抗网络进行图像修复。

Result: 用户研究表明，系统能帮助用户更高效地识别杂乱并提升照片质量。

Conclusion: 该系统通过灵活界面和准确算法，有效改善了用户的摄影体验和作品质量。
...
Result: 在24个预训练模型和10个下游数据集上的实验展示了其优异性能，尤其在低数据场景下表现突出。

Conclusion: LEAD通过单步优化有效解决了微调过程中的非线性问题，具有广泛适应性。
...
Method: 使用GANs、扩散模型和流匹配技术进行T1w到T2w的2D MRI图像转换，并在三个公开数据集上进行比较评估。

Result: GAN-based Pix2Pix模型表现最佳，流匹配技术在小数据集上容易过拟合。

Conclusion: 研究为实际MRI工作流程中的图像转换提供了实用指导，并指出了未来跨模态医学图像合成的方向。
...
<details>
  <summary>Details</summary>
Motivation: 研究不同深度学习框架在医学图像分类中的性能差异，以提升血液细胞分析的效率和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法仅适用于任务特定目标或场景内容，无法同时适应场景和用户查询。DiSCO-3D旨在解决这一问题。
...
Conclusion: Exp-Graph在实验室和现实环境中均表现出色，具有实际应用潜力。
...
### [37] [BusterX++: Towards Unified Cross-Modal AI-Generated Content Detection and Explanation with MLLM](https://arxiv.org/abs/2507.14632)
...
Method: 提出BusterX++框架，结合强化学习后训练策略（多阶段训练、思维奖励、混合推理）解决冷启动问题。

Result: 实验证明BusterX++在跨模态检测中表现优异，GenBuster++数据集支持全面评估。
...
<details>
  <summary>Details</summary>
Motivation: 现代多光谱特征融合方法存在过度偏好局部互补特征和计算复杂度与感受野大小的权衡问题，影响了泛化性能和可扩展性。

Method: MS2Fusion采用双路径参数交互机制：一条路径通过跨模态隐藏状态解码挖掘互补信息，另一条路径通过参数共享探索跨模态对齐的语义特征。

Result: 在FLIR、M3FD和LLVIP等主流基准测试中，MS2Fusion显著优于其他最先进的多光谱目标检测方法，并在RGB-T语义分割和RGBT显著目标检测任务中表现出泛化能力。

Conclusion: MS2Fusion通过统一框架实现了功能互补和共享语义空间，适用于多种多光谱感知任务，具有广泛的应用潜力。
...
Method: 结合计算机视觉、深度学习和边缘推断，实现动作的自动识别与分类。
...
Result: 模型表现良好，部分食物类型的像素比例一致性接近或超过90%，轻量模型实现实时推理。干燥和刚性食物分割效果更佳。

Conclusion: 该框架为大规模食品服务环境中的实时浪费监测提供了可扩展的无接触解决方案，并为减少食物浪费提供了可行方向。
...
TL;DR: Gene-DML通过多级双路径判别框架，提升病理图像与基因表达数据的跨模态对齐，实现更准确的基因表达预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分利用病理图像与基因表达数据的跨模态多层次对齐，限制了预测性能。

Method: 提出Gene-DML框架，通过双路径多级判别（实例级和组级）增强形态与转录模态的对齐。

Result: 在公开空间转录组数据集上，Gene-DML实现了最先进的基因表达预测性能。

Conclusion: Gene-DML通过多级对齐策略，显著提升了跨模态表征的鲁棒性和预测准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在复杂多页文档理解上表现不足，且检索增强生成方法存在上下文碎片化、多阶段错误累积等问题。

Method: 构建高质量文档级数据集Doc-750K，并开发原生多模态模型Docopilot，直接处理文档级依赖。
...
Conclusion: WSI-Agents在病理分析中实现了多功能性和准确性的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有的MLLM在零样本能力上表现强，但在复杂GSR任务中资源消耗大，而传统GSR模型泛化能力不足。论文旨在通过知识蒸馏提升小模型的性能。

Method: 提出MIPD框架，包括基于LLM的理性生成器（JRG）和负引导多模态提示对齐（NMPA）模块，通过场景感知和实例感知提示对齐视觉信息，蒸馏知识到小模型。

Result: 在Ov-SWiG和HICO-DET数据集上，MIPD在已知、罕见和未知场景中均表现优异，提升了未知场景检测能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有农业地块提取研究主要关注中分辨率或平坦农田，缺乏对复杂梯田地形的精细表达，无法满足精准农业的需求。

Method: 构建了GTPBD数据集，包含47,537张高分辨率图像和三级标签（像素级边界、掩码和地块标签），覆盖中国七大地理区域和全球跨气候区域。

Result: GTPBD在语义分割、边缘检测、地块提取和无监督域适应任务中进行了基准测试，展示了其挑战性和适用性。

Conclusion: GTPBD填补了梯田遥感研究的空白，为细粒度农业地形分析和跨场景知识迁移提供了基础设施。
...
<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是导致可预防性失明的主要原因，低收入社区患者因筛查机会有限，诊断时往往已进入晚期。共病情况加速疾病进展。

Method: 提出MultiRetNet，采用三种多模态融合方法，选择全连接层融合。通过对抗性低质量图像和对比学习训练延迟系统，识别需临床复查的样本。
...
Conclusion: 领域专用数据集对视频问答模型的性能至关重要，InterAct VideoQA为智能交通系统研究提供了基准。
...
Result: 在NExT-QA等数据集上实现了SOTA性能，显著提升了复杂推理任务的准确性。
...
TL;DR: 本文提出了一种基于信息互补的下采样方法HPD，用于解决传统下采样方法在语义分割任务中可能丢失关键空间信息的问题。实验表明HPD在性能上优于传统方法。
...
Method: 提出Hybrid Pooling Downsampling (HPD)方法，核心是用MinMaxPooling替代传统方法，通过提取局部区域的最大值信息，有效保留图像的明暗对比和细节特征。
...
Method: EPD通过并行梯度评估优化截断误差，参数可学习且完全并行化，可作为插件提升现有ODE采样器。

Result: 在多个图像合成基准测试中，EPD在5 NFE下FID显著优于现有方法，如CIFAR-10上4.47。
...
### [52] [An Evaluation of DUSt3R/MASt3R/VGGT 3D Reconstruction on Photogrammetric Aerial Blocks](https://arxiv.org/abs/2507.14798)
*Xinyi Wu,Steven Landgraf,Markus Ulrich,Rongjun Qin*

Main category: cs.CV

TL;DR: 论文评估了DUSt3R、MASt3R和VGGT等3D重建基础模型在航拍图像上的表现，发现它们能从稀疏图像（少于10张）中准确重建密集点云，但高分辨率和大图像集下表现受限。


<details>
  <summary>Details</summary>
Motivation: 探索这些模型在航拍图像上的潜力，填补其在摄影测量领域的空白。

Method: 在UseGeo数据集上对预训练模型进行姿态估计和密集3D重建的全面评估。

Result: 模型在稀疏图像集（少于10张）中表现优异，完整性提升50%，但高分辨率和大图像集下姿态估计可靠性下降。

Conclusion: Transformer方法无法完全替代传统SfM和MVS，但在低分辨率、稀疏场景下可作为补充。
...
TL;DR: 提出了一种基于视觉提示的统一低层视觉任务处理框架VPIP，通过输入-目标图像对作为提示，支持多种任务。模型GenLV在多任务基准测试中表现优异，并展示了良好的可扩展性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决低层视觉任务多样性带来的统一建模挑战，提出一种能够灵活适应不同任务的方法。

Method: 设计了包含图像处理主干、提示编码器和提示交互模块的VPIP框架，支持多任务联合训练。

Result: 在100多个任务的大规模基准测试中表现优异，增加训练任务数提升了泛化能力，尤其在数据有限的任务中。

Conclusion: VPIP框架作为一种统一基础，展示了在低层视觉任务中的有效性、可扩展性和潜力。
...
TL;DR: 提出了一种基于人类认知的多脸深度伪造检测框架HICOM，通过分析人类依赖的四种关键线索，显著提升了检测准确性和泛化能力。
...
Conclusion: 结合人类因素能有效增强对深度伪造的防御能力。
...
### [55] [Light Future: Multimodal Action Frame Prediction via InstructPix2Pix](https://arxiv.org/abs/2507.14809)
...
<details>
  <summary>Details</summary>
Motivation: 预测未来运动轨迹对机器人、自主系统等领域至关重要，但传统视频预测模型计算成本高且延迟大。

Method: 基于InstructPix2Pix模型，结合视觉和文本输入，实现多模态未来帧预测，仅需单张图像和文本指令。

Result: 在RoboTWin数据集上表现优于现有方法，SSIM和PSNR指标更优，计算效率和延迟显著改善。

Conclusion: 该方法为机器人动作预测提供了高效、轻量的解决方案，尤其适用于需要快速推理和多模态控制的应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型计算成本高，现有后训练量化方法通用性不足，难以与工业部署流程兼容。
...
TL;DR: FinChart-Bench是首个专注于金融图表的基准测试，包含1,200张图表和7,016个问题。评估25种先进LVLM模型，揭示了性能差距缩小、指令遵循困难等关键问题。


<details>
  <summary>Details</summary>
Motivation: 金融图表因复杂的时间结构和专业术语未被充分研究，需专门基准测试。

Method: 构建FinChart-Bench数据集，包含1,200张金融图表和7,016个问题，评估25种LVLM模型。

Result: 发现开源与闭源模型差距缩小、升级模型性能下降、指令遵循困难、空间推理能力有限、LVLM不可靠等问题。

Conclusion: 当前LVLM在金融图表理解上存在显著局限，需进一步改进。
...
TL;DR: PHATNet通过将目标域的雾霾模式迁移到源域的无雾图像上，生成特定领域的微调数据集，提升去雾模型的域适应能力。


<details>
  <summary>Details</summary>
Motivation: 现有去雾模型在未见过的真实雾霾图像上性能下降显著，原因是训练数据有限。

Method: 提出PHATNet，利用物理引导的雾霾迁移网络，结合Haze-Transfer-Consistency和Content-Leakage损失增强解耦能力。
...
TL;DR: 提出一种无需外部条件的配对图像生成方法，用于解决乳腺断层扫描图像中病灶分割的数据标注不足问题，提升生成质量和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 乳腺断层扫描图像中病灶分割对乳腺癌早期筛查至关重要，但高密度组织导致病灶隐蔽，人工标注困难且耗时，数据标注不足。现有扩散模型在生成病灶区域时质量低且无法生成对应标注。

Method: 提出一种配对图像生成方法，通过训练额外的扩散引导器实现条件扩散模型的配对图像生成，无需外部条件。实验生成了配对的乳腺断层扫描切片和病灶掩码。

Result: 实验表明，该方法无需外部条件即可提升生成质量，缓解标注数据短缺问题，并提升下游病灶分割任务的性能。

Conclusion: 该方法有效解决了病灶区域生成质量低和标注数据不足的问题，为乳腺病灶分割任务提供了高质量的训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 解决密集标注成本高和多帧依赖限制自监督方法在静态或单帧场景中的应用问题。

Method: 利用深度分布特性设计新损失函数，结合视觉基础模型生成的语义分割图增强深度估计。

Result: 实验证明该方法有效。

Conclusion: 该方法为深度补全提供了一种高效且适用范围广的解决方案。
...
### [61] [An Uncertainty-aware DETR Enhancement Framework for Object Detection](https://arxiv.org/abs/2507.14855)
*Xingshu Chen,Sicheng Yu,Chong Cheng,Hao Wang,Ting Tian*

Main category: cs.CV

TL;DR: 本文提出了一种基于DETR的不确定性感知增强框架，通过建模边界框为多元高斯分布并引入Gromov-Wasserstein距离，提升定位精度和预测可靠性。
...
Result: 在COCO基准测试中有效提升DETR变体性能，并在白细胞检测任务中达到SOTA结果。
...
### [62] [Hybrid-supervised Hypergraph-enhanced Transformer for Micro-gesture Based Emotion Recognition](https://arxiv.org/abs/2507.14867)
*Zhaoqiang Xia,Hexiang Huang,Haoyu Chen,Xiaoyi Feng,Guoying Zhao*

Main category: cs.CV

TL;DR: 提出了一种基于微手势的情感识别方法，采用超图增强的Transformer和混合监督框架，通过自监督和监督学习结合实现端到端训练。


<details>
  <summary>Details</summary>
Motivation: 微手势能反映人类情感状态，但基于微手势的情感建模研究不足，需要更有效的方法。

Method: 使用超图增强的Transformer编码器和解码器，结合自监督重建和监督学习，通过超图自注意力模块和多尺度时间卷积捕捉微手势的细微运动。

Result: 在两个公开数据集（iMiGUE和SMG）上表现优于现有方法，多项指标达到最佳。

Conclusion: 提出的方法在微手势情感识别中表现出色，验证了超图增强Transformer和混合监督框架的有效性。
...
### [63] [Region-aware Depth Scale Adaptation with Sparse Measurements](https://arxiv.org/abs/2507.14879)
...
<details>
  <summary>Details</summary>
Motivation: 基础模型的深度预测通常为相对尺度，限制了实际应用；现有尺度适应方法成本高且可能损害泛化能力。

Method: 非学习型方法，利用稀疏深度测量将相对尺度转换为度量尺度，无需额外训练或微调。

Result: 实验证明该方法有效，能在不增加计算成本或牺牲泛化能力的情况下实现度量尺度深度预测。

Conclusion: 该方法成功解决了相对尺度到度量尺度的转换问题，为实际应用提供了高效解决方案。
...
### [64] [BeatFormer: Efficient motion-robust remote heart rate estimation through unsupervised spectral zoomed attention filters](https://arxiv.org/abs/2507.14885)
*Joaquim Comas,Federico Sukno*

Main category: cs.CV

TL;DR: BeatFormer是一种轻量级光谱注意力模型，结合了手工方法和深度学习的优势，用于远程光电容积描记（rPPG）估计，并通过光谱对比学习（SCL）实现无监督训练。


<details>
  <summary>Details</summary>
Motivation: 当前rPPG估计中，深度学习方法依赖大数据集，而手工方法在未见场景中泛化能力有限。需要一种结合两者优势的混合方法。

Method: 提出BeatFormer模型，整合了放大的正交复注意力机制和频域能量测量，并引入SCL实现无监督训练。

Result: 在PURE、UBFC-rPPG和MMPD数据集上验证了模型的鲁棒性和性能，尤其在运动场景的跨数据集评估中表现优异。

Conclusion: BeatFormer结合了手工方法和深度学习的优势，提供了一种高效且泛化能力强的rPPG估计解决方案。
...
### [65] [TriCLIP-3D: A Unified Parameter-Efficient Framework for Tri-Modal 3D Visual Grounding based on CLIP](https://arxiv.org/abs/2507.14904)
...
Method: 利用2D CLIP双模态模型和适配器微调，结合GARF模块融合几何特征，实现三模态统一处理。

Result: 参数量减少58%，3D检测任务提升6.52%，3D视觉定位任务提升6.25%。

Conclusion: 该方法通过统一特征提取和融合，显著提升了3D视觉定位的效率和性能。
...
### [66] [Semantic-Aware Representation Learning for Multi-label Image Classification](https://arxiv.org/abs/2507.14918)
...
### [67] [Stereo-GS: Multi-View Stereo Vision Model for Generalizable 3D Gaussian Splatting Reconstruction](https://arxiv.org/abs/2507.14921)
*Xiufeng Huang,Ka Chun Cheung,Runmin Cong,Simon See,Renjie Wan*

Main category: cs.CV

TL;DR: 提出了一种解耦框架\method，用于高效预测3D高斯，通过局部图像对提取特征并融合，生成几何和外观的GS-maps，实现无姿态的高质量3D重建。
...
Method: 使用立体视觉主干从局部图像对提取特征，通过全局注意力块融合，生成几何的多视点点图和外观的高斯特征，结合为GS-maps，并通过细化网络提升重建质量。
...
### [68] [3-Dimensional CryoEM Pose Estimation and Shift Correction Pipeline](https://arxiv.org/abs/2507.14924)
...
<details>
  <summary>Details</summary>
Motivation: 冷冻电镜中姿态估计和位移校正是关键挑战，低信噪比直接影响3D重建的准确性。

Method: 利用MDS技术从二面角对估计旋转矩阵，结合鲁棒优化框架（ℓ₁范数）和迭代位移校正算法。

Result: 方法在欧拉角精度和重建保真度（FSC）上优于现有方法。

Conclusion: 提出的鲁棒优化和全局校正框架显著提升了低信噪比条件下的姿态估计和重建质量。
...
### [69] [Probabilistic smooth attention for deep multiple instance learning in medical imaging](https://arxiv.org/abs/2507.14932)
...
Result: 在三个医学数据集和十一个基线方法的评估中，该方法在预测性能上表现最佳，并提供了可解释的不确定性映射。

Conclusion: 该概率框架不仅提升了预测性能，还通过不确定性映射增强了模型的可解释性，尤其在疾病定位方面。
...
### [70] [Open-set Cross Modal Generalization via Multimodal Unified Representation](https://arxiv.org/abs/2507.14935)
...
Method: 提出MICU方法，包括FCMI（细粒度对比学习）和CUJP（跨模态拼图任务），增强开放集泛化能力。
...
### [71] [Polymorph: Energy-Efficient Multi-Label Classification for Video Streams on Embedded Devices](https://arxiv.org/abs/2507.14959)
...
<details>
  <summary>Details</summary>
Motivation: 嵌入式设备在计算和能源预算有限的情况下，需要高效的多标签视频分类方法。视频流的标签稀疏性、时间连续性和标签共现性等结构特性可被利用以优化推理效率。

Method: Polymorph框架动态激活每帧所需的轻量级LoRA适配器，这些适配器基于标签共现模式专用于特定子类，避免了全模型切换和权重合并。

Result: 在TAO数据集上，Polymorph比基线方法能耗降低40%，mAP提升9个百分点。

Conclusion: Polymorph通过模块化策略显著提升了嵌入式设备上多标签视频分类的效率和性能，且已开源。
...
### [72] [Decision PCR: Decision version of the Point Cloud Registration task](https://arxiv.org/abs/2507.14965)
*Yaojie Zhang,Tianlun Huang,Weijun Wang,Wei Feng*

Main category: cs.CV

TL;DR: 本文提出了一种基于深度学习的方法，用于解决低重叠点云配准（PCR）任务中的评估问题，显著提升了现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标在极低内点比例下失效，因此需要一种更可靠的评估方法来解决PCR任务中的决策问题。

Method: 构建基于3DMatch的数据集，训练深度学习分类器评估配准质量，并将其整合到标准PCR流程中。

Result: 与现有方法结合后，性能显著提升，例如在3DLoMatch基准上达到86.97%的配准召回率，并在ETH数据集上表现出强泛化能力。

Conclusion: 本文提出的数据驱动方法为低重叠PCR任务提供了有效的评估解决方案，并显著提升了配准性能。
...
### [73] [Hierarchical Cross-modal Prompt Learning for Vision-Language Models](https://arxiv.org/abs/2507.14976)
...
<details>
  <summary>Details</summary>
Motivation: 预训练的视觉语言模型（如CLIP）在下游任务中的泛化能力有限，现有提示学习方法存在模态隔离和层次语义衰减问题。

Method: 提出HiCroPL框架，通过分层知识映射器和轻量级知识代理实现文本与视觉模态的双向知识流，增强语义表示。
...
### [74] [Language Integration in Fine-Tuning Multimodal Large Language Models for Image-Based Regression](https://arxiv.org/abs/2507.14997)
...
Method: 提出RvTC方法，采用基于分箱的分类策略，并通过数据特定的语义提示优化模型表现。

Result: 在四个图像评估数据集上达到最优性能，AVA数据集中提示加入挑战标题后相关性从0.83提升至0.90。

Conclusion: 语义提示对多模态回归任务至关重要，RvTC方法为图像评估提供了更优解决方案。
...
### [75] [Axis-Aligned Document Dewarping](https://arxiv.org/abs/2507.15000)
...
<details>
  <summary>Details</summary>
Motivation: 现有学习方法依赖标注数据而未利用文档的几何特性，作者发现去扭曲文档的特征线应轴对齐。
...
### [76] [FastSmoothSAM: A Fast Smooth Method For Segment Anything Model](https://arxiv.org/abs/2507.15008)
*Jiasheng Xu,Yewang Chen*

Main category: cs.CV

TL;DR: 本文提出了一种基于B-Spline曲线拟合的新方法，用于优化FastSAM的边缘质量，解决了其生成的锯齿状边缘问题，同时保持实时处理能力。


<details>
  <summary>Details</summary>
Motivation: FastSAM在实时图像分割中表现优异，但其生成的边缘存在锯齿状问题，影响视觉质量和分析准确性。

Method: 采用B-Spline曲线拟合技术，通过四阶段细化过程（包括两轮曲线拟合）平滑锯齿边缘。

Result: 显著提升了边缘的视觉质量和分析准确性，同时保持了实时处理能力。

Conclusion: 该方法增强了FastSAM的实用性，适用于工业自动化、医疗成像等需要精确边缘识别的场景。
...
### [77] [Towards Video Thinking Test: A Holistic Benchmark for Advanced Video Reasoning and Understanding](https://arxiv.org/abs/2507.15028)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型的评测标准未能充分反映其在视频理解中的正确性和鲁棒性与人类智能的差距。

Method: 通过构建Video-TT测试，包含1,000个YouTube Shorts视频，每个视频配有一个开放式问题和四个对抗性问题。

Result: 评估结果显示，视频大语言模型在理解复杂视觉叙事和对抗性问题上的表现显著低于人类。

Conclusion: Video-TT揭示了视频大语言模型在视频理解任务中的不足，为未来改进提供了方向。
...
### [78] [OpenBreastUS: Benchmarking Neural Operators for Wave Imaging Using Breast Ultrasound Computed Tomography](https://arxiv.org/abs/2507.15035)
...
Method: 提出OpenBreastUS数据集，包含8,000个解剖学真实的人体乳腺模型和1,600万频域波动模拟，用于神经算子的全面测试。

Result: OpenBreastUS为神经PDE求解器的开发和实际医学成像应用提供了平台，首次实现了基于神经算子的人体乳腺高效体内成像。

Conclusion: OpenBreastUS通过提供真实且广泛的数据集，推动了神经算子在医学成像中的实际应用，展示了其在体内成像中的潜力。
...
### [79] [EBA-AI: Ethics-Guided Bias-Aware AI for Efficient Underwater Image Enhancement and Coral Reef Monitoring](https://arxiv.org/abs/2507.15036)
*Lyes Saad Saoud,Irfan Hussain*

Main category: cs.CV

TL;DR: EBA-AI框架通过CLIP嵌入和自适应处理解决了水下图像增强中的数据集偏差、高计算成本和透明度问题，实现了高效、公平和可解释的海洋监测。
...
Method: EBA-AI利用CLIP嵌入检测和缓解数据集偏差，并采用自适应处理优化能耗，同时结合不确定性估计和可解释性技术。

Result: 实验表明，EBA-AI在PSNR仅下降1.0 dB的情况下显著降低计算成本，支持实时大规模监测，并在公平性和可解释性上优于其他模型。

Conclusion: EBA-AI为可持续、无偏见且高效的水下图像处理提供了解决方案，推动了海洋保护的AI应用。
...
TL;DR: OmniVTON是一个无需训练的通用虚拟试穿框架，通过解耦服装和姿态条件，实现跨场景的高保真纹理和姿态一致性。
...
Method: 采用服装先验生成机制和连续边界缝合技术保留细节，结合DDIM反转实现精确姿态对齐。
...
<details>
  <summary>Details</summary>
Motivation: 挑战当前全色锐化领域大模型计算开销高、泛化性差的问题，提出更高效、通用的解决方案。

Method: 设计轻量级单步框架PanTiny，采用多数据集联合训练（WV2、WV3、GF2）和复合损失函数。

Result: PanTiny在性能和效率上优于大型专用模型，泛化性显著提升。

Conclusion: 通过模型设计、训练范式和损失函数的优化，证明了轻量化模型可以超越暴力扩展，推动全色锐化领域向高效、通用化发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前的人体图像动画扩散模型在参考图像和驱动视频差异较大时难以保持身份一致性，因此需要一种能够有效对齐姿态并保持身份的方法。

Method: StableAnimator++ 通过可学习层预测相似变换矩阵对齐姿态，使用预训练编码器提取图像和面部嵌入，并通过分布感知的身份适配器保持身份一致性。推理阶段引入基于 HJB 的面部优化。

Result: 实验证明 StableAnimator++ 在定性和定量上均优于现有方法，能够生成高质量且身份一致的视频。

Conclusion: StableAnimator++ 通过创新的姿态对齐和身份保持技术，显著提升了人体图像动画的身份一致性，为相关领域提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究生成模型是否能掌握文本图像生成和编辑的复杂性，推动其成为通用能力而非专用解决方案。

Method: 选择33个代表性任务，分为五类，评估六种闭源和开源模型，使用高质量输入和提示。

Result: 识别当前生成模型在OCR任务中的弱点，提出关键观察。

Conclusion: 逼真文本图像生成应内化为通用生成模型的基础技能，为社区提供实现目标的经验分析。
...
### [84] [Visual Place Recognition for Large-Scale UAV Applications](https://arxiv.org/abs/2507.15089)
...
<details>
  <summary>Details</summary>
Motivation: 无人机视觉定位（vPR）面临大规模高空数据集稀缺和图像旋转模糊的挑战，影响模型泛化能力。

Method: 引入LASED大规模数据集，并结合可转向CNN处理旋转方差。

Result: LASED训练的模型召回率显著提升，可转向CNN比传统网络平均提高12%召回率。

Conclusion: 结合大规模数据集和旋转等变网络，显著增强无人机vPR的鲁棒性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: ESD手术中出血源的实时定位和持续监测存在挑战，现有AI方法仅关注出血区域分割，缺乏对出血源准确检测和动态跟踪的研究，且缺乏专用数据集。

Method: 提出了BleedOrigin-Bench数据集和BleedOrigin-Net框架，结合检测和跟踪技术，从出血开始检测到持续空间跟踪。

Result: BleedOrigin-Net在出血开始检测、初始源检测和点跟踪方面表现优异，分别达到96.85%、70.24%和96.11%的准确率。

Conclusion: 该研究填补了ESD出血源检测和跟踪的空白，为AI辅助手术系统提供了有效工具。
...
TL;DR: LoopNet方法通过多任务ResNet架构和在线少样本学习，解决了SLAM闭环检测的精度和实时计算问题，并引入新数据集LoopDB。


<details>
  <summary>Details</summary>
Motivation: 解决SLAM系统中闭环检测精度低和嵌入式硬件实时计算限制的问题。

Method: 基于多任务ResNet架构，结合在线少样本学习和DISK描述符，优化嵌入式设备性能。

Result: LoopNet在多变条件下表现优于传统方法和手工特征，提供更高的闭环检测精度。

Conclusion: LoopNet为SLAM闭环问题提供了高效解决方案，并开源了代码和数据集LoopDB。
...
TL;DR: VPA（视觉规划辅助）通过视频预测用户行动序列，提出Auxiliary Task Augmentation和Multi-token Prediction解决数据稀缺和结构化动作空间建模问题，VideoPlan方法在COIN和CrossTask数据集上表现优异。
...
Method: 引入Auxiliary Task Augmentation增强规划能力，采用Multi-token Prediction显式建模动作空间。

Result: VideoPlan在COIN和CrossTask数据集上分别提升7.3%和3.4%，在Ego4D任务中表现接近SOTA。

Conclusion: VideoPlan通过辅助任务和多令牌预测有效提升视觉规划性能。
...
TL;DR: 提出了一种新颖的时空多图表示方法，通过解耦空间图和时间图，优化事件传感器的稀疏数据建模，显著提升了检测精度和效率。


<details>
  <summary>Details</summary>
Motivation: 事件传感器数据稀疏且异步，传统密集张量转换方法会丧失其优势，而现有图表示方法在时空动态建模上表现不佳。

Method: 构建解耦的空间图（使用B样条基函数建模全局结构）和时间图（基于运动向量的注意力机制建模局部动态变化），替代计算昂贵的3D核。

Result: 在Gen1和eTraM数据集上，检测精度提升6%，速度提升5倍，参数减少且计算成本不变。

Conclusion: 结构化图建模能有效提升异步视觉任务的性能，为事件传感器数据提供了高效解决方案。
...
Method: 通过顶点序列化技术将网格顶点排序，利用Mamba-SSMs处理；设计了MambaDiff3D（生成模型）和Mamba-HMR（重建模型）。

Result: MambaDiff3D在生成带衣物和手部的3D人体网格任务中表现优于现有方法；Mamba-HMR扩展了非参数化重建能力，支持全身模型并实现实时性能。

Conclusion: MeshMamba及其衍生模型在3D网格生成和重建任务中表现出高效性和扩展性，为复杂几何细节的处理提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 自监督学习在判别任务中表现优异，但生成模型在图像生成和细节增强上更优。结合两者可增强自监督学习的表征能力。

Method: 利用扩散噪声作为掩码状态，结合掩码图像建模（MIM），提出N-JEPA方法，通过多级噪声调度增强模型鲁棒性。

Result: 在分类任务中验证了方法的有效性。

Conclusion: N-JEPA通过结合扩散噪声与自监督学习，显著提升了模型在下游任务中的表现。
...
TL;DR: 提出了一种基于分层的3D血管生成框架，将全局拓扑与局部几何细节分离，通过三个阶段实现，并在真实数据集上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 准确表示复杂血管几何和拓扑结构是医学应用中的挑战，现有方法难以处理其分支模式和形状。

Method: 分三阶段：生成关键图建模全局结构、基于几何属性生成血管段、按关键图整合局部段。

Result: 在真实数据集上表现优于现有方法，首次成功应用基于部分的生成方法于3D血管建模。
...
TL;DR: 论文提出了一种基于稀疏自编码器（SAE）的可解释性方法，用于分析乳腺影像中的基础模型Mammo-CLIP，揭示了模型决策的潜在特征及其临床相关性。
...
Method: 通过训练一个基于Mammo-CLIP的SAE模型（Mammo-SAE），识别和探究与临床相关乳腺概念（如肿块和可疑钙化）相关的潜在特征。

Result: 研究发现，SAE潜在空间中激活的神经元通常与真实区域对齐，并揭示了影响模型决策的混杂因素。

Conclusion: 研究表明，可解释的SAE潜在表征为理解乳腺影像基础模型的内部机制提供了深入见解。
...
TL;DR: DINO预训练模型结合原型分类器在跨域少样本学习中表现优于最新SOTA方法，提出Coalescent Projection和伪类生成方法解决过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 解决跨域少样本学习中因标记样本稀缺导致Transformer参数更新过多引发的过拟合问题。

Method: 提出Coalescent Projection替代软提示，并结合自监督变换生成伪类，仅依赖基域数据准备网络应对未见域样本。

Result: 在BSCD-FSL基准测试的极端域偏移场景中表现有效。

Conclusion: Coalescent Projection和伪类生成方法有效解决了跨域少样本学习的过拟合问题，提升了模型性能。
...
Method: 提出注意力共享机制、改进的DiT变体和集成多模态大语言模型。

Result: 实验表明FreeCus在零样本主题合成中表现优异，兼容现有工具。

Conclusion: FreeCus成功解锁了DiT的零样本能力，为高质量主题合成提供了无需训练的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统微分PnP方法在图像到点云（I2P）配准中对噪声和异常值敏感，影响了对应学习的有效性。

Method: 提出MinCD-PnP方法，简化盲PnP为最小化Chamfer距离的任务，并设计轻量级多任务学习模块MinCD-Net。

Result: 在多个数据集上实验表明，MinCD-Net在跨场景和跨数据集设置中均优于现有方法，具有更高的内点比率（IR）和配准召回率（RR）。
...
<details>
  <summary>Details</summary>
Motivation: 随着生物识别系统的发展，攻击技术也日益复杂，传统深度学习模型在适应新攻击类型和环境条件方面存在不足，且需要大量训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 当前少样本目标检测方法在有限训练样本下难以准确提取有效特征，尤其是高权重通道未必有效，低权重通道可能仍有价值。

Method: 提出SCSM模块，包含空间特征建模（SFM）和基于Mamba的通道状态建模（CSM），以平衡空间和通道关系学习。

Result: 在VOC和COCO数据集上的实验表明，SCSM模块显著提升了特征表示质量，达到最先进性能。

Conclusion: SCSM模块通过建模通道间相关性，有效提升了少样本目标检测的特征提取能力。
...
TL;DR: 提出BenchDepth，一种新的深度基础模型（DFM）评估基准，通过五个下游代理任务评估DFM的实际应用价值，避免传统对齐指标的偏见。
...
Method: 提出BenchDepth基准，通过深度补全、立体匹配、单目3D场景重建、SLAM和视觉语言空间理解五个任务评估DFM。

Result: 对八种先进DFM进行了基准测试，提供了关键发现和观察的深入分析。

Conclusion: BenchDepth为深度模型评估提供了更公平实用的方法，希望推动社区讨论和未来深度估计研究。
...
<details>
  <summary>Details</summary>
Motivation: 工业缺陷检测系统在单类异常检测范式下面临均匀异常分布假设和数据稀缺的挑战，ExDD旨在通过显式建模双分布来解决这些问题。

Method: ExDD采用并行记忆库捕获正常和异常模式的特征分布，结合潜在扩散模型生成合成缺陷数据，并通过邻域感知比率评分机制融合距离度量。

Result: 在KSDD2数据集上，ExDD实现了94.2%的I-AUROC和97.7%的P-AUROC，最佳增强效果出现在100个合成样本时。

Conclusion: ExDD通过显式双分布建模和数据增强，显著提升了工业缺陷检测的性能，解决了单类异常检测的局限性。
...
TL;DR: 论文提出了一种使用高保真合成数据集训练模型的方法，能在保持精度的同时提高效率，并解决数据来源和多样性问题。


<details>
  <summary>Details</summary>
Motivation: 现有高精度模型需要大量数据和计算资源，而合成数据集能提供高质量标签、数据来源可控，并能解决模型不公平问题。

Method: 使用合成数据集训练模型，通过程序化数据合成控制数据多样性，并在三个密集预测任务上验证。

Result: 模型在真实图像上表现出高精度，且训练和推理成本显著低于同类模型。

Conclusion: 合成数据集是高效、可控且经济的替代方案，适用于人类中心计算机视觉任务。
...
TL;DR: ORSANet提出了一种新的面部表情识别方法，通过多模态语义引导和多尺度交互模块解决遮挡和数据集偏差问题，并设计了动态对抗排斥损失函数以提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有FER模型在面部部分遮挡时难以提取有效特征，导致分类不准确，因此需要一种更鲁棒的方法。

Method: 引入多模态语义引导（密集语义分割图和稀疏几何标志点），设计多尺度交互模块（MCM）和动态对抗排斥损失（DARELoss）。

Result: 在公共基准和自建Occlu-FER数据集上达到SOTA性能。

Conclusion: ORSANet通过多模态语义融合和动态损失设计，显著提升了遮挡条件下的FER性能。
...
TL;DR: EgoPrune是一种无需训练的令牌修剪方法，专为自我运动视频推理设计，显著降低计算成本并提升效率。


<details>
  <summary>Details</summary>
Motivation: 自我运动视频是具身AI代理的主要视觉输入，现有方法未能充分利用其时空连续性和运动约束，计算成本高。

Method: EgoPrune包括关键帧选择器、视角感知冗余过滤器和基于MMR的令牌选择器，结合时空连续性和多样性。

Result: 在两个基准测试中，EgoPrune优于现有方法，显著减少FLOPs、内存使用和延迟，并在边缘设备上验证了实用性。
...
TL;DR: 提出了一种名为RAda的简单有效方法，通过动态校准融合表示来优化视觉语言模型的微调，提升性能。
...
Method: RAda使用轻量级注意力层生成学习掩码，动态调整融合表示中每个元素的贡献，从而优化跨模态交互。

Result: 在不同设置下（如更新或冻结预训练编码器、测试时训练），RAda均显著提升基线性能，且代码简洁。

Conclusion: RAda是一种通用的微调技术，能有效提升视觉语言模型的下游任务性能。
...
<details>
  <summary>Details</summary>
Motivation: 在德国农村发生一起家庭谋杀案后，传统搜索方法因植被密集而失效，促使研究团队通过航空影像和众包搜索生成数据集。

Method: 利用高分辨率航空影像和众包标注，构建了一个包含遮挡条件下难以检测的异常数据集，并开发了交互式网络界面。

Result: 现有异常检测方法在该数据集上表现不佳，表明需要更上下文感知的方法。

Conclusion: 该数据集为复杂环境中的异常检测提供了基准，支持未来研究和实际应用。
...
TL;DR: 提出了一种结合LiDAR点云和图像的LiDAR-Visual里程计框架，通过深度补全和多尺度特征提取网络实现高精度位姿估计。
...
Result: 在KITTI里程计基准测试中表现优于或接近现有视觉和LiDAR里程计方法。

Conclusion: 该方法在动态环境和尺度模糊情况下具有鲁棒性，为自主系统提供了可靠的位姿估计解决方案。
...
### [110] [Quantifying and Narrowing the Unknown: Interactive Text-to-Video Retrieval via Uncertainty Minimization](https://arxiv.org/abs/2507.15504)
*Bingqing Zhang,Zhuo Cao,Heming Du,Yang Li,Xue Li,Jiajun Liu,Sen Wang*

Main category: cs.CV

TL;DR: UMIVR是一个基于不确定性最小化的交互式文本到视频检索框架，通过量化文本模糊性、映射不确定性和帧质量，生成针对性问题以减少检索模糊性。


<details>
  <summary>Details</summary>
Motivation: 当前交互式文本到视频检索方法依赖启发式策略，未显式量化不确定性，限制了效果。
...
### [111] [SAIGFormer: A Spatially-Adaptive Illumination-Guided Network for Low-Light Image Enhancement](https://arxiv.org/abs/2507.15520)
...
Method: 提出动态积分图像表示建模空间变化光照，并构建SAI²E估计器；引入光照引导的多头自注意力机制（IG-MSA）校准亮度相关特征。

Result: 在五个标准低光数据集和跨域基准（LOL-Blur）上，SAIGFormer在定量和定性指标上显著优于现有方法，尤其在非均匀光照增强中表现优异。

Conclusion: SAIGFormer在非均匀光照增强中具有卓越性能和强泛化能力，代码已开源。
...
TL;DR: 提出一种自监督过程学习框架，通过融合Gromov-Wasserstein最优传输和对比正则化，解决视频中关键步骤发现与顺序确定的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理顺序变化、背景/冗余帧和重复动作时性能受限，需改进。

Method: 结合Gromov-Wasserstein最优传输和结构先验进行帧间映射，并引入对比正则化避免嵌入空间退化。

Result: 在多个基准测试（EgoProceL、ProceL、CrossTask）上表现优于现有方法。

Conclusion: 所提框架有效解决了自监督过程学习中的关键挑战，性能显著提升。
...
Method: 提出了Endoscapes-SG201数据集和SSG-Com方法，通过图结构建模工具-动作-目标组合和手部身份。

Result: 实验表明，整合这些关键元素显著提升了手术场景理解任务（如安全关键视图评估和动作三元组识别）的性能。

Conclusion: 研究强调了工具-动作-目标组合和手部身份在图表示中的重要性，为手术场景理解提供了更全面的解决方案。
...
Method: 低秩分解VLM文本特征，生成类共享基础特征和可调权重，结合LLM动作正则化优化权重。

Result: 在HICO-DET上达到27.91的未见动作mAP，刷新零样本HOI检测的SOTA。
...
TL;DR: 提出了一种名为Dynamic-Image（DynImg）的视频表示方法，通过引入非关键帧作为时间提示，以突出快速移动物体的空间区域，从而提升多模态大语言模型（MLLM）在视频理解任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 传统方法将空间和时间信息分开处理，导致快速移动物体的空间信息难以准确表示，从而影响时空交互和视频理解的准确性。

Method: 提出DynImg方法，利用非关键帧作为时间提示，指导模型关注快速移动物体的细粒度空间特征，并结合4D视频旋转位置嵌入保持时空顺序。
...
### [116] [GeMix: Conditional GAN-Based Mixup for Improved Medical Image Augmentation](https://arxiv.org/abs/2507.15577)
*Hugo Carlesso,Maria Eliza Patulea,Moncef Garouani,Radu Tudor Ionescu,Josiane Mothe*

Main category: cs.CV

TL;DR: GeMix是一种基于类条件GAN的两阶段图像增强框架，通过生成视觉连贯的图像来替代传统的像素级混合方法，提升医学图像分类性能。


<details>
  <summary>Details</summary>
Motivation: 传统Mixup在医学图像中生成的图像可能不真实，影响学习效果，特别是在高风险应用中。

Method: 使用StyleGAN2-ADA生成器，通过Dirichlet和Beta分布采样标签向量，生成连续类流形上的图像。

Result: 在COVIDx-CT-3数据集上，GeMix结合真实数据显著提高了宏F1分数，降低了COVID-19检测的假阴性率。

Conclusion: GeMix是一种即插即用的Mixup替代方案，提供更强的正则化和语义保真度，适用于现有训练流程。
...
TL;DR: SegDT是一种基于扩散变压器（DiT）的新型皮肤病变分割模型，适用于低成本硬件，通过Rectified Flow提升生成质量并保持快速推理速度。
...
Conclusion: Being-H0通过利用人类视频数据，显著提升了VLA在复杂操作任务中的性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决SDF方法在细节捕捉上的不足和3DGS方法在全局几何一致性上的缺陷。

Method: 结合SDF的粗几何捕捉和3DGS的渲染能力，相互优化细节和全局一致性。

Result: 在DTU和MobileBrick数据集上，表面重建和新视角合成性能优于现有方法。

Conclusion: 混合方法有效结合了SDF和3DGS的优势，提升了稀疏视图重建和渲染的效果。
...
Method: 采用圆柱坐标系，明确分离不同角度的特征，并引入嵌套圆柱表示以处理复杂几何和多分辨率。

Result: 在合成数据集和真实图像上表现优于现有方法。

Conclusion: CylinderPlane能有效消除特征模糊问题，提升多视角一致性，适用于复杂场景。
...
TL;DR: 本文综述了深度神经网络（DNN）在视频分析中效率优化的技术，从硬件支持、数据处理等多角度进行了全面回顾，并探讨了现有挑战。


<details>
  <summary>Details</summary>
Motivation: 视频数据的爆炸式增长对视频分析的准确性和效率提出了更高要求，而DNN的效率优化仍是一个开放性问题。

Method: 采用自下而上的方式组织现有方法，涵盖硬件支持、数据处理、操作部署等多个视角。

Result: 提出了一个优化框架，并基于现有工作分析了DNN在视频分析中的性能优化问题和挑战。

Conclusion: 本文为DNN在视频分析中的效率优化提供了系统性综述，并指出了未来的研究方向。
...
TL;DR: 研究探讨了主动学习和顺序学习在光学音乐识别中的应用，通过YOLOv8选择不确定性高的样本进行迭代标注和训练，以减少标注需求。
...
Result: 实验表明，仅需少量标注即可达到与全监督训练相当的精度，但不确定性主动学习在特定手稿中效果不佳。
...
### [123] [Uncovering Critical Features for Deepfake Detection through the Lottery Ticket Hypothesis](https://arxiv.org/abs/2507.15636)
*Lisan Al Amin,Md. Ismail Hossain,Thanh Thi Nguyen,Tasnim Jahan,Mahbubul Islam,Faisal Quader*

Main category: cs.CV

TL;DR: 该研究应用彩票假设（LTH）于深度伪造检测，发现神经网络可高效剪枝且保持高检测精度。
...
Method: 研究采用彩票假设（LTH）和迭代幅度剪枝方法，在MesoNet、CNN-5和ResNet-18架构上进行实验。

Result: 实验显示，MesoNet在80%稀疏度下仍保持56.2%准确率，仅需3,000参数；LTH方法优于一次性剪枝。

Conclusion: 研究发现深度伪造检测网络存在关键子网络（winning tickets），可跨数据集迁移，为高效部署提供可能。
...
### [124] [Extracting Visual Facts from Intermediate Layers for Mitigating Hallucinations in Multimodal Large Language Models](https://arxiv.org/abs/2507.15652)
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在视觉识别和语言理解结合方面取得进展，但仍存在对象幻觉问题，即生成看似合理但实际错误的输出。现有研究表明先验知识会抑制深层视觉信息，但中间层的抑制机制尚不明确。

Method: 提出EVA方法，动态选择视觉事实信息最显著的中间层，通过对比原始输入和纯文本输入的输出分布，提取视觉事实知识并修正最终层的输出。

Result: 在广泛使用的基准测试中，EVA显著降低了幻觉率，优于基线方法。

Conclusion: EVA是一种模型无关的方法，能有效缓解MLLMs中的幻觉问题，且无需训练，兼容多种解码策略。
...
### [125] [HW-MLVQA: Elucidating Multilingual Handwritten Document Understanding with a Comprehensive VQA Benchmark](https://arxiv.org/abs/2507.15655)
...
Result: 该基准为多语言手写文档理解提供了严格的评估标准，推动了相关领域的研究。

Conclusion: HW-MLVQA有望促进多语言手写文档理解技术的进步，推动该领域的创新和学术研究。
...
### [126] [Visual-Language Model Knowledge Distillation Method for Image Quality Assessment](https://arxiv.org/abs/2507.15680)
*Yongkang Hou,Jiarun Song*

Main category: cs.CV

TL;DR: 提出了一种基于视觉语言模型知识蒸馏的方法，用于图像质量评估（IQA），解决了CLIP模型参数过多和局部失真特征识别不足的问题。
...
Conclusion: 该方法在IQA任务中表现出强大的实际部署潜力。
...
### [127] [Hi^2-GSLoc: Dual-Hierarchical Gaussian-Specific Visual Relocalization for Remote Sensing](https://arxiv.org/abs/2507.15683)
*Boni Hu,Zhenyu Xia,Lin Chen,Pengcheng Han,Shuhui Bu*

Main category: cs.CV

TL;DR: 论文提出了一种基于3D高斯点云的视觉重定位方法（Hi²-GSLoc），通过稀疏到密集、粗到细的双层次框架，解决了现有方法在精度和计算复杂度上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有视觉重定位方法在精度和计算效率之间存在权衡，尤其是在大规模遥感场景中。本文旨在利用3D高斯点云（3DGS）作为场景表示，结合双层次框架，提升重定位性能。

Method: 提出Hi²-GSLoc框架，分为稀疏阶段（基于高斯点云的采样和地标检测）和密集阶段（迭代优化位姿）。采用分区训练、GPU并行匹配和动态内存管理以应对大规模场景。

Result: 在仿真数据、公开数据集和实际飞行实验中，方法表现出高定位精度、召回率和计算效率，并能有效过滤不可靠位姿估计。

Conclusion: Hi²-GSLoc方法在遥感应用中具有实际有效性，解决了现有方法的局限性。
...
### [128] [LINR-PCGC: Lossless Implicit Neural Representations for Point Cloud Geometry Compression](https://arxiv.org/abs/2507.15686)
*Wenjie Huang,Qi Yang,Shuting Xia,He Huang,Zhu Li,Yiling Xu*

Main category: cs.CV

TL;DR: 本文提出了一种基于隐式神经表示（INR）的无损点云几何压缩方法LINR-PCGC，解决了现有方法依赖特定训练数据分布的问题，并通过优化编码框架和网络设计显著提升了压缩效率和速度。


<details>
  <summary>Details</summary>
Motivation: 现有基于AI的点云压缩方法依赖特定训练数据分布，限制了实际应用。INR方法虽能解决这一问题，但受限于编码时间和解码器大小，目前仅支持有损压缩。

Method: 提出LINR-PCGC方法，采用点云级编码框架和高效网络初始化策略，减少60%编码时间；设计基于多尺度SparseConv的轻量级编码网络，实现快速推理和解码器紧凑化。
...
### [129] [Efficient Face Image Quality Assessment via Self-training and Knowledge Distillation](https://arxiv.org/abs/2507.15709)
...
Method: 采用两阶段方法：先训练强大的教师模型，再通过自训练策略增强其能力，并蒸馏出轻量级学生模型。
...
### [130] [A Practical Investigation of Spatially-Controlled Image Generation with Transformers](https://arxiv.org/abs/2507.15724)
*Guoxuan Xia,Harleen Hanspal,Petru-Daniel Tudosiu,Shifeng Zhang,Sarah Parisot*

Main category: cs.CV

TL;DR: 该论文旨在为基于Transformer的空间控制图像生成模型提供清晰的比较和优化方法，填补文献中的知识空白。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决现有模型在空间控制生成任务中缺乏详细和公平的科学比较的问题，同时澄清某些方法的动机和细节。

Method: 方法包括在ImageNet上进行控制实验，比较扩散模型、流模型和自回归模型，并提出控制令牌预填充作为基线方法。此外，研究了采样时间增强技术，如扩展分类器自由指导和softmax截断。

Result: 结果表明，控制令牌预填充是一种简单且高性能的基线方法，采样时间增强技术对控制生成一致性有显著影响。适配器方法在有限数据下能保持生成质量，但在一致性上不如完整训练。

Conclusion: 结论是论文为基于Transformer的空间控制生成提供了实用的见解，填补了文献中的知识空白，并提出了有效的优化方法。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成短视频时表现优秀，但扩展到长视频时面临内存和一致性问题。

Method: 方法分为三部分：1) 训练To2V模型生成短视频；2) 引入T2To模型生成全局一致的token；3) 使用FIFO-Diffusion策略平滑过渡相邻片段。
...
### [132] [Appearance Harmonization via Bilateral Grid Prediction with Transformers for 3DGS](https://arxiv.org/abs/2507.15748)
*Jisu Shin,Richard Shaw,Seunghyun Shin,Anton Pelykh,Zhensong Zhang,Hae-Gon Jeon,Eduardo Perez-Pellitero*

Main category: cs.CV

TL;DR: 提出一种基于Transformer的方法，通过预测双边网格校正多视角图像中的光度不一致性，提升3D高斯泼溅管线的重建质量。


<details>
  <summary>Details</summary>
Motivation: 现代相机管线处理导致多视角图像的光度不一致，影响新视角合成的质量。现有方法通过联合优化场景表示和每张图像的外观嵌入来解决，但计算复杂且训练慢。

Method: 提出基于Transformer的方法，预测空间自适应的双边网格，以多视角一致的方式校正光度变化，无需场景特定重训练。

Result: 实验表明，该方法在重建保真度和收敛速度上优于或匹配现有场景特定优化方法。

Conclusion: 该方法通过高效训练和跨场景泛化能力，显著提升了多视角图像的光度一致性和重建质量。
...
### [133] [Learning from Heterogeneity: Generalizing Dynamic Facial Expression Recognition via Distributionally Robust Optimization](https://arxiv.org/abs/2507.15765)
...
Result: 在DFEW和FERV39k数据集上表现出色，显著提升了WAR和UAR指标，并保持了强泛化能力。
...
### [134] [Label tree semantic losses for rich multi-class medical image segmentation](https://arxiv.org/abs/2507.15777)
*Junwen Wang,Oscar MacCormac,William Rochford,Aaron Kujawa,Jonathan Shapey,Tom Vercauteren*

Main category: cs.CV

TL;DR: 提出两种基于树结构的语义损失函数，利用标签的层次结构改进医学图像分割任务，在稀疏标注和全监督任务中均达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法对所有错误同等惩罚，未能利用标签空间的语义信息，尤其是标签类别增多且差异细微时效果不佳。

Method: 提出两种树状语义损失函数，结合稀疏标注训练方法，应用于全监督和稀疏标注的医学图像分割任务。

Result: 在头MRI全脑分割和神经外科高光谱成像任务中，方法达到SOTA性能。
...
### [135] [Regularized Low-Rank Adaptation for Few-Shot Organ Segmentation](https://arxiv.org/abs/2507.15793)
*Ghassen Baklouti,Julio Silva-Rodríguez,Jose Dolz,Houda Bahig,Ismail Ben Ayed*

Main category: cs.CV

TL;DR: 提出了一种动态调整低秩适应（LoRA）中固有秩的新方法，用于医学图像分割，通过引入l_1稀疏正则化器自动优化秩，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决LoRA方法中固定秩选择对医学图像任务复杂性的挑战，借鉴自然图像处理进展，提出动态调整秩的解决方案。

Method: 将低秩表示视为奇异值分解，引入l_1稀疏正则化器到损失函数中，并使用近端优化器自动优化任务适应的秩。

Result: 在少量样本微调场景中，相比标准LoRA和其他PEFT方法，新方法在基础器官和新器官任务中表现显著更优。

Conclusion: 动态调整秩的方法在医学图像分割中高效且鲁棒，代码已开源。
...
### [136] [Exploring Superposition and Interference in State-of-the-Art Low-Parameter Vision Models](https://arxiv.org/abs/2507.15798)
*Lilian Hollard,Lucas Mohimont,Nathalie Gaveau,Luiz-Angelo Steffenel*

Main category: cs.CV

TL;DR: 研究了低参数深度神经网络在计算机视觉中的性能，重点分析了瓶颈架构及其在使用超线性激活函数时的行为，提出了一种减少特征图干扰的设计方法。


<details>
  <summary>Details</summary>
Motivation: 研究低参数深度神经网络中特征图干扰现象，探索如何通过设计优化网络性能。

Method: 通过分析多种瓶颈架构，识别减少干扰的关键设计元素，并提出名为NoDepth Bottleneck的概念验证架构。

Result: 在ImageNet数据集上验证了NoDepth Bottleneck的鲁棒性和准确性，表明其适用于低参数范围的网络。

Conclusion: 研究为低参数神经网络提供了更高效和可扩展的设计方案，并加深了对瓶颈架构的理解。
...
### [137] [ConformalSAM: Unlocking the Potential of Foundational Segmentation Models in Semi-Supervised Semantic Segmentation with Conformal Prediction](https://arxiv.org/abs/2507.15803)
...
Method: 提出ConformalSAM框架，结合不确定性校准和自依赖训练策略，利用SEEM生成高质量监督信号。

Result: 在三个标准基准测试中，ConformalSAM性能优于现有半监督语义分割方法，并可作为插件提升其他方法。

Conclusion: ConformalSAM有效利用基础分割模型解决标注稀缺问题，为半监督语义分割提供新思路。
...
### [138] [True Multimodal In-Context Learning Needs Attention to the Visual Context](https://arxiv.org/abs/2507.15807)
*Shuo Chen,Jianzhe Liu,Zhen Han,Yan Xia,Daniel Cremers,Philip Torr,Volker Tresp,Jindong Gu*

Main category: cs.CV

TL;DR: 当前多模态大语言模型（MLLMs）在多模态上下文学习（MICL）中过度依赖文本模式，忽视视觉信息。本文提出动态注意力重分配（DARA）和TrueMICL数据集，以提升模型对视觉内容的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在多模态上下文学习中未能有效利用视觉信息，导致性能受限且评估不足。

Method: 提出动态注意力重分配（DARA）策略和TrueMICL数据集，以促进模型对视觉内容的关注。

Result: 实验表明，该方法显著提升了多模态上下文学习的真实能力。

Conclusion: DARA和TrueMICL有效解决了MLLMs在MICL中的视觉信息利用不足问题。
...
TL;DR: PhysVidBench是一个评估文本到视频生成模型物理常识能力的基准，通过383个精心设计的提示和间接评估策略，填补了现有模型在物理合理性上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成模型在物理常识方面表现不佳，导致生成的视频违反直觉的因果关系和物体行为。

Method: 提出PhysVidBench基准，包含383个强调工具使用、材料属性和交互的提示，采用三阶段评估流程：生成物理问题、视频字幕描述、语言模型回答问题。

Result: 通过间接评估策略避免了直接视频评估中的幻觉问题，为生成视频模型的物理常识评估提供了结构化框架。

Conclusion: PhysVidBench填补了当前文本到视频评估的空白，为物理合理性的评估提供了可解释的方法。
...
TL;DR: 论文提出了一种概念驱动的视频对象分割框架（SeC），通过结合视觉-语言模型（LVLMs）构建高层次的对象表示，显著提升了在复杂场景下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频对象分割方法依赖外观匹配，难以处理剧烈视觉变化和复杂场景变化，缺乏人类对对象的概念理解能力。

Method: SeC框架利用LVLMs整合多帧视觉线索，构建对象的概念先验，并在推理时动态平衡语义推理和特征匹配。

Result: SeC在提出的SeCVOS基准上比SAM 2.1提升了11.8分，实现了概念感知视频对象分割的新SOTA。

Conclusion: SeC通过概念驱动的表示和动态计算调整，显著提升了复杂场景下的视频对象分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 现代生成模型共享类似的目标——从噪声输入（如高斯噪声或掩码）中重建干净信号。作者希望通过对齐分词器嵌入与去噪目标，使潜在嵌入更容易被重建。

Method: 提出l-DeTok，通过训练分词器从被插值噪声和随机掩码污染的潜在嵌入中重建干净图像。

Result: 在ImageNet 256x256上的实验表明，l-DeTok在六种代表性生成模型中均优于标准分词器。

Conclusion: 去噪是分词器设计的基本原则，未来分词器设计可借鉴这一视角。
...
### [142] [MiDeSeC: A Dataset for Mitosis Detection and Segmentation in Breast Cancer Histopathology Images](https://arxiv.org/abs/2507.14271)
...
<details>
  <summary>Details</summary>
Motivation: 由于有丝分裂形态多样，需要大数据集覆盖所有情况，以支持乳腺癌研究。

Method: 从40倍放大的玻璃切片中选择50个1024x1024像素区域，使用3D Histech和Olympus设备扫描。
...
### [143] [NuSeC: A Dataset for Nuclei Segmentation in Breast Cancer Histopathology Images](https://arxiv.org/abs/2507.14272)
*Refik Samet,Nooshin Nemati,Emrah Hancer,Serpil Sak,Bilge Ayca Kirmizi*

Main category: eess.IV

TL;DR: NuSeC数据集包含100张1024*1024像素的图像，来自25名患者，分为75%训练集和25%测试集，用于未来方法比较。


<details>
  <summary>Details</summary>
Motivation: 为研究人员提供一个标准化的数据集，以便未来开发的算法能够进行一致的比较分析。

Method: 从每位患者的4张图像中随机选择1张作为测试集，其余作为训练集，确保数据分布均匀。

Result: 训练集包含75张图像约30000个核结构，测试集包含25张图像约6000个核结构。

Conclusion: NuSeC数据集的设计支持公平和一致的算法评估。
...
### [144] [Self-Supervised Joint Reconstruction and Denoising of T2-Weighted PROPELLER MRI of the Lungs at 0.55T](https://arxiv.org/abs/2507.14308)
*Jingjia Chen,Haoyang Pei,Christoph Maier,Mary Bruno,Qiuting Wen,Seon-Hi Shin,William Moore,Hersh Chandarana,Li Feng*

Main category: eess.IV

TL;DR: 本研究通过自监督联合重建与去噪模型，提升了0.55T T2加权PROPELLER肺部MRI的图像质量。
...
Method: 使用44名COVID康复患者的MRI数据，开发了一种自监督学习框架，将PROPELLER采集的每个叶片分为两部分，分别用于训练重建网络和计算损失。对比方法包括MPPCA去噪和传统并行成像重建。

Result: 自监督模型显著提升了图像质量，与CT图像对齐良好，且扫描时间可减少一半。读者评估显示该方法优于MPPCA去噪（p<0.001）。

Conclusion: 通过利用k空间子集的结构冗余性，自监督学习模型有效实现了图像重建和噪声抑制。
...
### [145] [Classification of Histopathology Slides with Persistence Homology Convolutions](https://arxiv.org/abs/2507.14378)
*Shrunal Pothagoni,Benjamin Schweinhart*

Main category: eess.IV

TL;DR: 提出了一种基于局部持久同调的新方法（Persistent Homology Convolutions），用于改进医学图像分类中拓扑信息的利用，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统CNN在医学图像分类中可能丢失拓扑信息，而拓扑特征（如细胞形状）对疾病诊断至关重要。现有方法仅使用全局拓扑摘要，缺乏局部特征信息。

Method: 开发了一种改进的卷积算子（Persistent Homology Convolutions），生成局部持久同调数据，捕捉拓扑特征的局部性和平移不变性。

Result: 实验表明，使用该方法训练的模型在组织病理学切片分类中表现优于传统模型，且对超参数更鲁棒。

Conclusion: 持久同调卷积能有效提取组织病理学图像中的几何信息，提升诊断性能。
...
### [146] [Spatiotemporal Maps for Dynamic MRI Reconstruction](https://arxiv.org/abs/2507.14429)
...
TL;DR: 提出了一种新的动态MRI重建模型——时空映射（STM），通过利用(k, t)-空间的自回归特性，解决了部分可分离函数（PSF）模型在空间位置不同时表现不足的问题。


<details>
  <summary>Details</summary>
Motivation: PSF模型在动态MRI重建中广泛应用，但在空间位置不同时其表示能力下降，因此需要一种更灵活的模型。

Method: STM模型将时空MRI信号分解为多个组件的和，每个组件由空间函数和依赖于空间位置的时间函数组成。通过信号处理和随机线性代数技术从自校准数据中高效计算STM。

Result: 实验证明STM可用于2D单通道动物胃肠道MRI和3D多通道人类功能MRI数据的重建。

Conclusion: STM是PSF模型的扩展，能更灵活地表示动态MRI信号，适用于多种加速动态MRI重建框架。
...
### [147] [QUTCC: Quantile Uncertainty Training and Conformal Calibration for Imaging Inverse Problems](https://arxiv.org/abs/2507.14760)
*Cassandra Tong Ye,Shamus Li,Tyler King,Kristina Monakhova*

Main category: eess.IV

TL;DR: 论文提出QUTCC方法，通过非线性、非均匀的量化预测校准技术，为深度学习模型提供更紧致的不确定性估计，解决了医学图像去噪和重建中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像处理中可能产生幻觉（不真实的伪影），而现有不确定性量化方法因线性校准导致边界过大且信息不足。

Method: 提出QUTCC技术，结合U-Net架构和量化嵌入，预测完整的条件分布，并通过迭代查询上下分位数逐步优化不确定性边界。

Result: QUTCC在多个去噪任务和压缩MRI重建中，成功识别幻觉并实现更紧致的不确定性区间，同时保持统计覆盖。

Conclusion: QUTCC通过非线性校准显著提升了不确定性估计的精度和可靠性，适用于高精度要求的医学图像任务。
...
### [148] [PET Image Reconstruction Using Deep Diffusion Image Prior](https://arxiv.org/abs/2507.15078)
*Fumio Hashimoto,Kuang Gong*

Main category: eess.IV

TL;DR: 提出了一种基于扩散模型的解剖先验引导PET图像重建方法，解决了PET成像中对比度变化和高计算需求的问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在医学图像去噪和重建中表现优异，但在PET成像中因对比度变化和高计算需求应用受限。

Method: 采用扩散采样与模型微调交替进行，结合HQS算法提高计算效率，利用预训练分数函数实现多示踪剂图像重建。

Result: 实验证明该方法能跨示踪剂分布和扫描仪类型稳健重建，适用于低剂量PET成像。
...
### [149] [Performance Analysis of Post-Training Quantization for CNN-based Conjunctival Pallor Anemia Detection](https://arxiv.org/abs/2507.15151)
*Sebastian A. Cruz Romero,Wilfredo E. Lugo Beauchamp*

Main category: eess.IV

TL;DR: 该研究利用深度学习模型通过结膜苍白检测贫血，使用MobileNet架构，在CP-AnemiC数据集上取得了高准确率（0.9313），并探索了量化对模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 传统贫血检测方法成本高且依赖专家，阻碍早期诊断。研究旨在通过深度学习提供低成本、高效的解决方案。

Method: 使用MobileNet架构，结合数据增强和交叉验证策略，对模型进行端到端微调，并评估不同量化方案（FP32、FP16、INT8、INT4）的影响。

Result: 模型在数据集上表现优异（准确率0.9313），FP16量化后性能保持稳定，但更激进的量化（INT8、INT4）导致性能显著下降。
...
### [150] [A Study of Anatomical Priors for Deep Learning-Based Segmentation of Pheochromocytoma in Abdominal CT](https://arxiv.org/abs/2507.15193)
...
TL;DR: 该研究通过引入基于器官特异性解剖先验的多类标注策略，显著提升了深度学习模型对腹部CT扫描中嗜铬细胞瘤（PCC）的分割精度。
...
Method: 使用nnU-Net框架评估了11种标注策略，引入了基于邻近器官解剖先验的多类标注方案，并在105例增强CT扫描上进行了训练和测试。

Result: Tumor + Kidney + Aorta (TKA)标注策略在DSC、NSD和F1分数上显著优于传统方法，并在肿瘤负荷量化和遗传亚型分割中表现优异。

Conclusion: 研究表明，结合相关解剖背景可显著提升PCC分割的精确性，支持临床评估和长期监测。
...
### [151] [Personalized 3D Myocardial Infarct Geometry Reconstruction from Cine MRI with Explicit Cardiac Motion Modeling](https://arxiv.org/abs/2507.15194)
...
TL;DR: 提出了一种基于2D标准电影MRI的无对比剂高保真3D心肌梗死几何重建框架，利用动态几何运动模式定位梗死区域。
...
Method: 通过自动深度形状拟合模型biv-me重建4D双心室网格，并设计CMotion2Infarct-Net模型利用动态几何运动模式定位梗死区域。

Result: 在205例患者数据中，方法与手动描绘结果一致，验证了无对比剂3D梗死重建的可行性。

Conclusion: 该研究为心肌梗死的高效数字孪生提供了无对比剂的解决方案。
...
### [152] [Personalized 4D Whole Heart Geometry Reconstruction from Cine MRI for Cardiac Digital Twins](https://arxiv.org/abs/2507.15203)
...
<details>
  <summary>Details</summary>
Motivation: 心脏数字孪生（CDTs）在精准医学中潜力巨大，但目前全心脏CDT模型仍有限。

Method: 通过自监督映射从2D MRI生成4D心脏网格，实现个性化建模。

Result: 模型能高分辨率自动提取关键心脏参数，如射血分数和动态心室容积变化。

Conclusion: 验证了从MRI推断个性化4D心脏模型的可行性，为高效CDT平台铺路。
...
### [153] [EndoControlMag: Robust Endoscopic Vascular Motion Magnification with Periodic Reference Resetting and Hierarchical Tissue-aware Dual-Mask Contro](https://arxiv.org/abs/2507.15292)
*An Wanga,Rulin Zhou,Mengya Xu,Yiru Ye,Longfei Gou,Yiting Chang,Hao Chen,Chwee Ming Lim,Jiankun Wang,Hongliang Ren*

Main category: eess.IV

TL;DR: EndoControlMag是一种无需训练的、基于拉格朗日的框架，用于内窥镜手术中血管细微运动的放大，通过周期性参考重置和分层组织感知放大模块提升手术精度。
...
Method: EndoControlMag采用周期性参考重置（PRR）和分层组织感知放大（HTM）框架，结合双模式掩模扩张，通过运动或距离自适应软化策略放大血管运动。
...
### [154] [MedSR-Impact: Transformer-Based Super-Resolution for Lung CT Segmentation, Radiomics, Classification, and Prognosis](https://arxiv.org/abs/2507.15340)
...
TL;DR: TVSRN-V2是一种基于Transformer的超分辨率框架，用于低剂量CT体积的精细解剖细节重建，显著提升了临床肺部CT分析的准确性。
...
Method: 采用Transformer架构，包括Through-Plane Attention Blocks和Swin Transformer V2，结合伪低分辨率增强技术模拟扫描仪多样性。

Result: 在肺叶分割、放射组学和预后任务中表现优异，分割准确率提升4%，放射组学特征可重复性更高，预测性能提升（C-index和AUC增加0.06）。
...
### [155] [Latent Space Synergy: Text-Guided Data Augmentation for Direct Diffusion Biomedical Segmentation](https://arxiv.org/abs/2507.15361)
*Muhammad Aqeel,Maham Nazir,Zanxi Ruan,Francesco Setti*

Main category: eess.IV

TL;DR: SynDiff结合文本引导的合成数据生成和扩散模型，通过单步推理提升计算效率，显著改善医学图像分割性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割（如息肉检测）因标注数据稀缺且需专业知识，面临挑战。SynDiff旨在通过合成数据增强解决这一问题。

Method: 使用潜在扩散模型生成临床真实的合成息肉，通过文本条件修复增强数据多样性，并引入直接潜在估计实现单步推理。

Result: 在CVC-ClinicDB上达到96.0% Dice和92.9% IoU，同时保持实时性，适合临床部署。

Conclusion: SynDiff通过可控合成数据增强提升分割鲁棒性，为资源有限的医疗场景提供高效解决方案。
...
### [156] [A Steel Surface Defect Detection Method Based on Lightweight Convolution Optimization](https://arxiv.org/abs/2507.15476)
*Cong Chen,Ming Chen,Hoileong Lee,Yan Li,Jiyang Yu*

Main category: eess.IV

TL;DR: 提出了一种基于YOLOv9s的深度学习框架，结合C3Ghost、SCConv模块和CARAFE上采样算子，以提高钢表面多尺度缺陷检测的准确性和模型性能。


<details>
  <summary>Details</summary>
Motivation: 钢表面缺陷检测在工业制造中具有挑战性，传统方法在复杂环境中对小目标缺陷的检测精度不足且漏检率高。
...
### [157] [DeSamba: Decoupled Spectral Adaptive Framework for 3D Multi-Sequence MRI Lesion Classification](https://arxiv.org/abs/2507.15487)
*Dezhen Wang,Sheng Miao,Rongxin Chai,Jiufa Cui*

Main category: eess.IV

TL;DR: DeSamba框架通过解耦表示学习和自适应谱空间特征融合，显著提升了多序列MRI数据的3D病变分类性能。


<details>
  <summary>Details</summary>
Motivation: 多序列MRI数据的有效整合对病变分类至关重要，但现有方法难以实现稳健的3D分类。

Method: 提出DeSamba框架，包含解耦表示学习模块（DRLM）和谱自适应调制块（SAMB），动态融合空间和谱特征。

Result: 在脊柱转移和脊柱炎数据集上，DeSamba优于现有方法，显著提升了分类准确性和AUC。
...
### [158] [RARE-UNet: Resolution-Aligned Routing Entry for Adaptive Medical Image Segmentation](https://arxiv.org/abs/2507.15524)
*Simon Winther Albertsen,Hjalte Svaneborg Bjørnstrup,Mostafa Mehdipour Ghazi*

Main category: eess.IV

TL;DR: RARE-UNet是一种分辨率感知的多尺度分割架构，动态适应输入分辨率，显著提升低分辨率数据的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型假设输入为固定高分辨率，在低分辨率实际场景中性能显著下降。

Method: 提出RARE-UNet，结合多尺度块、分辨率感知路由机制和一致性驱动训练。

Result: 在脑成像任务中，RARE-UNet平均Dice分数最高（0.84和0.65），且推理时间显著减少。
...
### [159] [Exposing and Mitigating Calibration Biases and Demographic Unfairness in MLLM Few-Shot In-Context Learning for Medical Image Classification](https://arxiv.org/abs/2506.23298)
*Xing Shen,Justin Szeto,Mingyang Li,Hengguan Huang,Tal Arbel*

Main category: eess.IV

TL;DR: 本文研究了多模态大语言模型（MLLMs）在医学图像分类中的校准偏差和人口统计学不公平性，提出了一种名为CALIN的推理时校准方法，以减轻这些偏差。
...
Method: CALIN通过双层程序（从群体层面到子群层面）估计所需的校准量，并在推理时应用这些校准矩阵来调整预测置信度。

Result: 在三个医学影像数据集上的实验表明，CALIN能有效确保预测的公平校准，同时提高整体预测准确性，并最小化公平性与效用之间的权衡。

Conclusion: CALIN为MLLMs在医学图像分类中的公平性和校准性提供了有效的解决方案。
...
### [160] [Real-Time Scene Reconstruction using Light Field Probes](https://arxiv.org/abs/2507.14624)
*Yaru Liu,Derek Nowrouzezahri,Morgan Mcguire*

Main category: cs.GR

TL;DR: 提出了一种基于探针数据结构的神经渲染方法，用于高效重建大规模复杂场景，避免了显式依赖场景几何数据。
...
Method: 利用探针数据结构重建多尺度隐式场景表示，结合稀疏图像输入，实现高效渲染。

Result: 方法显著降低了计算成本，渲染效率与场景复杂度无关，适用于VR/AR应用。

Conclusion: 该方法为大规模场景重建提供了高效解决方案，具有潜在的实际应用价值。
...
### [161] [Towards Geometric and Textural Consistency 3D Scene Generation via Single Image-guided Model Generation and Layout Optimization](https://arxiv.org/abs/2507.14841)
*Xiang Tang,Ruotong Li,Xiaopeng Fan*

Main category: cs.GR

TL;DR: 提出了一种三阶段框架，通过单图像引导生成3D场景，解决了多对象场景中的对象质量和场景一致性问题。
...
Method: 采用图像实例分割和修复、伪立体视角构建及模型选择策略，最后通过点云距离优化布局参数。
...
### [162] [Time Series Information Visualization -- A Review of Approaches and Tools](https://arxiv.org/abs/2507.14920)
*Evandro S. Ortigossa,Fábio F. Dias,Diego C. Nascimento,Luis Gustavo Nonato*

Main category: cs.GR

TL;DR: 本文综述了时间序列数据的可视化技术，旨在帮助数据科学家通过视觉分析理解动态行为和发现模式。


<details>
  <summary>Details</summary>
Motivation: 时间序列数据普遍且复杂，需要高级工具和分析方法以揭示其动态行为和趋势。

Method: 探讨了多种可视化技术和工具，用于图形化表示时间序列数据的演变。

Result: 总结了时间序列可视化的挑战和未来研究方向，特别关注多特征时间序列。

Conclusion: 时间序列可视化是数据科学的关键工具，但仍需进一步研究以解决开放性问题。
...
### [163] [Model Simplification through refinement](https://arxiv.org/abs/2507.15186)
...
Method: 受向量量化文献中分裂算法的启发，从极粗糙近似开始逐步细化，利用表面曲率近似指导简化过程。

Result: 算法速度快，能在给定时间限制内生成可显示的高质量结果。

Conclusion: 该算法适合大型模型的交互式简化，并可进一步细化已生成的简化结果。
...
### [164] [Blended Point Cloud Diffusion for Localized Text-guided Shape Editing](https://arxiv.org/abs/2507.15399)
*Etai Sella,Noam Atia,Ron Mokady,Hadar Averbuch-Elor*

Main category: cs.GR

TL;DR: 本文提出了一种基于修复的框架，用于编辑点云表示的3D形状，通过结合3D扩散模型和坐标混合算法，实现了局部编辑的同时保持全局一致性和形状身份。
...
### [165] [ObjectGS: Object-aware Scene Reconstruction and Scene Understanding via Gaussian Splatting](https://arxiv.org/abs/2507.15454)
*Ruijie Zhu,Mulin Yu,Linning Xu,Lihan Jiang,Yixuan Li,Tianzhu Zhang,Jiangmiao Pang,Bo Dai*

Main category: cs.GR

TL;DR: ObjectGS是一个对象感知框架，结合3D场景重建与语义理解，通过动态调整对象锚点实现精确的对象级重建。
...
Method: 将场景中的对象建模为局部锚点，生成神经高斯并共享对象ID，通过动态调整锚点和优化特征实现重建。

Result: 在开放词汇和全景分割任务中优于现有方法，并能无缝集成网格提取和场景编辑应用。

Conclusion: ObjectGS成功实现了对象级的3D重建与语义理解的统一。
...
### [166] [Gaussian Splatting with Discretized SDF for Relightable Assets](https://arxiv.org/abs/2507.15629)
...
<details>
  <summary>Details</summary>
Motivation: 解决3DGS在逆渲染中因离散性难以应用几何约束的问题，同时避免现有SDF方法的内存和训练复杂性。

Method: 提出离散化SDF表示，将其编码到高斯中，并通过SDF到不透明度的转换实现渲染；设计投影一致性损失优化离散样本。

Result: 实验表明，该方法在逆渲染任务中优于现有高斯基方法，且无需额外内存或复杂优化。

Conclusion: 离散化SDF结合投影一致性损失，有效提升了3DGS在逆渲染中的性能，同时保持高效性。
...
### [167] [Geophysics-informed neural network for model-based seismic inversion using surrogate point spread functions](https://arxiv.org/abs/2507.14140)
*Marcus Saraiva,Ana Muller,Alexandre Maul*

Main category: physics.geo-ph

TL;DR: 提出了一种结合深度学习和地震建模的地球物理信息神经网络（GINN），用于解决传统地震反演方法的局限性，如依赖1D平均小波和不现实的横向分辨率假设。


<details>
  <summary>Details</summary>
Motivation: 传统地震反演方法存在局限性，如依赖1D平均小波和假设不现实的横向分辨率，需要一种更高效且准确的方法。

Method: 使用深度卷积神经网络（DCNN）同时估计点扩散函数（PSF）和声阻抗（IP），并通过自监督损失函数（MSE和SSIM）优化结果。

Result: GINN能够生成高分辨率IP和真实PSF，减少噪声并提高准确性，优于传统1D小波方法。

Conclusion: GINN展示了在地震反演中的潜力，未来工作将进一步优化训练过程并验证实际数据。
...
### [168] [Molecular Dynamics Simulations of Nanoscale Friction on Illite Clay: Effects of Solvent Salt Ions and Electric Double Layer](https://arxiv.org/abs/2507.14158)
*Ge Li,Astrid S. de Wijn*

Main category: physics.geo-ph

TL;DR: 通过分子动力学模拟研究了挪威快黏土中伊利石片层的滑动行为，分析了不同盐类对摩擦力的影响，发现二价盐能增强摩擦，为环保稳定剂设计提供了新思路。


<details>
  <summary>Details</summary>
Motivation: 快黏土因盐分流失导致强度急剧下降，传统稳定方法成本高且碳排放高，需开发更环保的替代方法。

Method: 使用分子动力学模拟，研究不同单价和二价盐对伊利石片层与表面相互作用的影响，重点关注摩擦力和双电层结构。

Result: 二价阳离子位于表面较高位置，增强摩擦；单价阳离子更靠近表面。

Conclusion: 研究为理解盐类在黏土力学中的作用提供了新框架，并指出了设计环保稳定剂的潜力。

Abstract: Quick clay is a highly sensitive soil that transforms rapidly from solid to
liquid under minor stress, as a result of long-term salt leaching that
drastically reduces shear strength. Stabilizing it is both costly and
carbon-intensive, significantly impacting construction emissions in regions
like Norway. Developing greener stabilization methods is challenging due to
limited understanding of the weakening mechanisms and the specific roles of
different salts. In this study, we use molecular dynamics (MD) simulations to
investigate the sliding behavior of illite platelets, the key component in
Norwegian quick clay, and how it is affected by the different ions in the
solution surrounding the surface. We examine the impact of monovalent (NaCl,
KCl, CsCl) and divalent (MgCl2 and CaCl2) salts on platelet-surface
interactions, focusing on the friction enhancement brought by divalent salts
and how the electric double layer (EDL) structure mediates frictional behavior.
We find that divalent cations sit higher on top of the surface, and lead to an
increase in friction, while monovalent cations sit closer to the surface. By
providing a detailed analysis of these interactions, the study offers a novel
framework for understanding the role of salts in clay mechanics and highlights
opportunities to design environmentally friendly stabilizers as alternatives to
traditional lime and cement.

</details>


### [169] [Integrating Newton's Laws with deep learning for enhanced physics-informed compound flood modelling](https://arxiv.org/abs/2507.15021)
*Soheil Radfar,Faezeh Maghsoodifar,Hamed Moftakhari,Hamid Moradkhani*

Main category: physics.geo-ph

TL;DR: ALPINE是一种基于物理信息的神经网络框架，用于复合洪水建模，通过强制完整的浅水动力学，显著提高了预测准确性。


<details>
  <summary>Details</summary>
Motivation: 解决传统流体动力学模型计算资源需求高和机器学习模型物理一致性不足的问题。

Method: 开发ALPINE框架，结合卷积编码器-解码器架构和ConvLSTM，使用复合损失函数平衡数据保真度和物理约束。

Result: 在六次历史风暴事件中，ALPINE显著降低了预测误差，提高了水表面高度和流速的预测能力。

Conclusion: ALPINE是一种物理一致的模拟器，适用于复合洪水预测和大规模风险分析，为沿海应急管理提供了可靠工具。
...
### [170] [Bayesian Surface Wave Inversion for 3D Shear Wave Velocity Structure Beneath the British Isles: comparing Direct-3D Variational Inversion to Two-step (2D+1D) Inversion Methods](https://arxiv.org/abs/2507.15390)
*Xuebin Zhao,Lily Irvin,Erica Galetti,Andrew Curtis*

Main category: physics.geo-ph

TL;DR: 比较了两种两步2D+1D反演方法和一种直接3D反演方法，发现直接3D方法在保持横向连续性和反演精度上更优。


<details>
  <summary>Details</summary>
Motivation: 研究不同反演方法对三维剪切波速度结构结果的影响，以优化表面波反演技术。

Method: 比较了两种两步2D+1D反演方法（变分反演和蒙特卡洛方法）与一种新的直接3D变分反演方法，并应用于英国群岛地下20km深度的3D速度模型。

Result: 直接3D方法在横向连续性和数据模拟准确性上优于两步方法，且计算成本相近。

Conclusion: 建议采用直接3D反演方法，因其在反演精度和计算效率上更具优势。

Abstract: We test the extent to which surface wave inversion results for
three-dimensional shear wave velocity structure depend on the tomography scheme
employed, by comparing two standard two-step 2D+1D inversion methods which use
variational inversion, Metropolis-Hastings and reversible jump Monte Carlo,
against a direct-3D inversion method. While it is possible to calculate a Monte
Carlo based solution for the 2-step methods since they neglect lateral spatial
correlations, a direct-3D Monte Carlo inversion proved too high-dimensional to
achieve statistical convergence. We therefore created a new variational method
which can solve the direct-3D tomographic problem efficiently. We tested the
methods in an inversion for 3D seismic velocity models of the subsurface of the
British Isles extending to a depth of 20km, given surface Love wave dispersion
data derived from ambient seismic noise. We repeated the tests using a 3D
synthetic velocity model consisting of a checkerboard of lower and higher shear
velocities. The direct-3D and one of the two-step methods used the same order
of computations to achieve apparently acceptable subsurface images. However,
the direct-3D scheme preserved better lateral continuity, and produced
synthetic data simulations that align more closely with observed data than
those from the two-step inversions, thus demonstrating higher inversion
accuracy. The inversion results are consistent with the known geology of the
British Isles, and for the first time provide clear seismologically imaged
evidence that seismic structure related to the Great Glen Fault extends to
depths of at least 9 km. On the basis of these and other previous results, we
suggest that direct-3D inversion schemes should be adopted for surface wave
inversion as they provide improved results at little or no additional
computational cost.
...
### [171] [APTx Neuron: A Unified Trainable Neuron Architecture Integrating Activation and Computation](https://arxiv.org/abs/2507.14270)
*Ravin Kumar*

Main category: cs.NE

TL;DR: APTx Neuron是一种新型神经计算单元，将非线性激活和线性变换整合为单一可训练表达式，提高了计算效率和架构简洁性。


<details>
  <summary>Details</summary>
Motivation: 传统神经元设计需要单独的激活层，增加了计算复杂性和架构冗余，APTx Neuron旨在解决这一问题。

Method: APTx Neuron基于APTx激活函数，形式为$y = \sum_{i=1}^{n} ((\alpha_i + \tanh(\beta_i x_i)) \cdot \gamma_i x_i) + \delta$，所有参数可训练。

Result: 在MNIST数据集上验证，仅用20个周期和约332K参数即达到96.69%的测试准确率。
...
### [172] [Self-Supervised Distillation of Legacy Rule-Based Methods for Enhanced EEG-Based Decision-Making](https://arxiv.org/abs/2507.14542)
*Yipeng Zhang,Yuanyi Ding,Chenda Duan,Atsuro Daida,Hiroki Nariai,Vwani Roychowdhury*

Main category: cs.CE

TL;DR: 提出了一种自监督学习框架SS2LD，用于改进传统高频振荡（HFO）检测器的结果，减少假阳性，无需依赖大量标注数据。


<details>
  <summary>Details</summary>
Motivation: 传统HFO检测器精度低且依赖专家标注，标注数据获取困难且不一致。SS2LD利用传统检测器的输出，通过自监督学习提高检测准确性。

Method: 使用变分自编码器（VAE）进行形态预训练，学习潜在表示并聚类生成弱监督信号，结合真实和增强数据训练分类器。

Result: 在多机构间期iEEG数据集上，SS2LD优于现有方法，提供了一种高效且临床有效的HFO检测策略。

Conclusion: SS2LD为HFO检测提供了一种可扩展、标签高效且临床有效的方法，解决了传统检测器的局限性。
...
### [173] [WebGuard: Building a Generalizable Guardrail for Web Agents](https://arxiv.org/abs/2507.14293)
*Boyuan Zheng,Zeyi Liao,Scott Salisbury,Zeyuan Liu,Michael Lin,Qinyuan Zheng,Zifan Wang,Xiang Deng,Dawn Song,Huan Sun,Yu Su*

Main category: cs.AI

TL;DR: WebGuard是首个用于评估网络代理行为风险的数据集，包含4,939条标注数据，分为SAFE、LOW和HIGH三个风险等级。前沿LLMs在预测行为结果和高风险行为召回率上表现不佳，而微调后的Qwen2.5VL-7B模型显著提升了性能，但仍未达到高可靠性要求。


<details>
  <summary>Details</summary>
Motivation: 随着LLM驱动的自主网络代理快速发展，其潜在风险（如无意或有害行为）亟需有效的安全保障措施。

Method: 提出WebGuard数据集，包含多领域网站的人类标注行为数据，采用三级风险分类，并支持训练和测试分割。通过微调Qwen2.5VL-7B模型提升性能。

Result: 前沿LLMs的预测准确率低于60%，高风险行为召回率低于60%。微调模型将准确率从37%提升至80%，高风险召回率从20%提升至76%。
...
### [174] [InsightX Agent: An LMM-based Agentic Framework with Integrated Tools for Reliable X-ray NDT Analysis](https://arxiv.org/abs/2507.14899)
*Jiale Liu,Huan Wang,Yue Zhang,Xiaoyu Luo,Jiaxiang Hu,Zhiliang Liu,Min Xie*

Main category: cs.AI

TL;DR: 本文提出InsightX Agent，一种基于LMM的框架，用于提升X射线无损检测的可靠性、可解释性和交互性。通过结合SDMSD和EGR工具，实现了高效的多尺度缺陷检测和主动推理，显著提高了检测准确性和可信任度。
...
Method: 提出InsightX Agent框架，利用LMM作为协调中心，结合SDMSD进行多尺度缺陷检测和EGR工具进行主动推理和验证。

Result: 在GDXray+数据集上，InsightX Agent实现了96.35%的F1分数，显著提升了检测的可解释性和可信任度。

Conclusion: InsightX Agent展示了基于LMM的代理框架在工业检测任务中的变革潜力，为可靠和可解释的NDT提供了新方向。
...
### [175] [Chart-R1: Chain-of-Thought Supervision and Reinforcement for Advanced Chart Reasoner](https://arxiv.org/abs/2507.15509)
*Lei Chen,Xuanle Zhao,Zhixiong Zeng,Jing Huang,Yufeng Zhong,Lin Ma*

Main category: cs.AI

TL;DR: Chart-R1是一种基于强化学习微调的图表领域视觉语言模型，用于复杂图表推理。通过程序化数据合成和两阶段训练策略，显著优于现有方法。
...
Result: Chart-R1在公开基准和自建数据集上表现优异，甚至媲美GPT-4o等大型模型。

Conclusion: Chart-R1在图表推理领域具有显著优势，为复杂多模态任务提供了有效解决方案。
...
### [176] [Hyper-spectral Unmixing algorithms for remote compositional surface mapping: a review of the state of the art](https://arxiv.org/abs/2507.14260)
*Alfredo Gimenez Zapiola,Andrea Boselli,Alessandra Menafoglio,Simone Vantini*

Main category: astro-ph.IM

TL;DR: 本文综述了用于遥感图像的数据分析方法，重点关注高光谱图像中地表材料的推断及其丰度和空间分布估计，比较了最成功的方法，并探讨了公开数据集和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 研究目的是解决从高光谱图像中推断地表材料及其分布的问题，为遥感数据分析提供方法支持。

Method: 综述并比较了最成功的高光谱解混方法，分析了最新技术，并系统探讨了公开数据集。

Result: 总结了高光谱解混方法的现状，指出了公开数据集的重要性，并提出了未来研究的建议。

Conclusion: 本文为高光谱图像分析提供了全面的方法综述，明确了未来研究方向，推动了该领域的发展。
...
### [177] [Uncertainty-aware Probabilistic 3D Human Motion Forecasting via Invertible Networks](https://arxiv.org/abs/2507.14694)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在不确定性量化方面表现不佳，而安全关键应用（如人机协作）需要准确的预测和不确定性估计。

Method: ProbHMI使用可逆网络将姿态参数化解耦到潜在空间，并通过预测模块显式建模未来潜在分布。
...
### [178] [Low-Latency Event-Based Velocimetry for Quadrotor Control in a Narrow Pipe](https://arxiv.org/abs/2507.15444)
*Leonard Bauersfeld,Davide Scaramuzza*

Main category: cs.RO

TL;DR: 提出了一种基于实时流场测量的四旋翼无人机闭环控制系统，用于在狭窄管道中悬停和机动飞行，解决了气流扰动问题。
...
Result: 系统在管道横截面横向机动中表现出色，能有效抵消瞬态气动效应，避免碰撞。
...
### [179] [GR-3 Technical Report](https://arxiv.org/abs/2507.15493)
...
<details>
  <summary>Details</summary>
Motivation: 目标是构建通用机器人策略，帮助人类日常生活。

Method: 通过多方面的训练方法，包括与网络规模视觉-语言数据共同训练、基于VR设备收集的人类轨迹数据高效微调，以及机器人轨迹数据的模仿学习。

Result: GR-3在多种挑战性任务上超越了现有基准方法π₀，尤其在长时程和灵巧任务中表现优异。

Conclusion: GR-3是迈向通用机器人发展的重要一步，展示了其在现实世界中的强大潜力。
...
### [180] [Look, Focus, Act: Efficient and Robust Robot Learning via Human Gaze and Foveated Vision Transformers](https://arxiv.org/abs/2507.15833)
*Ian Chuang,Andrew Lee,Dechen Gao,Jinyu Zou,Iman Soltani*

Main category: cs.RO

TL;DR: 论文探讨了将人类主动注视机制引入机器人视觉系统，以提高效率和性能。通过模拟人类头部和眼球运动，结合注视数据，提出了一种基于视觉Transformer的注视引导方法，显著降低了计算开销并提升了任务表现。


<details>
  <summary>Details</summary>
Motivation: 人类视觉通过注视主动引导注意力，而传统机器人视觉系统被动处理图像。研究旨在通过模拟人类注视机制，提升机器人视觉系统的效率和性能。

Method: 结合注视数据，提出了一种基于视觉Transformer的注视引导方法，包括两阶段模型和端到端模型，用于注视预测和动作生成。

Result: 方法显著降低了计算开销，提升了高精度任务的性能，并增强了对抗干扰的鲁棒性。
...
### [181] [LOVO: Efficient Complex Object Query in Large-Scale Video Datasets](https://arxiv.org/abs/2507.14301)
*Yuxin Liu,Yuezhang Peng,Hefeng Zhou,Hongze Liu,Xinyu Lu,Jiong Lou,Chentao Wu,Wei Zhao,Jie Li*

Main category: cs.IR

TL;DR: LOVO是一个高效处理大规模视频数据中复杂对象查询的系统，通过预训练视觉编码器生成紧凑视觉嵌入，结合倒排多索引结构和向量数据库，显著降低查询延迟并提高准确性。


<details>
  <summary>Details</summary>
Motivation: 随着摄像头广泛部署，视频数据激增，现有方法难以高效处理大规模视频中的复杂对象查询，存在适应性不足或高延迟问题。

Method: LOVO使用预训练视觉编码器一次性提取特征，生成视觉嵌入并构建倒排多索引结构；查询时通过近似最近邻搜索和跨模态重排优化结果。

Result: 在真实数据集上，LOVO在复杂查询中表现优异，查询准确率接近最优，搜索延迟降低85倍，同时显著降低索引构建成本。

Conclusion: LOVO为视频分析中的复杂对象查询设定了新标准，提供了一种可扩展且高效的方法，适用于动态环境。
...
### [182] [U-MARVEL: Unveiling Key Factors for Universal Multimodal Retrieval via Embedding Learning with MLLMs](https://arxiv.org/abs/2507.14902)
*Xiaojie Li,Chu Li,Shi-Zhe Chen,Xi Chen*

Main category: cs.IR

TL;DR: 论文提出了一种名为U-MARVEL的统一框架，用于多模态检索任务，通过系统分析嵌入学习和训练策略的关键因素，显著提升了性能。
...
Method: 实现了一个通用的MLLM嵌入学习流程，分析了嵌入生成和训练策略的关键因素，如渐进过渡、困难负样本挖掘和重排序蒸馏。

Result: U-MARVEL在M-BEIR基准测试中显著优于现有方法，并在零样本任务中表现出色。

Conclusion: U-MARVEL展示了在多模态检索任务中的强大泛化能力，为嵌入学习提供了关键见解。
...
### [183] [Generative Distribution Distillation](https://arxiv.org/abs/2507.14503)
*Jiequan Cui,Beier Zhu,Qingshan Xu,Xiaogang Xu,Pengguang Chen,Xiaojuan Qi,Bei Yu,Hanwang Zhang,Richang Hong*

Main category: cs.LG

TL;DR: 提出了一种基于条件生成的知识蒸馏框架GenDD，通过Split Tokenization和Distribution Contraction技术解决了高维优化和标签语义监督不足的问题，在无监督和监督设置下均表现出色。


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏（KD）在高维优化和缺乏标签语义监督时面临挑战，需要一种更稳定的方法。

Method: 提出GenDD框架，结合Split Tokenization（稳定无监督KD）和Distribution Contraction（引入标签监督）。

Result: 在ImageNet验证集上，无监督设置下超越KL基线16.29%；监督设置下ResNet-50达到82.28% top-1准确率。

Conclusion: GenDD在无监督和监督KD中均表现优异，为知识蒸馏提供了新思路。
...
### [184] [The Origin of Self-Attention: From Pairwise Affinity Matrices to Transformers](https://arxiv.org/abs/2507.14560)
*Giorgio Roffo*

Main category: cs.LG

TL;DR: 论文探讨了自注意力机制与更广泛的亲和矩阵计算原则之间的联系，将自注意力视为无限特征选择（Inf-FS）的特例。


<details>
  <summary>Details</summary>
Motivation: 揭示自注意力机制在不同领域（如计算机视觉、自然语言处理和图学习）中的共同数学基础，统一亲和矩阵的应用。

Method: 通过对比自注意力与Inf-FS的亲和矩阵定义和应用方式，分析其共性与差异。

Result: 自注意力是Inf-FS的单跳特例，两者均基于对成对关系的推理，差异在于亲和矩阵的构建和使用方式。

Conclusion: 自注意力是亲和矩阵计算范式的一部分，统一了多种机器学习模型和任务的数学基础。
...
### [185] [CXR-TFT: Multi-Modal Temporal Fusion Transformer for Predicting Chest X-ray Trajectories](https://arxiv.org/abs/2507.14766)
...
TL;DR: CXR-TFT是一个多模态框架，结合稀疏时间点的胸片和临床数据，预测ICU患者的胸片异常，提前12小时提供高精度预测。


<details>
  <summary>Details</summary>
Motivation: 现有胸片分析工具无法捕捉时间动态，限制了其在ICU患者管理中的效用。

Method: 通过视觉编码器生成潜在嵌入，与高频临床数据对齐，利用Transformer模型预测未来胸片嵌入。

Result: 在20,000名ICU患者中，CXR-TFT能提前12小时高精度预测胸片异常。
...
### [186] [Flow Equivariant Recurrent Neural Networks](https://arxiv.org/abs/2507.14793)
*T. Anderson Keller*

Main category: cs.LG

TL;DR: 论文提出了一种扩展等变网络理论的方法，将其应用于时间参数化的序列变换（如RNNs），并展示了这种模型在训练速度、长度和速度泛化方面的优势。


<details>
  <summary>Details</summary>
Motivation: 现有的等变网络理论仅适用于静态变换和前馈网络，限制了其在序列模型（如RNNs）中的应用。本文旨在扩展这一理论，以捕捉时间参数化的自然变换（如视觉运动）。

Method: 通过引入流等变性（flow equivariance），改进RNNs，使其隐藏状态能够以几何结构化的方式对动态刺激进行变换。

Result: 实验表明，改进后的模型在训练速度、长度泛化和速度泛化方面显著优于非等变模型，适用于下一步预测和序列分类任务。

Conclusion: 本文为构建尊重时间参数化对称性的序列模型迈出了第一步，展示了其在现实世界应用中的潜力。
...
### [187] [To Label or Not to Label: PALM -- A Predictive Model for Evaluating Sample Efficiency in Active Learning Models](https://arxiv.org/abs/2507.15381)
...
<details>
  <summary>Details</summary>
Motivation: 传统AL评估方法仅关注最终准确性，无法全面捕捉学习过程的动态特性，PALM旨在填补这一空白。

Method: PALM通过四个参数（可实现准确性、覆盖效率、早期性能和可扩展性）建模AL轨迹，并提供预测性描述。

Result: 实验表明，PALM能准确预测AL性能，并在不同数据集、预算和策略中具有普适性。
...
### [188] [GUI-G$^2$: Gaussian Reward Modeling for GUI Grounding](https://arxiv.org/abs/2507.15846)
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法使用稀疏的二元奖励，忽略了空间交互的连续性，而人类点击行为自然形成高斯分布，因此提出连续高斯建模。

Method: 引入GUI-G²框架，结合高斯点奖励和覆盖奖励，并开发自适应方差机制处理不同元素尺度。

Result: 在多个基准测试中，GUI-G²显著优于UI-TARS-72B，最高提升24.7%。

Conclusion: 连续高斯建模为GUI交互任务提供了更强的鲁棒性和泛化能力，确立了空间推理的新范式。
...
### [189] [Diffusion Beats Autoregressive in Data-Constrained Settings](https://arxiv.org/abs/2507.15857)
...
### [190] [Prompt-aware of Frame Sampling for Efficient Text-Video Retrieval](https://arxiv.org/abs/2507.15491)
*Deyu Zhang,Tingting Long,Jinrui Zhang,Ligeng Chen,Ju Ren,Yaoxue Zhang*

Main category: cs.MM

TL;DR: ProCLIP提出了一种用户中心的框架，通过动态帧采样和两阶段候选剪枝策略，显著提升了文本-视频检索的效率，同时保持高准确性。
...
Method: 采用提示感知的帧采样策略和两阶段候选剪枝策略（粗过滤和CLIP驱动的细粒度重排序）。

Result: 实验显示ProCLIP在MSR-VTT数据集上实现了75.3%的延迟降低，同时R@1达到49.0。

Conclusion: ProCLIP在效率和准确性上均优于现有方法，适用于边缘设备上的实时应用。
...
### [191] [A Multi-Modal IoT Node for Energy-Efficient Environmental Monitoring with Edge AI Processing](https://arxiv.org/abs/2507.14165)
*Philip Wiese,Victor Kartsch,Marco Guermandi,Luca Benini*

Main category: eess.SP

TL;DR: 论文介绍了一种紧凑型多模态MCU环境物联网节点，集成了11种传感器，支持实时边缘处理高级ML模型，显著提升了环境监测的效率和能耗表现。


<details>
  <summary>Details</summary>
Motivation: 当前环境监测IoT平台传感器种类有限且计算能力不足，无法支持高级ML/AI算法的边缘部署，限制了环境监测的全面性和效率。

Method: 开发了一种紧凑型多模态MCU环境IoT节点，集成11种传感器，并采用GAP9超低功耗SoC，实现实时边缘处理。通过YOLOv5占用检测管道和智能室内空气质量监测设置验证性能。

Result: 实现了42%的能耗节省，单次充电可支持长达143小时的运行，为高效AI驱动的边缘预测奠定了基础。

Conclusion: 该平台为创新的环境监测应用（如预测性室内空气质量控制）提供了高效、节能的边缘AI解决方案。
...
### [192] [Large-scale compressive microscopy via diffractive multiplexing across a sensor array](https://arxiv.org/abs/2507.14437)
...
TL;DR: 提出了一种新型显微镜，通过传感器阵列和衍射光学元件实现快照千兆像素级成像，显著提升时空吞吐量。


<details>
  <summary>Details</summary>
Motivation: 传统显微镜在空间分辨率、视场和帧率之间存在权衡，改进一项通常需牺牲其他性能。本研究旨在克服这一限制。

Method: 使用48个传感器阵列捕获数据，结合点扩散函数工程和压缩感知算法填补传感器间隙信息，设计衍射光学元件生成分布式PSF。

Result: 实现了约3微米分辨率、>5.2平方厘米视场、120帧/秒的性能，总时空吞吐量达252亿像素/秒。

Conclusion: 该显微镜在结构成像和功能荧光成像中表现出色，展示了其多功能性和高吞吐量优势。
...
### [193] [Breaking the Illusion of Security via Interpretation: Interpretable Vision Transformer Systems under Attack](https://arxiv.org/abs/2507.14248)
*Eldor Abdukhamidov,Mohammed Abuhamad,Simon S. Woo,Hyoungshick Kim,Tamer Abuhmed*

Main category: cs.CR

TL;DR: 本文提出了一种名为AdViT的攻击方法，能够同时欺骗视觉Transformer模型及其解释模型，实验证明其在白盒和黑盒场景下均具有高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉Transformer模型及其解释模型被认为安全性高，但成功攻击可能导致严重后果，而现有研究未充分考虑解释模型的影响。

Method: 提出AdViT攻击方法，生成能同时误导Transformer模型及其解释模型的对抗样本。

Result: AdViT在白盒和黑盒场景下攻击成功率分别达100%，且生成的对抗样本难以被检测。

Conclusion: 研究揭示了Transformer模型即使结合解释模型仍存在漏洞，AdViT攻击方法有效且隐蔽。
...
### [194] [Design of an Edge-based Portable EHR System for Anemia Screening in Remote Health Applications](https://arxiv.org/abs/2507.15146)
...
TL;DR: 本文提出了一种便携式、边缘支持的电子健康记录平台，专为资源有限的环境设计，支持离线操作、安全数据管理和模块化诊断集成。通过贫血筛查模块展示了其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决远程医疗系统在资源有限环境中的互操作性差、缺乏离线支持以及对昂贵基础设施依赖的问题，提升前线医疗工作者的效率。

Method: 开发了一个基于嵌入式设备的平台，支持AES-256加密本地存储和可选云同步。集成了基于随机森林的贫血筛查模块和YOLOv8n的指甲床检测器。

Result: 贫血筛查模块的测试RMSE为1.969 g/dL，MAE为1.490 g/dL，灵敏度达79.2%。YOLOv8n检测器优化后延迟从46.96 ms降至21.50 ms，mAP@0.5保持0.995。
...
### [195] [In-Depth and In-Breadth: Pre-training Multimodal Language Models Customized for Comprehensive Chart Understanding](https://arxiv.org/abs/2507.14298)
*Wan-Cyuan Fan,Yen-Chun Chen,Mengchen Liu,Alexander Jacobson,Lu Yuan,Leonid Sigal*

Main category: cs.CL

TL;DR: ChartScope是一种针对多样化图表类型优化的LVLM，通过高效数据生成管道和双路径训练策略，显著提升图表理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖有限图表类型的配对数据，缺乏针对性预训练，限制了模型的泛化能力和数据理解。

Method: 提出高效数据生成管道和双路径训练策略，结合底层数据推理。

Result: 实验显示ChartScope在多样化图表类型上显著提升理解能力。

Conclusion: ChartScope为图表理解提供了新方法，并通过新基准ChartDQA进行评估。
...
### [196] [Smart Eyes for Silent Threats: VLMs and In-Context Learning for THz Imaging](https://arxiv.org/abs/2507.15576)
*Nicolas Poggi,Shashank Agnihotri,Margret Keuper*

Main category: cs.CL

TL;DR: 论文提出了一种基于上下文学习（ICL）和视觉语言模型（VLM）的太赫兹（THz）图像分类方法，无需微调即可在低数据量下提升分类效果和可解释性。


<details>
  <summary>Details</summary>
Motivation: 太赫兹成像在安全筛查和材料分类等应用中具有潜力，但受限于标注数据少、分辨率低和视觉模糊等问题，传统分类方法效果不佳。

Method: 通过模态对齐的提示框架，将两种开放权重的视觉语言模型（VLM）适配到太赫兹领域，并在零样本和单样本设置下评估。
...
### [197] [GALE: Leveraging Heterogeneous Systems for Efficient Unstructured Mesh Data Analysis](https://arxiv.org/abs/2507.15230)
*Guoxi Liu,Thomas Randall,Rong Ge,Federico Iuricich*

Main category: cs.DC

TL;DR: 提出了一种名为GALE的新型异构任务并行数据架构，用于优化非结构化网格的可视化算法性能。


<details>
  <summary>Details</summary>
Motivation: 非结构化网格的复杂连接性导致计算和存储成为瓶颈，现有方法受限于CPU资源竞争。

Method: 采用异构CPU-GPU系统，将网格连接性计算卸载到GPU线程，CPU专注于可视化算法。

Result: 在20核CPU和NVIDIA V100 GPU上，GALE比现有技术快2.7倍，且内存高效。

Conclusion: GALE通过异构任务并行显著提升了非结构化网格分析的性能。
...
### [198] [Towards a Proactive Autoscaling Framework for Data Stream Processing at the Edge using GRU and Transfer Learning](https://arxiv.org/abs/2507.14597)
*Eugene Armah,Linda Amoako Bannning*

Main category: cs.DC

TL;DR: 论文提出了一种三步解决方案，用于主动边缘流处理自动扩展问题，包括负载预测、迁移学习和动态并行度调整。


<details>
  <summary>Details</summary>
Motivation: 边缘流处理面临快速工作负载波动，现有方法无法有效应对资源分配问题，可能导致性能下降或资源浪费。

Method: 使用GRU神经网络预测上游负载，结合迁移学习框架处理离线与在线数据差异，并通过动态并行度调整实现资源优化。

Result: GRU模型在真实数据集上表现出色，SMAPE值低至1.3%，优于CNN、ARIMA和Prophet模型。

Conclusion: 提出的方法在负载预测和资源分配方面优于现有技术，同时减少了计算开销。
...
### [199] [On Splitting Lightweight Semantic Image Segmentation for Wireless Communications](https://arxiv.org/abs/2507.14199)
*Ebrahim Abu-Helalah,Jordi Serra,Jordi Perez-Romero*

Main category: cs.NI

TL;DR: 提出一种新的语义通信方法，通过在资源受限的发射器和接收器之间分割语义图像分割过程，减少传输数据量和计算负载，同时保持分割精度。
...
Method: 将语义图像分割过程分割到资源受限的发射器和接收器之间，以减少传输数据量和发射器的计算负载。

Result: 实验结果显示，传输比特率减少72%，发射器计算负载降低19%以上。
