...
Motivation: 重新审视并振兴自回归范式，以实现高质量的图像生成及其他任务，同时避免现有方法的限制。

Method: 从头开始训练，采用统一的标记化方案和高效的解码策略（如推理时间缩放和推测性Jacobi采样）。
...
<details>
  <summary>Details</summary>
Motivation: 传统体育视频分析模型计算量大且缺乏对运动动作的细粒度理解，难以捕捉关键的运动阶段（如准备、执行和后续动作）。

Method: 采用DWT-VGG16-LDA关键帧提取机制和V-DWT-JEPA2编码器，结合自监督学习和LLM解码器，生成详细的运动动作描述。

Result: 在NSVA篮球数据集上，SV3.3B在文本生成和体育特定评估指标上表现优异，优于GPT-4o等大型闭源模型，计算需求更低。

Conclusion: SV3.3B在体育动作描述生成上表现出色，信息密度和动作复杂性显著提升，适用于高效的体育分析。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在处理复杂提示（多主体及属性）时表现不佳，受人类绘图过程启发，提出分阶段生成方法。

Method: 将复杂提示分解为子提示序列，利用自注意力机制控制全局布局，再通过交叉注意力机制和质心对齐损失细化属性绑定。

Result: 在T2I-CompBench和新构建的风格组合基准上，Detail++显著优于现有方法。

Conclusion: Detail++通过分阶段生成和属性绑定优化，有效提升复杂场景下的图像生成质量。
...
Result: 模型性能差异显著，CLIP-based框架能高效选择最优模型。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在皮肤癌筛查中的应用可能带来偏见，评估和改进公平性至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 传统Transformer的自注意力机制在高分辨率图像修复中计算成本高，而Restormer的通道自注意力可能忽略局部细节。DiNA作为一种替代方案，在高级视觉任务中表现良好，但在经典去模糊任务中直接应用效果不佳。

Method: 提出DiNAT-IR，结合滑动窗口注意力和混合膨胀因子，引入通道感知模块以整合全局上下文，同时保持像素级精度。
...
Method: AFR模块通过语义先验细化高分辨率特征，并整合高频组件以捕捉细粒度结构，同时通过不确定性驱动的注意力平衡局部与全局信息。
...
Method: 通过收集11名老年人六周虚拟学习会话的数据，提取面部、手部和身体关节特征，并标注参与状态、情感行为及上下文信息。

Result: 构建了OPEN数据集，训练多种机器学习模型，最高达到81%的参与度识别准确率。
...
Method: 使用五种YOLO变体（v5-v12）在自定义数据集上训练，通过视频分析和规则逻辑分类行为。

Result: YOLOv8s表现最佳，晒太阳检测可靠，但捕猎检测因蟋蟀检测不准确而较差。

Conclusion: 该系统为爬行动物行为监测提供了可扩展的解决方案，未来需优化蟋蟀检测以提高准确性。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像数据中的标签偏差（尤其是针对特定子群的偏差）是医学AI系统公平性中未被充分研究的问题。

Method: 使用EMBED数据集训练深度学习模型，模拟可分离和不可分离子群的标签偏差，分析其对模型特征和性能的影响。

Result: 标签偏差导致模型特征表示显著变化，且变化程度受子群大小和可分离性影响；验证集标签是否干净对子群性能有显著差异。
...
Result: 提出的方法在计算机视觉任务中优于现有解决方案，提高了收敛性。

Conclusion: 该研究为Grassmannian特征的配准问题提供了新的优化框架，具有全局最优解和表示无关性。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏多样性，无法支持泛化性检测方法的发展。

Method: 基于Celeb-DF数据集扩展，包含三种伪造场景和22种生成方法。

Result: 新数据集挑战现有检测方法，揭示其局限性。
...
<details>
  <summary>Details</summary>
Motivation: 3D场景修复因3D结构的不规则性和多视角一致性要求而具有挑战性，现有方法难以满足需求。

Method: 结合自动掩模优化（高斯场景过滤和反向投影）和区域不确定性引导的细粒度优化策略。
...
Result: 在多个基准数据集上表现优于现有方法，定量和定性评估均显示优势。

Conclusion: 双向扩散机制和模块设计有效提升了图像增强质量，适应多样化退化场景。
...
Method: 系统回顾骨骼情感识别技术，分类为姿势和步态方法，分析四种技术范式，并比较代表性工作。

Result: 总结了现有数据集和方法，提出统一分类法，并展示基准测试结果。
...
<details>
  <summary>Details</summary>
Motivation: 鸡肉加工行业需要自动化检测系统，但真实数据的获取和标注成本高昂，限制了深度学习模型的开发。

Method: 提出生成合成图像和自动标注的流程，并引入新的真实图像基准数据集，评估合成数据对分割模型的性能影响。
...
Method: ViGText将图像分块，构建图像和文本图，通过GNN整合分析，结合空间和频域多级特征提取。

Result: 实验显示ViGText显著提升泛化性（F1分数从72.45%升至98.32%），鲁棒性（召回率提升11.1%），对抗攻击性能下降小于4%。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI生成的视频在单场景短片段上表现良好，但在多场景长视频中缺乏连贯的场景转换能力。
...
Result: 在公开数据集上，TextSAM-EUS自动提示达到82.69% Dice和85.28% NSD，手动提示达83.10% Dice和85.70% NSD，优于现有方法。

Conclusion: TextSAM-EUS首次将提示学习引入SAM医学图像分割，提供高效、鲁棒的EUS自动分割方案。
...
<details>
  <summary>Details</summary>
Motivation: LULC制图对城市和资源规划至关重要，尤其是在发展智能和可持续城市中。研究旨在评估先进技术在此领域的应用效果。

Method: 采用LUT大气校正处理Cartosat多光谱图像，结合监督学习（DeeplabV3+）和半监督学习（CPS）模型，并通过动态加权优化CPS。

Result: 案例研究表明，这些技术能有效捕捉城市扩张、绿地减少和工业区扩展等变化，为城市规划提供实用工具。

Conclusion: 该技术组合在LULC制图中表现出高准确性和实用性，适用于城市规划和政策制定。
...
Method: 使用序列到序列Transformer模型，结合预训练模型提取的视觉、听觉和语言特征，通过双交叉注意力机制整合先验脑状态、当前刺激和叙事摘要信息。

Result: 模型在分布内和分布外数据上均表现出色，验证了时间感知多模态序列建模的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法如MC Dropout仅关注模型或数据不确定性，无法与OoD检测的语义目标对齐。

Method: 引入基于自由能的密度估计器（参数化为Beta分布）和集成后验网络的损失函数，直接估计不确定性。

Result: 在Fishyscapes等真实基准测试中验证了方法的有效性。

Conclusion: 该方法提供了语义明确且计算高效的不确定性感知分割解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析中，曲线弯曲度的精确量化对疾病辅助诊断和病理评估至关重要。
...
Method: 提出WaveMamba Fusion Block（WMFB），包括低频Mamba融合块（LMFB）和高频特征增强策略，结合DWT和IDWT实现跨模态融合。
...
TL;DR: 本文提出了一种基于YOLOv5优化的资源高效实时目标检测与分类系统，适用于FPGA边缘计算平台，实现了高精度和低功耗。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D LiDAR语义分割因域转移（如传感器类型、地理位置）导致的性能下降问题，避免手动标注目标数据的高成本。

Method: 1. 使用无监督对比学习预训练骨干网络，学习域不变特征；2. 引入多模型伪标签策略，通过硬投票生成高质量伪标签；3. 利用伪标签微调预训练网络。
...
Method: 提出Diff-UMamba架构，结合UNet和mamba机制，并引入噪声抑制模块（NRM）以减少噪声激活。
...
<details>
  <summary>Details</summary>
Motivation: 当前金属材料显微图像分析依赖监督方法，需为每个新数据集重新训练且标注数据少时表现不稳定。自监督学习虽有望解决此问题，但现有方法仍需大规模数据集。MatSSL旨在克服这一限制。
...
Method: LEAF在微调阶段直接预测分割图以减少结果方差，并通过特征蒸馏对齐卷积层与transformer编码器的特征。

Result: 实验表明，LEAF提升了原始扩散模型在多种疾病分割数据集上的性能。

Conclusion: LEAF高效且无需额外计算资源，适用于医学图像分割任务。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有3D TTA方法在空间域优化计算成本高且需要额外训练数据的问题。

Method: 将点云表示为图并通过图傅里叶变换转换到谱域，仅优化低频分量，结合自训练策略迭代优化。
...
Result: DATA在三个数据集上达到最优性能，对通信延迟和姿态误差具有鲁棒性。

Conclusion: DATA通过系统对齐特征和增强语义表示，显著提升了协作感知的性能。
...
Method: 提出高斯群压缩器进行非均匀量化，并利用两阶段扩散模型进行真实世界重建。

Result: 在多种数据稀缺场景下，该方法显著提升了检测性能。

Conclusion: 该方法有效解决了红外小目标检测在实际应用中的鲁棒性问题。
...
Result: 龋齿显著增加肺癌风险，尤其是鳞状细胞肺癌（OR=2.880），部分由肺功能下降介导；牙周炎无显著影响。

Conclusion: 龋齿是肺癌风险的潜在因果因素，建议将口腔护理和肺功能监测纳入癌症预防策略。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型多模态模型在多模态任务中表现优异，但其目标检测能力与专用检测器存在显著差距，因此需要一种无需额外检测模块的解决方案。

Method: 通过数据分布调整和推理优化提升召回率，并重新组织指令对话以增强检测能力。

Result: 实验证明LMM-Det有效，展示了大型多模态模型具备目标检测能力。

Conclusion: LMM-Det是一种简单有效的方法，无需专用检测模块即可实现目标检测。
...
Method: 整合多个公开数据集，采用五种半监督分割算法，在卷积网络和Transformer架构上评估，并提出ECG专用训练配置和增强策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统核范数（NN）在矩阵恢复问题中仅利用全局低秩结构，而结合局部信息的方法需要调参且缺乏理论保证。MNN旨在解决这些问题。

Method: 定义MNN族范数，通过对矩阵进行适当变换后应用核范数，联合捕获局部和全局信息，无需调参。

Result: 在Robust PCA和矩阵补全任务中，MNN提供了精确的理论恢复保证，实验验证了其有效性。

Conclusion: MNN框架灵活通用，适用于多种变换，为结构化低秩恢复提供了一种统一且高效的方法。
...
Method: 提出GVCCS数据集，包含地面可见光相机记录的尾迹视频序列，并开发了一个统一的深度学习框架进行尾迹分析。

Result: 数据集包含122个视频序列（24,228帧），每个尾迹均标注并追踪，为模型校准提供了高质量数据。
...
TL;DR: 通过将频率信息嵌入声谱图，提出了一种改进鸟类声音分类的方法，显著提升了分类性能。
...
Result: 实验显示，该方法在F1、ROC-AUC和CMAP上分别提升了7.3%、6.2%和6.6%，超越了BirdCLEF 2024的冠军模型。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够通过跨视角的知识传递与整合学习，但现有MLLMs在此能力上尚未被探索。

Method: 构建EgoExoBench基准测试，包含7,300多个问答对，覆盖11个子任务，分为语义对齐、视角关联和时间推理三大挑战。
...
TL;DR: VB-Mitigator是一个开源框架，旨在简化和统一视觉偏置缓解技术的开发、评估和比较分析，以推动公平性计算机视觉模型的研究。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉模型中的偏置问题导致不公平、不可靠和泛化能力差的AI系统，且现有研究因碎片化实现和评估不一致而进展缓慢。

Method: 开发了VB-Mitigator框架，整合了12种缓解方法和7个基准数据集，支持扩展性，并提供最佳评估实践和性能比较。

Result: VB-Mitigator为研究社区提供了一个统一的平台，便于开发和评估偏置缓解方法，并进行了全面的性能对比。

Conclusion: VB-Mitigator有望加速公平性计算机视觉模型的研究，为社区提供基础代码库和评估标准。
...
TL;DR: 提出了一种新型的可变形卷积模块，通过注意力机制和前馈网络学习偏移量，实现全局特征变形，并应用于眼底血管分割任务，取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 传统可变形卷积难以捕捉长距离全局特征，且卷积核大小与学习网络耦合。本文旨在解决这些问题，并应用于具有复杂全局自相似特征的眼底血管分割。

Method: 提出了一种可插拔的可变形卷积模块，通过学习亚像素位移场和自适应变形特征图，实现全局特征变形和卷积核大小与学习网络的解耦。

Result: 在眼底血管分割任务中，提出的GDCUnet模型在公开数据集上达到了最先进性能，且消融实验验证了模块的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 探索AI与人类在物理任务中的协作潜力，尤其是在日常和专业领域，以提升任务完成效率和减少错误。

Method: 引入评估框架和多模态数据集，开发AR辅助的AI代理，通过人类实验验证其效果。

Result: 实验表明，AI辅助协作显著提升了任务完成率。
...
TL;DR: 提出一种新颖的一阶段架构，直接从单张RGB图像和文本描述生成连续坐标空间中的姿态，避免中间表示或基于令牌的生成，显著优于现有方法。
...
Method: 采用相对运动预测机制和统一占位符令牌方法，直接在姿态坐标上操作，保持空间关系，实现训练和推理行为一致的单次前向生成。

Result: 在Penn Action和F-PHAB数据集上的实验表明，该方法在长期生成任务中显著优于量化和自回归方法。

Conclusion: 提出的方法通过消除中间表示，实现了更高效和一致的姿态生成，特别适用于长期任务。
...
Method: 构建OpenHumanBRDF数据集，设计渐进式训练模型HumanMaterial，引入CPR损失优化材料估计。

Result: 在OpenHumanBRDF数据集和真实数据上实现最优性能。

Conclusion: 提出的方法显著提升了材料估计的真实感，尤其在皮肤渲染方面表现突出。
...
TL;DR: Iwin Transformer是一种无需位置嵌入的分层视觉Transformer，通过创新的交错窗口注意力和深度可分离卷积，可直接从低分辨率到高分辨率进行微调，克服了Swin Transformer的局限性。
...
Conclusion: Iwin Transformer在视觉任务中具有竞争力，其核心模块和方法为未来研究（如视频生成）提供了新思路。
...
TL;DR: 论文提出了一种基于V-JEPA的自监督学习方法，用于解决超声图像数据标注困难的问题，并通过引入3D定位辅助任务提升了ViT模型的局部性理解能力，显著提高了分割性能。


<details>
  <summary>Details</summary>
Motivation: 超声图像数据标注困难，且噪声大、对比度低，传统方法需要大量时间和临床专业知识。自监督学习（SSL）可以利用未标注数据学习有用表示，但现有方法在医学小数据集上表现不佳。

Method: 采用V-JEPA框架进行自监督学习，避免像素级重建或负样本；提出3D定位辅助任务，增强ViT模型的局部性理解能力。

Result: 实验表明，V-JEPA结合辅助任务显著提升了分割性能，在使用100%和10%训练数据时分别提高了3.4%和8.35%。

Conclusion: V-JEPA结合3D定位辅助任务是一种有效的自监督学习方法，适用于超声视频数据，尤其在标注数据有限的情况下表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 头部姿态估计在计算机视觉应用中至关重要，但现有数据集常存在标注不准确的问题，且传统方法难以处理有限数据。

Method: 将头部姿态估计建模为回归问题，利用张量分解将欧拉角分解到不同子空间，并通过余弦曲线建模流形。生成精确的2D数据集进行训练。

Result: 方法在有限数据下实现了实时性能，准确捕捉了面部关键点的旋转特性，预测速度快。

Conclusion: NLML-HPE在有限数据下表现优异，解决了标注不准确问题，代码和模型已开源。
...
TL;DR: 论文提出了PDB-Eval基准，用于评估多模态大模型（MLLMs）在驾驶行为理解与推理中的表现，通过PDB-X和PDB-QA两部分提升模型在驾驶领域的零样本性能。
...
Method: CRUISE采用分解的高斯泼溅技术，动态交通参与者被分解为可编辑的高斯表示，支持场景的灵活修改和增强。

Result: CRUISE能够高保真重建V2X场景，提升3D检测和跟踪性能，并生成极端案例。
...
Method: 使用预训练视觉基础模型作为冻结特征提取器，结合Q-Former架构控制重建序列长度，并引入感知损失指导重建。
...
TL;DR: 论文提出了一种改进的疟疾数据集，用于支持深度学习目标检测模型的训练，并通过实验验证了其有效性。
...
Conclusion: 自动标注细化结合手动修正可生成高质量训练数据，提升模型性能，改进后的数据集已公开。
...
TL;DR: 提出了一种名为Rein-EAD的主动防御框架，通过自适应探索和交互提升3D对抗环境中的感知鲁棒性。
...
Conclusion: Rein-EAD是一种适用于复杂现实任务的高效主动防御框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前在线生成的高清地图可靠性不确定，而现有方法对地图不确定性在轨迹预测中的具体应用场景缺乏深入分析。

Method: 提出了Proprioceptive Scenario Gating方法，根据车辆未来运动状态动态整合地图不确定性，并引入Covariance-based Map Uncertainty方法。

Result: 在nuScenes数据集上，方法比现有技术提升了23.6%的轨迹预测性能。

Conclusion: 该方法显著提升了轨迹预测性能，并提供了对不确定性应用场景的可解释性。
...
TL;DR: SemBA-FAST是一个基于语义和概率的视觉注意力模型，用于预测目标搜索任务中的人类视觉注意力，性能优于基线和其他自上而下方法。


<details>
  <summary>Details</summary>
Motivation: 研究目标是利用深度学习和人类扫描路径数据，开发一个更接近人类视觉注意力的模型，以提升目标搜索任务的性能。

Method: SemBA-FAST结合深度目标检测和概率语义融合机制，动态生成注意力图，并通过预训练检测器和人工注视点更新知识，逐步优化注视预测。

Result: 在COCO-Search18数据集上，SemBA-FAST的注视序列与人类真实扫描路径高度匹配，性能优于基线和其他自上而下模型，部分情况下甚至与扫描路径信息模型竞争。

Conclusion: SemBA-FAST展示了语义-注视概率框架在模拟人类注意力方面的潜力，对实时认知计算和机器人技术有重要应用价值。
...
TL;DR: 提出了一种新指标，用于跨模态模型的稀疏自编码器（SAE）特征定量比较，并研究了视觉、文本和多模态编码器的共享特征。


<details>
  <summary>Details</summary>
Motivation: 解决现有研究仅限同模态模型比较的局限性，探索跨模态模型的共享特征。

Method: 提出新指标和Comparative Sharedness量化方法，对21种编码器进行比较研究。

Result: 发现视觉特征在多模态模型中与文本编码器共享，表明文本预训练的影响。

Conclusion: 研究为跨模态模型特征共享提供了新视角，并量化了其程度。
...
Conclusion: 该方法有效解决了遥感数据中稀有目标检测问题，并提供了甲烷排放的估算工具。
...
TL;DR: 研究利用基础模型进行语义对象分割，提出基于注视点的提示方法改进SAM模型，在真实世界数据上取得显著效果提升。
...
Conclusion: GaussianFusionOcc通过高效的多模态融合和高斯表示，实现了更精确和可扩展的占用预测。
...
TL;DR: 提出IntentVCNet，通过提示组合策略和参数高效适配器，增强大型视觉语言模型（LVLM）在视频序列中建模空间细节的能力，实现意图导向的精细控制视频描述生成。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM在时空理解上存在差距，难以直接响应用户指令实现精细空间控制，阻碍了意图导向的视频描述生成。

Method: 提出提示组合策略建模用户意图与视频序列的隐式关系，并设计参数高效适配器增强视觉上下文中的对象语义信息。

Result: 方法在多个开源LVLM上取得最优效果，并在IntentVC挑战赛中获亚军。

Conclusion: IntentVCNet通过统一时空理解知识，显著提升了LVLM在意图导向视频描述任务中的表现。
...
TL;DR: COT-AD是一个用于棉花作物分析的计算机视觉数据集，包含25,000张图像和5,000张标注图像，支持分类、分割、图像修复和增强等任务。
...
Method: 提出自适应批量大小调度和分层生成策略，包括基于聚类的多样性搜索（粗尺度）和基于重采样的潜力选择（细尺度）。

Result: 在Infinity模型上实现了8.7%的GenEval分数提升（从0.69到0.75）。

Conclusion: 早期结构特征对最终质量有显著影响，且重采样效果在不同生成尺度上存在差异。
...
<details>
  <summary>Details</summary>
Motivation: 现有MVS模型在大规模无位姿重建任务中受限于内存和精度，无法有效处理数百张户外图像。

Method: 将输入图像划分为子集，通过概率Procrustes映射将子图对齐到全局空间，并结合3DGS联合优化几何和位姿。使用软垃圾桶机制拒绝不确定对应关系，并通过联合优化框架细化场景和位姿。

Result: 在Waymo和KITTI数据集上实现了高精度重建，为无位姿3DGS重建设定了新标杆。

Conclusion: 该方法通过全局对齐和联合优化，显著提升了大规模无位姿图像序列的重建精度和效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决MRI和iUS在术中配准中的模态差异问题，如外观、分辨率和视野的不同。

Method: 采用患者特定的匹配-合成方法生成合成iUS，使用对比训练学习共享描述符空间，结合概率关键点检测和动态硬负样本挖掘的课程三元组损失。
...
TL;DR: VideoMind是一个视频为中心的跨模态数据集，用于深度视频内容认知和多模态特征表示，包含10.3万视频样本，每样本配有音频和三层文本描述（事实、抽象、意图）。
...
Method: 数据集包含10.3万视频样本，每样本配有三层文本描述（事实、抽象、意图），采用Chain-of-Thought方法生成意图表达。

Result: 建立了包含3000手动验证样本的基准测试，评估模型（如InternVideo、VAST等）在深度视频理解任务上的表现。
...
TL;DR: 提出了一种基于深度学习的年龄和性别分类方法，通过共享表示学习提升性能，实验结果显示性别分类准确率95%，年龄估计误差5.77年。


<details>
  <summary>Details</summary>
Motivation: 提升定向广告的效果，利用面部特征中年龄和性别的相关性。

Method: 设计了一种优化的CNN架构，学习共享表示，使用大规模多样化数据集训练。
...
TL;DR: 论文提出了一种新的去融合方法，能够从潜在空间分解融合图像，解决了现有方法在未见过融合技术和人脸风格时的局限性，并在真实人脸融合图像上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有去融合方法存在融合复制问题，且假设训练和测试融合使用相同技术。本文旨在克服这些问题，提供更通用的去融合方案。

Method: 通过潜在空间分解融合图像，支持处理未见过融合技术和人脸风格的图像。训练基于合成人脸融合，测试用于真实人脸融合。

Result: 方法显著优于现有技术，生成高保真度的去融合人脸图像。

Conclusion: 该方法为融合攻击检测提供了更强大的去融合能力，适用于多种融合技术和人脸风格。
...
TL;DR: DMDX通过对抗性蒸馏预训练和ADM微调，改进了DMD2的不足，在一步蒸馏任务中表现更优且更高效。
...
Method: 提出ADM框架，利用扩散判别器在潜在空间中对齐真实和虚假分数估计，并结合对抗性蒸馏和混合判别器。

Result: 在SDXL上的一步蒸馏表现优于DMD2，GPU时间更少；在多步蒸馏任务中为图像和视频合成设定了新基准。

Conclusion: DMDX通过对抗性方法显著提升了蒸馏效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: Transformer在3D语义分割中表现强大，但二次复杂度限制了其长距离依赖建模；Mamba虽高效但特征提取能力不足。结合两者优势成为挑战。

Method: 提出HybridTM架构，首次整合Transformer和Mamba，并设计Inner Layer Hybrid Strategy，在更细粒度上结合注意力与Mamba。
...
TL;DR: 论文提出了一种名为DRWKV的低光照图像增强模型，通过结合GER理论、Evolving WKV Attention和Bi-SAB模块，显著提升了边缘保真度和视觉自然性，并在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 低光照图像增强在极端光照退化下难以保持边缘连续性和细节结构，现有方法存在局限性。

Method: 1. 提出Global Edge Retinex (GER)理论解耦光照与边缘结构；2. 引入Evolving WKV Attention捕捉空间边缘连续性；3. 设计Bi-SAB模块和MS2-Loss对齐亮度和色度特征。

Result: 在五个低光照图像增强基准测试中，DRWKV在PSNR、SSIM和NIQE指标上领先，同时计算复杂度低。

Conclusion: DRWKV不仅提升了图像增强效果，还在低光照多目标跟踪任务中表现出泛化能力。
...
Method: 基于ScenethesisLang的中间表示，分阶段合成3D软件，支持独立验证、目标修改和系统约束满足。
...
TL;DR: 论文提出了一种名为SIDA的零样本域适应方法，通过合成图像替代文本描述，显著提升了适应效率和效果。
...
Method: 通过生成合成图像（源域风格图像经翻译后反映目标域风格），并引入Domain Mix和Patch Style Transfer模块，建模真实世界变化。

Result: 在多种零样本适应场景中表现优异，尤其在挑战性领域，同时显著减少适应时间。

Conclusion: SIDA方法通过合成图像和模块设计，实现了高效且有效的零样本域适应。
...
TL;DR: 论文介绍了一个用于识别文本到图像模型中提示艺术家的基准测试，包含1.95M图像和110位艺术家，评估了多种方法，发现不同方法在不同场景下表现各异。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过图像识别出文本到图像模型中使用的艺术家提示，以促进对模型的负责任管理。

Method: 构建了一个包含1.95M图像的数据集，覆盖110位艺术家，并评估了特征相似性、对比风格描述符、数据归因、监督分类器和少样本原型网络等方法。

Result: 监督和少样本模型在已知艺术家和复杂提示下表现优异，风格描述符在艺术家风格明显时表现更好，多艺术家提示最具挑战性。

Conclusion: 基准测试揭示了改进空间，并提供了公开数据集以推动文本到图像模型的负责任管理研究。
...
TL;DR: Captain Cinema是一个用于生成短电影的框架，通过关键帧规划和视频合成实现叙事连贯和视觉一致性。


<details>
  <summary>Details</summary>
Motivation: 解决短电影生成中叙事连贯性和视觉一致性的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有MEP重建算法耗时且效果不佳，需要一种更高效且高质量的解决方案。

Method: 开发了MEP-Diffusion扩散模型，结合Diffusion Posterior Sampling（DPS）作为生成先验，集成到现有迭代求解器中。

Result: 该方法显著提升了3D重建质量，SSIM指标提高了90.50%。

Conclusion: MEP-Diffusion为MEP重建提供了一种高效且高质量的解决方案。
...
TL;DR: 该论文研究了生物医学基础模型（FMs）在临床部署中对非生物技术特征的敏感性，提出了量化模型鲁棒性的方法，并开发了一个鲁棒性基准测试PathoROB。


<details>
  <summary>Details</summary>
Motivation: 生物医学FMs在临床应用中可能学习到非生物技术特征（如手术技术、实验室程序等），这可能导致临床错误，因此需要评估和改进其鲁棒性。

Method: 研究引入了PathoROB基准测试，包含三个新指标和四个数据集，覆盖28个生物类别和34个医疗中心，评估了20种FMs的鲁棒性。

Result: 实验显示所有评估的FMs均存在鲁棒性缺陷，非鲁棒性可能导致严重诊断错误。使用更鲁棒的FMs和后处理鲁棒化方法可减少错误风险。

Conclusion: 鲁棒性评估是临床验证FMs的关键，未来FMs开发需将鲁棒性作为核心设计原则。PathoROB为生物医学领域提供了鲁棒性评估的蓝图。
...
TL;DR: 研究利用高光谱成像和机器学习预测葡萄叶片和冠层的氮浓度，发现特定光谱区域对氮含量预测具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 氮是葡萄园中关键的营养元素，但土壤氮的空间和时间变异性高，需要精确估计叶片氮浓度以实现个体化施肥管理。

Method: 使用高光谱图像（400-1000nm）和两种特征选择方法，结合梯度提升和XGBoost模型预测氮浓度。

Result: 机器学习模型在冠层和叶片数据上的R平方分别为0.49和0.57，特定光谱区域（如500-525nm）对预测氮含量具有鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有2D方法在体积上不连续，3D方法需要大量训练数据，医学场景中数据有限。

Method: 采用轴向扩散模型生成粗修复，冠状扩散模型细化细节，结合自适应重采样。

Result: 实验表明，该方法在真实性和体积一致性上优于现有技术。
...
TL;DR: 论文比较了三种MRI腹部分割工具的性能，并引入了一种基于CT数据的合成训练模型ABDSynth。


<details>
  <summary>Details</summary>
Motivation: MRI分割因信号多变和标注困难而更具挑战性，现有方法训练数据有限，影响泛化能力。

Method: 对三种开源模型（MRSegmentator、MRISegmentator-Abdomen、TotalSegmentator MRI）及ABDSynth进行基准测试，使用三个公共数据集评估。

Result: MRSegmentator表现最佳且泛化性强，ABDSynth虽精度略低但训练数据需求低。

Conclusion: MRSegmentator是当前最佳选择，ABDSynth在标注预算有限时是可行替代方案。
...
Method: 结合光谱DECT模型和基于评分的去噪扩散先验，直接从投影数据优化生成材料图像。

Result: 在合成DECT数据上表现优异，优于无监督和有监督的现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D卷积操作在参数高效表示方面的研究不足，特别是在MRI图像生成中的实际需求。

Method: 利用张量网络建模，将3D卷积核表示为低维张量，设计出TenVOO方法，以少量参数捕捉复杂的空间依赖关系。

Result: 在三个脑MRI数据集上验证，TenVOO在MS-SSIM指标上表现优异，仅需原始模型0.3%的可训练参数。

Conclusion: TenVOO在参数效率和性能上均优于现有方法，为3D MRI图像生成提供了一种高效解决方案。
...
TL;DR: 提出了一种基于U-Net的新方法，用于从掩码输入图像中合成健康的3D脑组织，并在BraTS-Local-Inpainting数据集上表现出色。
...
Result: 在验证集上，SSIM为0.841，PSNR为23.257，MSE为0.007，表现稳定且优异。
...
TL;DR: 论文提出了一种基于预训练模型的深度学习方法来诊断胶质母细胞瘤，但在BraTS-Path验证集上表现不佳，准确率较低，但在特异性上表现优异，最终在测试阶段获得第二名。


<details>
  <summary>Details</summary>
Motivation: 胶质母细胞瘤的异质性使其诊断具有挑战性，传统方法依赖组织样本特征，而深度学习有望提供更准确的诊断。

Method: 利用预训练模型并在BraTS-Path训练数据集上进行微调。

Result: 模型在验证集上的准确率、召回率和F1-score均为0.392229，特异性为0.898704，MCC为0.255267，测试阶段获得第二名。

Conclusion: 尽管模型在验证集上表现不佳，但其特异性表现优异，展示了在负例分类上的潜力，未来需进一步优化。
...
TL;DR: 提出了首个专门用于AI驱动中医舌诊的数据集，包含6,719张标准化条件下的高质量图像，并标注了20种病理症状类别，为AI开发提供了关键基础。


<details>
  <summary>Details</summary>
Motivation: 解决中医舌诊因主观解释和图像协议不一致导致的标准化问题，以及缺乏大规模标注数据集的问题。

Method: 创建了一个包含6,719张标准化舌诊图像的数据集，标注了20种病理症状，并验证了九种深度学习模型的性能。

Result: 数据集支持多种标注格式，并通过多种深度学习模型验证了其可用性，为AI开发提供了可靠基础。

Conclusion: 该数据集填补了中医舌诊领域的数据短缺，推动了AI在研究和临床实践中的应用。
...
TL;DR: UniSegDiff是一种新型扩散模型框架，通过分阶段训练和动态调整预测目标，解决病灶分割问题，并在多模态和多器官任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 扩散概率模型（DPM）在生成任务中表现优异，但现有训练和推理策略导致注意力分布不均，影响效率和效果。

Method: 提出UniSegDiff框架，采用分阶段训练和动态调整预测目标的方法，预训练特征提取网络以实现统一病灶分割。

Result: 在六种不同器官和多种成像模态上，UniSegDiff显著优于现有最佳方法。

Conclusion: UniSegDiff通过优化注意力分布和统一分割框架，显著提升了病灶分割的性能和效率。
...
TL;DR: 该论文提出了一种针对胃肠道病理图像的多模态大模型改进方法，通过构建高质量数据集和引入提示论证策略，显著提升了生成文本的临床相关性和结构完整性。
...
Conclusion: 该方法在病理报告生成任务中表现出更高的准确性和临床实用性，为多模态病理分析提供了可靠解决方案。
...
TL;DR: 提出了一种零样本框架，用于文本到视频模型中的动态概念个性化，避免了每实例微调的限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要针对每个实例进行微调，限制了可扩展性。

Method: 利用结构化2x2视频网格训练轻量级Grid-LoRA适配器，并通过Grid Fill模块完成部分观测布局。

Result: 实验表明，该方法在广泛的主题和编辑场景中生成高质量且一致的结果。

Conclusion: 该方法无需测试时优化，能够泛化到未见过的动态概念。
...
TL;DR: DanceGraph是一种用于同步在线舞蹈的架构，通过实时带宽高效设计减少延迟和运动预测时间，并提供参数化舞蹈风格化方法。


<details>
  <summary>Details</summary>
Motivation: 解决网络化身体姿态共享的延迟问题，实现与音乐节奏同步的在线舞蹈。

Method: 开发实时带宽高效架构以减少延迟，并引入在线舞蹈修正的参数化风格化方法。

Result: 成功减少延迟和运动预测时间，实现与音乐节奏的同步舞蹈。

Conclusion: DanceGraph为在线舞蹈提供了一种高效且交互性强的解决方案。
...
TL;DR: GeoAvatar提出了一种自适应几何高斯泼溅框架，通过APS分割高斯为刚性和柔性集合，结合嘴部结构和部分变形策略，提升3D头部虚拟形象的动画保真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在3D头部虚拟形象生成中难以平衡身份保留（重建）与新姿势和表情（动画）的关系，导致质量不佳。

Method: GeoAvatar采用APS分割高斯为刚性和柔性集合，提出嘴部结构和部分变形策略，并引入正则化损失以优化高斯与3DMM面部的绑定。

Result: 实验表明，GeoAvatar在重建和新动画场景中优于现有方法。

Conclusion: GeoAvatar通过自适应几何高斯泼溅和嘴部优化，显著提升了3D头部虚拟形象的动画质量。
...
TL;DR: PS-GS方法通过高斯溅射和多层感知机，结合多视角光度立体视觉，高效估计几何、材质和光照，提升3D重建精度。


<details>
  <summary>Details</summary>
Motivation: 解决多视角光度立体视觉（MVPS）与逆渲染结合时的高效性问题，提升重建精度。

Method: 先构建2D高斯溅射初始几何，再通过多层感知机进行逆渲染，结合未标定光度立体视觉法线正则化。

Result: 在合成和真实数据集上，重建精度和计算效率优于现有方法。

Conclusion: PS-GS方法在多视角和多光照条件下，有效解决了逆渲染的病态问题，适用于新视角合成、重光照和材质编辑。
...
TL;DR: 论文提出了一种轻量级的实时面部动画模型，通过混合知识蒸馏和伪标签技术解决数据不足问题，显著减小模型大小和计算需求。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏高质量、多样化的音频-动画配对数据集，现有面部动画模型庞大且仅适用于离线推理，无法满足游戏开发中实时、设备端的需求。

Method: 采用混合知识蒸馏和伪标签技术，利用大型音频数据集和高效教师模型训练小型学生模型，去除注意力机制和循环更新。

Result: 实验显示，模型内存占用降至3.4 MB，所需未来音频上下文减少至81 ms，同时保持高质量动画。
...
### [109] [NWaaS: Nonintrusive Watermarking as a Service for X-to-Image DNN](https://arxiv.org/abs/2507.18036)
*Haonan An,Guang Hua,Yu Guo,Hangcheng Cao,Susanto Rahardja,Yuguang Fang*

Main category: cs.CR

TL;DR: 论文提出了一种非侵入式的DNN水印服务（NWaaS），通过ShadowMark实现，在不修改模型的情况下从输出中提取水印，解决了现有方法的侵入性问题。
...
Method: 提出NWaaS框架，通过ShadowMark在模型的黑盒API中建立非侵入式侧信道，利用编码器和解码器提取水印。

Result: 实验证明ShadowMark在不同DNN架构中具有绝对保真度和鲁棒性，适用于多种图像生成模型。

Conclusion: ShadowMark为非侵入式DNN水印提供了实用解决方案，消除了保真度与鲁棒性的权衡。
...
TL;DR: 论文探讨了在资源受限的嵌入式平台上部署现代CNN的挑战，提出利用异构架构和RISC-V Vector 1.0扩展优化性能，实验结果显示预处理速度提升9倍，YOLOv3回退层执行速度提升3倍。


<details>
  <summary>Details</summary>
Motivation: 现代CNN在嵌入式平台上的部署面临资源限制和性能瓶颈，需要异构架构和高效的编译/执行模型来优化。

Method: 利用RISC-V Vector 1.0扩展和缓存层次结构方案，优化预处理和CPU回退过程。

Result: 实验显示预处理速度提升9倍，YOLOv3回退层执行速度提升3倍，且功耗低于传统并行执行平台。

Conclusion: RISC-V Vector 1.0扩展为嵌入式SoC提供了灵活的编程模型，能够平衡计算和内存占用，支持现代深度学习数据流。
...
TL;DR: 论文提出了一种基于缓存的策略（FIFO、LRU和优先级）来减少联邦学习中不必要的模型更新传输，从而降低通信成本，同时保持模型准确性。
...
Method: 引入缓存策略（FIFO、LRU和优先级），选择性转发重要模型更新以减少带宽使用。

Result: 在CIFAR-10和医疗数据集上的实验表明，通信量减少且准确性损失最小。

Conclusion: 智能缓存提高了可扩展性和内存效率，支持边缘物联网网络中的可靠联邦学习，适用于智能城市、医疗等延迟敏感应用。
...
TL;DR: 该论文提出了一种多模态递归集成模型，用于预测自然刺激下的大脑皮层响应，结合视觉、听觉和语义信息，在Algonauts 2025挑战中表现优异。


<details>
  <summary>Details</summary>
Motivation: 预测自然刺激下的大脑皮层响应需要整合多模态信息，现有模型缺乏对时间动态和多模态融合的有效处理。

Method: 使用预训练的视频、音频和语言嵌入，通过双向RNN编码时间动态，融合后传递到第二层递归层，并输出1000个皮层区域的响应。采用复合损失函数和渐进式训练策略。

Result: 模型在Algonauts 2025挑战中排名第三，整体Pearson r为0.2094，单区域峰值r达0.63，对最具挑战性的受试者表现尤为突出。
...
### [113] [GrAInS: Gradient-based Attribution for Inference-Time Steering of LLMs and VLMs](https://arxiv.org/abs/2507.18043)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定干预向量，忽视输入标记的因果影响，且未充分利用多模态梯度信息。
...
Conclusion: GrAInS实现了细粒度、可解释的模型行为控制，且保持模型流畅性和通用能力。
...
### [114] [Evaluation of facial landmark localization performance in a surgical setting](https://arxiv.org/abs/2507.18248)
...
Method: 使用机器人手臂调整位置，固定手术灯和模型，测试MediaPipe算法。
...
### [115] [ReSem3D: Refinable 3D Spatial Constraints via Fine-Grained Semantic Grounding for Generalizable Robotic Manipulation](https://arxiv.org/abs/2507.18262)
*Chenyu Su,Weiwei Shang,Chen Qian,Fei Zhang,Shuang Cong*

Main category: cs.RO

TL;DR: ReSem3D 是一个统一的机器人操作框架，通过结合视觉基础模型（VFMs）和多模态大语言模型（MLLMs），实现细粒度视觉定位和动态构建分层3D空间约束，以解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在语义建模、实时闭环规划和语义多样性环境中的鲁棒性方面存在不足，ReSem3D 旨在解决这些问题。

Method: 利用 MLLMs 的分层递归推理与 VFMs 交互，从自然语言指令和 RGB-D 观测中分两阶段（部分级提取和区域级细化）自动构建3D空间约束，并将其编码为实时优化目标。

Result: 在模拟和真实世界的实验中，ReSem3D 在零样本条件下表现出强大的适应性和泛化能力，能够完成多样化的操作任务。

Conclusion: ReSem3D 通过结合 VFMs 和 MLLMs，实现了语义驱动的3D空间约束，提升了机器人操作的灵活性和鲁棒性。
...
### [116] [Adaptive Articulated Object Manipulation On The Fly with Foundation Model Reasoning and Part Grounding](https://arxiv.org/abs/2507.18276)
...
Method: 提出AdaRPG框架，利用基础模型提取部件级功能，构建部件级功能标注数据集，并生成高级控制代码。
...
### [117] [Topology-Preserving Coupling of Compressible Fluids and Thin Deformables](https://arxiv.org/abs/2507.18460)
*Jonathan Panuelos,Eitan Grinspun,David Levin*

Main category: physics.comp-ph

TL;DR: 提出了一种新的耦合可压缩流体与薄可变形结构的离散化方法，确保流体域路径连通性，防止泄漏。
...
Result: 验证了方法在多种复杂场景中的有效性，如气球推进、香槟塞弹出和超音速小行星模拟。

Conclusion: 该方法实现了流体与固体间的双向能量传递，同时确保无泄漏。
...
### [118] [On the Performance of Concept Probing: The Influence of the Data (Extended Version)](https://arxiv.org/abs/2507.18550)
...
<details>
  <summary>Details</summary>
Motivation: 概念探测用于解释人工神经网络，但以往研究忽视了训练数据对探测模型性能的影响。

Method: 在图像分类任务中，研究了训练数据对概念探测模型性能的影响。

Result: 分析了数据对探测模型性能的作用，并公开了两个数据集的概念标签。

Conclusion: 训练数据对概念探测模型性能有重要影响，未来研究需更多关注数据选择。
...
### [119] [SafeWork-R1: Coevolving Safety and Intelligence under the AI-45$^{\circ}$ Law](https://arxiv.org/abs/2507.18576)
...
Result: 在安全基准测试中平均提升46.54%，优于GPT-4.1和Claude Opus 4，且不影响通用能力。
...
TL;DR: QuaRC是一种在边缘设备上使用核心集进行量化感知训练的框架，通过相对熵评分和层间校正策略减少量化误差，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 边缘设备对低比特量化模型的需求增加，但隐私限制和数据规模问题导致传统量化感知训练难以高效实施。

Method: 提出QuaRC框架，包括基于相对熵评分的核心集选择和层间校正策略。
...
Method: 使用开源模型提取多模态特征，通过模态融合Transformer和时间预测Transformer（带旋转嵌入）进行解码。在CNeuroMod数据集的65小时电影数据上训练，并集成20个种子。

Result: 在分布内数据（Friends S07）上平均Parcel-wise Pearson相关系数为32.25，分布外数据上为21.25，优于之前版本。

Conclusion: VIBE在多模态fMRI预测任务中表现出色，并在Algonauts 2025挑战赛中取得优异成绩。
...
### [122] [ChronoSelect: Robust Learning with Noisy Labels via Dynamics Temporal Memory](https://arxiv.org/abs/2507.18183)
*Jianchao Wang,Qingfeng Li,Pengcheng Zheng,Xiaorong Pu,Yazhou Ren*

Main category: cs.LG

TL;DR: ChronoSelect是一个新颖的框架，通过四阶段内存架构和滑动更新机制，动态分析标签噪声，显著提升模型在噪声标签下的性能。


<details>
  <summary>Details</summary>
Motivation: 现实数据集中噪声标签会降低模型泛化性能，现有方法未能充分利用学习动态。

Method: 提出四阶段内存架构和滑动更新机制，压缩预测历史为时间分布，通过时间轨迹分析和双分支一致性划分样本。

Result: 理论证明框架的收敛性和稳定性，实验显示在合成和真实数据集上达到最先进性能。

Conclusion: ChronoSelect通过动态分析噪声标签，显著提升了模型在噪声环境下的表现。
...
Result: 实验显示DFAM在低信噪比条件下性能优于其他方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统CCA无法处理在线流数据的问题，提供实时和高维扩展能力。

Method: 使用流式PCA作为后端，结合滑动窗口样本实时估计CCA组件。

Result: 通过数值模拟和理论性能保证验证了方法的有效性，适用于极高维度数据。

Conclusion: SWICCA是一种高效、可扩展的在线流数据CCA方法，适用于实时和高维场景。
