...
Result: 在26个主流MLLMs上的实验表明，视觉细粒度推理的最高性能仅为0.46，显示改进潜力巨大。
...
TL;DR: 本文提出了一种结合ConvNeXt和EfficientNet的混合AI模型，用于准确分类猎鹰的三种健康状况（正常、肝病和曲霉病），其性能优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 猎鹰训练和狩猎中需要精确的健康监测，以确保这些珍贵鸟类的安全，尤其是在狩猎场景中。

Method: 采用ConvNeXt和EfficientNet混合的AI模型，利用大量数据集训练和验证，关注准确性、精确度、召回率和F1分数等关键指标。

Result: 混合模型在疾病检测中表现优于传统方法和单一模型架构。

Conclusion: 该混合AI模型的成功应用为猎鹰疾病检测提供了更精确的方法，并为未来AI驱动的鸟类医疗解决方案奠定了基础。
...
Result: 系统在动物图像任务中表现良好，能够将视觉内容与结构化查询联系起来。
...
TL;DR: 论文提出GraphGSOcc模型，通过结合语义和几何图Transformer改进3D高斯泼溅（3DGS）方法，解决特征聚合和边界模糊问题，实验显示性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在特征聚合中忽略语义相关性，且因缺乏几何约束导致边界模糊，需改进。
...
Conclusion: GraphGSOcc通过语义和几何图Transformer有效提升3D语义占用预测性能。
...
TL;DR: 论文提出了DAVID-X数据集和DAVID-XR1模型，用于可解释地检测AI生成视频，通过缺陷分类、时空定位和自然语言解释提供透明证据。


<details>
  <summary>Details</summary>
Motivation: AI生成视频在媒体平台广泛传播，现有方法仅提供二元分类，缺乏细粒度证据，难以说服用户和审计者。

Method: 引入DAVID-X数据集，包含缺陷级时空标注和文字解释；提出DAVID-XR1模型，通过视觉推理链实现可解释检测。

Result: 实验表明，通用主干网络在DAVID-X上微调后，结合思维链蒸馏，能泛化到多种生成器和模式。
...
TL;DR: 综述了2020年至2024年间基于深度学习的多智能体轨迹预测的最新进展，重点分析了ETH/UCY基准测试中的模型，并探讨了该领域的关键挑战与未来方向。
...
Method: 根据架构设计、输入表示和预测策略对现有方法进行分类，重点关注ETH/UCY基准测试中的模型。

Result: 总结了多智能体轨迹预测的最新方法，并分析了其性能。
...
TL;DR: 论文分析了人类设计与机器生成的3D建筑形式的差异，提出了ArchForms-4000数据集和ArchShapeNet模型，实验显示模型在区分形式来源上优于人类专家。
...
Method: 构建ArchForms-4000数据集（2000个人类设计和2000个Evomass生成形式），提出ArchShapeNet（带显著性模块的3D卷积网络），并进行比较实验。

Result: 模型在区分形式来源上表现优异，准确率94.29%，精确率96.2%，召回率98.51%。

Conclusion: 研究揭示了人类设计在空间组织、比例和谐及细节优化上的优势，为未来生成工具的改进提供了重要参考。
...
TL;DR: 提出了一种高性能、低延迟的视频监控系统，适用于资源受限环境，结合熵自适应帧缓冲算法与MobileNetV2，实现高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 针对资源受限设备（如Raspberry Pi、NVIDIA Jetson Nano）设计高效视频监控系统，满足低延迟和高准确率需求。

Method: 采用熵自适应帧缓冲算法，结合MobileNetV2，优化资源利用与处理效率。

Result: 系统在标准数据集上保持92%以上检测准确率，端到端推理延迟低于50ms，适应不同光照、背景和速度。

Conclusion: 系统具有可扩展性、低成本且符合严格数据隐私法规，适用于智能城市或嵌入式安全架构。
...
TL;DR: MonoVQD是一个新颖的框架，通过改进DETR架构，解决了单目3D检测中的关键挑战，包括稳定性、梯度消失和查询优化问题。


<details>
  <summary>Details</summary>
Motivation: 单目3D检测中，DETR架构的直接应用存在性能限制，MonoVQD旨在通过创新方法提升性能。

Method: 提出Mask Separated Self-Attention机制、Variational Query Denoising技术和自蒸馏策略，优化查询和去噪过程。

Result: 在KITTI和nuScenes数据集上表现优异，验证了其高效性和泛化能力。

Conclusion: MonoVQD通过核心技术创新显著提升了单目和多视角3D检测的性能。
...
Method: 将任务分解为视觉理解和代码翻译两部分，设计结构化指令（描述和差异）来转化视觉特征为语言表示，并采用两阶段（初始代码生成和迭代优化）流程。
...
TL;DR: PictSure是一个专注于图像嵌入模型的ICL框架，通过系统分析嵌入模型的架构、预训练和训练动态，显著提升了少样本图像分类的性能。
...
Result: 实验表明，嵌入模型的预训练方式显著影响训练成功率和跨域性能。PictSure在跨域任务中优于现有ICL模型，同时在域内任务中表现相当。

Conclusion: PictSure通过优化嵌入模型设计，为少样本图像分类提供了高效解决方案，尤其在跨域场景中表现突出。
...
TL;DR: 论文提出了一种基于数学和实证的框架BKSEF，用于优化CNN中逐层核大小的选择，平衡信息增益、计算效率和准确性。实验表明，BKSEF在多个数据集上显著提升模型性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 卷积神经网络中核大小的选择对模型性能至关重要，但常被忽视。传统方法通常使用固定大小的核，缺乏灵活性。

Method: 提出BKSEF框架，结合信息理论、信号处理和学习理论，通过数学和实证方法优化核大小。

Result: 在多个数据集上，BKSEF指导的模型实现了最高3.1%的准确率提升和42.8%的FLOPs减少。实际案例验证了其在医疗图像和交通标志识别中的有效性。

Conclusion: BKSEF为CNN设计提供了新的优化视角，可作为神经架构搜索和实时系统的实用工具。
...
TL;DR: 提出了一种基于深度学习的零售视频自动标注方法，通过关键帧识别和对象检测技术，显著降低了标注成本和时间。
...
TL;DR: 论文提出了一种基于神经不确定性预测的主动视角选择方法（UPNet），用于高效且准确的3D重建，显著降低了计算开销。
...
Result: 在仅使用一半视角的情况下，重建精度与基准方法相当，同时计算开销大幅降低（400倍加速，资源使用减少50%以上）。

Conclusion: UPNet方法在高效3D重建中表现出色，且无需额外训练即可推广到新物体类别。
...
TL;DR: 论文提出DPO-Kernels方法，通过混合损失、核化表示和散度选择提升文本到图像模型的校准能力，并引入DETONATE基准和AQI指标。


<details>
  <summary>Details</summary>
Motivation: 确保文本到图像模型生成的图像符合用户意图，同时保持安全性和公平性。

Method: 提出DPO-Kernels方法，包括混合损失、核化表示和散度选择，并引入DETONATE基准和AQI指标。

Result: 实验表明DPO-Kernels通过HT-SR保持强泛化能力，DETONATE基准和代码已公开。

Conclusion: DPO-Kernels在提升模型校准能力方面有效，DETONATE基准为相关研究提供了支持。
...
### [18] [MSNeRV: Neural Video Representation with Multi-Scale Feature Fusion](https://arxiv.org/abs/2506.15276)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于INR的视频压缩方法在处理细节密集和快速变化内容时表现不佳，主要原因是网络内部特征利用不足和缺乏视频特定设计。

Method: 提出MSNeRV框架，包括时间窗口增强时间一致性、GoP级网格背景表示、多尺度空间解码器和尺度自适应损失函数，以及多尺度特征块。

Result: 在HEVC ClassB和UVG数据集上，MSNeRV在INR方法中表现最佳，压缩效率在动态场景中超过VTM-23.7。
...
### [19] [Frequency-Calibrated Membership Inference Attacks on Medical Image Diffusion Models](https://arxiv.org/abs/2506.14919)
*Xinkai Zhao,Yuta Tokuoka,Junichiro Iwasawa,Keita Oda*

Main category: cs.CV

TL;DR: 论文提出了一种针对医学图像扩散模型的频率校准重建误差（FCRE）方法，用于成员推断攻击（MIA），通过聚焦中频区域重建误差，有效量化隐私风险。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在医学图像生成中的广泛应用引发了隐私担忧，现有MIA方法因依赖重建误差而面临挑战。

Method: 提出FCRE方法，通过分析反向扩散过程，计算中频重建误差和结构相似性指数得分，确定成员资格。

Result: 实验表明，FCRE方法在多个医学图像数据集上优于现有MIA方法。

Conclusion: FCRE方法通过频率选择性策略，有效解决了医学图像MIA中的挑战。
...
### [20] [Vision Transformers for End-to-End Quark-Gluon Jet Classification from Calorimeter Images](https://arxiv.org/abs/2506.14934)
*Md Abrar Jahin,Shahriar Soudeep,Arian Rahman Aditta,M. F. Mridha,Nafiz Fahad,Md. Jakir Hossen*

Main category: cs.CV

TL;DR: 本文评估了Vision Transformer (ViT)及其混合模型在夸克-胶子喷注分类中的表现，发现其优于传统CNN方法。


<details>
  <summary>Details</summary>
Motivation: 区分夸克和胶子喷注是高能物理中的关键挑战，对LHC的新物理搜索和精确测量至关重要。ViT架构在全局上下文建模方面表现优异，但尚未充分探索其在直接分析量能器图像中的应用。

Method: 使用模拟的2012年CMS开放数据，构建多通道喷注图像（ECAL、HCAL和重建轨迹），并评估ViT和ViT-CNN混合模型的性能。

Result: ViT及其混合模型（如ViT+MaxViT和ViT+ConvNeXt）在F1分数、ROC-AUC和准确率上均优于传统CNN。

Conclusion: ViT架构在喷注分类中表现出色，本文为其在量能器图像分析中的应用提供了系统框架和性能基准。
...
### [21] [Advances in Compliance Detection: Novel Models Using Vision-Based Tactile Sensors](https://arxiv.org/abs/2506.14980)
...
<details>
  <summary>Details</summary>
Motivation: 传统顺应性检测方法缺乏便携性和可扩展性，且依赖昂贵设备；现有基于神经网络的视觉触觉传感器方法预测精度不足。

Method: 提出两种模型：基于LRCN和Transformer架构，利用GelSight传感器的RGB触觉图像及其他信息预测顺应性指标。

Result: 模型在多种指标下验证有效，显著优于基线；发现传感器与物体顺应性相关性，较硬物体更难估计。

Conclusion: 所提模型显著提升顺应性预测精度，为工程、农业和生物医学应用提供更优解决方案。
...
TL;DR: PALETTE是一种端到端的文本识别方法，用于从多样化的历史地图中提取文本，通过超局部采样模块和合成数据训练，显著提升了长文本和倾斜文本的识别效果。


<details>
  <summary>Details</summary>
Motivation: 历史地图中的文本提供了丰富的地理、历史和文化信息，但提取文本面临方法不足和训练数据缺乏的挑战。

Method: PALETTE采用超局部采样模块学习文本实例的边界点和字符特征，并结合合成数据SynthMap+进行训练。

Result: PALETTE在历史地图文本识别任务中优于现有方法，尤其在长文本和倾斜文本上表现突出，并已应用于大规模地图处理。

Conclusion: PALETTE为历史地图文本提取提供了高效解决方案，结合合成数据训练，显著提升了识别效果和应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统风格迁移方法的局限性，统一不同任务的框架，实现高质量的风格迁移和文本控制。

Method: 1. 基于语义的风格注入方法；2. 基于人类反馈的数据增强策略；3. 训练自由的三重扩散过程。

Result: 实现了高质量的图像驱动风格迁移和文本驱动风格化，首次在风格迁移中实现图像颜色编辑。

Conclusion: StyleWallfacer框架在风格迁移领域取得了突破性进展，提供了高效、高质量的风格迁移解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有向量量化方法存在训练不稳定和码本崩溃问题，主要源于特征与码向量分布不匹配，导致信息损失和码本利用率低。

Method: 采用Wasserstein距离对齐特征和码向量的分布，以减少量化误差并提升码本利用率。

Result: 实验和理论分析表明，该方法实现了接近100%的码本利用率，并显著降低了量化误差。

Conclusion: 通过Wasserstein距离对齐分布，有效解决了向量量化中的关键问题，提升了模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LVMs的无训练方法在低对比度医学图像上表现不佳，主要原因是未能有效利用负提示。

Method: 设计置信图协同模块结合DINOv2和SAM的优势，选择高质量正负点提示，并通过K-means聚类优化提示集。
...
TL;DR: 论文提出了一种名为PDSA的模块，通过高维空间相关性修正特征分布，解决了点云分析中局部坐标的干扰和特征层次差异问题，提高了计算效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 点云分析中，局部坐标的局限性导致无关点干扰和特征层次差异问题，现有方法计算开销大且对噪声敏感。

Method: 提出PDSA模块，利用高维空间相关性修正特征分布，通过轻量级跨阶段结构描述符区分点相关性，并通过长距离建模增强结构同质性。
...
TL;DR: 本文提出了一种名为Echo-DND的双噪声扩散模型，用于超声图像中左心室的分割，通过结合高斯和伯努利噪声以及多尺度融合模块，显著提高了分割精度。


<details>
  <summary>Details</summary>
Motivation: 超声图像噪声大、对比度低且左心室边界模糊，导致分割困难，需要一种更有效的解决方案。

Method: Echo-DND结合高斯和伯努利噪声，采用多尺度融合条件模块和空间一致性校准技术。

Result: 在CAMUS和EchoNet-Dynamic数据集上，模型分别达到0.962和0.939的Dice分数，优于现有SOTA模型。

Conclusion: Echo-DND为超声图像分割设立了新标准，并有望扩展到其他医学影像任务中。
...
TL;DR: ReSeDis任务首次将大规模检索与像素级定位结合，解决现有技术无法同时满足检索和定位的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索技术要么专注于检索（如文本到图像检索），要么专注于定位（如视觉接地），无法同时满足大规模检索和精确定位的需求。

Method: 提出ReSeDis任务，结合检索与定位，并设计了一个基准测试和任务特定指标。

Result: 提供了一个零样本基线，显示未来研究有显著改进空间。

Conclusion: ReSeDis为构建下一代稳健且可扩展的多模态搜索系统提供了现实测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学图像分析模型虽然性能优异，但局限于特定任务且开发成本高，通用模型能提供更灵活的实用性。

Method: 采用视觉上下文学习（VICL）训练通用模型，基于少量示例实现任务泛化，并提出针对OCT的评估协议。

Result: 在多个视网膜OCT数据集上评估了先进的医学VICL方法，建立了首个基线结果，展示了其潜力与局限性。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言预训练模型语义理解能力的提升，公开图像容易被搜索引擎等工具利用，因此需要保护用户隐私。

Method: 提出PSIC方法，结合CLTG模块和UAEO优化函数，实现多解码选项和隐私保护。

Result: 实验证明PSIC在多种下游任务中有效，同时保持图像质量和压缩功能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在捕捉细节特征和跨模态特征交互方面表现不足，导致融合图像质量不理想。

Method: 采用两阶段方法：第一阶段通过扩散过程训练UNet进行图像重建；第二阶段将不同步骤的噪声图像输入融合网络，结合三个关键融合模块自适应处理不同模态图像。
...
<details>
  <summary>Details</summary>
Motivation: 视频包含丰富信息，生成准确的自然语言描述是视频理解的关键。现有模型在字幕生成上仍有改进空间。

Method: 提出视频-SALMONN 2模型，结合LoRA和DPO优化。引入MrDPO方法，周期性更新参考模型并重新初始化LoRA模块，结合真实字幕指导训练。

Result: MrDPO显著提升模型性能，字幕错误率降低28%。模型在7B参数规模下超越GPT-4o和Gemini-1.5-Pro。
...
<details>
  <summary>Details</summary>
Motivation: SAR船舶检测面临尺度变化大、小目标噪声干扰和复杂背景等挑战，需要提升检测精度和鲁棒性。

Method: C-AFBiFPN结合了卷积特征增强模块（CFE）和创新的AFBiFPN网络，后者整合了BiFormer注意力机制，以增强局部细节和全局建模能力。

Result: 在SSDD数据集上的实验表明，该方法显著提升了小目标检测精度、抗遮挡能力和多尺度特征适应性。

Conclusion: C-AFBiFPN框架有效解决了SAR船舶检测中的关键问题，为实际应用提供了可靠的技术支持。
...
Method: 采用两阶段训练范式（学习阶段和回忆阶段），设计了DMA捕获多尺度视觉信息，DMM和IPR利用历史知识重构推理。

Result: 在多个数据集上实验表明，RetroMem显著优于现有最先进方法。
...
TL;DR: 论文探讨了在半导体领域中使用领域自适应（DA）技术，特别是在半监督和无监督设置下，以减少重新标记和训练的需求，并提出了改进的DBACS方法。


<details>
  <summary>Details</summary>
Motivation: 半导体行业竞争激烈，时间和质量是关键因素。深度学习和DA技术在此领域的应用可以显著提升效率，减少资源消耗。

Method: 提出DBACS方法，基于CycleGAN并增加额外损失项，以提升性能。在半监督和无监督设置下测试DA技术。

Result: 在真实电子显微镜图像上验证了方法的有效性，证明了DA技术在半导体领域的实用性。

Conclusion: DBACS方法有效提升了DA技术在半导体领域的应用，为减少资源消耗和提高效率提供了新思路。
...
TL;DR: BCRNet通过Bezier曲线细化策略提升腹腔镜肝脏手术中的关键解剖标志检测，结合多模态特征提取和分层曲线细化机制，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜肝脏手术中准确识别解剖结构具有挑战性，AR系统结合MRI/CT图像需要精确的2D-3D配准，而曲线标志检测是关键。

Method: 提出BCRNet框架，包括多模态特征提取（MFE）、自适应曲线提议初始化（ACPI）和分层曲线细化（HCR）机制。

Result: 在L3D和P2ILF数据集上，BCRNet显著优于现有方法。

Conclusion: BCRNet通过Bezier曲线细化策略有效提升标志检测精度，为手术导航提供可靠支持。
...
TL;DR: ViMAT是一种无需视觉标记或固定工作空间设置的AI系统，用于实时监控工业装配任务，结合感知和推理模块，在复杂场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 工业装配任务的视觉监控对防止设备损坏和确保工人安全至关重要，但现有商业解决方案通常需要严格的工作空间设置或视觉标记。

Method: ViMAT结合感知模块（从多视角视频流提取视觉观察）和推理模块（基于观察到的装配状态和任务知识推断最可能的动作）。

Result: 在LEGO组件更换和液压机模具重新配置任务中，ViMAT通过定量和定性分析验证了其有效性，尤其在部分和不确定视觉观察的挑战性场景中。

Conclusion: ViMAT无需传统约束即可有效监控装配任务，适用于复杂工业环境。
...
TL;DR: 论文探讨了面部微表情（MEs）的识别、检测与生成，提出结合检测与识别的统一任务（ME-STR）和基于多模态大语言模型的视觉问答任务（ME-VQA）。


<details>
  <summary>Details</summary>
Motivation: 传统方法将微表情检测与识别分开处理，在现实场景中效果不佳；多模态大语言模型为ME分析提供了新思路。

Method: 提出ME-STR（检测与识别的统一管道）和ME-VQA（基于视觉问答的ME理解），利用MLLMs/LVLMs的多模态推理能力。

Result: MEGC 2025挑战赛引入ME-STR和ME-VQA任务，要求算法在测试集上运行并提交结果。

Conclusion: 结合多模态大语言模型的新方法有望提升ME分析的效率和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，高精地图和鸟瞰图语义地图对精确定位、规划和决策至关重要，但现有方法在特征表示和预测质量上仍有提升空间。

Method: 采用端到端模型MapFM，结合基础模型编码相机图像，并集成辅助预测头进行鸟瞰图语义分割，实现多任务学习。

Result: 模型显著提升了特征表示质量和预测准确性，生成的高精地图质量更高。

Conclusion: MapFM通过多任务学习和丰富的上下文监督，提供了一种更全面的场景表示方法，适用于自动驾驶中的高精地图生成。
...
TL;DR: OpenPath是一种新型的开放集主动学习方法，用于病理图像分类，通过预训练的视觉语言模型和任务特定提示，显著提高了标注效率和模型性能。


<details>
  <summary>Details</summary>
Motivation: 病理图像分类需要大规模标注数据，但获取成本高且耗时。传统主动学习方法在开放集场景下效率低，且初始随机选择浪费标注资源。

Method: OpenPath结合任务特定提示和多样化信息样本选择策略（PIS和EGSS），确保样本纯度和信息量，避免选择分布外数据。

Result: 在两个公共病理图像数据集上，OpenPath显著提升了模型性能，优于现有开放集主动学习方法。

Conclusion: OpenPath通过高效样本选择和避免分布外数据，显著提高了病理图像分类的标注效率和模型性能。
...
TL;DR: 论文提出了一种新任务：开放世界视频目标计数，通过文本描述或图像示例指定目标对象，统计视频中所有独特实例。方法结合图像计数和视频分割跟踪模型，性能优于基线。


<details>
  <summary>Details</summary>
Motivation: 解决拥挤场景中目标对象重复计数和遮挡问题，实现开放世界的自动化对象计数。

Method: 提出CountVid模型，结合图像计数和可提示视频分割跟踪技术，实现跨帧计数。

Result: 在VideoCount数据集上验证，CountVid表现优于基线，计数准确。

Conclusion: CountVid和VideoCount数据集为开放世界视频目标计数提供了有效解决方案，代码开源。
...
TL;DR: 提出了一种几何感知的纹理映射方法，将动物皮毛图案解缠到规范的UV空间，提升特征匹配的鲁棒性。
...
Method: 利用表面法线估计指导解缠过程，保持3D表面与2D纹理空间的几何一致性，无需真实UV标注，可自监督训练。

Result: 在Saimaa环斑海豹和豹的数据集上，重识别准确率提升达5.4%。

Conclusion: 该方法通过几何感知的纹理映射显著提升了动物个体重识别的准确性，尤其在复杂姿态和视角下表现优异。
...
Conclusion: DDIS通过扩散模型和领域对齐指导，显著提升了数据无关图像合成的质量和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 在医学影像领域，确保深度学习模型的可靠性至关重要，尤其是能够识别OOD样本以辅助诊断决策。现有方法在捕捉OOD多样性方面存在不足。

Method: NERO通过聚类神经元级相关性形成代表性中心，并引入相关性距离度量来量化新样本与中心的偏差，同时结合缩放相关性和特征范数以优化性能。
...
TL;DR: 本教程以Hunyuan3D 2.1为例，提供了从3D数据处理到模型训练与评估的完整指南，旨在降低3D生成模型的开发门槛。


<details>
  <summary>Details</summary>
Motivation: 尽管3D AI生成内容（AIGC）发展迅速，但其复杂性仍限制了非专业人士的参与。本教程旨在通过Hunyuan3D 2.1案例，简化3D生成模型的开发流程。

Method: 教程详细介绍了Hunyuan3D 2.1系统的两个核心组件（Hunyuan3D-DiT用于形状生成，Hunyuan3D-Paint用于纹理合成），涵盖数据准备、模型架构、训练策略、评估指标和部署。

Result: 通过本教程，用户可以掌握开发或优化3D生成模型的能力，适用于游戏、虚拟现实和工业设计等领域。

Conclusion: 本教程为开发者和设计师提供了实用的工具和知识，推动了3D AIGC的普及和应用。
...
TL;DR: MRG-LLM是一种新型多模态大语言模型，通过动态提示定制机制结合冻结的LLM和可学习的视觉编码器，实现了医学影像报告的精准生成。


<details>
  <summary>Details</summary>
Motivation: 医学影像报告生成在临床实践中仍具挑战性，现有大语言模型（LLMs）与医学影像数据的结合需要深入探索。

Method: 提出MRG-LLM，结合冻结的LLM和可学习的视觉编码器，引入动态提示定制机制，通过视觉特征生成实例特定的提示。

Result: 在IU X-ray和MIMIC-CXR数据集上的实验表明，MRG-LLM在医学报告生成中达到最先进性能。

Conclusion: MRG-LLM通过动态提示定制机制，显著提升了医学影像报告生成的精准性和针对性。
...
Method: 第一阶段使用Object-AnchorNet从3D HOI数据集中学习稀疏3D HOI关键帧；第二阶段通过Contact-Aware Diffusion Model（ContactDM）插值生成密集4D HOI序列。

Result: 在OMOMO和3D-FUTURE数据集上取得最优结果，展示了强大的泛化能力和高保真生成效果。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在通过挑战赛推动阴影去除技术的发展，并评估不同方法在真实场景中的表现。

Method: 使用WSRD+数据集，模拟自阴影和投射阴影的交互，评估重建保真度和用户视觉感知。

Result: 306名参与者注册，17支团队成功提交解决方案。

Conclusion: 挑战赛展示了阴影去除技术的多样性，并验证了WSRD+数据集的有效性。
...
TL;DR: CLAIM框架通过临床引导的LGE增强技术，生成解剖学一致的多样化心肌瘢痕图像，并联合训练分割网络，提升瘢痕分割的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决LGE图像稀缺和多样性不足对心肌瘢痕分割模型开发的限制。

Method: 提出CLAIM框架，结合SMILE模块（基于临床知识的瘢痕生成）和联合训练策略，生成解剖学一致的瘢痕图像并优化分割网络。

Result: 实验表明CLAIM生成的瘢痕更符合解剖学分布，Dice相似度高于基线模型。
...
TL;DR: RaCalNet提出了一种无需密集监督的毫米波雷达深度估计方法，通过稀疏LiDAR监督实现高效学习，性能优于密集监督方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖密集LiDAR监督，成本高且数据需求大。RaCalNet旨在通过稀疏监督实现高效深度估计。
...
Conclusion: RaCalNet证明了稀疏监督的可行性，性能优于密集监督方法，为低成本高效深度估计提供了新思路。
...
Method: WinWinLay采用非局部注意力能量函数和基于Langevin动力学的自适应更新策略，提升控制精度和图像真实性。

Result: 实验表明，WinWinLay在元素布局控制和视觉保真度上优于现有方法。

Conclusion: WinWinLay通过创新策略显著提升了布局到图像生成的质量，无需额外训练。
...
Conclusion: Show-o2展示了多模态模型的潜力，为跨模态任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 超高空间分辨率土地覆盖分类对细粒度分析至关重要，但面临像素级标注成本高、尺度变化大和大规模视觉模型适应性有限等挑战。现有方法多依赖标注数据且适用于1米分辨率影像，而实际应用需在弱监督下处理更高分辨率影像。

Method: 提出参数高效的半监督分割框架，结合SAM2知识并引入遥感专用FreqWeaver适配器，以轻量化设计（仅占5.96%参数）增强细粒度细节建模。

Result: 该方法有效利用未标注数据且参数开销小，分割结果具有优异的结构一致性，比现有参数高效调优策略提升1.78%，比最先进高分辨率遥感分割方法提升3.44%。

Conclusion: 该框架在超高分辨率遥感影像分割中表现出色，兼顾轻量化和性能提升，为实际应用提供了有效解决方案。
...
TL;DR: 提出了一种基于图的新型统一框架，用于大规模点云的端到端处理，显著提高了森林地上生物量（AGB）估计的鲁棒性和可扩展性。
...
Method: 通过创新的基于图的流程，无缝集成了树木分割、叶木分离和3D骨架重建，利用路径和抽象等图操作进行树拓扑推理。

Result: 在多种条件下（如叶覆盖和低密度点云）表现出色，相对误差约为20%（叶覆盖）和30%（低密度点云）。

Conclusion: 该框架为大规模非破坏性AGB估计提供了鲁棒且可扩展的解决方案，减少了预处理工具依赖，并确立了无人机激光扫描（ULS）作为地面激光扫描（TLS）的可行替代方案。
...
TL;DR: 论文提出了一种名为DLoRAL的双LoRA学习方法，用于在视频超分辨率任务中同时实现细节丰富和时间一致性。
...
Result: DLoRAL在准确性和速度上均表现出色。

Conclusion: DLoRAL能够高效且高质量地恢复视频，同时保持细节和时间一致性。
...
TL;DR: 提出了一种名为M2M-Reg的新框架，用于解决多模态图像配准问题，通过单模态相似性训练模型，并引入GradCyCon正则化器，实验证明其在PET-MRI和FA-MRI配准中表现优异。


<details>
  <summary>Details</summary>
Motivation: 由于多模态图像（如PET、FA）与结构参考（如MRI、CT）之间存在极大异质性，传统无监督配准方法难以学习可靠的空间映射，导致图像失真。

Method: 提出了M2M-Reg框架，利用单模态相似性训练多模态配准模型，并引入GradCyCon正则化器以促进微分同胚。框架还支持半监督设置，无需真实变换或分割掩码。

Result: 在ADNI数据集上的实验表明，M2M-Reg在PET-MRI和FA-MRI配准中的DSC指标比现有方法高2倍。

Conclusion: M2M-Reg有效解决了高度异质性多模态图像配准问题，为临床实践提供了更可靠的解决方案。
...
Conclusion: 该框架无需重建，计算高效，适用于大范围场景，具有强泛化能力。
...
TL;DR: HOIDiNi是一个基于文本驱动的扩散框架，用于生成逼真且合理的人-物交互（HOI）动作。通过扩散噪声优化（DNO）直接在预训练扩散模型的噪声空间中优化，实现了高真实性和物理准确性。
...
Method: 采用两阶段方法：物体中心阶段选择手-物接触位置，人体中心阶段优化全身动作以实现接触。使用扩散噪声优化（DNO）在噪声空间中直接优化。
...
Method: 在Habitat模拟器中引入了一个新的基准测试，包含60个需要长期记忆和上下文感知的任务，并提供了基线模型。

Result: 基准测试能够评估模型在长期记忆任务中的表现，并展示了现有模型的局限性。

Conclusion: 该基准测试为未来研究提供了方向，强调了在机器人控制任务中改进长期记忆和推理能力的必要性。
...
TL;DR: 研究发现，多模态大语言模型（MLLMs）在视觉质量与任务表现之间存在矛盾，即图像质量偏离人类感知时模型表现可能更好。为此，作者提出了一种轻量级适配模块VQ-TTT，通过动态调整输入图像提升任务表现。


<details>
  <summary>Details</summary>
Motivation: 探讨输入视觉质量对MLLMs响应的影响，揭示视觉质量与模型表现之间的矛盾关系。

Method: 提出VQ-TTT模块，通过可学习的低秩核和浅层视觉编码器微调，动态调整输入图像以适应模型偏好。

Result: VQ-TTT显著提升了所有测试MLLMs和数据集的平均准确率，无需额外模型或数据。
...
TL;DR: ViMaR是一种两阶段推理框架，结合时间差分价值模型和边界感知奖励调整，显著提升视觉语言模型的效率和输出保真度。
...
Method: 两阶段框架：首阶段筛选高价值候选描述，次阶段选择性优化弱视觉基础部分，并引入边界惩罚。

Result: ViMaR生成更可靠、准确、详细的描述，速度提升4倍，且能跨模型泛化。

Conclusion: ViMaR具有灵活性和可扩展性，适用于快速、准确的视觉语言模型推理，并能通过自训练提升性能。
...
TL;DR: 提出了一种联合估计反照率并生成重光照输出的单阶段方法，利用视频扩散模型的生成能力，提升场景理解和光照效果的真实性。
...
Method: 采用单阶段联合估计反照率和生成重光照输出的方法，利用视频扩散模型，结合合成多光照数据和自动标注的真实视频进行训练。
...
Method: 开发高效工具箱收集、预处理和标注视频，包括位置、场景、天气等信息，并训练交互式视频世界探索模型YUME。

Result: 实验验证了数据集质量，YUME模型展示了其潜力。
...
Method: 提出ECAD，利用遗传算法为每个模型学习高效的缓存调度，形成帕累托前沿，仅需少量校准提示。
...
### [66] [Empirical Studies of Large Scale Environment Scanning by Consumer Electronics](https://arxiv.org/abs/2506.14771)
...
<details>
  <summary>Details</summary>
Motivation: 评估Matterport Pro3在大规模环境重建中的有效性，探索其性能提升和局限性，并与iPhone等其他设备进行对比。

Method: 通过1,099个扫描点对六层建筑（17,567平方米）进行详细扫描，分析设备性能并提出解决方案。

Result: Matterport Pro3生成的点云密度更高（1,877,324点vs iPhone的506,961点），对齐精度更高（RMSE 0.0118米），C2C平均距离误差为0.0408米。
...
### [67] [Deploying and Evaluating Multiple Deep Learning Models on Edge Devices for Diabetic Retinopathy Detection](https://arxiv.org/abs/2506.14834)
*Akwasi Asare,Dennis Agyemanh Nana Gookyi,Derrick Boateng,Fortunatus Aabangbio Wulnye*

Main category: eess.IV

TL;DR: 该研究提出了一种基于边缘计算和深度学习的方法，用于实时检测糖尿病视网膜病变（DR），通过优化多种CNN模型在边缘设备上的部署，实现了高精度和低延迟的诊断。
...
Method: 研究使用Edge Impulse平台，结合TensorFlow设计了多种CNN模型（如MobileNet、ShuffleNet等），并通过数据增强、归一化和量化优化模型，最终在边缘设备上部署。

Result: MobileNet达到96.45%的准确率，SqueezeNet在GPU上仅需17ms延迟，模型大小176KB，适合实时应用；其他模型在资源效率上表现优异。
...
### [68] [Improving Prostate Gland Segmenting Using Transformer based Architectures](https://arxiv.org/abs/2506.14844)
*Shatha Abudalou*
...
<details>
  <summary>Details</summary>
Motivation: 解决T2加权MRI图像中前列腺分割的读者间变异性和跨站点域偏移问题，探索transformer模型在此类异构数据中的精度保持能力。
...
### [69] [Foundation Artificial Intelligence Models for Health Recognition Using Face Photographs (FAHR-Face)](https://arxiv.org/abs/2506.14909)
...
TL;DR: FAHR-Face是一个基于4000万张面部图像训练的基础模型，用于生物年龄估计和生存风险预测。FAHR-FaceAge在公开数据集上表现最佳，FAHR-FaceSurvival能稳健预测癌症患者死亡率。两者结合提高了预后准确性。
...
Method: FAHR-FaceAge通过两阶段微调训练，FAHR-FaceSurvival在癌症患者数据上微调。测试了模型鲁棒性和独立性，并在临床数据中验证。

Result: FAHR-FaceAge在年龄估计上误差最低（5.1年），FAHR-FaceSurvival预测死亡率风险最高组是低风险组的3.22倍。两者结合提高了预后准确性。

Conclusion: 单一基础模型可生成低成本、可扩展的面部生物标志物，有效捕捉生物衰老和疾病相关死亡风险。
...
### [70] [Recursive Variational Autoencoders for 3D Blood Vessel Generative Modeling](https://arxiv.org/abs/2506.14914)
...
Conclusion: RvNN首次用于血管合成，为医学领域提供了高精度和多样性的血管生成方法。
...
### [71] [NeuroMoE: A Transformer-Based Mixture-of-Experts Framework for Multi-Modal Neurological Disorder Classification](https://arxiv.org/abs/2506.14970)
...
Method: 使用Transformer编码器捕获MRI数据的空间关系，结合模态特定的专家进行特征提取，并通过自适应融合门控机制动态整合专家输出。

Result: 验证准确率达到82.47%，优于基线方法10%以上。

Conclusion: 该框架在多模态学习应用于临床数据方面具有潜力，可显著提升神经退行性疾病的诊断效果。
...
### [72] [Classification of Multi-Parametric Body MRI Series Using Deep Learning](https://arxiv.org/abs/2506.15182)
*Boah Kim,Tejas Sudharshan Mathai,Kimberly Helm,Peter A. Pinto,Ronald M. Summers*

Main category: eess.IV

TL;DR: 提出了一种基于深度学习的多参数MRI序列分类模型，DenseNet-121表现最佳，准确率高且适用于不同数据分布。


<details>
  <summary>Details</summary>
Motivation: 解决DICOM头信息错误导致的多参数MRI序列分类问题，提高放射科医生的工作效率。

Method: 使用ResNet、EfficientNet和DenseNet训练分类器，比较性能并研究训练数据量和外部数据集的影响。

Result: DenseNet-121在内部和外部数据集上分别达到0.972和0.872/0.810的准确率，性能随训练数据量增加而提升。

Conclusion: DenseNet-121模型在多参数MRI序列分类任务中表现优异，适用于不同来源的数据。
...
### [73] [ABC: Adaptive BayesNet Structure Learning for Computational Scalable Multi-task Image Compression](https://arxiv.org/abs/2506.15228)
...
<details>
  <summary>Details</summary>
Motivation: 神经图像压缩（NIC）因计算需求高而难以广泛应用，现有方法缺乏全面的计算复杂度控制。

Method: ABC框架提出异构二分贝叶斯网络（管理神经主干计算）、同构多分贝叶斯网络（优化自回归单元处理）和自适应控制模块。

Result: 实验显示ABC在保持压缩性能的同时，实现了更好的计算适应性和更广的复杂度控制范围。
...
TL;DR: 提出了一种基于VQGAN和同态加密（HE）的医疗图像推理框架，通过压缩图像和优化激活函数，显著降低了计算负担，同时保持了图像质量。


<details>
  <summary>Details</summary>
Motivation: 医疗图像包含敏感信息，需要隐私保护；HE虽然能保护隐私，但计算成本高，尤其是大图像。

Method: 使用VQGAN压缩图像为潜在表示，用低阶多项式逼近激活函数，结合Squeeze and Excitation模块优化HE框架。
...
### [75] [Brain Stroke Classification Using Wavelet Transform and MLP Neural Networks on DWI MRI Images](https://arxiv.org/abs/2506.15364)
*Mana Mohammadi,Amirhesam Jafari Rad,Ashkan Behrouzi*

Main category: eess.IV

TL;DR: 本文提出了一种基于小波变换和多层感知机（MLP）的轻量级框架，用于从DWI MRI扫描中分类脑卒中类型，旨在平衡诊断准确性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 准确及时的脑卒中检测对治疗和患者预后至关重要，但传统CNN方法在资源受限的临床环境中难以部署。

Method: 结合小波变换和紧凑型MLP，使用'Brain Stroke MRI Images'数据集进行分类。

Result: 使用'db4'小波（3级分解）和'Haar'小波（2级分解）分别达到82.0%和86.00%的分类准确率。

Conclusion: 该方法为自动化卒中诊断提供了实用解决方案，未来将增强模型鲁棒性并整合更多MRI模态。

Abstract: This paper presents a lightweight framework for classifying brain stroke
...
### [76] [FedWSIDD: Federated Whole Slide Image Classification via Dataset Distillation](https://arxiv.org/abs/2506.15365)
...
Method: 提出FedWSIDD，利用数据集蒸馏生成合成切片并传输，结合染色归一化优化蒸馏过程。
...
### [77] [A Real-time Endoscopic Image Denoising System](https://arxiv.org/abs/2506.15395)
...
<details>
  <summary>Details</summary>
Motivation: 超小型内窥镜传感器因感光面积小、动态范围受限及模拟信号传输引入噪声，导致图像质量下降，亟需有效的去噪方法。

Method: 开发了全面的噪声模型，结合传统图像处理算法与学习技术，提出混合去噪系统。

Result: 实验显示系统有效降噪，PSNR从21.16提升至33.05，且实时运行于FPGA平台。

Conclusion: 混合去噪系统显著改善内窥镜图像质量，适用于实际医疗场景。
...
### [78] [Advanced cervical cancer classification: enhancing pap smear images with hybrid PMD Filter-CLAHE](https://arxiv.org/abs/2506.15489)
...
Result: 混合PMD-CLAHE方法显著提升了分类性能，最高提升13.62%的准确率。
...
### [79] [Construction of an Organ Shape Atlas Using a Hierarchical Mesh Variational Autoencoder](https://arxiv.org/abs/2506.15557)
*Zijie Wang,Ryuichi Umehara,Mitsuhiro Nakamura,Megumi Nakao*

Main category: eess.IV

TL;DR: 提出了一种基于分层潜在变量的网格变分自编码器（MeshVAE）的器官形状图谱，用于表示生物器官的复杂形状和非线性个体差异，并通过分层潜在变量实现低维形状表示。


<details>
  <summary>Details</summary>
Motivation: 由于软器官的形状和位置在患者间差异大，线性模型难以重建具有大局部变化的形状，而传统非线性模型难以控制和解释生成的形状，因此需要一种新的方法。

Method: 使用MeshVAE和分层潜在变量，通过模板定义顶点对应关系，实现网格数据的分层表示，并控制器官形状的全局和局部特征。

Result: 在124例肝脏和胃器官网格数据上训练模型，测试数据（19例）显示肝脏和胃形状的平均顶点距离分别为1.5 mm和1.4 mm，平均距离分别为0.7 mm和0.8 mm。

Conclusion: 该方法能够连续表示插值形状，并通过分层潜在变量分离形状特征，优于PCA方法。

Abstract: An organ shape atlas, which represents the shape and position of the organs
...
### [80] [Automated MRI Tumor Segmentation using hybrid U-Net with Transformer and Efficient Attention](https://arxiv.org/abs/2506.15562)
*Syed Haider Ali,Asrar Ahmad,Muhammad Ali,Asifullah Khan,Muhammad Shahban,Nadeem Shaukat*

Main category: eess.IV

TL;DR: 该研究提出了一种基于混合UNet-Transformer模型的肿瘤自动分割方法，用于本地医院的MRI数据集，通过数据增强和高效架构设计，取得了竞争性的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI分割模型通常基于大型公共数据集训练，缺乏对本地患者群体的适应性，因此需要开发适用于本地数据集的模型以优化肿瘤放疗计划。

Method: 结合UNet和Transformer的混合架构，引入多种注意力模块（如SE、CBAM等），并通过数据增强和预训练权重初始化提升模型性能。

Result: 在本地MRI数据集上，Dice系数为0.764，IoU为0.736，表现竞争性。
...
### [81] [You Only Render Once: Enhancing Energy and Computation Efficiency of Mobile Virtual Reality](https://arxiv.org/abs/2506.15183)
...
<details>
  <summary>Details</summary>
Motivation: 移动VR需要高效且节能的渲染技术，传统方法因双目图像两次渲染导致计算和能耗瓶颈。

Method: 利用逐像素属性，EffVR从单目图像生成双目VR图像，仅需一次渲染。

Result: EffVR平均节省27%能耗，图像质量高（SSIM 0.9679，PSNR 34.09），帧率提升115.2%。

Conclusion: EffVR在计算和节能方面表现优异，为可持续移动VR铺平道路。
...
### [82] [Human Motion Capture from Loose and Sparse Inertial Sensors with Garment-aware Diffusion Models](https://arxiv.org/abs/2506.15290)
*Andela Ilic,Jiaxi Jiang,Paul Streli,Xintong Liu,Christian Holz*

Main category: cs.GR

TL;DR: 提出了一种使用稀疏、松散附着IMU传感器进行全身姿态估计的新任务，通过模拟数据和基于transformer的扩散模型解决，实验表明其性能优于现有方法。
...
Method: 模拟松散IMU数据，开发基于transformer的扩散模型，并结合服装参数训练模型。

Result: 提出的扩散模型在模拟和合成数据上训练后，定量和定性均优于现有方法。

Conclusion: 该方法为松散附着IMU的姿态估计提供了新方向，具有潜在的研究价值。
...
### [83] [One-shot Face Sketch Synthesis in the Wild via Generative Diffusion Prior and Instruction Tuning](https://arxiv.org/abs/2506.15312)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在大规模数据稀缺时性能下降的问题，并降低人工成本。

Method: 利用扩散模型优化文本指令，通过梯度优化生成指令用于推理，并引入OS-Sketch数据集进行测试。

Result: 实验表明，该方法在单次训练下能生成高质量且一致的素描，具有更高的便利性和适用性。

Conclusion: 该方法在数据稀缺场景下表现优异，为实际应用提供了更高效的解决方案。
...
### [84] [Nabla-R2D3: Effective and Efficient 3D Diffusion Alignment with 2D Rewards](https://arxiv.org/abs/2506.15684)
...
### [85] [Forecasting the spatiotemporal evolution of fluid-induced microearthquakes with deep learning](https://arxiv.org/abs/2506.14923)
*Jaehong Chung,Michael Manga,Timothy Kneafsey,Tapan Mukerji,Mengsu Hu*

Main category: physics.geo-ph

TL;DR: 论文提出了一种基于Transformer的深度学习模型，用于预测微地震（MEQ）的时空演化，应用于增强地热系统（EGS）等场景，取得了高精度的预测结果。
...
Method: 使用Transformer模型，输入水力刺激历史和先前的MEQ观测数据，预测MEQ数量、地震矩及MEQ云的分布范围。

Result: 在EGS Collab Experiment 1数据集上，模型在1秒和15秒预测范围内均取得高R²值（>0.98和>0.88），并提供不确定性估计。
...
### [86] [Blended parameterization in an atmospheric model: Improving severestorm ensemble prediction by considering uncertainties in model physics](https://arxiv.org/abs/2506.15472)
*Khanh Hung Mai,Duc Le,Kazuo Saito,Tomizawa Futo,Yohei Sawada*

Main category: physics.geo-ph

TL;DR: 论文提出了一种名为“混合参数化”的方法，通过权重参数优化物理参数化方案的选择，以减少强风暴预测中的不确定性。


<details>
  <summary>Details</summary>
Motivation: 由于大气模型分辨率不足，无法显式解析与降水系统相关的次网格尺度过程，物理参数化方案的选择引入不确定性，影响降水模拟。

Method: 采用“混合参数化”方法，通过权重参数优化物理参数化方案的选择，并利用MCMC方法估计权重参数的后验概率分布，通过代理模型降低计算成本。

Result: 优化的“混合参数化”集合预报系统在越南的降雨预测中表现优于传统物理集合预报系统。

Conclusion: “混合参数化”方法有效减少了模型物理不确定性，提升了强风暴预测的准确性。

Abstract: Physics parameterizations are often needed for numerical weather prediction
(NWP) of precipitation forecast. This is mainly because the resolutions of most
computational atmospheric models are not fine enough to explicitly resolve
sub-grid scale processes associated with precipitation systems. The various
options in each physical parameterization scheme introduce model physics
uncertainty, leading to variations in simulated precipitation due to differing
representations of physical processes. We aim to quantify and reduce
uncertainties in severe storm prediction arising from selecting physics
parameterization schemes. In this study, we introduced a method called "blended
parameterization" in an atmospheric model. This method parameterizes the
selection of physical parameterization schemes using weighting parameters. This
approach reduces the model selection problem to the optimization of these
weighting parameters. The Markov Chain Monte Carlo (MCMC) method was used to
estimate the posterior probability distribution of the weighting parameters.
The large computational cost of MCMC was resolved by a surrogate model that
efficiently mimics the relationship between weighting parameters and
likelihood. Subsequently, the weighting parameters were sampled from the
posterior distribution, followed by conducting an ensemble simulation to
predict rainfall in Vietnam. Our optimized "blended parameterized" ensemble
prediction system outperformed a conventional physical ensemble prediction
system similar to that operationally used in Vietnam.
...
### [87] [pycnet-audio: A Python package to support bioacoustics data processing](https://arxiv.org/abs/2506.14864)
...
<details>
  <summary>Details</summary>
Motivation: 传统手动处理大规模音频数据效率低下，需要自动化工具来高效分析野生动物声音和环境噪声。

Method: 利用自动化录音设备（ARUs）采集数据，通过PNW-Cnet模型自动检测目标物种的叫声及其他信号。
...
### [88] [An accurate and revised version of optical character recognition-based speech synthesis using LabVIEW](https://arxiv.org/abs/2506.15029)
...
<details>
  <summary>Details</summary>
Motivation: 视障人士通常依赖盲文和音频书籍，但这些方式受限，无法满足个性化需求。语音作为更有效的沟通方式，可以解决这一问题。
...
### [89] [An Empirical Study of Bugs in Data Visualization Libraries](https://arxiv.org/abs/2506.15084)
*Weiqi Lu,Yongqiang Tian,Xiaohan Zhong,Haoyang Ma,Zhenyang Xu,Shing-Chi Cheung,Chengnian Sun*

Main category: cs.SE

TL;DR: 该研究首次全面分析了数据可视化库中的错误，收集了五个常用库的564个错误，系统分析了症状和根本原因，并提出了一种分类法。研究发现，错误/不准确的图表普遍存在，主要原因是图形计算错误，需要进一步开发自动化测试方法。此外，研究还探索了视觉语言模型（VLMs）在检测错误图表中的可行性。


<details>
  <summary>Details</summary>
Motivation: 数据可视化库的准确性对用户体验和信息传达至关重要，但其中的视觉错误可能导致误导性决策。因此，深入了解这些错误的特性对检测和修复至关重要。

Method: 研究收集了五个广泛使用的数据可视化库中的564个错误，系统分析了其症状和根本原因，并提出了一种分类法。同时探索了视觉语言模型（VLMs）在检测错误图表中的应用。

Result: 研究发现错误/不准确的图表普遍存在，主要原因是图形计算错误。VLMs在检测错误图表中的有效性介于29%到57%之间，且提示信息的增加不一定提高效果。

Conclusion: 研究强调了数据可视化库中错误的普遍性和潜在影响，提出了自动化测试方法的必要性，并探索了VLMs的应用潜力，为未来研究提供了方向。
...
### [90] [Omnidirectional Video Super-Resolution using Deep Learning](https://arxiv.org/abs/2506.14803)
*Arbind Agrahari Baniya,Tsz-Kwan Lee,Peter W. Eklund,Sunil Aryal*

Main category: cs.MM

TL;DR: 论文提出了一种新的360度视频超分辨率方法S3PO，并创建了360VDS数据集，解决了现有方法在360度视频中的失真问题。


<details>
  <summary>Details</summary>
Motivation: 360度视频在VR中广泛应用，但分辨率不足影响视觉质量，且现有超分辨率技术未解决其投影失真问题。

Method: 提出S3PO模型，采用循环建模和注意力机制，设计了专用特征提取器和损失函数。

Result: S3PO在360度视频数据集上优于现有超分辨率模型。

Conclusion: S3PO有效解决了360度视频超分辨率问题，并通过消融研究验证了其设计。
...
### [91] [Embodied Web Agents: Bridging Physical-Digital Realms for Integrated Agent Intelligence](https://arxiv.org/abs/2506.15677)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI代理在物理世界交互与网络信息处理上存在割裂，限制了其在需要综合能力的任务中的应用。

Method: 开发了统一的仿真平台和任务环境，构建了包含多种跨领域任务的基准测试。

Result: 实验显示现有AI系统与人类能力存在显著差距，突出了跨领域智能的挑战与机遇。
...
### [92] [Towards Perception-based Collision Avoidance for UAVs when Guiding the Visually Impaired](https://arxiv.org/abs/2506.14857)
*Suman Raj,Swapnil Padhi,Ruchi Bhoot,Prince Modi,Yogesh Simmhan*

Main category: cs.RO

TL;DR: 无人机通过机载传感器结合机器学习和计算机视觉算法实现自主导航，应用于农业、物流和灾害管理等领域。本文探讨无人机辅助视障人士（VIPs）在城市户外环境中的导航，提出一种基于感知的路径规划系统，结合全局规划器实现避障。


<details>
  <summary>Details</summary>
Motivation: 无人机导航技术已广泛应用于多个领域，但其在辅助视障人士方面的潜力尚未充分开发。本文旨在填补这一空白，帮助VIPs在城市环境中更安全地导航。

Method: 提出一种几何问题建模方法，结合多深度神经网络（DNN）框架，实现无人机和VIP的避障。系统包括局部感知规划和基于GPS的全局规划。

Result: 在大学校园环境中对无人机-人系统进行评估，验证了算法在三种场景（人行道、停车区、拥挤街道）中的可行性。

Conclusion: 研究表明，所提出的系统能够有效辅助VIPs在城市环境中导航，为未来实际应用提供了技术基础。
...
### [93] [Robust Instant Policy: Leveraging Student's t-Regression Model for Robust In-context Imitation Learning of Robot Manipulation](https://arxiv.org/abs/2506.15157)
*Hanbit Oh,Andrea M. Salcedo-Vázquez,Ixchel G. Ramirez-Alpizar,Yukiyasu Domae*

Main category: cs.RO

TL;DR: 论文提出了一种鲁棒的上下文模仿学习算法（RIP），通过学生t回归模型减少LLM生成的幻觉轨迹，显著提高了任务成功率。
...
Method: 利用学生t分布聚合LLM生成的多个候选轨迹，忽略异常值（幻觉），生成鲁棒轨迹。

Result: 在模拟和真实环境中，RIP显著优于现有方法，任务成功率提升至少26%。

Conclusion: RIP算法有效解决了幻觉问题，提升了模仿学习的可靠性和性能。
...
### [94] [MCOO-SLAM: A Multi-Camera Omnidirectional Object SLAM System](https://arxiv.org/abs/2506.15402)
...
Conclusion: MCOO-SLAM通过多相机配置和语义增强，显著提升了SLAM系统的鲁棒性和实用性。
...
### [95] [Particle-Grid Neural Dynamics for Learning Deformable Object Models from RGB-D Videos](https://arxiv.org/abs/2506.15680)
...
Method: 采用粒子-网格混合表示，粒子表示物体形状，空间网格确保空间连续性并提升学习效率，结合高斯渲染生成3D视频。
...
Result: 在110个SJTU-TMQA网格上，迭代次数减少2.4倍，曲率分布更紧密，UV失真与MOS相关性达-0.93，单次迭代仅增加0.25毫秒。

Conclusion: MicroRicci适用于实时资源受限的图形和模拟应用，因其线性时间更新、自适应超参数和高几何感知质量。
...
### [97] [PIPE: Physics-Informed Position Encoding for Alignment of Satellite Images and Time Series](https://arxiv.org/abs/2506.14786)
...
Result: 在最大的开源卫星图像数据集上，PIPE在深度学习和气候领域方法中均达到最先进性能，台风强度预测比之前工作提升了12%。
...
### [98] [Reinforcing VLMs to Use Tools for Detailed Visual Reasoning Under Resource Constraints](https://arxiv.org/abs/2506.14821)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型模型在推理能力上取得显著进展，但视觉语言模型（VLMs）在计算资源有限时仍难以进行详细视觉推理。

Method: 采用Group Relative Policy Optimization（GRPO）训练小规模模型，结合简单奖励结构、简化工具调用接口、额外分配工具调用结果的令牌，以及过采样视觉困难样本的训练数据。

Result: 相比同类基线模型，该方法在某些视觉问答（VQA）任务中表现更优，得益于从外部工具获取的详细视觉信息。
...
### [99] [CACTUS as a Reliable Tool for Early Classification of Age-related Macular Degeneration](https://arxiv.org/abs/2506.14843)
...
TL;DR: 论文提出CACTUS工具，用于改进AMD分期分类，解决医疗数据不完整和模型透明度问题，并提高分类准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 医疗数据通常不完整且有限，影响机器学习模型性能，同时模型缺乏透明度。AMD早期诊断至关重要，但缺乏有效治疗方法，需结合多因素分类方法。

Method: 引入CACTUS工具，结合遗传、饮食、临床和人口统计因素，提供可解释性和灵活性，优化分类性能。

Result: CACTUS优于标准机器学习模型，能识别关键因素并提供结果置信度，与现有医学知识一致。

Conclusion: CACTUS通过消除不相关或有偏数据，为临床决策提供支持，并允许临床医生反馈以解决偏见。
...
### [100] [Pixel-level Certified Explanations via Randomized Smoothing](https://arxiv.org/abs/2506.15499)
...
<details>
  <summary>Details</summary>
Motivation: 现有的事后归因方法对输入扰动高度敏感，导致归因图不稳定，影响了其可信度。因此，需要一种能够提供严格鲁棒性保证的方法。

Method: 通过随机平滑技术，将归因图稀疏化和平滑化，并将其重新表述为分割问题，以认证每个像素的重要性。提出了三种评估指标来衡量认证鲁棒性、定位性和忠实性。
