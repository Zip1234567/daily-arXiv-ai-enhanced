...
TL;DR: 提出了一种动态调整对抗训练扰动预算的方法（DES），结合梯度代理、预测置信度和模型不确定性，显著提升了对抗鲁棒性和标准准确性。


<details>
  <summary>Details</summary>
Motivation: 现有对抗训练方法依赖固定扰动预算，无法适应实例特定的鲁棒性需求，导致效果受限。
...
TL;DR: RSVP是一个结合多模态推理与视觉分割的两阶段框架，通过视觉提示和分割模块提升视觉定位与分割精度，显著优于现有方法。
...
Method: RSVP采用两阶段框架：1）推理阶段，通过多模态链式视觉提示生成区域建议；2）分割阶段，利用视觉语言分割模块（VLSM）结合文本与视觉线索优化分割掩码。

Result: RSVP在ReasonSeg上比现有方法提升+6.5 gIoU和+9.2 cIoU，在SegInW上零样本设置下达到49.7 mAP。

Conclusion: RSVP通过显式建模推理与分割的交互，为可解释的推理分割提供了新范式，验证了其高效性和可扩展性。
...
TL;DR: 论文提出了首个多图像推理基准MMRB，评估多模态大语言模型在多图像任务中的结构化推理能力，发现开源模型与商业模型存在显著差距。
...
Method: 设计MMRB基准，包含92个子任务，覆盖空间、时间和语义推理，并采用GPT-4o生成多解法和CoT式注释。提出基于开源LLM的句子级匹配框架。

Result: 实验显示开源MLLM在多图像推理任务中显著落后于商业模型，且当前多模态奖励模型几乎无法处理多图像奖励排序任务。

Conclusion: MMRB填补了多图像推理评估的空白，揭示了开源模型和奖励模型在此领域的不足。
...
TL;DR: 提出了一种弱监督的3D人体生成流程，通过图像扩散模型生成可控属性数据集，结合Transformer架构映射到3D点云，并训练点云扩散模型，显著提升了生成速度、真实性和文本对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在3D人体生成的细节、真实性和可控性方面存在不足，且缺乏多样化和标注数据，阻碍了基础模型的开发。

Method: 1. 使用图像扩散模型生成可控属性的真实人体图像数据集；2. 提出基于Transformer的图像特征到3D点云的高效映射方法；3. 训练基于文本提示的点云扩散模型。

Result: 相比现有方法，实现了数量级的生成速度提升，显著改善了文本对齐、真实性和渲染质量。

Conclusion: 提出的流程有效解决了3D人体生成的挑战，为未来研究提供了数据和代码支持。
...
<details>
  <summary>Details</summary>
Motivation: 为胸片视觉问答提供多样化和临床真实的评估任务，弥补现有模板化查询的不足，推动AI系统在放射学推理能力上的发展。

Method: 构建ReXVQA基准，包含五种放射学推理任务，评估8种多模态大语言模型，并与3名放射科住院医师进行人类读者研究对比。

Result: MedGemma模型以83.24%的准确率表现最佳，首次超越人类专家（77.27%），并在人类读者研究中显示出与人类不同的性能模式。
...
### [6] [Physics Informed Capsule Enhanced Variational AutoEncoder for Underwater Image Enhancement](https://arxiv.org/abs/2506.04753)
*Niki Martinel,Rita Pucci*

Main category: cs.CV

TL;DR: 提出了一种新颖的双流架构，通过结合物理模型和胶囊聚类特征学习，实现了无需参数的图像增强，显著提升了性能并降低了计算复杂度。
...
Method: 采用双流架构，分别通过物理估计器和胶囊聚类学习特征，结合物理指导和感知质量优化目标。
...
TL;DR: WorldPrediction是一个基于视频的基准测试，用于评估AI模型的世界建模和程序规划能力，强调具有时间和语义抽象的动作。当前前沿模型表现远低于人类。


<details>
  <summary>Details</summary>
Motivation: AI代理需要具备类似人类的世界模型以进行动作规划，但目前不清楚生成模型如何学习此类模型。

Method: 引入WorldPrediction基准，通过区分正确动作或动作序列来评估模型，使用视觉观察表示状态和动作，并提供“动作等效”以避免低级别线索的利用。

Result: 当前模型在WorldPrediction-WM和WorldPrediction-PP上的准确率分别为57%和38%，远低于人类的完美表现。

Conclusion: WorldPrediction为评估世界建模和规划能力提供了可靠基准，揭示了当前模型的局限性。
...
TL;DR: PLUCC是一种基于上下文线索的冰球定位方法，利用球员行为和姿态改进冰球检测性能。
...
TL;DR: 论文研究了使用自我中心设备进行高动态范围场景光真实重建的挑战，提出了视觉惯性束调整（VIBA）和高斯泼溅物理图像形成模型，显著提升了重建质量。
...
Result: 实验表明，VIBA和图像形成模型分别提升PSNR +1 dB，总提升+2 dB。

Conclusion: 该方法在多种光照条件下均表现优异，显著提升了光真实重建的质量。
...
TL;DR: 该论文研究了基于视频变换器架构（VideoMAE、ViViT和TimeSformer）的孟加拉手语（BdSL）识别方法，并在小型（BdSLW60）和大型（BdSLW401）数据集上进行了评估。结果显示VideoMAE表现最佳，准确率分别达到95.5%和81.04%。


<details>
  <summary>Details</summary>
Motivation: 提高孟加拉手语的自动识别能力，为听力障碍群体提供更好的无障碍服务。

Method: 使用VideoMAE、ViViT和TimeSformer等视频变换器架构，结合数据增强技术和分层交叉验证，对BdSLW60和BdSLW401数据集进行微调和评估。

Result: VideoMAE在BdSLW60和BdSLW401数据集上分别达到95.5%和81.04%的准确率，显著优于传统方法。

Conclusion: 视频变换器模型在孟加拉手语识别中表现出色，尤其是VideoMAE，具有扩展性和高准确性的潜力。
...
TL;DR: 论文提出了一种方法，将激活最大化技术应用于基于DNN的脑编码模型，以生成能驱动特定脑区响应的图像，并验证了其有效性。
...
Conclusion: 激活最大化可成功应用于DNN编码模型，为人类视觉系统的响应特征和调控提供了灵活方法。
...
Result: 实验表明，保护方法未能完全阻止编辑，反而可能使编辑结果更符合文本提示，导致更好的编辑效果。
...
Method: 提出滤波器归一化方法，随后进行可学习的缩放和平移，类似于批量归一化，确保滤波器的大气等变性。
...
TL;DR: HMAR是一种新的图像生成算法，通过改进VAR的并行生成问题，采用分层掩码自回归建模，实现了更高质量的图像生成和更快的采样速度。


<details>
  <summary>Details</summary>
Motivation: 解决VAR在并行生成时图像质量下降、序列长度随分辨率超线性增长以及需要重新训练调整采样计划的问题。

Method: HMAR将下一尺度预测重新定义为马尔可夫过程，仅基于直接前驱尺度的令牌进行预测，并采用可控的多步掩码生成过程。

Result: 在ImageNet 256x256和512x512基准测试中，HMAR表现优于或匹配VAR、扩散模型和自回归基线模型，训练和推理速度分别提升2.5倍和1.75倍，内存占用降低3倍。

Conclusion: HMAR不仅提升了图像生成质量和效率，还提供了更高的灵活性，无需重新训练即可调整采样计划，并能零样本应用于图像编辑任务。
...
TL;DR: 提出了一种称为“面部去正面化”的方法，通过增强训练数据集来提高面部特征提取模型的性能，避免了推理时的额外时间开销。
...
Conclusion: 面部去正面化方法在大规模数据集上表现优越，但现有方法可能在小数据集上过拟合。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测技术依赖于训练时学习的统计特征，难以应对新的伪造方法。

Method: 结合分类与图像-文本对比学习，利用MLLM生成文本，并集成数据不确定性学习。

Result: 在DFDC和DF40数据集上AUC分别提升6.15%和16.68%，在DDVQA数据集上性能提升24.69%。

Conclusion: AuthGuard在泛化性、准确性和可解释性上均表现优异，成为深度伪造检测的新标杆。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型复杂度高且计算成本大，现有剪枝方法仅针对神经元或层，无法同时剪枝，限制了效率提升。

Method: 通过比较层剪枝和神经元剪枝生成的子网络，选择与父网络表示相似性最高的子网络，迭代生成稀疏模型。

Result: 在ResNet56和ResNet110上分别实现86.37%和95.82%的FLOPs减少，同时保持或提升准确性，并增强对抗样本的鲁棒性。

Conclusion: 该方法为剪枝领域开辟了新方向，显著减少计算成本和碳排放，推动GreenAI发展。
...
TL;DR: 提出了一种名为LSKs的即插即用模块，通过可学习的可分离核直接增强图像频率成分，显著减少参数和计算需求，同时提升模型性能。
...
Method: 设计了一种可学习的可分离核（LSKs），将其分解为可合并的一维核，减少参数和计算量，并通过频率视角分析其适用性。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型中视觉与推理解耦时描述生成不准确的问题。

Method: 采用推理导向的强化学习策略（RACRO），通过奖励优化视觉提取器的描述生成。

Result: 在数学和科学多模态基准测试中达到最优性能，并支持低成本适配更先进的推理模型。

Conclusion: RACRO方法有效提升了视觉与推理的对齐，具有可扩展性和灵活性。
...
TL;DR: 提出了一种轻量级全局建模网络（LGM-Pose），通过单分支结构和创新模块解决多分支CNN网络在全局上下文捕捉和高延迟上的问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决从单目视频生成和编辑4D内容时因相机轨迹变化或用户编辑导致的缺失内容填充问题。

Method: 利用视频修复基础模型作为生成先验，生成复合掩码数据并微调模型，结合自迭代调优策略和时序包装模块提升生成质量。

Result: 方法在质量和多功能性上显著优于现有技术，支持基于提示的内容编辑，生成具有多视角一致性的4D视频。

Conclusion: 该框架有效利用基础模型的先验知识，实现了高质量的4D视频生成与编辑。
...
TL;DR: 提出了一种分层持续学习框架（HEC）和任务感知的增量LoRA专家混合方法（Task-aware MoILE），通过分层学习和任务识别减少灾难性遗忘。
...
Method: 将学习分为高低两层，通过聚类视觉-文本嵌入识别任务，使用任务级和令牌级路由器选择LoRA专家，并利用SVD保留关键参数。

Result: 实验表明，该方法在减少旧任务遗忘方面优于其他方法。

Conclusion: HEC和Task-aware MoILE能有效支持智能体在持续学习中保留旧知识。
...
TL;DR: SmartAvatar是一个基于视觉-语言-代理的框架，通过单张照片或文本提示生成可动画的3D人体化身，结合大型视觉语言模型和参数化人体生成器，实现高质量、可定制的化身生成。
...
Result: 生成的化身支持姿势操作且身份一致，定量评估和用户研究表明其在网格质量、身份保真度等方面优于现有方法。
...
TL;DR: 提出了一种名为Perfecting Depth的两阶段传感器深度增强框架，结合扩散模型的随机性和确定性细化，提升深度图的可靠性和质量。


<details>
  <summary>Details</summary>
Motivation: 传感器深度测量中存在不可靠区域，需通过自动检测和保留几何线索来增强深度图。
...
TL;DR: 本文综述了深度学习如何逐步改变传统图像匹配流程，包括替换传统步骤为可学习模块和合并多步骤为端到端模块，并评估了代表性方法，讨论了未来研究方向。
...
Method: 通过替换传统流程中的步骤为可学习模块（如检测器-描述符、离群值过滤器）和合并多步骤为端到端模块（如稀疏匹配器、姿态回归器），分类并评估代表性方法。

Result: 在相对姿态恢复、单应性估计和视觉定位任务中，深度学习方法表现出更高的鲁棒性和准确性。
...
TL;DR: STARE是一个评估多模态大语言模型在空间认知任务中表现的基准，涵盖几何变换、空间推理和现实世界问题。模型在简单任务中表现良好，但在复杂任务中接近随机水平，且无法有效利用视觉模拟。
...
Method: STARE包含4K个任务，涵盖几何变换（2D和3D）、综合空间推理（如立方体展开和七巧板）和现实世界空间推理（如视角和时间推理）。

Result: 模型在简单2D任务中表现良好，但在复杂任务（如3D立方体展开和七巧板）中接近随机水平。人类表现优异但耗时较长，而模型无法有效利用视觉模拟。

Conclusion: STARE揭示了模型在复杂空间认知任务中的局限性，表明其无法像人类一样有效利用视觉模拟，为未来研究提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决生成模型在图像超分辨率中导致的文本结构失真问题。

Method: 提出TADiSR框架，结合文本感知注意力和联合分割解码器，并设计合成高质量图像的完整流程。

Result: 显著提升超分辨率图像中文本的可读性，在多项评估指标中达到最优性能。
...
TL;DR: FPSAttention是一种结合FP8量化和稀疏化的视频生成方法，通过联合优化显著提升推理速度，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散生成模型推理速度慢且计算需求高，现有方法无法有效结合量化和稀疏化技术。

Method: 提出FPSAttention，采用3D分块粒度、去噪步感知策略和硬件友好内核设计。

Result: 在720p分辨率下，注意力操作提速7.09倍，端到端视频生成提速4.96倍。

Conclusion: FPSAttention在保持生成质量的同时显著提升了视频生成的效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统表示学习假设解耦的独立特征轴是好的表示，但无法解释条件独立性。需要一种更通用的表示方法。

Method: 通过结合特征提取和对象分割，将像素平移替换为特征平移，并将对象分割视为同一变换下的特征分组。
...
TL;DR: Gen-n-Val 是一种新型数据生成框架，利用 Layer Diffusion、LLMs 和 VLLMs 生成高质量单对象掩码和多样化背景，显著减少无效数据并提升性能。
...
Method: Gen-n-Val 包含两个代理：LD 提示代理优化提示生成高质量单对象数据，数据验证代理过滤低质量数据，并通过 TextGrad 优化系统提示。

Result: 将无效合成数据从 50% 降至 7%，在 COCO 实例分割中提升 1% mAP，开放词汇目标检测中提升 7.1% mAP。
...
Conclusion: MARS展示了强大的实际应用潜力，能够高效重建无线电地图。
...
TL;DR: 论文提出了HoliSafe数据集和SafeLLaVA模型，以解决现有视觉语言模型（VLM）安全性的不足，通过全面覆盖图像-文本组合和引入可学习的安全元标记与专用安全头，显著提升了模型的安全性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM安全性方法存在两个主要问题：1）数据集和基准测试未全面考虑图像-文本交互可能产生的有害内容；2）方法主要依赖数据调整，缺乏架构创新。

Method: 引入HoliSafe数据集，覆盖五种安全/不安全图像-文本组合；提出SafeLLaVA模型，包含可学习的安全元标记和安全头，用于编码有害视觉线索和分类危害性。

Result: SafeLLaVA在多个VLM基准测试中达到最先进的安全性能；HoliSafe基准揭示了现有模型的关键漏洞。
...
TL;DR: 论文研究了多模态语言模型中图像概念的表示方式，发现线性可解码特征存在于残差流中，并通过编辑模型输出来验证其因果性。


<details>
  <summary>Details</summary>
Motivation: 探索多模态模型（如LlaVA-Next）如何在其隐藏激活中表示图像概念。

Method: 使用线性解码和稀疏自编码器（SAEs）分析图像特征，并通过目标编辑验证特征的因果性。

Result: 发现ImageNet类别的线性可解码特征，且不同模态的表示在深层逐渐共享。

Conclusion: 多模态模型的图像和文本表示在深层逐渐融合，稀疏自编码器提高了特征的可解释性。
...
TL;DR: 论文研究了如何通过微调视觉编码器提升预训练视觉语言模型（VLM）在少样本适应中的分布内（ID）和分布外（OOD）准确性，并提出了一种两阶段的SRAPF方法。
...
Method: 比较了多种适应方法（如提示调优、线性探测、对比微调等），发现仅微调视觉编码器的顶层效果最佳，并提出了结合检索增强和对抗扰动的SRAPF方法。

Result: SRAPF在ImageNet OOD基准测试中达到了最先进的ID和OOD准确性。

Conclusion: 通过两阶段的SRAPF方法，成功平衡了ID和OOD准确性，为VLM的少样本适应提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 预测ESD视频中的剥离轨迹对手术技能训练至关重要，但现有方法在处理不确定的未来动作、几何对称性和多样化手术场景时存在挑战。

Method: 结合扩散模型和等变表示，通过联合状态动作分布建模专家行为，并采用前向过程引导的动作推断策略。

Result: 在近2000个ESD视频片段上的实验表明，iDPOE在轨迹预测上优于现有方法。

Conclusion: iDPOE首次将模仿学习应用于手术技能训练中的剥离轨迹预测，展现了显著优势。
...
TL;DR: 论文提出了一种基于AI的自动标注系统，用于显示屏面板缺陷检测，通过上下文学习能力优化SegGPT架构，并引入涂鸦标注机制。实验显示，该方法显著提升了性能。
...
TL;DR: LAT是一种标签对齐转移框架，通过伪标签生成和特征融合解决多数据集标注不一致问题，显著提升目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 多数据集结合可提升泛化能力，但标注语义和边界框不一致阻碍了其应用。现有方法或假设共享标签分类，或需手动重标注，无法满足固定目标标签空间的需求。

Method: LAT通过训练数据集特定检测器生成伪标签，结合Privileged Proposal Generator（PPG）和Semantic Feature Fusion（SFF）模块，实现标签空间转移和特征优化。

Result: LAT在多个基准测试中显著提升目标域检测性能，最高提升+4.8AP。

Conclusion: LAT无需共享标签空间或手动标注，有效解决了类别和空间标注不一致问题，适用于异构数据集。
...
TL;DR: 论文提出了一种名为Semantic Reward Defense（SRD）的强化学习框架，用于防御视觉语言模型（VLMs）中的后门攻击，通过扰动敏感图像区域来破坏恶意路径的激活，同时保持生成标题的质量。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在图像描述任务中表现优异，但易受后门攻击影响，攻击者通过注入微小扰动使模型生成恶意标题。这些攻击隐蔽且跨模态，难以检测和防御。

Method: 提出SRD框架，利用深度Q网络学习对敏感图像区域施加离散扰动（如遮挡、颜色掩码）的策略，并通过语义保真度评分作为奖励信号，指导模型生成鲁棒且准确的标题。

Result: 实验表明，SRD将攻击成功率降至5.6%，同时在干净输入上保持标题质量，性能下降小于10%。
...
TL;DR: 论文提出了一种名为RAP的数据选择方法，通过识别高价值的认知样本，仅用9.3%的训练数据实现了优于全数据集的性能，并降低了43%的计算成本。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）通常需要大量训练数据以提高推理能力，但数据冗余和计算成本高。论文质疑这一假设，认为仅需稀疏的认知样本即可触发有效的多模态推理。

Method: 提出RAP方法，包含两个互补的估计器：1）基于潜在结果模型的因果差异估计器（CDE），消除依赖语言先验的样本；2）注意力置信度估计器（ACE），剔除无关但被过度强调的样本。此外，引入难度感知替换模块（DRM）提升样本复杂性。

Result: 在六个数据集上的实验表明，RAP仅用9.3%的数据即可实现更优性能，同时降低43%的计算成本。

Conclusion: RAP方法通过高效数据选择，显著提升了多模态推理的效率与性能，为MLLMs的训练提供了新思路。
...
Result: 新SSIM损失函数在KITTI-2015数据集上显著优于基线方法，实现了更平滑的梯度和更高性能。

Conclusion: 新SSIM形式有效提升了无监督单目深度估计的性能，为相关研究提供了优化方向。
...
TL;DR: HypeVPR是一个新颖的层次化嵌入框架，利用双曲空间解决P2E VPR的挑战，显著提升检索速度和准确性。
...
Method: 采用双曲空间表示层次特征关系，提出层次化特征聚合机制和粗到细的搜索策略。
...
TL;DR: Object-X是一种多模态3D对象表示框架，支持编码和解码几何与视觉重建，适用于多种任务，且存储需求极低。
...
Method: Object-X通过将多模态数据（如图像、点云、文本）嵌入3D体素网格，并学习融合体素与对象属性的非结构化嵌入，支持3D高斯泼溅重建和其他下游任务。
...
TL;DR: LotusFilter是一种后处理模块，用于多样化近似最近邻搜索（ANNS）结果，通过预计算邻近向量表并贪婪删除冗余向量，实现快速（0.02 ms/query）且多样化的搜索结果。


<details>
  <summary>Details</summary>
Motivation: 在RAG等应用中，ANNS结果可能过于相似，需要同时保证结果与查询相似且多样化。

Method: 预计算邻近向量表，过滤时贪婪查找并删除冗余向量。

Result: 在类似真实RAG应用的设置中，LotusFilter运行速度快（0.02 ms/query）。

Conclusion: LotusFilter是一种高效的ANNS结果多样化方法，适用于需要多样性的应用场景。
...
Method: 提出了SupeRANSAC，一种统一的RANSAC管道，分析了使其在特定视觉任务中有效的技术。

Result: SupeRANSAC在多个任务和数据集上显著优于现有方法，例如在基础矩阵估计中平均提高6 AUC点。

Conclusion: SupeRANSAC为计算机视觉任务提供了一种更一致且高效的鲁棒性估计解决方案。
...
TL;DR: 论文介绍了MegaHan97K数据集，支持GB18030-2022标准，包含97,455个汉字类别，解决了长尾分布问题，并揭示了超大类识别的新挑战。


<details>
  <summary>Details</summary>
Motivation: 解决超大类汉字识别中数据集不足的问题，支持文化传承和数字应用。

Method: 构建MegaHan97K数据集，包含手写、历史和合成三个子集，平衡样本分布。

Result: 数据集覆盖97,455个类别，支持GB18030-2022标准，揭示了存储需求、形态相似字符识别和零样本学习等新挑战。

Conclusion: MegaHan97K为超大类识别研究提供了重要资源，推动了OCR和模式识别领域的发展。
...
TL;DR: 提出了一种基于时间二进制表示（TBR）和脉冲神经网络的噪声鲁棒事件流编码方法Spike-TBR。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高时间分辨率、低延迟和动态范围等优势，但如何高效且抗噪地转换事件流以适应标准计算机视觉流程仍具挑战。

Method: 结合TBR的帧基优势与脉冲神经网络的噪声过滤能力，提出Spike-TBR方法，并评估了四种不同脉冲神经元的变体。

Result: 在噪声场景下表现优异，同时提升了干净数据上的结果。
...
TL;DR: 论文展示了如何通过打印的对抗性补丁攻击交通灯检测的CNN模型，提出了威胁模型和训练策略，并在真实场景中验证了攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究较少关注交通灯检测器的对抗攻击，本文填补了这一空白。

Method: 提出了一种威胁模型，通过在交通灯下方放置对抗性补丁，并设计了相应的训练策略。

Result: 实验证明了通用的对抗性补丁攻击在目标标签翻转（如红变绿）和图形分类中的有效性，并在真实场景中进行了验证。

Conclusion: 研究表明，交通灯检测器易受对抗性补丁攻击，需加强防御措施。
...
TL;DR: DualX-VSR提出了一种新的双轴向时空注意力机制，解决了现有Transformer模型在视频超分辨率任务中的局限性，无需运动补偿即可实现高性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在视频超分辨率任务中因像素级精度要求和时空建模的局限性表现不佳，DualX-VSR旨在解决这些问题。

Method: 采用双轴向时空注意力机制，沿正交方向整合时空信息，简化结构并避免运动补偿。
...
Method: 引入SEG令牌和对象标识符，处理点云数据和文本提示生成分割掩码。

Result: 在ScanNet数据集上验证了模型的有效性。

Conclusion: OpenMaskDINO3D填补了3D推理分割的空白，实现了高精度分割。
...
<details>
  <summary>Details</summary>
Motivation: 从确定性图像修复技术中汲取灵感，解决稀疏观测下地质场重建问题。

Method: 结合张量补全和地质统计学，通过全局低秩结构恢复缺失值。

Result: 在合成地质场实验中，张量补全方法显著提高了重建精度。
...
TL;DR: 本文提出了一种通过毒化训练数据在图像编辑过程中嵌入隐形触发器的攻击框架，利用现成的水印模型作为触发器，成功实现了高攻击成功率。
...
Method: 利用现成的水印模型将不可见水印编码为后门触发器，通过毒化训练数据嵌入到图像编辑过程中，使模型在接收水印输入时输出预定义目标，而对干净图像正常编辑。

Result: 在不同水印模型上的实验表明，该方法攻击成功率较高，水印特性分析进一步验证了其有效性。

Conclusion: 提出的隐形触发器攻击框架在图像编辑中具有高效性和实用性，代码已开源。
...
Method: 使用监督对比学习训练Transformer编码器，将棋盘状态嵌入到按位置评估结构化的潜在空间中，距离反映评估相似性。

Result: 模型在6层束搜索下达到2593 Elo评分，性能随模型规模和嵌入维度提升，表明潜在规划可能是传统搜索的可行替代方案。
...
TL;DR: Anywhere3D-Bench是一个新的3D视觉基准测试，涵盖四种不同的定位级别，揭示了现有模型在空间和部件级别任务上的显著不足。
...
Method: 提出Anywhere3D-Bench基准测试，包含2,632对表达-边界框数据，评估了多种3D视觉定位方法和LLMs/MLLMs。

Result: 空间和部件级别任务表现最差，最佳模型OpenAI o4-mini在空间级别任务上仅23.57%准确率，部件级别为33.94%。

Conclusion: 当前模型在3D场景的空间和部件级别理解与推理能力存在明显不足。
...
TL;DR: 本文提出了一种基于3D高斯泼溅（3DGS）的立体数据集生成方法，相比基于神经辐射场（NeRF）的方法更高效。通过结合显式3D表示的重建几何和FoundationStereo模型的深度估计，实现了零样本泛化基准的竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 探索一种低成本、高保真度的立体数据集生成方法，以替代传统NeRF方法，并提升深度立体模型的快速微调能力。

Method: 利用3DGS生成数据集，结合显式3D几何重建和FoundationStereo模型的深度估计，进行专家知识迁移。

Result: 3DGS生成的数据集在零样本泛化基准上表现竞争性，但直接使用重建几何会引入噪声；FoundationStereo的视差估计更干净，性能更优。

Conclusion: 3DGS方法为低成本、高保真数据集生成提供了潜力，但其在复杂场景中的鲁棒性仍需进一步研究。
...
TL;DR: 本文探讨了文化遗产数字化中3D摄影方法的多样性，强调应根据对象特性和未来用途选择最合适的数字化工具，而非追求绝对分类。


<details>
  <summary>Details</summary>
Motivation: 文化遗产数字化已成为共识，但现有方法多样且无统一标准，需针对不同对象灵活选择工具。

Method: 通过法国国家考古博物馆的藏品案例，分析不同3D摄影数字化方法的适用性，强调跨领域协作选择工具。

Result: 研究表明，单一方法无法适用于所有对象，需根据对象特性和数字孪生的用途定制工具。

Conclusion: 文化遗产数字化需灵活选择工具，跨领域协作是关键，避免绝对分类，注重实际需求。
...
TL;DR: CzechLynx是一个大规模、开放访问的数据集，用于欧亚猞猁的个体识别、2D姿态估计和实例分割。


<details>
  <summary>Details</summary>
Motivation: 为猞猁的个体识别和姿态估计提供首个公开数据集，支持跨时空域的研究。

Method: 收集30k相机陷阱图像并标注，生成100k合成图像，定义三种评估协议。

Result: 数据集覆盖219个个体，15年监测数据，支持多种任务。

Conclusion: CzechLynx将推动猞猁识别及相关领域的研究和模型发展。
...
TL;DR: 论文提出了一种基于视频的胚胎分级任务，利用时间延迟监测视频预测胚胎质量，并设计了互补时空模式挖掘框架（CoSTeM）来整合形态和动态特征。


<details>
  <summary>Details</summary>
Motivation: 当前胚胎评估方法存在局限性，无法全面评估胚胎质量或受胚胎外因素干扰，因此需要一种直接利用时间延迟监测视频预测胚胎质量的新方法。

Method: 提出了CoSTeM框架，包含形态分支（局部结构特征选择）和形态动力学分支（全局发育轨迹建模），整合静态和动态特征。

Result: 实验结果表明CoSTeM框架优于现有方法，为AI辅助胚胎选择提供了方法论支持。
...
TL;DR: 论文提出了一种新的IQA模型设计方法，通过架构设计而非数据驱动的方式提升对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统IQA模型易受对抗攻击，现有防御方法依赖数据驱动，但可能并非最优解。本文探讨是否可以通过设计模型架构本身来提升鲁棒性。

Method: 通过正交信息流和范数保持操作重塑模型内部结构，并结合剪枝和微调进一步稳定系统。

Result: 提出了一种无需对抗训练的鲁棒IQA架构，能够有效抵御对抗攻击。

Conclusion: 论文主张从数据优化转向设计优化，为提升模型鲁棒性提供了新视角。
...
Method: APVR包含两个部分：Pivot Frame Retrieval（语义扩展和多模态置信度评分）和Pivot Token Retrieval（查询感知的注意力驱动令牌选择）。

Result: 在LongVideoBench和VideoMME上表现优异，达到SOTA效果，且兼容现有MLLM架构。
...
Method: 比较了SAM模型在三种森林类型中的应用，并研究了DSM信息的集成效果，提出了BalSAM模型。

Result: BalSAM在种植园场景中表现优于其他方法，但未经调优的SAM模型表现不及定制Mask R-CNN。

Conclusion: 端到端调优SAM和集成DSM信息是提升树冠实例分割模型性能的有效途径。
...
TL;DR: 论文介绍了TextVidBench，首个专注于长视频文本问答的基准测试，解决了现有数据集视频时长短、评估范围窄的问题，并提出了一种改进大型模型的方法。


<details>
  <summary>Details</summary>
Motivation: 现有短视频文本视觉问答（ViteVQA）数据集因视频时长和评估范围有限，难以充分评估多模态大语言模型（MLLMs）的能力。

Method: 1）提出TextVidBench基准，覆盖9个领域的长视频；2）设计三阶段评估框架；3）引入IT-Rope机制、非均匀位置编码和轻量级微调。

Result: TextVidBench对现有模型提出了显著挑战，提出的方法在多个数据集上验证有效。

Conclusion: TextVidBench为长视频文本理解提供了新基准，提出的方法为提升模型能力提供了实用方向。
...
TL;DR: 论文提出了一种基于视觉自回归（VAR）模型的图像超分辨率（ISR）方法，通过多尺度图像标记化和直接偏好优化（DPO）解决了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有VARSR方法存在固定分辨率限制和依赖大规模数据的问题，论文旨在通过新方法解决这些问题。

Method: 提出多尺度图像标记化方法和DPO正则化项，训练模型在单次前向传递中实现超分辨率。

Result: 模型在ISR任务中达到最先进水平，且参数更少（300M vs 1B），无需外部数据。

Conclusion: 新方法在性能和效率上均优于现有VARSR，展示了多尺度标记化和偏好优化的潜力。
...
TL;DR: PATS是一种新颖的时间采样策略，用于多视角技能评估，通过保留完整的基本动作片段，显著提升了技能评估的准确性。
...
Result: 在EgoExo4D基准测试中，PATS在所有视角配置下均优于现有方法（+0.65%至+3.05%），在挑战性领域表现尤为突出（+26.22%攀岩，+2.39%音乐，+1.13%篮球）。

Conclusion: PATS作为一种自适应时间采样方法，能够适应多样化的活动特性，显著提升了自动化技能评估的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂多样的中文文本布局时表现不佳，亟需一种专门针对中文场景的解决方案。
...
TL;DR: 论文探讨了单目深度估计的进展，重点介绍了基于深度学习的方法及其挑战，特别是泛化到未见域的问题。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计旨在从单目相机捕获的RGB图像中确定每个像素的深度。深度学习的发展显著推动了这一领域，但泛化到未见域仍具挑战性。

Method: 论文回顾了多种方法，包括多尺度融合网络、将回归任务重新定义为分类问题、引入额外先验以及开发更有效的目标函数。近期方法采用仿射不变损失实现多数据集联合训练。

Result: Depth Anything在零样本单目深度估计中表现领先，尽管在度量深度估计上存在不足，但在提取未见图像的结构信息方面表现出色。
...
TL;DR: 提出了一种利用3D高斯泼溅和透明度场生成合成数据的管道，用于训练点云分割模型，无需真实数据即可达到高性能。
...
Result: 模型仅用合成数据训练，PTv3的mIoU达91.35%，某些场景下优于真实数据训练的模型。

Conclusion: 合成数据可替代真实数据，且模型能泛化到未训练的语义类别。
...
TL;DR: UAV4D框架解决了无人机拍摄动态场景的渲染问题，通过结合3D基础模型和人体网格重建模型，实现了单目视频数据下的多移动行人场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有动态神经渲染方法无法处理无人机拍摄场景的特殊挑战，如单目相机、俯视视角和多个小移动行人，且现有数据集缺乏相关数据。

Method: 结合3D基础模型和人体网格重建模型，通过识别人与场景接触点解决尺度模糊问题，并利用SMPL模型和背景网格初始化高斯样条进行整体渲染。

Result: 在三个无人机数据集上测试，PSNR提升1.5 dB，视觉清晰度优于现有方法。

Conclusion: UAV4D框架在无人机拍摄动态场景的渲染中表现优异，解决了现有方法的不足。
...
<details>
  <summary>Details</summary>
Motivation: 传统屏幕标注方法效率低且不够直观，无法充分利用人工检查员的专业知识。新系统旨在通过物理交互直接捕获标注数据，提升ML训练效率。

Method: 使用校准的追踪指针记录用户输入，结合投影界面提供视觉引导，将物理交互转化为标准化标注格式。

Result: 初步评估证实系统能有效捕获详细标注轨迹，并与CVAT集成优化ML工作流。

Conclusion: 该系统填补了人工经验与自动化数据生成之间的空白，为非IT专家参与ML训练提供了可能，未来有望推动自动光学检测的数据生成。
...
TL;DR: FlowDirector是一种无需反转的视频编辑框架，通过ODE直接演化数据空间，保持时间一致性和结构细节，结合注意力掩码和增强策略实现高效编辑。
...
<details>
  <summary>Details</summary>
Motivation: 越南语文本识别因复杂的变音符号、声调变化及缺乏大规模标注数据集而面临独特挑战，现有方法在泛化和数据稀缺方面表现不佳。

Method: 综述了现有越南文档识别技术，分析了传统OCR和深度学习的局限性，并探讨了LLMs和视觉语言模型的应用潜力。

Result: LLMs和视觉语言模型在文本识别和文档理解方面表现出显著改进潜力，但仍需解决领域适应、多模态学习和计算效率等问题。
...
TL;DR: SeedEdit 3.0结合T2I模型Seedream 3.0，显著提升了编辑指令遵循和图像内容保留能力，并通过数据增强和联合学习管道进一步优化。


<details>
  <summary>Details</summary>
Motivation: 改进现有图像编辑工具在真实图像输入上的表现，提升编辑指令的准确性和内容保留能力。

Method: 1. 开发增强的数据整理管道，采用元信息范式嵌入策略；2. 引入联合学习管道，结合扩散损失和奖励损失。

Result: 在真实图像编辑测试中，SeedEdit 3.0实现了56.1%的高可用率，优于其他版本和竞品。

Conclusion: SeedEdit 3.0在编辑性能和内容保留方面取得了显著进步，提供了更高的实用性。
...
TL;DR: 提出了一种融合视觉和语言模型的多模态街道评估框架（MSEF），用于同时评估客观特征和主观感知，并在中国哈尔滨的街道数据上验证了其有效性。
...
Method: 结合视觉变换器（VisualGLM-6B）和大型语言模型（GPT-4），使用LoRA和P-Tuning v2进行参数高效微调，分析15,000多张标注的街景图像。

Result: 模型在客观特征上F1得分为0.84，与居民感知的吻合度为89.3%，并揭示了上下文依赖的矛盾和非线性模式。

Conclusion: MSEF为城市感知建模提供了方法创新，并为规划系统提供了实用工具，能够兼顾基础设施精确性和居民体验。
...
TL;DR: 研讨会旨在探讨情感AI面部情感分析（FAA）工具的信任问题，聚焦公平性、可解释性和安全性。


<details>
  <summary>Details</summary>
Motivation: 随着FAA工具的普及，其信任问题（如偏见、隐私）日益突出，需推动相关研究。

Method: 通过研讨会汇集研究者，讨论FAA任务中的挑战（如可解释性、不确定性）。

Result: 研讨会支持FG2025的伦理目标，促进FAA的信任研究。

Conclusion: TrustFAA研讨会为FAA的信任问题提供了研究平台，推动伦理与技术结合。
...
TL;DR: ASTRAEA是一个自动框架，通过轻量级令牌选择和内存高效的稀疏注意力策略，显著提升视频扩散变压器的推理速度，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 视频扩散变压器（vDiT）在文本到视频生成方面表现优异，但计算需求高，限制了实际部署。现有加速方法依赖启发式方法，适用性有限。

Method: ASTRAEA提出轻量级令牌选择机制和GPU并行的稀疏注意力策略，并结合进化算法自动优化令牌预算分配。

Result: 在单GPU上实现2.4倍推理加速，8GPU上可达13.2倍，视频质量损失极小（VBench分数损失<0.5%）。

Conclusion: ASTRAEA在显著提升推理速度的同时，保持了视频生成的高质量，具有实际部署潜力。
...
Result: 发现模型参数增加时泛化能力提升但默认模式多样性下降，并识别细粒度失败案例。训练数据多样性与默认模式多样性相关性为0.85。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测模型在非理想条件下性能不稳定，容易被规避，因此需要一种更全面的伪造模型来提升检测器的鲁棒性。

Method: 采用Poisson混合、多样化掩码、生成器伪影和干扰物扩展伪伪造空间，并通过添加强退化增强训练图像的多样性。

Result: 在DFDC和DFDCP数据集上分别实现了3.51%和6.21%的AUC提升，显著提高了模型的鲁棒性和性能。

Conclusion: PMM不仅提升了检测器对常见图像退化的鲁棒性，还在标准基准数据集上表现优异，解决了以往检测器鲁棒性不足的问题。
...
Result: VLMs仅能识别有限基本属性，性能受对象位置影响，且难以理解对象关系，表现不及人类。

Conclusion: VLMs在场景理解上仍有显著局限性，需进一步改进以接近人类水平。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在捕捉快速移动的小型无人机时存在局限，尤其是在复杂光照条件下。事件相机虽有优势，但现有基准数据集缺乏高时间分辨率或无人机特定运动模式。

Method: 提出FRED数据集，包含7小时密集标注的无人机轨迹，涵盖5种无人机模型及恶劣天气和光照条件。
...
TL;DR: 论文提出了一种不使用神经网络的穿墙雷达人类活动识别方法，通过模板匹配和拓扑相似性计算实现智能识别。


<details>
  <summary>Details</summary>
Motivation: 当前穿墙雷达人类活动识别领域过度依赖神经网络训练，而早期基于模板匹配的方法具有更强的物理可解释性，作者希望回归这一路径。

Method: 生成距离-时间图和多普勒-时间图，使用角点检测确定目标区域，通过多相主动轮廓模型分割微多普勒特征，将其离散化为点云，最后通过Mapper算法计算拓扑相似性。
...
TL;DR: TAO是一个新的视频异常检测框架，通过像素级跟踪异常对象，统一了多粒度异常检测，无需阈值调整，提高了定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法多关注异常帧或对象，忽略像素级分析，限制了异常检测的广度。

Method: 提出TAO框架，将异常检测转化为像素级跟踪问题，结合分割和跟踪任务，避免阈值调整。

Result: 实验表明TAO在准确性和鲁棒性上设定了新基准。

Conclusion: TAO通过统一多粒度异常检测，显著提升了视频异常检测的性能。
...
TL;DR: TAPFM是一种针对病理基础模型（PFMs）的任务适应方法，通过视觉变换器（ViT）注意力机制和多实例学习（MIL）优化特征表示和注意力权重，在单GPU上实现高效适应。


<details>
  <summary>Details</summary>
Motivation: 适应预训练的PFMs用于特定临床任务面临挑战，主要由于仅有的弱标签（WSI级别）和需要MIL范式。

Method: 提出TAPFM方法，利用ViT注意力进行MIL聚合，同时优化特征表示和注意力权重，保持MIL聚合器和PFM的独立计算图以实现稳定训练。
...
TL;DR: 本文提出了一种多模态低秩适配方法（MokA），针对当前高效多模态微调方法的局限性，通过模态特定参数压缩单模态信息并增强跨模态交互，显著提升了多模态大语言模型的微调效果。


<details>
  <summary>Details</summary>
Motivation: 当前高效多模态微调方法多借鉴自大语言模型（LLMs），忽视了多模态场景的固有差异，导致无法充分利用所有模态信息。

Method: 提出MokA方法，通过模态特定参数压缩单模态信息，并显式增强跨模态交互，实现单模态和跨模态适配。

Result: 在多种多模态场景（如视听文本、视觉文本、语音文本）和多种LLM骨干网络上均表现优异，验证了方法的有效性和通用性。

Conclusion: MokA为多模态大语言模型的高效适配提供了更具针对性的解决方案，为未来研究奠定了基础。
...
TL;DR: 本文提出了一种基于视觉辅助的自主反射器系统，用于在非视距条件下增强毫米波通信性能。
...
Result: 实验结果显示，在60GHz频段下，接收信号强度平均增益为23dB，信号接收概率为0.89（阈值-65dB），显著优于静态和无反射器基准。

Conclusion: 该系统在复杂动态环境中展现了增强毫米波通信的潜力。
...
TL;DR: 研究探讨了神经网络在多模态学习中的记忆特性，特别是视觉-语言模型的跨模态记忆能力，发现跨模态信息传递存在显著差距，并提出了一种缓解方法。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络在多模态学习中的记忆行为，既涉及隐私保护，也关乎知识密集型任务的有效学习。

Method: 引入合成人物数据集，通过单模态训练和跨模态评估，量化知识记忆和跨模态传递性。

Result: 发现跨模态信息可传递，但源模态和目标模态间存在显著差距，且该差距在多种场景下普遍存在。

Conclusion: 提出基线方法缓解跨模态传递差距，希望推动更鲁棒的多模态学习技术发展。
...
TL;DR: 研究发现，现有的3D检测模型在无需特定训练的情况下，其性能已优于专门训练的3D grounding模型，表明当前方法在3D grounding任务中存在不足。为此，作者提出了DEGround框架，通过共享DETR查询和引入新模块，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 探索3D grounding任务是否真正受益于检测模型，并发现当前方法在细粒度任务中的不足。

Method: 提出DEGround框架，共享DETR查询，并引入区域激活模块和查询调制模块以增强语言理解。

Result: DEGround在EmbodiedScan验证集上的整体准确率比BIP3D高出7.52%。

Conclusion: DEGround通过改进框架设计，显著提升了3D grounding任务的性能，证明了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 新兴应用（如虚拟现实和具身AI）需要从稀疏视图高效重建3D场景，但现有方法在视野外区域表现不佳且计算成本高。

Method: OGGSplat通过开放高斯的语义属性提供先验，结合RGB-语义一致性修复模块和双向控制扩散模型，逐步优化高斯参数。

Result: OGGSplat在Gaussian Outpainting基准测试中表现出色，并能通过智能手机拍摄的两张视图实现语义感知的场景重建。

Conclusion: OGGSplat为稀疏视图下的3D场景重建提供了一种高效且语义一致的新方法。
...
TL;DR: 提出了一种名为Follow-Your-Motion的高效视频运动迁移框架，通过空间-时间解耦的LoRA和改进的训练策略，解决了现有方法在运动一致性和调优效率上的问题。
...
Method: 提出空间-时间解耦的LoRA，分离空间外观和时间运动的处理；在第二阶段训练中，设计了稀疏运动采样和自适应RoPE以加速调优。

Result: 在提出的MotionBench基准测试中，Follow-Your-Motion表现出优越性能。

Conclusion: Follow-Your-Motion框架有效解决了运动一致性和调优效率问题，为视频运动迁移提供了高效解决方案。
...
TL;DR: VLG模型通过结合视觉和语言信息生成服装，展示了多模态基础模型在专业领域（如时尚设计）的迁移潜力。
...
Result: 初步结果表明VLG在未见过的服装风格和提示下具有迁移潜力。

Conclusion: 多模态基础模型在专业领域（如时尚设计）具有有效的适应潜力。
...
Conclusion: DSG-World为真实世界的3D重建和模拟提供了一种高效且物理一致的解决方案。
...
Method: 采用SRR（结构-识别-关系）三元范式，将文档解析分解为三个核心问题，分别对应布局分析、内容识别和逻辑排序。

Result: 在MonkeyDoc数据集上表现优异，平均性能提升5.1%，尤其在公式和表格处理上显著改进，且速度更快。

Conclusion: MonkeyOCR在文档解析任务中实现了最先进的性能，同时保持了高效部署能力。
...
Method: 提出SAM-TTA框架，包括SBCT（自适应转换医学图像为SAM兼容输入）和DUMT（双尺度不确定性驱动的均值教师适配）。
...
TL;DR: MoViX是一种自监督跨视角视频定位框架，用于GPS缺失的越野环境，通过学习和季节不变表示和方向感知实现精确定位。


<details>
  <summary>Details</summary>
Motivation: 解决GPS缺失环境下因植被重复、地形无结构和季节变化导致的定位挑战。

Method: 采用自监督学习框架，结合姿态依赖正采样、时间对齐硬负采样、运动信息帧采样器和轻量级时间聚合器。

Result: 在TartanDrive 2.0数据集上，93%的情况下定位误差小于25米，100%小于50米，优于现有方法。

Conclusion: MoViX在复杂环境中表现优异，具有泛化能力，适用于不同地理区域和机器人平台。
...
TL;DR: 论文提出LeanPO方法，通过重新定义奖励和动态标签平滑策略，优化视频大语言模型的偏好对齐，减少非目标响应的概率提升。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型偏好对齐技术（如DPO）在训练中可能导致目标和非目标响应的对数概率同时下降，从而无意中提升非目标响应的概率。

Method: 提出LeanPO方法，采用参考无关的奖励定义和自生成偏好数据管道，结合动态标签平滑策略。

Result: 实验表明LeanPO显著提升视频大语言模型性能，且额外训练开销极小。

Conclusion: LeanPO为视频大语言模型提供了一种简单有效的偏好对齐方案，提升模型可靠性和效率。
...
TL;DR: 本文探讨了如何利用基础模型（FM）提升身份证展示攻击检测（PAD）的泛化能力，特别是在面对多国身份证时的表现。


<details>
  <summary>Details</summary>
Motivation: 由于隐私保护限制，现有PAD系统通常仅针对少数几种身份证训练，导致在新国家身份证上的泛化能力不足。

Method: 研究采用零样本学习和微调两种方法，测试了基于智利身份证的私有数据集和包含芬兰、西班牙、斯洛伐克身份证的开放数据集。

Result: 研究发现，真实图像（bona fide）是提升泛化能力的关键。

Conclusion: 基础模型在提升PAD泛化能力方面具有潜力，尤其是通过真实图像优化模型表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有CoVR基准无法捕捉细微的时间差异，TF-CoVR填补了这一空白，专注于体操和跳水视频。

Method: 提出TF-CoVR-Base框架，包括预训练视频编码器和对比学习对齐查询与候选视频。

Result: TF-CoVR-Base在零样本和微调后均显著提升性能，mAP@50从5.92提升至25.82。

Conclusion: TF-CoVR为时间细粒度视频检索提供了新基准，TF-CoVR-Base框架表现出色。
...
TL;DR: Rectified Point Flow 是一种统一的参数化方法，将点云配准和多部件形状组装建模为单一的条件生成问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在点云配准和形状组装中对称性处理不足的问题，并实现无监督学习。

Method: 通过学习连续点速度场将噪声点传输到目标位置，并利用自监督编码器专注于重叠点。

Result: 在六个基准测试中达到最新性能，并通过联合训练提升几何先验学习。

Conclusion: Rectified Point Flow 提供了一种统一且高效的解决方案，显著提升了点云处理任务的性能。
...
TL;DR: 论文提出了一种基于几何空间记忆的框架，用于提升视频世界模型的长时一致性，解决了现有模型因时间窗口限制导致的场景遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型因时间上下文窗口有限，难以在场景重访时保持一致性，导致环境遗忘。受人类记忆机制启发，研究旨在解决这一问题。

Method: 引入了一种基于几何空间的长时记忆框架，包括信息存储与检索机制，并使用定制数据集训练和评估带有3D记忆机制的模型。

Result: 实验表明，该框架在生成质量、一致性和上下文长度上优于基线模型。

Conclusion: 该研究为长时一致的世界生成提供了新方向。
...
TL;DR: RaySt3R将3D形状补全问题重新定义为新视角合成问题，通过单张RGB-D图像和新视角生成深度图、物体掩码和置信度分数，实现高效且一致的3D重建。
...
Method: 利用单张RGB-D图像和新视角（查询射线），训练前馈Transformer预测深度图、物体掩码和置信度分数，并通过多视角融合重建完整3D形状。
...
TL;DR: 论文提出了Vision Concept Transformer (VCT)和Stable Vision Concept Transformer (SVCT)，解决了现有概念瓶颈模型在医学领域中性能下降和解释不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 医学领域对透明度的需求促使研究可解释AI（XAI），但现有概念瓶颈模型（CBMs）仅依赖概念特征，忽略了医学图像的固有特征嵌入，且性能不稳定。

Method: VCT结合概念特征与图像特征；SVCT基于VCT，采用视觉变换器（ViT）和去噪扩散平滑技术，提升决策能力和解释稳定性。

Result: 在四个医学数据集上的实验表明，VCT和SVCT在保持准确性的同时具有可解释性，SVCT在扰动下仍能提供稳定解释。

Conclusion: VCT和SVCT满足了医学领域对模型透明性和稳定性的需求。
...
TL;DR: EOC-Bench是一个新的基准测试，用于评估动态自我中心场景中对象为中心的认知能力，填补了现有基准测试的不足。
...
Method: 开发了EOC-Bench，包含3,277个标注QA对，分为三个时间类别，涵盖11个评估维度和3种视觉对象引用类型，并设计了混合格式注释框架和多尺度时间准确性指标。

Result: 基于EOC-Bench对多种MLLMs进行了全面评估，证明了其作为评估工具的有效性。

Conclusion: EOC-Bench为提升MLLMs的具身对象认知能力提供了重要工具，为开发可靠的具身系统核心模型奠定了基础。
...
TL;DR: 提出了一种新的Aligned Tokenizer（AliTok），通过因果解码器建立单向依赖关系，解决了现有图像标记器在自回归模型中建模效率低的问题，并在生成友好性和重建性能上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有图像标记器在压缩过程中编码的双向依赖关系阻碍了自回归模型的有效建模，因此需要一种新的标记器来对齐自回归模型的建模方式。

Method: 提出AliTok，利用因果解码器建立单向依赖关系，结合前缀标记和两阶段训练提升重建一致性。

Result: 在ImageNet-256基准测试中，AliTok在177M参数下gFID为1.50，IS为305.9；662M参数下gFID为1.35，超越现有扩散方法且采样速度快10倍。
...
TL;DR: SeedVR2是一种基于扩散的一步视频修复模型，通过对抗训练和动态窗口注意力机制，显著降低了计算成本，同时在高分辨率视频修复中表现优异。
...
TL;DR: Perceive Anything Model (PAM) 是一个高效的区域级视觉理解框架，结合了 SAM 2 和 LLMs，支持多模态语义输出，速度快且资源消耗低。


<details>
  <summary>Details</summary>
Motivation: 旨在实现图像和视频中全面的区域级视觉理解，同时提升效率和实用性。

Method: 通过 Semantic Perceiver 将 SAM 2 的视觉特征转化为多模态 tokens，结合 LLMs 生成语义输出，并开发了数据增强流程。

Result: PAM 在多种任务中表现优异，速度提升 1.2-2.4 倍，GPU 内存消耗更低。
...
Method: 训练一个适配器，利用3D感知链生成的伪标签优化现成特征，通过松弛循环一致性和3D球形原型映射约束过滤错误标签。
...
TL;DR: MARBLE提出了一种基于CLIP空间和预训练文本到图像模型的方法，用于图像中物体的材质编辑，支持材质混合和细粒度属性控制。


<details>
  <summary>Details</summary>
Motivation: 研究目标是改进基于示例的图像材质编辑方法，通过CLIP空间和预训练模型实现更精细的材质控制。

Method: 方法包括在CLIP空间中寻找材质嵌入，利用预训练UNet中的材质块进行编辑，并通过浅层网络预测材质属性变化方向。

Result: 实验通过定性和定量分析验证了方法的有效性，支持单次前向传播中的多次编辑和绘画应用。

Conclusion: MARBLE在材质编辑方面表现出色，能够灵活控制细粒度属性，适用于多种场景。
...
TL;DR: ProJo4D提出了一种渐进式联合优化框架，通过逐步增加优化参数集，解决了稀疏多视角视频下物理参数估计的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏多视角视频下因误差累积和高度非凸问题效果不佳，限制了物理准确数字孪生等应用。

Method: 采用渐进式联合优化框架，逐步增加优化参数集，最终联合优化几何、外观、物理状态和材料属性。
...
TL;DR: 论文探讨了3D视觉语言模型（VLMs）的性能问题，发现3D场景中心模型依赖3D编码器不足，并提出新数据集以促进真正的3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 研究3D VLMs性能不如2D VLMs的原因，并探索改进方法。

Method: 分类3D VLMs为3D对象中心、2D图像基础和3D场景中心方法，分析其性能差异。

Result: 发现3D场景中心模型依赖3D编码器不足，数据扩展效果不明显，且易过度依赖语言线索。

Conclusion: 提出新数据集以改进3D理解，强调需更先进的评估和改进策略。
...
<details>
  <summary>Details</summary>
Motivation: 深度图在3D高斯泼溅中因物体边界处的不连续性导致点云稀疏或碎片化，影响渲染质量。

Method: 引入PM-Loss，利用预训练变换器预测的点图作为正则化损失，增强几何平滑性。

Result: 改进后的深度图显著提升了3D高斯泼溅的渲染效果，适用于多种架构和场景。

Conclusion: PM-Loss有效解决了深度图在物体边界处的几何不连续性问题，提升了渲染质量。
...
Method: 提出CG-AV-Counting基准，并开发AV-Reasoner模型，使用GRPO和课程学习提升计数能力。

Result: AV-Reasoner在多个基准上达到SOTA，但在域外基准上表现不佳。

Conclusion: CG-AV-Counting是一个全面的计数测试平台，AV-Reasoner展示了强化学习的有效性，但在域外任务中仍有局限性。
...
TL;DR: MINT-CoT通过动态插入视觉标记到文本推理步骤中，解决了多模态数学推理中的视觉区域依赖和感知限制问题，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多模态数学推理中存在视觉区域依赖、感知限制和外部修改需求的问题，需要一种更灵活的方法。

Method: 提出MINT-CoT，通过Interleave Token动态选择视觉区域，构建数据集并采用三阶段训练策略（文本CoT SFT、插入CoT SFT、插入CoT RL）。

Result: MINT-CoT-7B在MathVista、GeoQA和MMStar上分别提升了34.08%、28.78%和23.2%。

Conclusion: MINT-CoT在多模态数学推理中表现优异，为视觉与文本结合的推理提供了有效解决方案。
...
TL;DR: VideoMarathon数据集填补了长视频标注的空白，支持长达1小时的视频训练，并提出了Hour-LLaVA模型，在多个长视频语言基准测试中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有长视频标注数据稀缺，限制了长视频多模态模型（Video-LMMs）的训练和研究。
...
Conclusion: VideoMarathon和Hour-LLaVA为长视频多模态理解提供了重要资源和工具。
...
TL;DR: VideoMolmo是一个多模态大模型，用于基于文本描述的细粒度时空定位，结合了时序模块和掩码融合技术，显著提升了时空一致性和推理能力。
...
Method: 结合Molmo架构，引入时序模块和双向点传播的掩码融合技术，分两步生成定位坐标和分割掩码。

Result: 在多个真实场景和任务中，VideoMolmo显著提升了时空定位准确性和推理能力。

Conclusion: VideoMolmo通过创新的架构和数据集，为时空定位任务提供了更优的解决方案。
...
TL;DR: 提出了一种从室内空间纹理网格和多视角全景图像生成无家具复制品的流程，通过简化网格和ControlNet修复技术实现高质量结果。
...
Method: 1. 从网格中分割并移除家具，生成简化无家具网格（SDM）。2. 从SDM提取Canny边缘作为控制信号。3. 使用ControlNet修复全景图像并重新纹理化网格。

Result: 方法生成的资产质量高于神经辐射场和RGB-D修复，避免了模糊和幻觉问题。

Conclusion: 提出的流程能有效生成高质量无家具场景，优于现有技术。
...
TL;DR: DirectLayout是一个基于大语言模型的框架，直接从文本描述生成3D室内布局，解决了现有方法在开放词汇和用户指令对齐上的不足。
...
Method: DirectLayout分三阶段生成布局：鸟瞰图生成、3D空间提升和对象放置优化，结合Chain-of-Thought激活和奖励机制。

Result: 实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异。

Conclusion: DirectLayout通过直接生成布局和迭代优化，实现了高质量的3D场景合成。
...
TL;DR: 论文提出了一种新的任务——全模态引用表达分割（ORES），并提出了一个名为RAS的框架，通过多模态交互提升分割模型的能力。
...
Conclusion: RAS框架为多模态交互下的图像分割提供了有效解决方案。
...
Result: 在VBench上达到85.14分的SOTA性能，支持多种分辨率和时长。
...
TL;DR: 研究发现多模态大语言模型（MLLMs）中仅约5%的注意力头对视觉理解有贡献，称为视觉头。提出了一种无需训练的框架来识别这些头，并基于此设计了KV-Cache优化策略SparseMM，显著提升了推理效率和内存利用率。


<details>
  <summary>Details</summary>
Motivation: 探索MLLMs如何处理视觉输入，揭示注意力机制的稀疏性，以优化模型效率。

Method: 通过分析注意力机制识别视觉头，设计训练无关的框架量化视觉相关性，并开发KV-Cache优化策略SparseMM。

Result: SparseMM在主流多模态基准测试中实现了1.38倍实时加速和52%内存减少，同时保持性能。

Conclusion: 视觉头的稀疏性可用于高效优化MLLMs，SparseMM在效率和准确性间取得了优越平衡。
...
TL;DR: 首个基于物理的神经逆向渲染系统，利用多视角视频捕捉光的传播，通过时间分辨的神经辐射缓存技术实现高效渲染。


<details>
  <summary>Details</summary>
Motivation: 解决多视角视频中光传播的逆向渲染问题，尤其是间接光的处理，以实现高质量的3D重建和视图合成。

Method: 采用时间分辨的神经辐射缓存技术，存储任意点从任意方向接收的无限反弹辐射，从而加速逆向渲染。

Result: 在闪光激光雷达系统捕获的数据上，实现了包含直接和间接光的高质量3D重建，并展示了时间分辨的视图合成和场景重照明。

Conclusion: 该方法为光传播的逆向渲染提供了高效且准确的解决方案，支持多种新颖应用。
...
TL;DR: 论文提出FreeTimeGS，一种新的4D表示方法，用于处理复杂运动的动态3D场景重建，优于现有方法。
...
TL;DR: VideoMathQA是一个评估模型在视频中进行跨模态数学推理能力的基准，涵盖10个数学领域，强调对视觉、音频和文本信息的整合。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的视频数学推理与静态图像或文本不同，需要整合多模态信息并处理时间分散的细节。

Method: 通过设计包含直接问题解决、概念迁移和深度教学理解的多样化问题，结合专家标注，创建高质量基准。

Result: 基准包含920+人工标注小时，覆盖10秒至1小时以上的视频，提供多步推理注释以诊断模型能力。

Conclusion: VideoMathQA揭示了现有方法的局限性，并为跨模态数学推理模型提供了系统评估框架。
...
TL;DR: 论文提出了一种对比流匹配方法，通过显式增强条件流之间的独特性，解决了传统流匹配在条件设置下生成模糊的问题。实验表明，该方法显著提升了训练速度、减少了去噪步骤并降低了FID。
...
Result: 实验显示，对比流匹配将训练速度提升至9倍，减少5倍去噪步骤，并降低FID达8.9。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中参数高效微调（PEFT）虽能提升隐私和训练效率，但其隐私保护能力尚未充分验证。

Method: 通过梯度反转攻击，利用可访问的适配器梯度重构用户本地数据样本。

Result: 实验表明，攻击者可以高保真地重构大批量微调图像。

Conclusion: 研究揭示了PEFT的隐私漏洞，呼吁未来开发隐私保护机制。
...
Method: 设计了一个统一的编码器-解码器网络，结合Retinex分解和泊松去噪损失，无需先验反射和光照信息。
...
TL;DR: DACN是一种双注意力卷积网络，用于解决2D CNN在高光谱图像超分辨率任务中缺乏全局上下文理解的问题。通过多头注意力和通道-空间注意力，结合优化的损失函数，提升了性能。
...
Method: 引入DACN，采用多头注意力捕捉局部和全局特征依赖，推断通道和空间维度的注意力图，并设计结合L2正则化和空间-光谱梯度损失的优化损失函数。

Result: 在两个高光谱数据集上的实验表明，多头注意力和通道注意力的组合优于单独使用任一注意力机制。

Conclusion: DACN通过结合多种注意力机制和优化损失函数，显著提升了高光谱图像超分辨率的性能。
...
TL;DR: PixCell是一种基于扩散的生成基础模型，用于组织病理学，能够生成高质量图像，解决标注数据稀缺、隐私保护和虚拟染色等问题。


<details>
  <summary>Details</summary>
Motivation: 解决病理学中图像合成的独特问题，如标注数据稀缺、隐私保护和生成任务（如虚拟染色）。

Method: 使用PanCan-30M数据集，采用渐进式训练策略和自监督条件化方法，无需标注数据。

Result: PixCell生成多样且高质量的图像，可用于训练自监督模型、数据增强和教育用途，并展示了对图像生成的精确控制能力。

Conclusion: PixCell为计算病理学研究提供了强大的工具，模型已公开以加速研究。
...
TL;DR: DM-SegNet是一种双Mamba架构，通过方向性状态转换和解剖感知分层解码，解决了现有医学SSMs在编码器-解码器兼容性上的问题，实现了3D医学图像分割的全局上下文建模与空间拓扑保持。


<details>
  <summary>Details</summary>
Motivation: 现有医学SSMs在编码器的1D序列扁平化中破坏了空间结构，而传统解码器无法利用Mamba的状态传播能力，因此需要一种新架构来兼顾全局上下文建模与空间拓扑保持。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义的噪声计划，常导致感知一致性受损，因此需要一种更平衡且忠实的方法。

Method: 采用连续时间扩散模型，引入SSIMBaD（基于结构相似性的平衡扩散），通过sigma空间变换对齐感知退化。

Result: 在大规模动漫面部数据集上，该方法在像素精度和感知质量上均优于现有技术，并能适应多样风格。

Conclusion: SSIMBaD框架在动漫面部上色任务中表现出色，具有更高的重建质量和泛化能力。
...
TL;DR: 论文提出了一种实例中心的3D语义占用预测框架VoxDet，通过将体素级分类任务解耦为偏移回归和语义预测，提升了实例级别的完整性和区分性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D语义占用预测视为密集分割任务，忽略了实例级别的区分性，导致实例不完整和相邻模糊。论文发现体素级标签隐含实例信息，提出利用这一信息改进预测。

Method: 提出VoxDet框架，包括：（a）空间解耦体素编码器生成任务特定特征；（b）任务解耦密集预测器，通过回归4D偏移场和语义预测实现实例感知预测。

Result: VoxDet在相机和LiDAR输入上均表现优异，在SemanticKITTI测试集上达到63.0 IoU，排名第一。

Conclusion: VoxDet通过实例中心的设计显著提升了3D语义占用预测的性能和效率。
...
TL;DR: 提出了一种快速无监督的多边形近似闭合数字曲线的方案，速度快于现有技术，并在Rosin度量及美观性上具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在速度和美观性上存在不足，需一种更高效且视觉上令人满意的多边形近似方案。

Method: 方案分三阶段：初始分割检测高曲率顶点，迭代顶点插入补充低曲率顶点，迭代合并去除冗余顶点，最后调整顶点优化美观性。
...
TL;DR: 本文提出了一种无偏接触交互方法，无需区分主从表面，通过中平面单次评估接触力，确保力平衡，适用于多种几何形状和动态问题。


<details>
  <summary>Details</summary>
Motivation: 传统接触分析方法需要区分主从表面，可能导致偏差。本文旨在提出一种更公平、高效的方法，适用于复杂几何和动态场景。

Method: 基于中平面单次评估接触力，惩罚实际穿透，并通过离散接触段的积分计算力。详细研究了3D段的几何配置以提高精度。

Result: 方法通过接触补丁测试、两梁弯曲、赫兹接触和平冲头测试验证，证明其能准确处理平面、曲面和尖角接触。

Conclusion: 该方法在多种接触问题中表现出高精度和鲁棒性，适用于非共形网格和动态碰撞问题。
...
TL;DR: NeuroMix是一种基于混合现实（MR）的模拟器，用于训练脑室外引流（EVD）放置手术。研究发现，结合2D和3D视觉辅助的训练方式在无辅助测试中显著提高了44%的精确度，且不影响认知负荷和技术接受度。


<details>
  <summary>Details</summary>
Motivation: 神经外科手术中，从2D切片重建复杂3D解剖结构需要毫米级精度，但MR技术在临床中应用有限，因此需要开发训练系统以提升无辅助条件下的技能保留。

Method: 研究比较了三种训练方式（无辅助、仅2D辅助、2D+3D辅助），通过48名参与者的实验评估其对精确度、执行时间、认知负荷和技术接受度的影响。

Result: 结合2D和3D辅助的训练组在无辅助测试中精确度提升44%，且所有训练方式均获得高可用性和技术接受度评分。

Conclusion: 2D和3D视觉辅助结合的训练方式显著提高了手术精确度，且不影响认知负荷，但操作时间较长。
...
TL;DR: 提出了一种基于随机射线与表面交点的简单通用方法，用于在表面上均匀采样点，特别适用于隐式距离函数。


<details>
  <summary>Details</summary>
Motivation: 在几何处理中，表面点采样是基本操作，但现有方法在隐式表面等非显式表示上效率较低。

Method: 通过随机射线与表面的交点实现均匀采样，利用球面行进法高效计算隐式距离函数的交点。

Result: 实验证明该方法比替代策略更高效，且支持蓝噪声采样和分层采样。
...
TL;DR: 该研究提出了一种通过残差叠前深度偏移（RPSM）分析监测地震调查的时移速度变化的方法，成功应用于两种各向同性深度模型。


<details>
  <summary>Details</summary>
Motivation: 旨在估计地震时移调查中地震速度的横向变化，以改进监测调查的深度偏移效果。

Method: 利用已知基础调查的速度模型对监测调查进行深度偏移，并通过RPSM分析预期的时差，结合菲涅耳孔径实现局部速度分布确定。

Result: 成功恢复了两种模型（透镜状和断层储层结构）的监测调查速度，且迭代应用和模型压缩对速度估计影响较小。

Conclusion: 该方法能准确估计速度变化，适用于低信噪比环境，且对模型变化具有鲁棒性。
...
TL;DR: 本文研究了多孔介质中的固体变形与孔隙压力相互作用的孔隙弹性问题，通过多尺度分解方法分析位移和孔隙压力，以揭示数据中的隐藏结构。


<details>
  <summary>Details</summary>
Motivation: 孔隙弹性在储层管理中具有重要意义，例如地热能开发中，流体与多孔介质的相互作用会影响周围材料的变形和孔隙压力。本文旨在通过多尺度分解方法，更详细地分析数据中的结构和界面。

Method: 基于线性弹性、动量守恒、质量守恒和达西定律，推导出准静态孔隙弹性方程（QEP）。通过构造物理驱动的尺度函数（基于基本解的平滑化）和多尺度分解方法，分析位移和孔隙压力。

Result: 通过多尺度分解，成功揭示了数据中隐藏的结构和界面，提供了更详细的信息。

Conclusion: 多尺度分解方法为孔隙弹性问题的分析提供了新的视角，有助于更深入地理解数据中的结构和动态变化。
...
<details>
  <summary>Details</summary>
Motivation: 解决从水分测量中估计土壤特性的参数识别问题。

Method: 使用Fokas方法模拟垂直入渗数据，并比较多种机器学习模型（如SVM和NN）的性能。

Result: SVM和NN在预测扩散率D和水力传导率K时表现最稳健，D的预测更准确。
...
TL;DR: 论文研究了非平面断层在粘弹性半空间中的应力积累及其对断层运动的影响，通过数值模拟分析了断层几何和运动对周围位移、应力和应变的影响。


<details>
  <summary>Details</summary>
Motivation: 在地震活跃区，了解断层应力积累及其对断层运动的影响有助于预测未来地震活动。

Method: 采用二维边界值问题，结合拉普拉斯变换、分数阶导数和对应原理进行数值求解。

Result: 计算结果表明断层几何和运动对周围位移、应力和应变有显著影响。

Conclusion: 研究为理解地下变形及其对断层运动的影响提供了新视角，有助于地震活动研究。
...
TL;DR: M6.0地震的复发时间Tr在40到1500年之间变化较大，科林斯裂谷的断层段可分为三组，其复发行为不同，部分段落的更新模型概率显著低于泊松模型。


<details>
  <summary>Details</summary>
Motivation: 研究科林斯裂谷断层段的地震复发行为，以了解其复发时间的变异性及模型适用性。

Method: 通过分析M6.0地震的复发时间Tr，将断层段分为三组，比较更新模型与泊松模型的概率差异。

Result: 南部科林斯裂谷断层段的复发时间Tr可达350年，而反向断层段为400至1500年；不同段落的模型概率差异显著。

Conclusion: 科林斯裂谷断层段的复发行为具有多样性，更新模型与泊松模型的适用性因段而异。
...
TL;DR: 论文研究了凯法利尼亚转换断层带（KTFZ）的地震活动性及其构造背景，重点关注断层段之间的应力转移及其对长期构造加载的影响。


<details>
  <summary>Details</summary>
Motivation: KTFZ是地中海地震最活跃的区域，研究其地震活动和构造机制有助于理解该区域的长期构造加载和地震发生规律。

Method: 通过分析历史地震数据和断层段的应力转移机制，研究KTFZ的地震活动性。

Result: 研究表明，KTFZ的强地震活动与断层段之间的应力转移密切相关。

Conclusion: 理解KTFZ的应力转移机制对预测该区域的长期地震活动和构造演化具有重要意义。
...
TL;DR: 研究探讨了页岩中二氧化碳储存的可行性及反应输运模型的可靠性，通过模拟高压力高温下CO2饱和盐水与页岩的化学-力学相互作用。


<details>
  <summary>Details</summary>
Motivation: 由于岩石的异质性，实验测试成本高且耗时，因此采用反应输运模型模拟页岩中的孔隙尺度反应和输运过程。

Method: 使用Crunch Tope模拟一维反应输运模型，模拟Permian岩石样本在100°C和12.40 MPa下暴露于酸性盐水14和28天的过程。

Result: 结果显示石英相显著溶解后沉淀，黏土相沉淀和膨胀，长石相在酸性盐水-岩石界面附近溶解。孔隙度变化显示14和28天后矿物沉淀不足1.00%，无法完全填充孔隙。

Conclusion: 反应输运模型能有效模拟页岩中CO2储存的化学-力学过程，但矿物沉淀量不足以完全填充孔隙。
...
### [147] [Ontology-based knowledge representation for bone disease diagnosis: a foundation for safe and sustainable medical artificial intelligence systems](https://arxiv.org/abs/2506.04756)
*Loan Dao,Ngoc Quoc Ly*

Main category: cs.AI

TL;DR: 该研究提出了一种基于本体的骨病诊断框架，结合了分层神经网络、视觉语言模型和多模态深度学习，旨在提升医学AI系统的诊断可靠性。


<details>
  <summary>Details</summary>
Motivation: 医学AI系统常缺乏系统的领域专业知识整合，可能影响诊断的可靠性，因此需要一种更结构化的方法来整合专业知识。

Method: 研究开发了一个基于本体的框架，包括分层神经网络架构、增强的视觉问答系统和多模态深度学习模型，整合了影像、临床和实验室数据。

Result: 框架展示了可扩展性和标准化潜力，但实验验证因数据集和计算资源限制尚未完成。

Conclusion: 未来工作将集中于扩展临床数据集和进行系统验证，以进一步验证框架的有效性。
...
### [148] [Deep learning image burst stacking to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2506.04781)
*Christoph Schirninger,Robert Jarolim,Astrid M. Veronig,Christoph Kuckein*

Main category: astro-ph.SR

TL;DR: 提出了一种基于深度学习的实时图像重建方法，用于解决地面太阳望远镜观测中大气湍流导致的图像退化问题。
...
Method: 采用无配对图像到图像转换的深度学习模型，利用短曝光图像序列和斑点重建作为参考进行训练。

Result: 方法在感知质量上表现更优，尤其在斑点重建存在伪影时，且能高效利用图像序列信息。
...
### [149] [HSSBench: Benchmarking Humanities and Social Sciences Ability for Multimodal Large Language Models](https://arxiv.org/abs/2506.03922)
...
TL;DR: HSSBench是一个针对人文与社会科学（HSS）任务的多语言基准测试，填补了现有MLLM评估在跨学科思维和知识整合方面的空白。


<details>
  <summary>Details</summary>
Motivation: 当前MLLM评估基准主要关注STEM领域，忽视了HSS领域的独特需求，如跨学科思维和抽象概念与视觉表征的关联。

Method: 提出HSSBench，包含13,000多个样本，覆盖六类任务，并通过专家与自动化代理协作的数据生成流程创建。

Result: 测试了20多个主流MLLM，发现即使最先进模型也面临显著挑战。

Conclusion: HSSBench旨在推动MLLM在跨学科推理能力上的研究，尤其是知识整合与关联能力的提升。
...
### [150] [ViCocktail: Automated Multi-Modal Data Collection for Vietnamese Audio-Visual Speech Recognition](https://arxiv.org/abs/2506.04635)
...
<details>
  <summary>Details</summary>
Motivation: 传统语音识别系统仅依赖音频特征，易受噪声干扰，而AVSR虽具鲁棒性，但缺乏多语言数据集。本文旨在解决这一问题。

Method: 通过改进现有技术，从原始视频自动生成AVSR数据集，并开发越南语基线模型。

Result: 实验表明，自动收集的数据集在干净环境下表现与强ASR相当，在嘈杂环境中显著优于ASR。
...
### [151] [MMRefine: Unveiling the Obstacles to Robust Refinement in Multimodal Large Language Models](https://arxiv.org/abs/2506.04688)
...
<details>
  <summary>Details</summary>
Motivation: 随着对推理过程中增强能力的关注增加，需要一种框架来评估MLLMs在多种场景下的错误细化能力，而不仅仅是最终准确率的比较。

Method: MMRefine通过六种不同场景和六种错误类型分类，评估MLLMs的错误检测与纠正能力，并对多种开放和封闭MLLMs进行实验。
...
### [152] [ComfyUI-Copilot: An Intelligent Assistant for Automated Workflow Development](https://arxiv.org/abs/2506.05010)
...
Conclusion: ComfyUI-Copilot有效解决了ComfyUI的使用难题，适合不同水平的用户。
...
### [153] [Parking, Perception, and Retail: Street-Level Determinants of Community Vitality in Harbin](https://arxiv.org/abs/2506.05080)
*HaoTian Lan*

Main category: cs.CL

TL;DR: 研究提出了一种基于图像的可解释框架，分析中国哈尔滨社区街道的商业活力与街道特征（如停车密度、绿化、清洁度和街道宽度）的关系，发现车辆密度、绿化与清洁度对商业表现和用户满意度的影响。


<details>
  <summary>Details</summary>
Motivation: 探讨社区街道的商业活力如何受车辆可达性、环境质量和行人感知的复杂影响，为城市设计和社区振兴提供科学依据。

Method: 利用街景图像和多模态大语言模型（VisualGLM-6B）构建社区商业活力指数（CCVI），并结合GPT-4的感知建模分析空间属性。

Result: 适度车辆存在可提升商业可达性，但过度停车（尤其在窄街道）会降低步行性和满意度；绿化与清洁度显著提高满意度，但对定价影响较弱；街道宽度调节车辆影响。
...
### [154] [RoboRefer: Towards Spatial Referring with Reasoning in Vision-Language Models for Robotics](https://arxiv.org/abs/2506.04308)
...
TL;DR: RoboRefer是一个3D感知的视觉语言模型，通过监督微调（SFT）和强化微调（RFT）提升空间理解和推理能力，在RefSpatial-Bench上表现优异。
...
Method: 结合专用的深度编码器进行SFT，并通过RFT和多步推理奖励函数优化空间推理能力。

Result: SFT训练的RoboRefer在空间理解上达到89.6%的成功率，RFT训练的模型在RefSpatial-Bench上超越其他基线17.4%。

Conclusion: RoboRefer在复杂场景中表现出色，并能与多种控制策略集成，适用于多样化机器人任务。
...
### [155] [Learning Smooth State-Dependent Traversability from Dense Point Clouds](https://arxiv.org/abs/2506.04362)
*Zihao Dong,Alan Papalia,Leonard Jung,Alenna Spiro,Philip R. Osteen,Christa S. Robison,Michael Everett*

Main category: cs.RO

TL;DR: SPARTA是一种通过点云估计角度依赖地形可通行性的方法，利用傅里叶基函数输出平滑的风险分布函数，显著提高了计算效率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决越野自主性中地形可通行性依赖于车辆状态的问题，特别是某些障碍物仅能从特定方向通过，传统方法需要大量数据和计算资源。

Method: 提出SPARTA方法，通过输出基于傅里叶基函数的平滑解析函数，预测任意接近角度的风险分布，减少计算开销。

Result: 在高保真仿真中，SPARTA在40米巨石场地的通过成功率为91%，优于基线的73%，并在硬件测试中验证了泛化能力。

Conclusion: SPARTA通过几何结构设计和傅里叶基函数，有效解决了角度依赖地形可通行性问题，提高了计算效率和实际应用能力。
...
### [156] [MineInsight: A Multi-sensor Dataset for Humanitarian Demining Robotics in Off-Road Environments](https://arxiv.org/abs/2506.04842)
*Mario Malizia,Charles Hamesse,Ken Hasselmann,Geert De Cubber,Nikolaos Tsiogkas,Eric Demeester,Rob Haelterman*

Main category: cs.RO

TL;DR: MineInsight是一个公开的多传感器、多光谱数据集，用于越野地雷检测，填补了现有数据集的不足。
...
Method: 数据集包含35个目标（15个地雷和20个常见物体），分布在三条不同路径上，整合了双视角传感器扫描（无人地面车辆和机械臂）、多种LiDAR和多光谱图像（RGB、VIS-SWIR、LWIR）。
...
### [157] [Synthetic Dataset Generation for Autonomous Mobile Robots Using 3D Gaussian Splatting for Vision Training](https://arxiv.org/abs/2506.05092)
*Aneesh Deogan,Wout Beks,Peter Teurlings,Koen de Vos,Mark van den Brand,Rene van de Molengraft*

Main category: cs.RO

TL;DR: 提出了一种在Unreal Engine中自动生成标注合成数据的新方法，用于解决手动创建数据集的耗时、易错和多样性不足问题。


<details>
  <summary>Details</summary>
Motivation: 手动标注数据集耗时、易错且多样性有限，尤其在机器人领域动态多变场景中更为突出。

Method: 利用3D高斯泼溅技术快速生成合成数据，并结合真实数据提升性能。
...
### [158] [You Only Train Once](https://arxiv.org/abs/2506.04349)
*Christos Sakaridis*
...
<details>
  <summary>Details</summary>
Motivation: 传统方法需要多次训练以优化损失权重，效率低下。YOTO旨在通过单次训练解决这一问题。

Method: 将损失权重超参数视为网络参数，通过梯度优化自动学习，并引入软最大操作和正则化损失。

Result: 在3D估计和语义分割任务中，YOTO优于传统网格搜索方法。

Conclusion: YOTO提供了一种高效的损失权重优化方法，显著提升了模型性能。
...
### [159] [StatsMerging: Statistics-Guided Model Merging via Task-Specific Teacher Distillation](https://arxiv.org/abs/2506.04567)
*Ranjith Merugu,Bryan Bo Cao,Shubham Jain*

Main category: cs.LG

TL;DR: StatsMerging是一种基于统计学习的轻量级模型合并方法，通过权重分布统计指导合并，无需真实标签或测试样本，表现优于现有技术。
...
Method: 利用SVD的奇异值捕捉任务特定权重分布，通过轻量级学习器StatsMergeLearner建模权重分布，并引入任务特定教师蒸馏合并异构架构模型。

Result: 在八项任务中表现优异，总体准确性、泛化能力和对图像质量变化的鲁棒性均优于现有技术。

Conclusion: StatsMerging是一种高效、通用的模型合并方法，适用于异构架构模型，且无需真实标签。
...
### [160] [Scaling Laws for Robust Comparison of Open Foundation Language-Vision Models and Datasets](https://arxiv.org/abs/2506.04598)
*Marianna Nezhurina,Tomer Porian,Giovanni Pucceti,Tommie Kerssies,Romain Beaumont,Mehdi Cherti,Jenia Jitsev*

Main category: cs.LG

TL;DR: 本文研究了可迁移学习中的缩放定律，用于比较模型和数据集，并首次为CLIP和MaMMUT两种语言-视觉学习方法推导了完整的缩放定律。结果表明MaMMUT在规模和样本效率上优于CLIP。


<details>
  <summary>Details</summary>
Motivation: 研究如何利用缩放定律进行模型和数据集比较，以优化预训练过程。

Method: 通过密集测量不同模型和样本规模的性能，推导缩放定律，并比较CLIP和MaMMUT两种方法。

Result: MaMMUT在规模和样本效率上表现优于CLIP，且在不同下游任务和数据集上趋势一致。

Conclusion: 准确的缩放定律推导为模型和数据集比较提供了系统方法，避免了单一规模测量的误导性结论。
...
### [161] [Exploring bidirectional bounds for minimax-training of Energy-based models](https://arxiv.org/abs/2506.04609)
...
<details>
  <summary>Details</summary>
Motivation: 传统EBMs训练中，仅最小化下界可能导致不稳定性，因此需要更稳定的训练方法。

Method: 提出四种不同的对数似然边界（基于生成器雅可比矩阵的奇异值、互信息、梯度惩罚和扩散过程），并设计算法评估这些边界。

Result: 双向边界方法显著提高了EBMs训练的稳定性，并实现了高质量的密度估计和样本生成。

Conclusion: 双向边界是训练EBMs的有效方法，能够提升模型性能。
...
### [162] [Identifying and Understanding Cross-Class Features in Adversarial Training](https://arxiv.org/abs/2506.05032)
*Zeming Wei,Yiwen Guo,Yisen Wang*

Main category: cs.LG

TL;DR: 论文通过类间特征视角研究对抗训练（AT），发现跨类特征在AT初期对鲁棒性至关重要，但随着训练深入，模型更依赖类特定特征，导致鲁棒过拟合。
...
Method: 通过类间特征分析，结合理论模型和系统实验，研究AT过程中跨类特征的作用。
...
### [163] [Aligning Latent Spaces with Flow Priors](https://arxiv.org/abs/2506.05240)
*Yizhuo Li,Yuying Ge,Yixiao Ge,Ying Shan,Ping Luo*

Main category: cs.LG

TL;DR: 提出了一种利用流模型作为先验对齐可学习潜在空间的新框架，通过对齐损失优化潜在空间，避免了昂贵的似然计算和ODE求解。


<details>
  <summary>Details</summary>
Motivation: 现有方法在潜在空间对齐时计算成本高且复杂，需要一种更高效的方法。

Method: 预训练流模型捕获目标分布，通过对齐损失优化潜在空间，避免似然计算和ODE求解。
...
### [164] [CMIE: Combining MLLM Insights with External Evidence for Explainable Out-of-Context Misinformation Detection](https://arxiv.org/abs/2505.23449)
...
### [165] [Beyond the Desktop: XR-Driven Segmentation with Meta Quest 3 and MX Ink](https://arxiv.org/abs/2506.04858)
*Lisle Faray de Paiva,Gijs Luijten,Ana Sofia Ferreira Santos,Moon Kim,Behrus Puladi,Jens Kleesiek,Jan Egger*

Main category: cs.HC

TL;DR: 研究提出了一种基于扩展现实（XR）的医学影像分割工具，结合Meta Quest 3头显和Logitech MX Ink触控笔，旨在简化手动标注流程并提升临床适用性。


<details>
  <summary>Details</summary>
Motivation: 医学影像分割在临床中至关重要，但手动标注繁琐且耗时。

Method: 开发了一个沉浸式界面，支持实时交互2D和3D医学影像数据，结合触控笔标注和即时3D渲染。

Result: 用户研究显示工具具有基础可行性（SUS得分66），参与者赞赏其直观控制和空间交互设计。
...
### [166] [From Screen to Space: Evaluating Siemens' Cinematic Reality](https://arxiv.org/abs/2506.04972)
...
TL;DR: 研究团队评估了Siemens的Cinematic Reality在Apple Vision Pro上的可用性和临床潜力，医学专家反馈了其可行性和改进需求。


<details>
  <summary>Details</summary>
Motivation: 探索Cinematic Reality在医学影像中的沉浸式渲染潜力，以推动临床工作流程的改进。

Method: 通过可视化肝脏CT和MRCP扫描，14位医学专家使用System Usability Scale等工具评估可用性和临床潜力。

Result: 反馈揭示了可行性、优势及需改进的功能，为临床工作流程的适应提供了方向。

Conclusion: 沉浸式电影渲染在医学影像中具有潜力，但需进一步优化以支持临床实际应用。
