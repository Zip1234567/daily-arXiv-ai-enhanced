...
Motivation: 尽管VR在培训中具有优势，但其开发仍面临时间、专业知识和资源的高要求挑战。

Method: 系统包括LLM模块（从文本提取任务信息）和智能模块（将信息转化为VR中的动画演示和视觉提示）。
...
<details>
  <summary>Details</summary>
Motivation: 圆拟合算法在噪声数据中表现不佳，需要有效的异常值检测方法以提高精度。
...
Method: 提出两种方法：1）基于转换因子，通过已知参考零件估计转换因子计算未知零件直径；2）基于像素，直接利用参考零件的像素信息估计直径。

Result: 实验显示，测量误差从13-114微米降至1-2微米，显著提高了精度。

Conclusion: 该方法仅需少量已知参考零件即可实现高精度测量，显著减少误差并提升可靠性。
...
Method: 使用Gemini生成推理轨迹作为先验，结合平衡双对比学习方法优化损失函数。

Result: 在MER2024基准测试中，性能显著提升，证明了MLLM推理与轻量级融合网络的协同效果。

Conclusion: MLLM的先验推理知识与轻量级融合网络结合，可实现鲁棒且可扩展的情感识别。
...
<details>
  <summary>Details</summary>
Motivation: AVS通过结合视觉和音频模态识别视频中的发声物体，是细粒度对象理解的重要研究方向。

Method: 分析了单模态和多模态编码架构、音频-视觉融合策略、解码器设计及监督学习到无监督学习的训练范式。

Result: 比较了不同方法在标准基准上的表现，揭示了架构选择、融合策略和训练范式对性能的影响。

Conclusion: 当前挑战包括时间建模不足、视觉模态偏见等，未来方向包括改进时间推理、多模态融合、减少标注依赖等。
...
<details>
  <summary>Details</summary>
Motivation: 由于IC机械图纸的非结构化和抽象标注，自动化解析和几何建模极具挑战性，目前缺乏直接自动化标注的方法。
...
Conclusion: 该方法在复杂环境中表现出色，适用于实际机器人部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖微调或重新训练模型，计算成本高且影响模型原有能力，需要一种高效且不影响预训练模型的方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统智能城市映射技术（如卫星图像、LiDAR扫描）存在成本高、准确性不足等问题，开源地图数据因人为错误和环境变化引入偏差，影响神经网络性能。

Method: 采用基于视觉变换器的架构，统一处理射频数据和地图数据，捕捉空间依赖性和结构先验。

Result: 模型在合成数据集上表现优异，宏观IoU达65.3%，显著优于基线方法（40.1%）和其他对比方法（37.3%和42.2%）。
...
Method: 采用Swin Transformer提取语义特征，结合VQ模块实现离散化；设计注意力机制优化索引传输；通过KLD和EMA稳定训练。

Result: 实验结果显示系统在重建质量上优于基准方法。

Conclusion: VQ-DeepISC系统在数字语义通信中表现出高效性和鲁棒性，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决GeoAI中训练数据不足及忽视空间原则的问题，推动AI与地理空间研究的深度融合。

Method: 开发基于Tobler地理第一定律的空间显式模型，结合注意力图和多阶段训练策略。

Result: 成功应用于火星撞击坑检测，模型可推广至地球和其他行星的自然与人造特征。

Conclusion: 研究推动了GeoAI的理论与方法基础，为弱监督目标检测提供了新思路。
...
Method: 比较了三种计算机视觉方法（目标检测、人群分类、语义分割）并提出一种基于线性优化的新方法。

Result: 在600小时视频数据上测试，证明计算机视觉方法能有效估计拥挤度。

Conclusion: CCTV数据可独立支持实时拥挤度估计，助力运营响应。
...
<details>
  <summary>Details</summary>
Motivation: 解决精准农业中无人机视频杂草分割的效率和准确性问题，应对常见的图像退化（如模糊和噪声）。
...
Conclusion: 提出的质量感知模块化框架为无人机视频杂草分割提供了高效准确的解决方案，具有实际应用潜力。
...
Result: 生成了视觉逼真的新发票图像和完美对齐的结构化数据文件（JSON）。
...
Result: 去噪均方误差降低50%，中心定位误差低于0.04像素，显著优于传统算法。

Conclusion: 4D-PreNet为4D-STEM的高通量实时分析提供了可靠解决方案，提升了自动化表征的效率和准确性。
...
Method: 1. 发布HPDv3数据集，包含1.08M文本-图像对和1.17M标注比较；2. 提出基于VLM的偏好模型和不确定性感知排名损失；3. 引入CoHP方法迭代优化图像质量。

Result: HPSv3成为广泛图像评估的稳健指标，CoHP无需额外数据即可提升生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 陨石坑的空间分布和形态特征对行星科学研究至关重要，深度学习模型的发展为自动化陨石坑检测提供了新机遇。

Method: 采用两阶段框架：第一阶段使用CNN、ResNet-50和YOLO进行陨石坑识别；第二阶段通过YOLO实现陨石坑定位。

Result: YOLO在陨石坑检测中表现均衡，ResNet-50在大型陨石坑识别中精度更高。

Conclusion: 提出的方法在火星和月球数据上验证了有效性，为行星科学研究提供了自动化工具。
...
Method: 设计了一种扩散模型，利用OASIS-3数据训练，生成保留点对应关系的形状表示。

Result: 模型生成的形状高度逼真，并能应用于下游任务，如条件生成和反事实生成预测疾病形态变化。

Conclusion: 该模型成功生成了具有对应关系的形状，为形状生成和相关应用提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 放射科医生在阅读医学图像时通常混合多种策略，但现有机器学习模型缺乏动态选择能力。

Method: 训练一个策略网络，动态推荐最佳成像模态和区域，结合预训练分割网络迭代优化分割结果。

Result: 在1325个多参数MRI图像数据集上验证，显著提高了分割准确性和注释效率。

Conclusion: 该方法不仅优于标准分割网络，还展示了与放射科医生策略不同的优化路径，具有交互应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖昂贵的高质量手动标注视频，难以扩展到开放世界的多样化音频-视频类别。

Method: 两阶段训练：第一阶段利用噪声视频预训练学习多样但不完美的对齐；第二阶段在小规模高质量数据上微调。采用多特征条件和窗口注意力增强同步。

Result: 在AVSync48基准上，方法显著减少手动标注需求10倍以上，并能泛化到多种开放类别。

Conclusion: 提出的方法高效且可扩展，显著降低了对高质量手动标注的依赖。
...
Method: RAVID利用微调的CLIP编码器（RAVID CLIP）和VLM，动态检索相关图像并与查询图像结合，形成增强输入。
...
<details>
  <summary>Details</summary>
Motivation: 从2D图像估计食物营养含量对健康监测至关重要，但现有方法依赖专有数据集，限制了可复现性。

Method: 使用ImageNet和COYO预训练的ViT模型，与CNN基线（InceptionV2、ResNet-50）及JFT-300M预训练方法对比，在Nutrition5k数据集上进行实验。
...
Method: 采用Compiler-as-a-Judge Module（CJM）作为快速奖励信号，结合代理生成和Compiler-as-a-Review Module（CRM）验证模型。

Result: 在挑战性CAD数据集上实现了最先进的性能，同时保持高效。

Conclusion: CAD-Judge方法显著提升了文本到CAD模型的生成效率和验证效果。
...
TL;DR: 论文提出了一种名为S²Q-VDiT的后训练量化框架，用于视频扩散模型（V-DMs），通过显著数据和稀疏令牌蒸馏解决长令牌序列带来的校准和学习挑战。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型因联合建模时空信息导致长令牌序列，增加了校准方差和学习难度，需高效量化方法。

Method: 提出Hessian-aware显著数据选择和注意力引导稀疏令牌蒸馏，优化校准数据选择和令牌学习。

Result: 在W4A6量化下，实现无损性能，模型压缩3.9倍，推理加速1.3倍。

Conclusion: S²Q-VDiT有效解决了视频扩散模型的量化挑战，显著提升效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究显示大型语言模型被动接受缺陷输入，但LMMs是否具备主动检测错误的能力尚未探索。

Method: 引入ISEval框架，包含七类缺陷前提和三种评估指标，对十种先进LMMs进行测试。

Result: 多数模型需显式提示才能识别错误，逻辑错误检测表现较好，但语言错误和条件缺陷表现较差。不同模型对模态信任度不同。

Conclusion: 研究强调需提升LMMs的主动验证能力，为缓解问题提供新见解。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像语义分割中，完整地获取地物对象对精确分析至关重要，但高类内方差和高类间相似性严重阻碍了任务完成。传统方法无法有效统一类表示和区分相似特征，导致分割结果不完整。

Method: 设计了三个关键模块：自适应原型提取模块（APEM）确保语义准确性；语义-结构协调模块（SSCM）遵循语义优先、结构其次的原则；通道相似性调整模块（CSAM）动态调整步长以关注类间区分性特征。

Result: 实验表明，PDSSNet在性能上优于现有最先进方法。

Conclusion: PDSSNet通过结合类语义和空间结构信息，显著提升了遥感图像语义分割的完整性和准确性。
...
Method: 提出SSD-Net，采用不对称分解机制，结合CNN和Transformer，通过并行特征分解块（PFDB）和双向特征通信块（BFCB）实现特征解耦与融合。

Result: SSD-Net在性能上优于多尺度方法，同时显著降低了复杂度。

Conclusion: 单尺度特征提取在水下图像增强中具有潜力，SSD-Net通过创新设计实现了高效的特征解耦与融合。
...
Method: DCAR框架动态调整语义和视觉维度的提示向量，联合优化属性和类别特征，并通过负样本和类别匹配权重学习子类别区分。
...
<details>
  <summary>Details</summary>
Motivation: 路边停车导致的NLoS盲区对道路安全构成威胁，现有方法依赖预定义空间信息或简单反射假设，泛化性和实用性受限。

Method: 通过图像分割检测停车车辆，估计深度推断空间特征，再用2D雷达点云数据精确定位行人。
...
Method: 使用CycleGAN预训练合成数据，采用ECAB和SECAB融合机制优化特征表示。

Result: 在Person/Vehicle ReID数据集上表现优于现有方法，支持轻量级骨干网络。

Conclusion: CORE-ReID V2为UDA-based ReID提供了高效、可扩展的解决方案，并推动进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有暗图像恢复方法存在效率瓶颈，主要源于对外部先验的依赖、多阶段增强管道的冗余操作以及频率域方法中的全局计算需求过高。

Method: 提出SPJFNet，包括自我挖掘引导模块（SMGM）生成轻量级内生引导，通过无损小波分解和联合傅里叶增强优化频率处理，并设计双频引导框架（DFGF）分离高低频处理。

Result: SPJFNet在多个基准测试中表现优于现有方法，显著降低了模型复杂性和计算开销。

Conclusion: SPJFNet通过内生引导和高效频率处理，实现了高性能和高效能的暗图像恢复。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试存在模拟与现实的差距、任务复杂度有限和推理覆盖不完整的问题，限制了其在实际场景中的应用。

Method: 通过基于第一人称操作视频的可扩展数据构建流程，整合任务选择、图像对提取、自动化元数据标注和结构化问题生成，构建了包含12个任务的VisualTrans基准。
...
Result: 在内部和公共数据集上的实验表明，IPA-CP优于现有的半监督学习方法。
...
<details>
  <summary>Details</summary>
Motivation: 手语视频生成需要自然动作和精确语义控制，但面临数据需求高和泛化能力差的挑战。

Method: 构建签名者无关的多模态运动词典，通过离散到连续的运动合成和身份感知神经渲染生成视频。

Result: 实验表明解耦运动与身份不仅可行，还能实现高质量合成和签名者个性化。

Conclusion: 该方法为手语视频生成提供了灵活且高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 跨视角物体对应是视觉理解中的关键但具有挑战性的任务，现有方法通常直接匹配单个物体掩码，而忽略了物体之间的位置和语义关系。

Method: DOMR框架包含Dense Object Matcher (DOM)模块，联合建模多物体的位置和语义关系，并结合掩码细化模块提升预测掩码的完整性和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要利用局部可预测性，忽略了k空间的全局依赖性。Transformer在自然语言处理和图像分析中表现出色，但其缺乏可解释性。

Method: 从结构化低秩（SLR）模型的角度提出GPI，将全局湮灭滤波器作为可学习参数，并通过展开SLR的优化算法构建白盒Transformer。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有文档修复方法因独立处理任务导致的系统复杂性和扩展性不足问题，以及未能充分利用任务间协同效应的问题。

Method: 提出Uni-DocDiff模型，结合可学习的任务提示设计、Prior Pool机制（整合局部高频和全局低频特征）和Prior Fusion Module（自适应选择任务相关先验信息）。

Result: 实验表明，Uni-DocDiff在性能上媲美甚至超越专用模型，同时具备对新任务的无缝扩展能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统Transformer在医学图像分割中存在计算复杂度高和局部上下文信息捕获不足的问题，限制了其性能。

Method: 提出TCSAFormer，包含压缩注意力模块（CA）减少计算复杂度，以及双分支前馈网络（DBFFN）增强局部特征捕获能力。
...
<details>
  <summary>Details</summary>
Motivation: 遮挡感知是空间理解的关键，但现有MLLM模型在此任务上的表现尚未充分探索，因此需要专门的基准进行评估。

Method: 通过分层合成方法构建语义连贯的遮挡场景图像，并标注问答对，评估22个MLLM模型的性能。

Result: 当前MLLM模型在遮挡感知任务上与人类表现差距显著，模型规模或思维过程无法完全弥补。

Conclusion: O-Bench为遮挡感知提供了重要评估工具，并有望推动MLLM模型的视觉智能发展。
...
Method: 提出TNet，利用卷积和加法操作逐步将低分辨率特征（全局上下文丰富）融合到高分辨率特征（局部细节丰富）中。

Result: 在ISPRS Vaihingen、ISPRS Potsdam和LoveDA数据集上，TNet-R分别取得了85.35%、87.05%和52.19%的mIoU。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如图像上采样或视频超分辨率）未考虑或仅隐式处理3D一致性，3DSR旨在通过显式3D表示解决这一问题。

Method: 基于3D高斯溅射的场景表示，结合2D扩散超分辨率模型，确保多视角一致性和空间连贯性。

Result: 在MipNeRF360和LLFF数据上验证，3DSR生成高分辨率结果，视觉质量高且保持3D结构一致性。

Conclusion: 3DSR通过显式3D表示和现成模型，高效提升超分辨率质量，代码将开源。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图条件下实现精确几何重建是一个基本挑战，现有方法依赖非局部深度正则化，无法捕捉细粒度结构且对深度估计噪声敏感，传统平滑方法忽略语义边界并破坏边缘和纹理。

Method: DET-GS引入了分层几何深度监督框架，自适应地强制执行多级几何一致性；设计了基于Canny边缘检测的边缘感知深度正则化；提出了RGB引导的边缘保留总变差损失。

Result: 实验表明，DET-GS在几何精度和视觉保真度上均有显著提升，优于现有稀疏视图新视图合成方法。

Conclusion: DET-GS通过深度和边缘感知正则化，有效解决了稀疏视图条件下的几何重建问题，提升了整体质量。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析中标注数据有限，现有视觉语言模型因领域差异难以直接应用，且单模态方法易导致模态不对齐。

Method: 提出 NEARL-CLIP 框架，包含 USEformer（动态生成跨模态查询）和 OCA（正交技术分离新知识）。

Result: NEARL-CLIP 仅引入 1.46M 可学习参数，实现了高效的跨模态交互。

Conclusion: NEARL-CLIP 通过跨模态交互和正交技术，显著提升了医学图像分析的性能。
...
<details>
  <summary>Details</summary>
Motivation: 人体感知研究对理解CV模型性能至关重要，但传统方法复杂且难以扩展。AR的丰富上下文和交互性为简化这一过程提供了机会。

Method: 设计了ARCADE平台，支持跨平台AR数据收集、可插拔模型推理的自定义实验协议，以及AR流媒体用于用户研究。

Result: 通过深度和光照估计模型验证了ARCADE的有效性，表明AR任务能有效引发人类对模型质量的感知判断。

Conclusion: ARCADE是一个灵活且有效的人体感知评估平台，适用于多种CV研究场景。
...
Method: 提出MLLMSeg框架，利用MLLM视觉编码器的细节特征，设计DSFF模块融合细节与语义特征，使用轻量级解码器（34M参数）进行分割。
...
TL;DR: 提出了一种联合车辆检测与重识别的方法CLIPVehicle，通过双粒度语义区域对齐模块和多级车辆识别学习策略，解决了检测与重识别目标冲突的问题，并在新构建的数据集上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有方法需先检测并存储所有车辆图像再进行重识别，资源消耗大且不实用，因此需要一种联合检测与重识别的高效方法。

Method: 提出CLIPVehicle框架，包含双粒度语义区域对齐模块和多级车辆识别学习策略，利用视觉语言模型（VLMs）进行车辆区分建模。

Result: 在真实数据集CityFlowVS和合成数据集SynVS-Day、SynVS-All上，CLIPVehicle优于现有车辆重识别和人物搜索任务的最先进方法。

Conclusion: CLIPVehicle通过联合优化检测与重识别任务，解决了目标冲突问题，显著提升了车辆搜索性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决实例分割任务中对象实例的有效解耦问题，特别是在零样本场景下。

Method: 提出条件潜在扩散框架，结合对象模板和图像特征生成实例掩码；包括粗模型生成初始提案和细化模型并行优化提案。

Result: 在多个真实世界基准测试中达到最佳性能，无需目标数据重新训练。
...
### [50] [Learning Using Privileged Information for Litter Detection](https://arxiv.org/abs/2508.04124)
...
Method: 首次将特权信息与深度学习目标检测结合，并提出一种将边界框信息编码为二进制掩码的方法。
...
### [51] [SVC 2025: the First Multimodal Deception Detection Challenge](https://arxiv.org/abs/2508.04129)
...
TL;DR: DS²Net提出了一种多视角深度监督网络，结合细节和语义特征监督，通过DEM和SEM模块增强特征监督，并引入不确定性损失优化监督强度，显著提升医学图像分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅单独监督粗粒度语义或细粒度细节特征，忽视了二者在医学图像分析中的互补关系。

Method: 提出DS²Net，通过DEM和SEM模块分别监督低层细节和高层语义特征，并采用不确定性损失自适应调整监督强度。

Result: 在六个基准测试中，DS²Net均优于现有方法。

Conclusion: DS²Net通过多视角深度监督和不确定性损失，显著提升了医学图像分割的准确性和鲁棒性。
...
### [53] [UniFGVC: Universal Training-Free Few-Shot Fine-Grained Vision Classification via Attribute-Aware Multimodal Retrieval](https://arxiv.org/abs/2508.04136)
*Hongyu Guo,Kuan Zhu,Xiangzhao Hao,Haiyun Guo,Ming Tang,Jinqiao Wang*

Main category: cs.CV

TL;DR: UniFGVC是一个无需训练的通用框架，通过将细粒度视觉分类（FGVC）转化为多模态检索任务，利用多模态大语言模型（MLLMs）生成结构化文本描述，显著提升少样本场景下的分类性能。
...
Method: 提出Category-Discriminative Visual Captioner（CDV-Captioner），利用MLLMs生成结构化文本描述，构建多模态类别模板，并通过现成的视觉和文本编码器进行检索。

Result: 在12个FGVC基准测试中表现优于现有的少样本CLIP方法和部分全监督MLLMs方法。

Conclusion: UniFGVC框架具有广泛的兼容性和适应性，在少样本FGVC任务中展现出卓越的性能和泛化能力。
...
### [54] [IDCNet: Guided Video Diffusion for Metric-Consistent RGBD Scene Generation with Precise Camera Control](https://arxiv.org/abs/2508.04147)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将RGB和深度生成分开处理，缺乏几何一致性，IDC-Net旨在通过联合学习提升几何对齐和相机控制精度。

Method: 采用统一的几何感知扩散模型联合生成RGB和深度图，并引入几何感知Transformer块实现细粒度相机控制。

Result: 实验表明IDC-Net在视觉质量和几何一致性上优于现有方法，生成的RGB-D序列可直接用于3D场景重建。

Conclusion: IDC-Net通过联合学习框架实现了高质量的RGB-D序列生成，无需后处理即可用于下游任务。
...
### [55] [ICM-Fusion: In-Context Meta-Optimized LoRA Fusion for Multi-Task Adaptation](https://arxiv.org/abs/2508.04153)
*Yihua Shao,Xiaofeng Lin,Xinwei Long,Siyu Chen,Minxi Yan,Yang Liu,Ziyang Yan,Ao Ma,Hao Tang,Jingcai Guo*

Main category: cs.CV

TL;DR: 提出了一种名为ICM-Fusion的新框架，结合元学习和上下文适应，解决了预训练LoRA模型在多任务适应中的权重冲突和领域遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有预训练LoRA融合方法在分解权重矩阵时会导致权重冲突和领域遗忘，增量学习在少样本场景下泛化能力不足。
...
### [56] [Audio-Assisted Face Video Restoration with Temporal and Identity Complementary Learning](https://arxiv.org/abs/2508.04161)
...
### [57] [ToxicTAGS: Decoding Toxic Memes with Rich Tag Annotations](https://arxiv.org/abs/2508.04166)
*Subhankar Swain,Naquee Rizwan,Nayandeep Deb,Vishwajeet Singh Solanki,Vishwa Gangadhar S,Animesh Mukherjee*

Main category: cs.CV

TL;DR: 论文介绍了首个包含6300个真实世界模因帖子的数据集，用于毒性内容检测，并提出了一种标签生成模块以提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 社交媒体模因常传播有害内容，但数据获取和标注成本高，阻碍了有效的毒性内容检测系统的开发。

Method: 构建两阶段标注的数据集（毒性分类和细粒度标签），并设计标签生成模块以补充缺失的上下文标签。

Result: 实验表明，引入社会相关标签显著提升了视觉语言模型（VLM）的检测性能。

Conclusion: 该研究为多模态在线环境中的内容审核提供了可扩展的新方法。
...
### [58] [AD-FM: Multimodal LLMs for Anomaly Detection via Multi-Stage Reasoning and Fine-Grained Reward Optimization](https://arxiv.org/abs/2508.04175)
*Jingyi Liao,Yongyi Su,Rong-Cheng Tu,Zhao Jin,Wenhao Sun,Yiting Li,Dacheng Tao,Xun Xu,Xulei Yang*

Main category: cs.CV

TL;DR: 提出了一种改进的多模态大语言模型（MLLM）在异常检测中的应用框架，通过多阶段推理和细粒度奖励机制解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于GRPO的方法在领域适应中存在数据利用不足和推理过程监督不足的问题，限制了其在专业异常检测中的应用。

Method: 引入多阶段推理过程和细粒度奖励机制，优化GRPO训练，提升模型对异常检测的适应能力。

Result: 在多个工业数据集上验证了方法的有效性，显著提升了模型在专业异常检测中的性能。

Conclusion: 该方法成功弥合了通用MLLM能力与专业异常检测需求之间的差距，实现了高效适应和准确检测。
...
### [59] [Uncertainty-Aware Spatial Color Correlation for Low-Light Image Enhancement](https://arxiv.org/abs/2508.04176)
*Jin Kuang,Dong Liu,Yukuang Zhang,Shengsheng Wang*

Main category: cs.CV

TL;DR: U2CLLIE框架通过不确定性感知增强和空间-颜色因果建模，解决了低光图像增强中的梯度消失和噪声主导问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视特征表示中的不确定性，尤其在极暗条件下，梯度退化和噪声主导影响模型可靠性和因果推理。

Method: 提出U2CLLIE框架，包含不确定性感知双域去噪模块（UaD）和分层因果感知框架（LEN、NeCo、AsC）。

Result: 在多个基准数据集上达到最先进性能，表现出鲁棒性和强泛化能力。

Conclusion: U2CLLIE有效解决了低光图像增强中的关键问题，具有广泛适用性。
...
### [60] [Deeper Inside Deep ViT](https://arxiv.org/abs/2508.04181)
*Sungrae Hong*

Main category: cs.CV

TL;DR: 论文研究了ViT-22B模型在本地环境中的训练表现，提出了稳定性改进，并探索了其在图像生成任务中的适用性。


<details>
  <summary>Details</summary>
Motivation: 尽管ViT-22B等大规模视觉模型已有研究，但其实际应用价值尚未完全明确，因此需要进一步分析其训练行为和性能。

Method: 在本地环境中训练ViT-22B模型，分析其训练不稳定性并提出改进方案；同时设计基于ViT的图像生成架构，比较ViT与ViT-22B在图像生成中的表现。

Result: ViT-22B在相同参数规模下性能优于ViT；通过改进提升了训练稳定性；初步探索了ViT-22B在图像生成任务中的潜力。

Conclusion: ViT-22B在性能上优于ViT，且通过改进可稳定训练；其在图像生成任务中也展现出潜力，但需进一步研究。
...
### [61] [RPCANet++: Deep Interpretable Robust PCA for Sparse Object Segmentation](https://arxiv.org/abs/2508.04190)
*Fengyi Wu,Yimian Dai,Tianfang Zhang,Yixuan Ding,Jian Yang,Ming-Ming Cheng,Zhenming Peng*

Main category: cs.CV

TL;DR: RPCANet++结合RPCA的可解释性与高效深度架构，提出了一种稀疏对象分割框架，解决了传统RPCA的计算负担和适应性限制。


<details>
  <summary>Details</summary>
Motivation: 传统RPCA模型存在计算负担大、超参数调优依赖性强、动态场景适应性差等问题，限制了其应用。

Method: 通过将松弛RPCA模型展开为包含背景近似模块(BAM)、对象提取模块(OEM)和图像恢复模块(IRM)的结构化网络，并引入记忆增强模块(MAM)和深度对比先验模块(DCPM)优化性能。

Result: 在多样化数据集上的实验表明，RPCANet++在各种成像场景下均达到最先进性能。
...
### [62] [From Learning to Unlearning: Biomedical Security Protection in Multimodal Large Language Models](https://arxiv.org/abs/2508.04192)
...
<details>
  <summary>Details</summary>
Motivation: 生物医学MLLMs的训练数据可能包含隐私信息和错误知识，导致部署后隐私泄露或错误输出，但完全重新训练成本高昂。

Method: 提出MLLMU-Med基准，通过合成隐私数据和错误知识构建数据集，并设计遗忘效率评分。

Result: 评估五种遗忘方法，发现其效果有限，表明需进一步改进。
...
Conclusion: GAT框架在视频文本视觉问答任务中具有显著优势，准确性和效率均优于现有方法。
...
### [64] [Bootstrap Deep Spectral Clustering with Optimal Transport](https://arxiv.org/abs/2508.04200)
*Wengang Guo,Wei Ye,Chunchun Chen,Xin Sun,Christian Böhm,Claudia Plant,Susanto Rahardja*

Main category: cs.CV

TL;DR: BootSC是一种端到端的深度谱聚类模型，通过联合优化谱聚类的所有阶段，解决了传统方法的两个主要缺点。


<details>
  <summary>Details</summary>
Motivation: 传统谱聚类方法存在优化过程不连贯和表示能力有限的问题。

Method: 提出BootSC模型，通过单一网络端到端学习谱聚类的所有阶段（亲和矩阵构建、谱嵌入和k均值聚类），并利用最优传输监督和正交重参数化技术。

Result: BootSC在ImageNet-Dogs数据集上实现了16%的NMI提升，达到最先进的聚类性能。

Conclusion: BootSC通过端到端学习和新技术显著提升了谱聚类的效果。
...
Method: 构建基于视觉推理核心维度的子问题模板，通过多轮QA构建有效推理路径，动态分析推理一致性，并引入自适应CoT机制。

Result: 在A-OKVQA等数据集上，ViFP提升准确率最高达5.4%，显著减少假阳性推理。

Conclusion: ViFP有效提升视觉语言模型的推理可靠性，并通过VoC指标量化评估模型表现。
...
### [66] [Small Lesions-aware Bidirectional Multimodal Multiscale Fusion Network for Lung Disease Classification](https://arxiv.org/abs/2508.04205)
*Jianxun Yu,Ruiquan Ge,Zhipeng Wang,Cheng Yang,Chenyu Lin,Xianjun Fu,Jikui Liu,Ahmed Elazab,Changmiao Wang*

Main category: cs.CV

TL;DR: 提出MMCAF-Net模型，通过多尺度交叉注意力模块解决医学影像与电子健康记录数据的维度差异问题，显著提升诊断准确性。
...
### [67] [What Holds Back Open-Vocabulary Segmentation?](https://arxiv.org/abs/2508.04211)
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇方法在语言-图像预训练中性能停滞，需解决瓶颈问题。

Method: 提出利用真实信息的oracle组件，识别并解耦性能瓶颈。
...
### [68] [SplitGaussian: Reconstructing Dynamic Scenes via Visual Geometry Decomposition](https://arxiv.org/abs/2508.04224)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于高斯泼溅的动态场景重建方法常将静态和动态元素混为一谈，导致运动泄漏、几何失真和时间闪烁。
...
### [69] [Continual Learning for VLMs: A Survey and Taxonomy Beyond Forgetting](https://arxiv.org/abs/2508.04227)
*Yuyang Liu,Qiuhe Hong,Linlan Huang,Alexandra Gomez-Villa,Dipam Goswami,Xialei Liu,Joost van de Weijer,Yonghong Tian*

Main category: cs.CV

TL;DR: 本文首次系统综述了视觉语言模型（VLM）的持续学习（CL）问题，提出了三种核心失败模式及解决方案，并呼吁改进评估基准。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在持续学习中面临跨模态特征漂移、参数干扰等独特挑战，亟需系统性研究。

Method: 提出基于挑战的分类法，包括多模态重放策略、跨模态正则化和参数高效适应。

Result: 综述了现有方法、评估协议和数据集，指出需改进基准以更好地衡量VLM的遗忘和组合泛化能力。

Conclusion: 本文为开发终身视觉语言系统提供了全面诊断参考，并提出了未来研究方向。
...
### [70] [LayerT2V: Interactive Multi-Object Trajectory Layering for Video Generation](https://arxiv.org/abs/2508.04228)
...
Method: 通过分层生成背景和前景物体，将每个物体置于独立层，实现多物体运动的灵活合成与控制。
...
### [71] [Intention Enhanced Diffusion Model for Multimodal Pedestrian Trajectory Prediction](https://arxiv.org/abs/2508.04229)
*Yu Liu,Zhijie Liu,Xiao Ren,You-Fu Li,He Kong*

Main category: cs.CV

TL;DR: 提出了一种基于扩散模型的多模态行人轨迹预测方法，结合行人运动意图，提高预测的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 行人轨迹预测对自动驾驶至关重要，但现有扩散模型未能充分结合行人运动意图，限制了预测的精确性和可解释性。
...
### [72] [DocVCE: Diffusion-based Visual Counterfactual Explanations for Document Image Classification](https://arxiv.org/abs/2508.04233)
*Saifullah Saifullah,Stefan Agne,Andreas Dengel,Sheraz Ahmed*

Main category: cs.CV

TL;DR: 论文提出DocVCE方法，通过生成视觉反事实解释提高文档图像分类模型的透明度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 黑盒AI决策系统在文档处理中的广泛应用需要更高的透明度和可靠性，尤其是避免偏见或虚假相关性带来的严重后果。

Method: 利用潜在扩散模型和分类器指导生成视觉反事实解释，并通过分层块细化寻找最接近目标事实图像的反事实。
...
### [73] [A machine learning approach for image classification in synthetic aperture RADAR](https://arxiv.org/abs/2508.04234)
*Romina Gaburro,Patrick Healy,Shraddha Naidu,Clifford Nolan*

Main category: cs.CV

TL;DR: 论文探讨了使用卷积神经网络（CNN）在合成孔径雷达（SAR）数据中识别和分类地面物体的问题，包括模拟数据和真实卫星数据，并比较了不同方法的分类效果。
...
Conclusion: CNN在SAR数据分类中表现优异，且数据采集高度对分类效果有影响。
...
### [74] [PIS3R: Very Large Parallax Image Stitching via Deep 3D Reconstruction](https://arxiv.org/abs/2508.04236)
...
Method: 结合视觉几何变换器和深度3D重建，先获取相机参数和密集3D场景重建，再通过点云重投影和图像扩散模块优化拼接结果。
...
### [75] [From eye to AI: studying rodent social behavior in the era of machine Learning](https://arxiv.org/abs/2508.04255)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法存在偏见且难以捕捉复杂的社会行为，现代方法结合计算机视觉、行为学和神经科学，为研究提供更全面的视角。

Method: 讨论了分析啮齿动物社会行为的主要步骤和可用工具，并提出了解决常见问题的实用方案。

Result: 现代方法能提供更深入的行为洞察，但AI的整合也带来挑战。

Conclusion: 旨在指导年轻研究者采用这些方法，并促进专家讨论工具的科学应用需求。
...
### [76] [Segment Any Vehicle: Semantic and Visual Context Driven SAM and A Benchmark](https://arxiv.org/abs/2508.04260)
...
<details>
  <summary>Details</summary>
Motivation: 由于SAM模型在车辆部件分割任务中缺乏语义标签和公开的文本提示功能，限制了其应用，因此需要改进。

Method: SAV框架包括SAM编码解码器、车辆部件知识图谱和上下文样本检索编码模块，利用结构化知识和上下文先验提升分割性能。

Result: 在VehicleSeg10K等数据集上的实验验证了SAV的有效性，为未来研究提供了基准。

Conclusion: SAV框架解决了SAM在车辆部件分割中的局限性，并通过新数据集推动了相关研究。
...
### [77] [Revisiting Continual Semantic Segmentation with Pre-trained Vision Models](https://arxiv.org/abs/2508.04267)
...
Method: 通过系统实验分析DFT在两种基准数据集（Pascal VOC 2012和ADE20K）和两种PVM骨干（ResNet101和Swin-B）下的表现，提出改进方法DFT*，包括冻结骨干和分类器、预分配未来分类器等策略。

Result: 实验表明DFT*在性能上优于16种现有方法，且训练参数和时间更少。

Conclusion: PVMs本身具有抗遗忘能力，DFT*是一种简单高效的CSS方法，挑战了现有复杂方法的必要性。
...
### [78] [PKSS-Align: Robust Point Cloud Registration on Pre-Kendall Shape Space](https://arxiv.org/abs/2508.04286)
*Chenlei Lv,Hui Huang*

Main category: cs.CV

TL;DR: 提出了一种基于Pre-Kendall形状空间的鲁棒点云配准方法PKSS-Align，能够处理相似变换、非均匀密度、噪声和缺陷部分，无需数据训练或复杂特征编码。


<details>
  <summary>Details</summary>
Motivation: 点云配准对相似变换、噪声和不完整几何结构敏感，非均匀尺度和缺陷部分容易导致局部最优解，需要一种鲁棒的方法。

Method: 在Pre-Kendall形状空间（PKSS）上测量形状特征相似性，无需点对点或点对面度量，直接生成变换矩阵。

Result: 实验表明，该方法在效率和实用性上显著优于现有技术。
...
### [79] [MuGS: Multi-Baseline Generalizable Gaussian Splatting Reconstruction](https://arxiv.org/abs/2508.04297)
*Yaopeng Lou,Liao Shen,Tianqi Liu,Jiaqi Li,Zihao Huang,Huiqiang Sun,Zhiguo Cao*

Main category: cs.CV

TL;DR: MuRF是一种新颖的视图合成方法，通过结合多视角立体视觉和单目深度估计特征，以及深度深度融合机制，实现了高效的3D高斯表示，适用于多种基线设置和复杂场景。
...
Method: 结合MVS和MDE特征，提出深度深度融合的投影采样机制，引入参考视图损失，利用3D高斯表示加速训练和推理。

Result: 在DTU、RealEstate10K等数据集上达到SOTA性能，并在LLFF和Mip-NeRF 360上展示了零样本性能。

Conclusion: MuRF通过多基线设置和高效表示，显著提升了新视图合成的质量和效率。
...
### [80] [Length Matters: Length-Aware Transformer for Temporal Sentence Grounding](https://arxiv.org/abs/2508.04299)
...
<details>
  <summary>Details</summary>
Motivation: 现有DETR模型在时序句子定位任务中存在查询角色重叠和预测冗余的问题，需要一种方法使每个查询专注于其特定功能。

Method: 提出LATR，将查询分为短、中、长三组，并引入长度分类任务，抑制不匹配长度的预测。

Result: 实验表明LATR在三个公开基准上达到最优性能，消融研究验证了各组件和长度先验的重要性。
...
### [81] [A Foundation Model for DAS Signal Recognition and Visual Prompt Tuning of the Pre-trained Model for Downstream Tasks](https://arxiv.org/abs/2508.04316)
*Kun Gui,Hongliang Ren,Shang Shi,Jin Lu,Changqiu Yu,Quanjun Cao,Guomin Gu,Qi Xuan*

Main category: cs.CV

TL;DR: 论文提出了一种基于掩码自编码器（MAEPD）的基础模型，用于解决分布式声学传感（DAS）信号识别中的跨领域泛化问题。通过自监督预训练和视觉提示调优（VPT），模型在少量参数调整下实现了高精度和高效训练。


<details>
  <summary>Details</summary>
Motivation: DAS技术在不同领域的应用中，数据分布差异和标记数据不足限制了AI模型的泛化能力。

Method: 提出MAEPD模型，通过自监督掩码重建任务预训练，结合VPT进行下游任务微调。

Result: 在步态识别任务中达到96.94%的准确率，仅微调0.322%的参数，训练时间减少45%。
...
### [82] [TempFlow-GRPO: When Timing Matters for GRPO in Flow Models](https://arxiv.org/abs/2508.04324)
*Xiaoxuan He,Siming Fu,Yuke Zhao,Wanli Li,Jian Yang,Dacheng Yin,Fengyun Rao,Bo Zhang*

Main category: cs.CV

TL;DR: TempFlow-GRPO提出了一种改进的GRPO框架，通过时间感知优化解决了现有流匹配模型在人类偏好对齐中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配模型在强化学习中对人类偏好对齐效果不佳，主要因时间均匀假设导致奖励分配不精准。

Method: 引入轨迹分支机制和噪声感知加权方案，实现时间感知优化。

Result: 在人类偏好对齐和文本到图像基准测试中达到最优性能。
...
### [83] [RiemanLine: Riemannian Manifold Representation of 3D Lines for Factor Graph Optimization](https://arxiv.org/abs/2508.04335)
*Yanyan Li,Ze Yang,Keisuke Tateno,Federico Tombari Liang Zhao,Gim Hee Lee*

Main category: cs.CV

TL;DR: 论文提出了一种名为RiemanLine的3D线最小参数化方法，适用于单条线和并行线组，通过全局和局部解耦优化，显著提升了相机定位和结构映射的精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要处理独立线，忽略了人造环境中常见的并行线结构规律，导致参数冗余和优化效率低下。

Method: RiemanLine将线地标解耦为全局（单位球上的消失方向）和局部（正交子空间上的缩放法向量），实现并行线的紧凑编码，并将参数空间从4n降至2n+2。

Result: 在ICL-NUIM、TartanAir和合成数据集上的实验表明，该方法显著提高了姿态估计和线重建的精度，同时降低了参数维度和提升了收敛稳定性。

Conclusion: RiemanLine为3D线提供了一种高效的最小参数化方法，特别适用于包含并行线结构的场景，具有实际应用潜力。
...
### [84] [RotatedMVPS: Multi-view Photometric Stereo with Rotated Natural Light](https://arxiv.org/abs/2508.04366)
*Songyun Yang,Yufei Han,Jilong Zhang,Kongming Liang,Peng Yu,Zhaowei Qu,Heng Guo*

Main category: cs.CV

TL;DR: RotatedMVPS提出了一种在旋转自然光下恢复形状和反射率的方法，通过结合数据先验和光一致性，显著提高了恢复精度。
...
Method: 通过旋转台实现光一致性，减少复杂环境光的未知性，并结合单视角光度立体学习方法的数据先验。

Result: 在合成和真实数据集上的实验证明了该方法的有效性。

Conclusion: RotatedMVPS在自然光照下实现了高精度的形状和反射率恢复，具有广泛的应用潜力。
...
### [85] [TSPO: Temporal Sampling Policy Optimization for Long-form Video Language Understanding](https://arxiv.org/abs/2508.04369)
*Canhui Tang,Zifan Han,Hongbo Sun,Sanping Zhou,Xuchong Zhang,Xin Wei,Ye Yuan,Jinglin Xu,Hao Sun*

Main category: cs.CV

TL;DR: 论文提出了一种名为TSPO的强化学习方法，通过事件感知的时间代理和联合决策优化，解决了多模态大语言模型在长视频理解中的帧采样问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理长视频输入时存在上下文限制和训练成本问题，现有方法可能遗漏关键事件或受限于预训练模型的事件理解能力。

Method: 提出TSPO方法，包括事件感知时间代理、强化学习范式、长视频训练数据构建流程和基于规则的奖励机制。

Result: TSPO在多个长视频理解基准测试中取得了最先进的性能，并展示了在不同前沿视频MLLMs中的可迁移能力。

Conclusion: TSPO通过强化学习优化帧采样策略，显著提升了长视频语言理解能力。
...
### [86] [VisionTS++: Cross-Modal Time Series Foundation Model with Continual Pre-trained Visual Backbones](https://arxiv.org/abs/2508.04379)
*Lefei Shen,Mouxiang Chen,Xu Liu,Han Fu,Xiaoxue Ren,Jianling Sun,Zhuo Li,Chenghao Liu*

Main category: cs.CV

TL;DR: VisionTS++通过视觉模型改进时间序列预测，解决了数据模态、多变量预测和概率预测的差距，并在多个基准测试中取得最佳结果。


<details>
  <summary>Details</summary>
Motivation: 解决视觉模型在时间序列预测中的三个关键差距：数据模态、多变量预测和概率预测。

Method: 提出VisionTS++，包括高质量数据筛选机制、多变量转换方法和多分位数预测方法。
...
### [87] [ProtoN: Prototype Node Graph Neural Network for Unconstrained Multi-Impression Ear Recognition](https://arxiv.org/abs/2508.04381)
*Santhoshkumar Peddi,Sadhvik Bathini,Arun Balasubramanian,Monalisa Sarma,Debasis Samanta*

Main category: cs.CV

TL;DR: ProtoN是一个基于图的少样本学习框架，用于耳部生物特征识别，通过原型图神经网络（PGNN）处理多印象数据，显著提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 耳部生物特征识别因标注数据稀缺和类内变异性大而受限，现有方法难以提取一致且具区分性的特征。

Method: 提出ProtoN框架，使用图结构表示多印象数据，通过PGNN层和跨图原型对齐策略优化特征表示。

Result: 在五个基准数据集上，ProtoN达到99.60%的Rank-1识别准确率和0.025的EER。

Conclusion: ProtoN在少样本条件下显著提升耳部识别性能，为数据稀缺问题提供有效解决方案。
...
### [88] [Deep Learning-based Scalable Image-to-3D Facade Parser for Generating Thermal 3D Building Models](https://arxiv.org/abs/2508.04406)
...
<details>
  <summary>Details</summary>
Motivation: 现有建筑改造对气候影响至关重要，但早期改造规划需要基于LoD3热模型，而现有方法在可扩展性和准确性上存在不足。

Method: SI3FP直接从正交图像平面建模几何基元，结合计算机视觉和深度学习，支持稀疏和密集数据源。

Result: 在瑞典典型住宅建筑测试中，SI3FP在窗墙比估计中误差约为5%，满足早期改造分析需求。

Conclusion: SI3FP为大规模能源改造规划提供了高效工具，并在城市发展和规划中有广泛应用潜力。
...
### [89] [Thinking With Videos: Multimodal Tool-Augmented Reinforcement Learning for Long Video Reasoning](https://arxiv.org/abs/2508.04416)
*Haoji Zhang,Xin Gu,Jiawen Li,Chixiang Ma,Sule Bai,Chubin Zhang,Bowen Zhang,Zhichao Zhou,Dongliang He,Yansong Tang*

Main category: cs.CV

TL;DR: 提出了一种名为VITAL的视频推理框架，通过工具增强学习和多模态链式思维解决长视频推理中的跨模态交互不足和幻觉问题，并在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在视频推理任务中跨模态交互不足和幻觉问题，特别是在长视频或复杂推理链场景下的性能限制。

Method: 提出VITAL框架，结合视觉工具箱动态采样视频帧，生成多模态链式思维推理；构建两个高质量数据集（MTVR-CoT-72k和MTVR-RL-110k）并设计DGRPO算法优化多任务强化学习。

Result: 在11个视频理解基准测试中表现优异，尤其在长视频场景下的视频问答和时间定位任务中超越现有方法。

Conclusion: VITAL框架通过工具增强学习和多任务优化显著提升了视频推理能力，为复杂视频理解任务提供了有效解决方案。
...
### [90] [Efficient Inter-Task Attention for Multitask Transformer Models](https://arxiv.org/abs/2508.04422)
...
<details>
  <summary>Details</summary>
Motivation: 在多任务学习中，传统的Transformer架构由于注意力矩阵规模随任务数量呈二次增长，计算效率低下，难以满足实际硬件限制的需求。

Method: 提出Deformable Inter-Task Self-Attention方法，通过更高效地跨任务特征图聚合信息，减少计算开销。

Result: 在NYUD-v2和PASCAL-Context数据集上，FLOPs和推理延迟降低了一个数量级，同时任务预测质量指标提升了7.4%。

Conclusion: 该方法显著提升了多任务学习的计算效率和性能，为实际应用提供了可行的解决方案。
...
### [91] [Composed Object Retrieval: Object-level Retrieval via Composed Expressions](https://arxiv.org/abs/2508.04424)
*Tong Wang,Guanyu Yang,Nian Liu,Zongyan Han,Jinxing Zhou,Salman Khan,Fahad Shahbaz Khan*

Main category: cs.CV

TL;DR: 论文提出了一种新任务Composed Object Retrieval (COR)，旨在通过结合参考对象和检索文本实现对象级检索和分割，并构建了大规模基准COR127K和统一模型CORE。
...
Conclusion: COR任务和CORE模型为对象级检索提供了有效解决方案，并推动了多模态检索研究的发展。
...
### [92] [Benchmarking Foundation Models for Mitotic Figure Classification](https://arxiv.org/abs/2508.04441)
*Jonas Ammeling,Jonathan Ganz,Emely Rosbach,Ludwig Lausser,Christof A. Bertram,Katharina Breininger,Marc Aubreville*

Main category: cs.CV

TL;DR: 研究探讨了基础模型在病理学有丝分裂图像分类中的应用，通过低秩适应（LoRA）提升性能，接近100%数据可用性，并在未见肿瘤领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决病理学中标记数据有限的问题，利用自监督学习训练基础模型，提升分类性能和泛化能力。

Method: 比较线性探测和LoRA适应方法，评估基础模型在数据扩展和未见肿瘤领域的鲁棒性，并与传统CNN和Vision Transformer基线对比。

Result: LoRA适应模型性能优于线性探测，仅需10%训练数据即可接近100%数据可用性表现，且在未见领域表现接近全微调模型。

Conclusion: LoRA适应基础模型在有限数据和跨领域任务中表现优异，但传统全微调方法仍具竞争力。
...
### [93] [Boosting Visual Knowledge-Intensive Training for LVLMs Through Causality-Driven Visual Object Completion](https://arxiv.org/abs/2508.04453)
...
<details>
  <summary>Details</summary>
Motivation: 现有LVLMs在需要深度视觉感知的任务（如图像细微差异识别）中表现不足，原因是训练数据中视觉知识稀缺。

Method: 通过自动化实例构建管道生成CVC任务实例，无需依赖复杂模型或人工辅助，LVLMs通过试错学习自我改进。

Result: 在四个专业任务和四个综合基准测试中表现显著提升，专业任务上平均提升5.4%（7B模型）和4.0%（13B模型）。
...
### [94] [4DVD: Cascaded Dense-view Video Diffusion Model for High-quality 4D Content Generation](https://arxiv.org/abs/2508.04467)
...
<details>
  <summary>Details</summary>
Motivation: 高维数据（如4D）生成复杂，现有方法难以同时建模3D空间和时间特征。

Method: 4DVD将任务分解为粗多视角布局生成和结构感知条件生成，并统一处理。

Result: 实验表明4DVD在新视角合成和4D生成上表现优异。

Conclusion: 4DVD通过解耦和统一方法，显著提升了4D内容生成的质量和应用范围。
...
### [95] [FrEVL: Leveraging Frozen Pretrained Embeddings for Efficient Vision-Language Understanding](https://arxiv.org/abs/2508.04469)
*Emmanuelle Bourigault,Pauline Bourigault*

Main category: cs.CV

TL;DR: FrEVL框架探索冻结预训练嵌入是否支持有效的视觉-语言理解，发现其在判别任务中表现接近SOTA，且计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 解决视觉-语言模型部署时的高计算需求问题。

Method: 使用冻结预训练嵌入，分析其在下游任务中的有效性。

Result: 在标准基准测试中达到SOTA性能的85%-95%，计算速度提升2.3倍，能耗降低52%。
...
### [96] [Zero-Residual Concept Erasure via Progressive Alignment in Text-to-Image Model](https://arxiv.org/abs/2508.04472)
...
Method: ErasePro引入零残差约束和渐进式层更新策略，确保目标概念与锚概念完美对齐，并减少深层参数变化。

Result: 实验证明ErasePro在不同概念消除任务中表现优异。

Conclusion: ErasePro在概念消除任务中更有效且能保持生成质量。
...
### [97] [QuantVSR: Low-Bit Post-Training Quantization for Real-World Video Super-Resolution](https://arxiv.org/abs/2508.04485)
*Bowen Chai,Zheng Chen,Libo Zhu,Wenbo Li,Yong Guo,Yulun Zhang*

Main category: cs.CV

TL;DR: QuantVSR提出了一种针对真实世界视频超分辨率（VSR）的低比特量化模型，通过时空复杂度感知机制和可学习偏置对齐模块，解决了量化VSR模型的挑战，并在性能上接近全精度模型。
...
Method: 提出QuantVSR，采用时空复杂度感知（STCA）机制测量各层的时空复杂度，分配特定秩的低秩全精度辅助分支，并联合优化全精度和低比特分支；引入可学习偏置对齐（LBA）模块减少量化误差。
...
### [98] [Learning Robust Intervention Representations with Delta Embeddings](https://arxiv.org/abs/2508.04492)
...
<details>
  <summary>Details</summary>
Motivation: 当前因果表示学习主要关注场景变量的识别与表示，而对干预本身的表示研究较少。本文旨在通过干预的表示提升模型的泛化性和鲁棒性。

Method: 提出因果增量嵌入的概念，其具有视觉场景不变性和对因果变量的稀疏影响。基于此，设计了一个无需额外监督的框架，从图像对中学习因果表示。

Result: 在Causal Triplet挑战中，因果增量嵌入在合成和真实世界基准测试中显著优于基线性能，尤其在OOD设置下表现突出。

Conclusion: 因果增量嵌入是提升OOD鲁棒性的有效策略，为干预表示提供了新思路。
...
### [99] [MonoCloth: Reconstruction and Animation of Cloth-Decoupled Human Avatars from Monocular Videos](https://arxiv.org/abs/2508.04505)
...
Method: 采用分部分解策略，将化身分为身体、面部、手部和衣物，并为衣物设计专用布料模拟模块，利用时间运动线索和几何约束捕捉变形。

Result: 实验表明，MonoCloth在视觉重建质量和动画真实感上优于现有方法，并支持衣物转移等附加任务。

Conclusion: MonoCloth通过分部分解和专用模块设计，提升了单目视频重建的逼真度和实用性。
...
### [100] [Skeleton Motion Words for Unsupervised Skeleton-Based Temporal Action Segmentation](https://arxiv.org/abs/2508.04513)
*Uzay Gökay,Federico Spurio,Dominik R. Bach,Juergen Gall*

Main category: cs.CV

TL;DR: 提出了一种无监督的基于骨架的时间动作分割方法，利用序列到序列的时间自编码器，通过量化骨架运动词实现动作聚类。
...
Method: 使用序列到序列时间自编码器，将潜在骨架序列分割为不重叠的块并量化，生成骨架运动词以发现动作簇。
...
### [101] [RAIDX: A Retrieval-Augmented Generation and GRPO Reinforcement Learning Framework for Explainable Deepfake Detection](https://arxiv.org/abs/2508.04524)
...
Method: RAIDX整合了RAG和GRPO，利用外部知识提高检测准确性，并自动生成细粒度解释和显著性图。

Result: 在多个基准测试中，RAIDX表现出色，实现了最先进的检测性能，并提供了可解释的文本描述和显著性图。
...
### [102] [No Masks Needed: Explainable AI for Deriving Segmentation from Classification](https://arxiv.org/abs/2508.04534)
...
### [103] [TopKD: Top-scaled Knowledge Distillation](https://arxiv.org/abs/2508.04539)
*Qi Wang,Jinjia Zhou*
...
Result: 在CIFAR-100、ImageNet等多个数据集上，TopKD优于现有蒸馏方法，且在Vision Transformers上表现突出。
...
### [104] [InceptoFormer: A Multi-Signal Neural Framework for Parkinson's Disease Severity Evaluation from Gait](https://arxiv.org/abs/2508.04540)
...
Method: 使用Inception1D捕获多尺度时间特征，Transformer建模长程依赖，并通过过采样处理类别不平衡。

Result: InceptoFormer在PD严重程度评估中达到96.6%的准确率，优于现有方法。

Conclusion: InceptoFormer通过多尺度特征和长程依赖建模，显著提升了PD严重程度评估的性能。
...
### [105] [Hierarchical Event Memory for Accurate and Low-latency Online Video Temporal Grounding](https://arxiv.org/abs/2508.04546)
*Minghang Zheng,Yuxin Peng,Benyuan Sun,Yi Yang,Yang Liu*

Main category: cs.CV

TL;DR: 提出了一种基于分层事件记忆的在线视频时间定位方法，解决了现有方法无法有效建模事件和保留长期历史信息的问题，并在多个数据集上实现了最佳性能。
...
Method: 提出了一个基于事件的OnVTG框架，通过事件提案建模事件级信息，并引入分层事件记忆保留历史事件信息。此外，还提出了一个未来预测分支以实现实时预测。

Result: 在TACoS、ActivityNet Captions和MAD数据集上实现了最先进的性能。

Conclusion: 分层事件记忆和未来预测分支有效提升了在线视频时间定位的性能，解决了现有方法的局限性。
...
### [106] [MSC: A Marine Wildlife Video Dataset with Grounded Segmentation and Clip-Level Captioning](https://arxiv.org/abs/2508.04549)
...
Result: 构建了全面的视频理解基准，提升了海洋视频字幕生成和分析能力。
...
### [107] [Two-Way Garment Transfer: Unified Diffusion Framework for Dressing and Undressing Synthesis](https://arxiv.org/abs/2508.04551)
*Angang Zhang,Fang Deng,Hao Chen,Zhongjian Chen,Junyan Li*

Main category: cs.CV

TL;DR: 论文提出了一种双向服装转移模型（TWGTM），首次统一解决了虚拟试穿（VTON）和虚拟试脱（VTOFF）的双向任务，通过特征解耦和分阶段训练实现了高效性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究将虚拟试穿和虚拟试脱视为独立任务，忽略了它们的互补性，因此需要一种统一框架来解决这一根本性差距。

Method: 提出TWGTM模型，利用潜在空间和像素空间的双重条件引导，并通过分阶段训练解决掩模依赖不对称问题。

Result: 在DressCode和VITON-HD数据集上的实验验证了模型的有效性和竞争力。

Conclusion: TWGTM为服装中心图像合成提供了首个统一框架，成功解决了双向任务，具有显著优势。
...
### [108] [Augmentation-based Domain Generalization and Joint Training from Multiple Source Domains for Whole Heart Segmentation](https://arxiv.org/abs/2508.04552)
*Franz Thaler,Darko Stern,Gernot Plank,Martin Urschler*

Main category: cs.CV

TL;DR: 提出了一种基于深度学习的全心脏分割方法，通过平衡联合训练和强化的数据增强技术，解决了医学图像分割中的域偏移问题，在CT和MR数据上均表现出色。
...
Method: 采用平衡联合训练策略（同时使用CT和MR数据）和强化的强度与空间数据增强技术，以应对训练和测试数据分布不同的域偏移问题。

Result: 在CT数据上达到93.33% DSC和0.8388 mm ASSD，在MR数据上达到89.30% DSC和1.2411 mm ASSD，性能优于或接近单模态训练模型。

Conclusion: 该方法能高效生成精确的心脏语义分割，为心脏数字孪生模型提供了可靠基础。
...
### [109] [One Model For All: Partial Diffusion for Unified Try-On and Try-Off in Any Pose](https://arxiv.org/abs/2508.04559)
*Jinxi Liu,Zijian He,Guangrun Wang,Guanbin Li,Liang Lin*

Main category: cs.CV

TL;DR: OMFA是一个统一的扩散框架，支持无需展示衣物和任意姿势的虚拟试穿和试脱，通过部分扩散策略实现高效的双向衣物-人像转换。
...
### [110] [Drone Detection with Event Cameras](https://arxiv.org/abs/2508.04564)
...
<details>
  <summary>Details</summary>
Motivation: 无人机扩散带来安全和安保挑战，传统摄像头因目标小、机动性高及光线条件差而难以可靠检测。

Method: 采用事件摄像头技术，消除运动模糊，在极端光照下实现稳定检测，并利用稀疏异步输出抑制静态背景。

Result: 事件视觉技术为下一代反无人机系统提供了可靠、低延迟和高效的基础。

Conclusion: 事件视觉技术是解决无人机检测问题的强大方案，支持实时跟踪、轨迹预测等高级任务。
...
### [111] [TAlignDiff: Automatic Tooth Alignment assisted by Diffusion-based Transformation Learning](https://arxiv.org/abs/2508.04565)
...
Result: 实验表明TAlignDiff在牙齿对齐中效果显著，优于传统方法。
...
<details>
  <summary>Details</summary>
Motivation: 探索在弱监督条件下（仅视频级标签）进行视听事件定位的挑战，提出跨模态显著锚点作为解决方案。

Method: 使用跨模态显著锚点，结合事件一致性评估和锚点传播模块，增强事件语义编码。
...
TL;DR: 论文研究了大规模视觉语言模型（LVLMs）的幻觉问题，提出新基准POPEv2和轻量级方法Obliviate，通过解学习训练偏置显著减少幻觉。


<details>
  <summary>Details</summary>
Motivation: LVLMs在视觉输入不一致时生成错误文本（幻觉问题），研究训练数据在其中的作用。

Method: 引入POPEv2基准，通过掩码对象收集反事实图像；提出Obliviate方法，仅更新LM头部参数。

Result: Obliviate显著减少幻觉，仅更新2%参数，适用于不同模型规模和数据量。

Conclusion: Obliviate高效且可扩展，为LVLMs幻觉问题提供解决方案。
...
TL;DR: DDTracking是一种基于深度生成框架的扩散MRI纤维束成像方法，通过条件去噪扩散过程实现纤维束传播，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决当前扩散MRI纤维束成像方法在捕捉局部细节和全局一致性方面的不足，提出一种端到端可学习的解决方案。

Method: 采用双路径编码网络联合建模局部空间编码和全局时间依赖，结合条件扩散模型预测纤维束传播方向。

Result: 在多个数据集上验证，DDTracking性能显著优于现有方法，并展现出强大的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决医疗图像中异常定位问题，现有通用VLM在医疗领域表现不佳，而专业医疗VLM需要大量资源和标注。

Method: K2Sight框架将临床概念分解为视觉属性（如形状、密度、解剖位置），并通过指令式提示指导区域-文本对齐。

Result: 仅用1.5%数据训练的小型模型（0.23B和2B参数）性能优于7B+医疗VLM，mAP50提升9.82%。

Conclusion: K2Sight通过结构化语义监督，高效结合领域知识与空间结构，显著提升模型性能并减少资源需求。
...
TL;DR: 论文提出了一种用于多类皮肤病分类的深度学习框架，评估了三种图像预处理技术，并比较了多种预训练模型的性能，最终DinoV2结合RGB预处理表现最佳。


<details>
  <summary>Details</summary>
Motivation: 皮肤病分类因类间相似性高、类内变异大和病变纹理复杂而具有挑战性，需要高效的预处理和模型架构。

Method: 系统评估了RGB、CMY颜色空间转换和CLAHE三种预处理技术，并测试了DenseNet201、Efficient-NetB5、ViT、Swin Transformer和DinoV2等模型的性能。
...
TL;DR: FAME 2026挑战赛旨在多语言环境下探索人脸与声音的关联，使用MAV-Celeb数据集，并提供了挑战赛的细节、基线模型和任务说明。


<details>
  <summary>Details</summary>
Motivation: 全球一半人口为双语者，多语言场景下的交流普遍存在，因此研究多语言环境下的人脸与声音关联具有重要意义。

Method: 挑战赛使用MAV-Celeb数据集，通过基线模型探索人脸与声音在多语言环境中的关联。

Result: 报告详细介绍了挑战赛的细节、数据集和任务，为相关研究提供了基础。
...
TL;DR: 提出了一种基于3D高斯SLAM的在线3D重建方法，结合前馈循环预测模块，显著提升了跟踪速度，并在性能上媲美现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理长序列或依赖深度传感器时表现不佳，需要一种更高效且不依赖深度传感器的3D重建方法。
...
<details>
  <summary>Details</summary>
Motivation: 单目和立体深度估计各有优缺点，但实践中两者通常分离。OmniDepth旨在通过结合两者的优势，解决立体方法在反射或无纹理表面上的模糊性问题。

Method: OmniDepth采用交叉注意力对齐机制，动态同步单目上下文线索与立体假设表示，在单个网络中实现两者的相互优化。

Result: 实验表明，OmniDepth在Middlebury和ETH3D数据集上零样本泛化误差减少超过40%，并解决了透明和反射表面的长期问题。
...
TL;DR: 论文研究了双耳对称性对CNN耳部识别效果的影响，提出通过耳侧分类器区分左右耳，实验表明分开处理左右耳可提升性能。


<details>
  <summary>Details</summary>
Motivation: 双耳对称性对CNN学习特征的影响尚未被充分研究，论文旨在填补这一空白。

Method: 开发耳侧分类器自动区分左右耳，并在训练和测试中利用这一信息，进行跨数据集评估。

Result: 实验表明分开处理左右耳能显著提升性能，消融研究提供了CNN耳部识别系统的实用建议。

Conclusion: 双耳对称性信息对CNN耳部识别系统性能有重要影响，分开处理左右耳是有效的优化策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准在金融领域的多模态推理能力评估上存在不足，FinMMR旨在填补这一空白，推动MLLMs在金融场景中的实际应用。

Method: 通过转换现有金融推理基准和构建新问题，FinMMR整合了多模态数据（文本和图像），涵盖14个金融子领域，并设计了多步精确数值推理任务。

Result: 最佳MLLM在困难问题上仅达到53.0%的准确率，表明任务具有挑战性。

Conclusion: FinMMR将推动MLLMs在金融领域推理能力的提升，尤其是在复杂现实场景中的应用。
...
TL;DR: 论文介绍了EncQA基准测试，用于评估视觉语言模型在图表理解中的视觉推理能力，发现模型性能在不同任务和编码方式下差异显著，且模型规模并非关键因素。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在图表理解基准测试中的进步未能全面覆盖视觉推理能力的需求，因此需要新的评估方法。

Method: 提出EncQA基准测试，包含2,076个合成问题-答案对，覆盖六种视觉编码通道和八种任务。

Result: 评估9个先进视觉语言模型发现，性能在不同编码和任务间差异显著，且模型规模对许多任务-编码组合无显著提升。
...
TL;DR: X-SAM是一个多模态大语言模型框架，通过统一架构和视觉提示提升像素级感知能力，支持多种分割任务，并在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在像素级感知上的不足，以及Segment Anything Model在多掩码预测和类别特定分割任务中的局限性。

Method: 提出X-SAM框架，引入视觉接地分割任务和统一训练策略，支持多数据集协同训练。

Result: X-SAM在多个图像分割基准测试中达到最先进性能。

Conclusion: X-SAM通过统一框架和视觉提示，显著提升了多模态大语言模型的像素级视觉理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 家禽行业中，传统人工检测鸡只疾病耗时且易出错，需一种更高效的方法。

Method: 开发基于YOLO v8的AI系统，通过分析高分辨率鸡只照片检测疾病迹象（行为和外貌异常）。

Result: 训练后的算法能实时准确识别患病鸡只，并向农场主发出警报。

Conclusion: 该技术可早期识别感染，减少人工检查需求，提升大规模农场生物安全性。
...
<details>
  <summary>Details</summary>
Motivation: 粗粒度房间布局估计为许多下游任务提供重要几何线索，但现有方法多基于单视图且假设全景图像，限制了其应用。

Method: 通过端到端训练优化密集深度特征的多视图对齐，学习具有大收敛域和平滑损失景观的特征图，并使用简单启发式初始化房间布局。

Result: 在ScanNet++和2D-3D-Semantics新基准测试中，PixCuboid显著优于现有方法，并支持扩展到多房间估计。

Conclusion: PixCuboid通过优化方法实现了高效且灵活的房间布局估计，适用于单房间和多房间场景。
...
TL;DR: HierarchicalPrune是一种新型压缩框架，通过分层修剪、权重保护和敏感性蒸馏技术，显著减少大规模扩散模型的内存占用和延迟，同时保持输出质量。
...
Conclusion: HierarchicalPrune有效压缩模型，适用于设备端推理，性能优于现有方法。
...
Method: ANPrompt通过融合原始和噪声扰动的文本嵌入构建噪声提示，生成抗噪声提示并注入编码器深层，同时计算噪声抵抗视觉提示原型（NRVPP）和引入抗噪声目标。
...
TL;DR: InterVLA是首个大规模人-物-人交互数据集，结合了第一人称视角和多模态数据，为AI助手在物理世界中的发展提供了新基准。
...
Method: 采用混合RGB-MoCap系统，结合视觉-语言-动作框架，通过GPT生成的脚本收集多模态交互数据。

Result: 构建了包含11.4小时、1.2M帧多模态数据的InterVLA数据集，并建立了新的人体运动估计、交互合成和预测基准。

Conclusion: InterVLA数据集和基准测试将推动物理世界中AI代理的发展。
...
TL;DR: TurboTrain是一个高效的多智能体训练框架，通过多智能体时空预训练和平衡多任务学习策略，简化训练流程并提升性能。
...
Conclusion: 预训练有效捕捉时空多智能体特征，平衡多任务学习策略进一步提升检测和预测性能。
...
TL;DR: BEVCon是一个简单有效的对比学习框架，旨在提升自动驾驶中的鸟瞰图（BEV）感知能力，通过两个对比学习模块改进特征表示，实验显示性能显著提升。
...
Result: 在nuScenes数据集上，BEVCon实现了最高+2.4% mAP的性能提升。
...
TL;DR: ST-Occ是一种用于3D占用预测的时空一致性学习框架，通过高效的历史信息捕获和动态感知，显著提升了性能。
...
Result: 性能优于现有方法3 mIoU，时空不一致性降低29%。
...
### [132] [Technical specification of a framework for the collection of clinical images and data](https://arxiv.org/abs/2508.03723)
*Alistair Mackenzie,Mark Halling-Brown,Ruben van Engen,Carlijn Roozemond,Lucy Warren,Dominic Ward,Nadia Smith*

Main category: eess.IV

TL;DR: 报告描述了一个用于收集临床图像和数据的框架，用于训练和验证人工智能工具，涵盖数据收集、伦理、信息治理和共享基础设施。
...
Method: 提出自动化持续收集数据的框架，同时介绍半自动化方法作为补充。

Result: 框架能确保数据更新及时且具有代表性，同时支持长期随访数据的整合。

Conclusion: 自动化框架适用于大规模长期数据收集，但半自动化方法可作为初期选择。
...
### [133] [A Survey of Multimodal Ophthalmic Diagnostics: From Task-Specific Approaches to Foundational Models](https://arxiv.org/abs/2508.03734)
*Xiaoling Luo,Ruli Zheng,Qiaojian Zheng,Zibo Du,Shuo Yang,Meidan Ding,Qihao Xu,Chengliang Liu,Linlin Shen*

Main category: eess.IV

TL;DR: 本文综述了截至2025年眼科多模态深度学习方法的最新进展，包括任务特定方法和基础模型，并讨论了数据集、评估指标、创新方法及未来方向。


<details>
  <summary>Details</summary>
Motivation: 视觉障碍是全球重大健康挑战，多模态成像为眼科诊断提供了互补信息，推动了多模态深度学习方法的发展。

Method: 综述分为两类：任务特定方法（如病灶检测、疾病诊断）和基础模型（结合视觉-语言架构和大语言模型）。

Result: 方法包括自监督学习、注意力融合和对齐技术，但仍面临数据变异性、标注不足和泛化性等挑战。
...
### [134] [Improve Retinal Artery/Vein Classification via Channel Couplin](https://arxiv.org/abs/2508.03738)
*Shuang Zeng,Chee Hong Lee,Kaiwen Li,Boxu Xie,Ourui Fu,Hangzhou He,Lei Zhu,Yanye Lu,Fangxiao Cheng*

Main category: eess.IV

TL;DR: 提出了一种新的损失函数和正则化方法，用于视网膜血管分割和动静脉分类，解决了现有方法忽略血管结构耦合关系的问题。


<details>
  <summary>Details</summary>
Motivation: 手动分割和分类视网膜血管耗时且不一致，现有自动化方法将动静脉和整体血管分割视为独立任务，忽略了其内在耦合关系。
...
Conclusion: 新方法通过耦合关系和精细特征提取，显著提升了视网膜动静脉分类的准确性。
...
### [135] [A Modified VGG19-Based Framework for Accurate and Interpretable Real-Time Bone Fracture Detection](https://arxiv.org/abs/2508.03739)
*Md. Ehsanul Haque,Abrar Fahim,Shamik Dey,Syoda Anamika Jahan,S. M. Jahidul Islam,Sakib Rokoni,Md Sakib Morshed*

Main category: eess.IV

TL;DR: 提出了一种基于改进VGG-19模型的自动化骨折检测框架，结合预处理技术和可解释AI方法，实现高精度和快速诊断。


<details>
  <summary>Details</summary>
Motivation: 早期准确检测骨折对治疗至关重要，但现有方法存在误分类和缺乏可解释性，资源有限时尤其突出。

Method: 使用改进的VGG-19模型，结合CLAHE、Otsu阈值和Canny边缘检测等预处理技术，并采用Grad-CAM提供可解释性。

Result: 模型分类准确率达99.78%，AUC为1.00，实时应用反馈时间仅0.5秒。

Conclusion: 该框架为骨折检测提供了高效、可靠且可解释的解决方案，有助于提升临床诊断效率和患者护理。
...
### [136] [Boosting Vision Semantic Density with Anatomy Normality Modeling for Medical Vision-language Pre-training](https://arxiv.org/abs/2508.03742)
*Weiwei Cao,Jianpeng Zhang,Zhongyi Shui,Sinuo Wang,Zeli Chen,Xi Li,Le Lu,Xianghua Ye,Tingbo Liang,Qi Zhang,Ling Zhang*

Main category: eess.IV

TL;DR: 该论文提出了一种提升视觉语义密度的方法，通过疾病级视觉对比学习和解剖学正常性建模，改善了医学图像与报告的语义对齐，显著提升了诊断性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像与报告之间存在语义密度差距，导致视觉对齐偏差，影响了多功能和通用医学诊断能力的开发。

Method: 采用疾病级视觉对比学习增强视觉语义，并通过解剖学正常性建模（利用VQ-VAE）重构正常视觉嵌入，放大异常信号。

Result: 在两个胸部CT数据集和一个腹部CT数据集上进行了实验，实现了最先进的零样本性能，平均AUC达到84.9%。

Conclusion: 该方法有效提升了视觉语义密度和诊断性能，并展示了优越的迁移学习能力。
...
### [137] [Do We Need Pre-Processing for Deep Learning Based Ultrasound Shear Wave Elastography?](https://arxiv.org/abs/2508.03744)
*Sarah Grube,Sören Grünhagen,Sarah Latus,Michael Meyling,Alexander Schlaefer*

Main category: eess.IV

TL;DR: 研究探讨了深度学习在超声剪切波弹性成像中的应用，评估了不同预处理程度对预测性能的影响，结果表明深度学习可直接利用原始射频数据可靠区分弹性组。


<details>
  <summary>Details</summary>
Motivation: 超声剪切波弹性成像的非侵入性诊断潜力受限于系统间通用性和标准化问题，图像处理步骤对可靠性和可重复性的影响尚未明确。

Method: 使用3D卷积神经网络从时空超声图像预测剪切波速度，比较不同预处理程度（从完全波束形成图像到原始射频数据）的性能，并与传统飞行时间法对比。

Result: 无论预处理程度如何，深度学习均能显著区分不同弹性组，且原始射频数据表现可靠，预处理仅轻微提升性能。

Conclusion: 深度学习可减少传统预处理步骤的需求和偏差，实现更快、更可靠的临床弹性评估。
...
### [138] [M$^3$HL: Mutual Mask Mix with High-Low Level Feature Consistency for Semi-Supervised Medical Image Segmentation](https://arxiv.org/abs/2508.03752)
*Yajun Liu,Zenghui Zhang,Jiang Yue,Weiwei Guo,Dongying Li*

Main category: eess.IV

TL;DR: 提出了一种名为M3HL的新方法，结合动态可调掩码和层次一致性正则化，改进了半监督医学图像分割任务中的CutMix数据增强。


<details>
  <summary>Details</summary>
Motivation: 现有CutMix方法在医学图像分割中表现僵硬且缺乏特征级一致性约束，限制了性能提升。

Method: M3HL包含两部分：1) M3：基于MIM的动态可调掩码数据增强；2) HL：高低层次特征一致性正则化框架。

Result: 在ACDC和LA数据集上取得了最先进的性能。

Conclusion: M3HL通过动态掩码和层次一致性，显著提升了半监督医学图像分割的效果。
...
### [139] [Classification non supervis{é}es d'acquisitions hyperspectrales cod{é}es : quelles v{é}rit{é}s terrain ?](https://arxiv.org/abs/2508.03753)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有评估方法中存在的问题，如类别定义不明确、类内变异性高和分类错误。

Method: 基于类内光谱变异性的简单模型，利用有限数量的编码采集数据进行无监督分类。

Result: 在Pavia University场景中，通过简单假设检测到光谱更一致的区域，表明需要重新评估分类方法。

Conclusion: 强调了在无监督场景下重新思考分类方法评估的必要性。
...
### [140] [FUTransUNet-GradCAM: A Hybrid Transformer-U-Net with Self-Attention and Explainable Visualizations for Foot Ulcer Segmentation](https://arxiv.org/abs/2508.03758)
*Akwasi Asare,Mary Sagoe,Justice Williams Asare*

Main category: eess.IV

TL;DR: 提出了一种名为FUTransUNet的混合架构，结合了Vision Transformers的全局注意力机制和U-Net的局部特征提取能力，用于糖尿病足溃疡的自动分割，取得了较高的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 糖尿病足溃疡（DFU）的自动分割在临床诊断和治疗规划中至关重要，但由于溃疡区域的异质性外观、不规则形态和复杂背景，这一任务具有挑战性。传统卷积神经网络（如U-Net）难以建模长距离空间依赖关系。

Method: 提出FUTransUNet，将Vision Transformers的全局注意力机制集成到U-Net框架中，结合全局上下文特征和局部空间分辨率。在Foot Ulcer Segmentation Challenge数据集上训练和验证。

Result: 训练集Dice系数为0.8679，IoU为0.7672，验证集Dice系数为0.8751，IoU为0.7780。Grad-CAM可视化展示了模型预测时的关注区域。

Conclusion: FUTransUNet成功整合了全局和局部特征提取，提供了高鲁棒性、准确性、可解释性和临床可转化性的解决方案，为糖尿病足溃疡的自动分析提供了可靠方法。
...
### [141] [Assessing the Impact of Image Super Resolution on White Blood Cell Classification Accuracy](https://arxiv.org/abs/2508.03759)
...
<details>
  <summary>Details</summary>
Motivation: 低分辨率的显微图像可能影响白细胞分类的准确性，因此需要探索图像增强技术对分类性能的影响。

Method: 结合标准数据和增强数据训练深度学习模型，并评估图像分辨率提升对分类效果的影响。

Result: 通过实验验证了图像分辨率提升对模型性能的积极影响，并优化了分类准确性。

Conclusion: 研究为特定白细胞数据集设计更高效的图像识别算法提供了理论基础和实践指导。
...
### [142] [Scaling Artificial Intelligence for Prostate Cancer Detection on MRI towards Population-Based Screening and Primary Diagnosis in a Global, Multiethnic Population (Study Protocol)](https://arxiv.org/abs/2508.03762)
...
TL;DR: 该研究通过多国MRI数据训练并验证了PI-CAI-2B模型，用于检测Gleason等级≥2的前列腺癌，目标是验证其与标准诊断的一致性。


<details>
  <summary>Details</summary>
Motivation: 开发高效AI系统以改进前列腺癌的MRI检测，并验证其在不同人群和地区的适用性。

Method: 使用22,481例MRI数据（来自22个国家）训练和验证PI-CAI-2B模型，分为内部和外部测试组。

Result: 主要终点是AI与标准诊断的一致性，次要终点包括AUROC分析以评估模型性能。

Conclusion: PI-CAI-2B模型在检测前列腺癌方面具有潜力，需进一步验证其临床适用性。
...
### [143] [When Deep Learning Fails: Limitations of Recurrent Models on Stroke-Based Handwriting for Alzheimer's Disease Detection](https://arxiv.org/abs/2508.03773)
*Emanuele Nardone,Tiziana D'Alessandro,Francesco Fontanella,Claudio De Stefano*

Main category: eess.IV

TL;DR: 研究探讨了深度学习能否通过手写分析实现非侵入性阿尔茨海默病检测，发现传统集成方法优于循环神经网络架构。
...
Conclusion: 循环网络架构假设与离散笔画特征不匹配，性能受限，但研究为未来数据表示和模型兼容性提供了方向。
...
### [144] [UNISELF: A Unified Network with Instance Normalization and Self-Ensembled Lesion Fusion for Multiple Sclerosis Lesion Segmentation](https://arxiv.org/abs/2508.03982)
...
TL;DR: UNISELF是一种深度学习方法，用于多发性硬化（MS）病变的自动分割，通过测试时自集成和实例归一化技术，在单源训练数据下实现了高准确性和强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在单源训练数据下难以同时优化域内准确性和域外泛化能力，UNISELF旨在填补这一空白。

Method: UNISELF采用测试时自集成病变融合和测试时实例归一化（TTIN）技术，以应对域偏移和缺失输入对比度的问题。

Result: 在ISBI 2015测试数据集上表现优异，并在多个域外测试数据集（如MICCAI 2016和UMCL）上超越基准方法。

Conclusion: UNISELF在单源训练数据下实现了高准确性和强泛化能力，适用于多变的实际应用场景。
...
### [145] [PET2Rep: Towards Vision-Language Model-Drived Automated Radiology Report Generation for Positron Emission Tomography](https://arxiv.org/abs/2508.04062)
...
TL;DR: PET2Rep是一个针对PET图像报告生成的大规模基准数据集，填补了现有基准的空白，并评估了30种前沿视觉语言模型（VLMs）的性能。结果表明，当前最先进的VLMs在PET报告生成任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: PET成像在临床决策中至关重要，但手动生成报告耗时费力。尽管VLMs在医学领域有潜力，但现有研究主要关注结构成像，忽略了PET的代谢特性。

Method: 引入PET2Rep数据集，包含全身图像-报告对，并设计临床效率指标评估报告质量。对30种VLMs进行对比分析。

Result: 当前VLMs在PET报告生成任务上表现不佳，无法满足实际需求。

Conclusion: 需解决VLMs的关键不足，以推动其在医学应用中的发展。
...
### [146] [Edge2Prompt: Modality-Agnostic Model for Out-of-Distribution Liver Segmentation](https://arxiv.org/abs/2508.04305)
*Nathan Hollet,Oumeymah Cherkaoui,Philippe C. Cattin,Sidaty El hadramy*

Main category: eess.IV

TL;DR: Edge2Prompt是一种新型的模态无关肝脏分割方法，结合边缘检测和基础模型，适用于分布外数据，性能优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 肝脏分割在术前规划中至关重要，但临床应用中面临模态特异性工具和数据稀缺的挑战。

Method: 通过边缘检测提取模态无关边缘图，利用U-Net生成基于logit的提示，结合SAM-2模型生成2D分割结果并重建为3D体积。

Result: 在CHAOS数据集上表现优异，分布外任务中Dice分数达86.4%，优于U-Net和其他自提示方法。
...
### [147] [Discriminating Distal Ischemic Stroke from Seizure-Induced Stroke Mimics Using Dynamic Susceptibility Contrast MRI](https://arxiv.org/abs/2508.04404)
*Marijn Borghouts,Richard McKinley,Josien Pluim,Manuel Köstner,Roland Wiest,Ruisheng Su*

Main category: eess.IV

TL;DR: 研究探讨了磁共振灌注成像（MRP）在区分远端急性缺血性卒中（AIS）与癫痫发作（一种常见卒中模拟）中的潜力，通过分析灌注图描述符（PMDs）取得了较高的诊断性能。


<details>
  <summary>Details</summary>
Motivation: 区分急性缺血性卒中（AIS）与卒中模拟（SMs），尤其是涉及中小血管闭塞的病例，是诊断中的重大挑战。CT的敏感性有限，因此探索MRP的潜力。

Method: 使用162名患者（129名AIS，33名癫痫发作）的回顾性数据集，从动态磁敏感对比（DSC）图像中提取区域灌注图描述符（PMDs），并进行统计分析。

Result: 模型在区分AIS与癫痫发作时，AUROC为0.90，AUPRC为0.74，特异性92%，敏感性73%。

Conclusion: MRP-based PMDs可作为区分真实卒中与模拟的有效特征，值得进一步研究。代码已开源。
...
### [148] [Unmasking Interstitial Lung Diseases: Leveraging Masked Autoencoders for Diagnosis](https://arxiv.org/abs/2508.04429)
...
TL;DR: MAEs用于无标签数据预训练，提取临床相关特征，提升扩散性肺病诊断性能。


<details>
  <summary>Details</summary>
Motivation: 扩散性肺病研究中标注影像数据稀缺，MAEs能利用无标签数据学习稳健特征。

Method: 在5000多张胸部CT扫描上预训练MAE，结合内部和公开数据，并在下游分类任务中微调。

Result: MAEs能有效提取临床相关特征，提升诊断性能，无需大规模标注数据。

Conclusion: MAEs在扩散性肺病诊断中表现优异，适用于标注数据稀缺的场景。
...
### [149] [TotalRegistrator: Towards a Lightweight Foundation Model for CT Image Registration](https://arxiv.org/abs/2508.04450)
...
TL;DR: TotalRegistrator是一种轻量级图像配准框架，能同时对齐多个解剖区域，表现优于基线方法，并展示了强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像配准方法多针对单器官，限制了在其他解剖区域的通用性。

Method: 采用标准UNet架构和新颖的场分解策略，构建轻量级框架，仅需11GB GPU内存。

Result: 在多器官腹部配准中表现优于基线方法，在外部数据集上取得竞争性结果。

Conclusion: TotalRegistrator具有强泛化能力，适用于多解剖区域配准，代码将公开。
...
### [150] [OpenDCVCs: A PyTorch Open Source Implementation and Performance Evaluation of the DCVC series Video Codecs](https://arxiv.org/abs/2508.04491)
...
<details>
  <summary>Details</summary>
Motivation: 解决以往公开代码仅限于评估代码的问题，推动可重复性、基准测试和进一步开发。

Method: 提供统一的、支持端到端训练和评估的框架，包括详细文档和评估协议。

Result: 实现了显著的比特率降低，并提供了透明一致的比较基础。

Conclusion: OpenDCVCs为社区提供了加速研究和促进协作的工具，所有代码和实验工具均已公开。
...
### [151] [Conditional Fetal Brain Atlas Learning for Automatic Tissue Segmentation](https://arxiv.org/abs/2508.04522)
*Johannes Tischer,Patric Kienast,Marlene Stümpflen,Gregor Kasprian,Georg Langs,Roxane Licandro*

Main category: eess.IV

TL;DR: 提出了一种基于深度学习的框架，用于生成连续的、年龄特定的胎儿脑图谱，支持实时脑组织分割，具有高精度和实时性能。


<details>
  <summary>Details</summary>
Motivation: 胎儿脑MRI评估因脑成熟度、成像协议和孕龄估计的不确定性而具有挑战性，需要标准化参考框架。

Method: 结合直接配准模型和条件判别器，基于219例21至37周孕龄的胎儿MRI数据训练。

Result: 实现了高配准精度和86.3%的平均Dice相似系数，揭示了详细的神经典型生长轨迹。
...
### [152] [LA-CaRe-CNN: Cascading Refinement CNN for Left Atrial Scar Segmentation](https://arxiv.org/abs/2508.04553)
*Franz Thaler,Darko Stern,Gernot Plank,Martin Urschler*

Main category: eess.IV

TL;DR: 提出了一种名为LA-CaRe-CNN的两阶段3D CNN模型，用于从LGE MR扫描中精确分割左心房和疤痕组织，以支持个性化消融治疗。


<details>
  <summary>Details</summary>
Motivation: 心房颤动（AF）治疗需要消融手术，而患者特异性心脏数字孪生模型需要准确的健康与疤痕组织分割。

Method: 采用两阶段CNN级联结构，第一阶段预测左心房，第二阶段结合原始图像信息细化疤痕组织预测，并通过数据增强应对域偏移。

Result: 模型在5折集成下表现优异，左心房分割DSC为89.21%，疤痕组织DSC为64.59%。

Conclusion: LA-CaRe-CNN的分割结果有望用于生成心脏数字孪生模型和个性化消融治疗。
...
### [153] [A Comprehensive Framework for Uncertainty Quantification of Voxel-wise Supervised Models in IVIM MRI](https://arxiv.org/abs/2508.04588)
...
Method: 使用深度集成（DE）和混合密度网络（MDN）构建概率框架，分解不确定性为偶然性和认知性，并通过合成数据训练和真实数据评估。

Result: MDN在D和f参数上生成更校准和锐利的预测分布，尽管D*存在轻微过度自信。认知性不确定性揭示了真实数据与模拟数据的差异。

Conclusion: 该框架为IVIM拟合提供了全面的不确定性量化方法，可识别不可靠估计，并适用于其他物理模型。
...
### [154] [RLGS: Reinforcement Learning-Based Adaptive Hyperparameter Tuning for Gaussian Splatting](https://arxiv.org/abs/2508.04078)
...
<details>
  <summary>Details</summary>
Motivation: 3DGS的超参数调优过程繁琐且依赖专家经验，导致重建结果不一致且不理想。

Method: RLGS通过轻量级策略模块动态调整关键超参数（如学习率和密度阈值），并与现有3DGS流程无缝集成。

Result: RLGS在多个3DGS变体（如Taming-3DGS和3DGS-MCMC）上表现优异，例如在TNT数据集上将Taming-3DGS的PSNR提升了0.7dB。

Conclusion: RLGS为3DGS训练中的超参数调优提供了高效通用的自动化解决方案，填补了强化学习在3DGS中应用的空白。
...
### [155] [Radiance Fields in XR: A Survey on How Radiance Fields are Envisioned and Addressed for XR Research](https://arxiv.org/abs/2508.04326)
*Ke Li,Mana Masuda,Susanne Schmidt,Shohei Mori*

Main category: cs.GR

TL;DR: 本文系统调查了辐射场（RF）在XR领域的应用现状，分析了研究差距，并提供了XR社区的资源导航。


<details>
  <summary>Details</summary>
Motivation: 尽管辐射场（如3DGS和NeRF）在交互式逼真视图合成方面取得了突破，但XR领域的相关研究仍较少，需填补这一研究空白。

Method: 收集并分析了365篇与XR相关的RF文献，重点研究了其中66篇详细探讨RF在XR中应用的论文。

Result: 研究发现XR领域的RF研究仍处于早期阶段，但已显示出巨大潜力。

Conclusion: 本文为XR社区提供了RF研究的导航资源，并指出了未来的研究方向。
...
### [156] [Surf3R: Rapid Surface Reconstruction from Sparse RGB Views in Seconds](https://arxiv.org/abs/2508.04508)
*Haodong Zhu,Changbai Li,Yangyang Ren,Zichao Feng,Xuhui Liu,Hanlin Chen,Xiantong Zhen,Baochang Zhang*

Main category: cs.GR

TL;DR: Surf3R是一种端到端的前馈方法，无需相机标定即可从稀疏视图中重建3D表面，并在10秒内完成整个场景。
...
Method: 采用多分支和多视图解码架构，通过分支级处理、跨视图注意力和分支间融合捕获几何线索，并引入D-Normal正则化器优化3D几何。

Result: 在ScanNet++和Replica数据集上达到最先进的性能，表现出优秀的泛化能力和效率。

Conclusion: Surf3R无需相机标定即可高效重建3D表面，显著提升了3D一致性和表面细节精度。
...
### [157] [MienCap: Realtime Performance-Based Facial Animation with Live Mood Dynamics](https://arxiv.org/abs/2508.04687)
*Ye Pan,Ruisi Zhang,Jingying Wang,Nengfu Chen,Yilin Qiu,Yu Ding,Kenny Mitchell*

Main category: cs.GR

TL;DR: 论文提出了一种结合传统混合形状动画技术和机器学习模型的方法，以提高基于性能的动画表现，生成感知上更真实的3D风格化角色表情。


<details>
  <summary>Details</summary>
Motivation: 改进基于性能的动画技术，使其能够驱动感知上更真实的3D风格化角色表情。

Method: 结合传统混合形状动画技术和多种机器学习模型，提出非实时和实时解决方案。非实时系统使用3D情感转移网络从2D图像生成3D角色参数；实时系统通过混合形状适应网络生成几何一致且时间稳定的角色参数。

Result: 与商业产品Faceware相比，系统的表情识别度、强度和吸引力评分显著更高。

Conclusion: 该系统可集成到动画制作流程中，帮助动画师更快、更准确地创建所需表情。
...
### [158] [VLF Remote Sensing across and along the Totality Path during the April 8, 2024 Total Solar Eclipse using a VLF Receiver Network](https://arxiv.org/abs/2508.04030)
*Oleksiy Agapitov,Mark Gołkowski,Lucas Colomban,Ryan Eskola,Favour Ogbinaka,Iryna Agapitova,Kyung-Eun Choi,Varvara Bashkirova,Aaron Brenemean*

Main category: physics.geo-ph

TL;DR: 在2024年4月8日的日全食期间，研究者通过部署在日全食路径上的VLF接收器观测了五个美国海军VLF发射器的信号振幅和相位变化。


<details>
  <summary>Details</summary>
Motivation: 研究日全食对VLF信号传播的影响，特别是通过独特的接收器布局捕捉日全食路径上的信号变化。

Method: 在日全食路径上部署了九个接收器站点，观测了来自两个海军发射器的28条无线电传播路径的信号变化。

Result: 观测到振幅和相位的显著变化，包括5-10 dB的振幅波动和负相位变化，并发现了一种动态的“W”形振幅响应。

Conclusion: 这些观测结果可用于研究日全食对低电离层的影响，并揭示了信号变化的时空动态特征。

Abstract: During the total solar eclipse in the United States on April 8, 2024, we
observed the amplitude and phase of VLF signals from five U.S. Navy VLF
transmitters using a novel geometry of VLF receivers deployed along and across
the totality path. Nine receiver sites (four of them inside the totality path)
were deployed to collect the data in different transmitter-receiver
configurations relative to the totality path, which intersected with the radio
propagation paths from the Cutler, MA (NAA, 24 kHz) and the LaMoure, ND (NML,
25.2 kHz) Navy transmitters. The transmitters themselves experienced 98.7 (NAA)
and 68 (NML) solar obscuration at 75 km altitude. The novelty of the
observations is the near-total obscuration of one of the transmitters and
observations of several radio propagation paths closely aligned to the path of
totality. This configuration enabled observations of the effects of the Moon
shadow progression from Texas to Maine for over three hours on a total of 28
radio paths (with coverage from 55 to 100) with transmitter-to-receiver
distances ranging from 780 km to 7700 km. Both positive and negative (5- 10 dB)
amplitude changes were observed by receivers throughout the eclipse period. The
observed phase changes were mostly negative. The unique observations of VLF
propagation along the totality path produced a temporally dynamic quasiperiodic
`W' shaped response in amplitude that can be used to determine the gradients
and spatial scales of the eclipse effect on the lower ionosphere. For
observations within 780 km of NAA, the eclipse produced a 13 dB surge in
amplitude.

</details>


### [159] [Evolution of the Tohoku earthquake aftershocks in the framework of the phenomenological theory of aftershocks](https://arxiv.org/abs/2508.04323)
*O. D. Zotov,A. V. Guglielmi*

Main category: physics.geo-ph

TL;DR: 论文分析了东北地震余震，基于地震源动态系统的现象学理论，提出了“地下时钟”与“世界时间”同步的概念，并展示了时间相对性的有效性。研究发现余震源的松弛过程分为三个阶段。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过动态系统理论解释地震余震现象，探索地震源状态随时间的变化规律。

Method: 采用现象学理论，将地震源视为动态系统，引入“地下时钟”概念，分析余震源的松弛过程。

Result: 发现余震源的松弛过程分为三个阶段：初始阶段（去激活系数为零）、主阶段（去激活系数恒定）、恢复阶段（去激活系数随机变化）。

Conclusion: 研究验证了时间相对性概念的有效性，揭示了余震源松弛的动态特性，为地震预测提供了新视角。

Abstract: The aftershocks of the Tohoku earthquake are analyzed in light of the
phenomenological theory of aftershocks. The theory is based on the concept of
an earthquake source as a dynamic system, the state of which is described by a
deactivation coefficient. The concept of the proper time of the source is used,
which in general differs from world time. The "underground clock" and the clock
showing world time have been synchronized. For an observer using the
underground impacts as time markers, the flow of world time will be uneven. The
admissibility and effectiveness of the idea of this specific relativity of time
is shown. Three phases of relaxation of the source after the main shock were
discovered. In the initial phase, the deactivation coefficient is zero. In the
main phase, the deactivation coefficient has a finite value and does not change
over time. In the recovery phase, the deactivation coefficient changes randomly
over time. The sharp transitions between phases resemble the phenomenon of
bifurcation.
...
### [160] [Super Resolved Imaging with Adaptive Optics](https://arxiv.org/abs/2508.04648)
*Robin Swanson,Esther Y. H. Lin,Masen Lamb,Suresh Sivanandam,Kiriakos N. Kutulakos*

Main category: astro-ph.IM

TL;DR: 提出一种利用自适应光学系统（AO）通过计算成像技术克服望远镜视场与分辨率矛盾的方法，显著提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 解决望远镜视场（FoV）与分辨率之间的矛盾，利用现有AO系统提升成像质量。

Method: 利用AO系统的变形镜施加学习控制的波前畸变，生成高频子像素位移图像序列，联合上采样得到超分辨率图像。

Result: 实验和模拟显示信噪比（SNR）提升高达12 dB，且无需硬件改动。

Conclusion: 该方法可无缝应用于现有望远镜系统，显著提升成像性能。
...
### [161] [CONVERGE: A Multi-Agent Vision-Radio Architecture for xApps](https://arxiv.org/abs/2508.04556)
*Filipe B. Teixeira,Carolina Simões,Paulo Fidalgo,Wagner Pedrosa,André Coelho,Manuel Ricardo,Luis M. Pessoa*

Main category: cs.NI

TL;DR: 论文提出了一种结合电信与计算机视觉的新架构，通过多智能体方法实时传输无线和视频感知信息，实验显示延迟低于1毫秒。


<details>
  <summary>Details</summary>
Motivation: 电信与计算机视觉领域独立发展，高频无线链路需要视觉数据预测信道动态并优化通信。

Method: 提出多智能体架构，实时传输无线和视频感知信息，并引入视频功能生成阻塞信息。

Result: 实验结果表明，感知信息延迟低于1毫秒，xApp能实时控制5G/6G RAN。

Conclusion: 该架构成功实现了集成感知与通信，为实时控制无线网络提供了新方法。
...
### [162] [GlaBoost: A multimodal Structured Framework for Glaucoma Risk Stratification](https://arxiv.org/abs/2508.03750)
...
Method: GlaBoost整合临床特征、眼底图像嵌入和专家文本描述，使用预训练卷积编码器和基于Transformer的语言模型提取特征，并通过增强的XGBoost模型进行分类。

Result: 在真实数据集上验证，GlaBoost准确率达98.71%，特征重要性分析显示临床一致的决策模式。
...
### [163] [Continual Multiple Instance Learning for Hematologic Disease Diagnosis](https://arxiv.org/abs/2508.04368)
*Zahra Ebrahimi,Raheleh Salehi,Nassir Navab,Carsten Marr,Ario Sadafi*

Main category: cs.LG

TL;DR: 提出了一种针对多实例学习（MIL）的持续学习方法，通过选择实例并保留数据多样性，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 实验室和临床环境中数据流动态变化，需要持续更新模型以避免灾难性遗忘，但现有方法在多实例学习中效果不佳。

Method: 基于实例注意力分数和距离选择样本，存储代表性实例集，保留数据多样性。

Result: 在白血病实验室的真实数据中，该方法显著优于现有持续学习方法。

Conclusion: 该方法为MIL提供了首个持续学习解决方案，适应数据分布随时间变化的需求。
...
TL;DR: 提出了CX-Mind模型，通过课程强化学习和可验证过程奖励实现CXR任务的交替推理，显著提升诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型在CXR诊断中缺乏可验证的推理监督，导致多任务诊断效率低。

Method: 构建CX-Set数据集，采用CuRL-VPR框架分两阶段优化模型。

Result: CX-Mind在视觉理解、文本生成和对齐任务上平均提升25.1%，临床数据集表现优异。

Conclusion: CX-Mind在多任务CXR诊断中表现卓越，具有临床实用性。
...
### [165] [LRTuckerRep: Low-rank Tucker Representation Model for Multi-dimensional Data Completion](https://arxiv.org/abs/2508.03755)
*Wenwu Gong,Lili Yang*

Main category: cs.LG

TL;DR: 提出了一种新的低秩Tucker表示（LRTuckerRep）模型，结合全局和局部先验建模，通过自适应的加权核范数和参数自由的拉普拉斯正则化，解决了多维数据补全问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如低秩近似或平滑正则化）存在计算成本高或泛化能力差的问题，需要一种更高效且无需手动调参的解决方案。

Method: LRTuckerRep模型通过Tucker分解统一全局低秩和局部平滑先验，采用自适应加权核范数和拉普拉斯正则化，并开发了两种迭代算法。

Result: 实验表明，LRTuckerRep在高缺失率下优于基线方法，具有更高的补全精度和鲁棒性。

Conclusion: LRTuckerRep为多维数据补全提供了一种高效且无需手动调参的解决方案，具有广泛的应用潜力。
...
### [166] [FLAT: Latent-Driven Arbitrary-Target Backdoor Attacks in Federated Learning](https://arxiv.org/abs/2508.04064)
*Tuan Nguyen,Khoa D Doan,Kok-Seng Wong*

Main category: cs.LG

TL;DR: FLAT是一种新型的联邦学习后门攻击方法，利用条件自编码器生成多样化的目标特定触发器，具有高度灵活性和隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击方法通常使用固定模式或单一目标触发器，灵活性差且易被检测。

Method: 通过潜在驱动的条件自编码器生成多样化的触发器，支持任意目标选择且无需重新训练。

Result: FLAT攻击成功率高，能有效规避现有防御机制。

Conclusion: 研究揭示了联邦学习中潜在驱动的多目标后门威胁，亟需新的防御策略。
...
### [167] [Think Before You Segment: An Object-aware Reasoning Agent for Referring Audio-Visual Segmentation](https://arxiv.org/abs/2508.04418)
*Jinxing Zhou,Yanghao Zhou,Mingfei Han,Tong Wang,Xiaojun Chang,Hisham Cholakkal,Rao Muhammad Anwer*

Main category: cs.MM

TL;DR: 论文提出TGS-Agent方法，通过显式参考理解将音频-视觉分割任务分解为Think-Ground-Segment过程，无需像素级监督，并在新基准上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖多模态融合学习潜在嵌入，缺乏可解释性且需要强监督。本文从显式参考理解角度出发，模仿人类推理过程。

Method: 提出TGS-Agent，包含Ref-Thinker（多模态语言模型）和Grounding-DINO、SAM2，通过指令微调数据集实现显式推理。

Result: 在标准Ref-AVSBench和新提出的R²-AVSBench上达到最优性能。

Conclusion: TGS-Agent通过显式推理和分解任务，实现了高效且可解释的音频-视觉分割。
...
### [168] [RoboTron-Sim: Improving Real-World Driving via Simulated Hard-Case](https://arxiv.org/abs/2508.04642)
*Baihui Xiao,Chengjian Feng,Zhijian Huang,Feng yan,Yujie Zhong,Lin Ma*

Main category: cs.RO

TL;DR: RoboTron-Sim通过模拟高风险场景提升自动驾驶系统在复杂情况下的性能，使用HASS数据集和SPE、I2E Encoder技术，实验显示性能提升50%。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶系统在罕见高风险场景和复杂交互中数据不足导致的性能问题。
...
### [169] [Spectral Efficiency-Aware Codebook Design for Task-Oriented Semantic Communications](https://arxiv.org/abs/2508.04223)
*Anbang Zhang,Shuaishuai Guo,Chenyuan Feng,Shuai Liu,Hongyang Du,Geyong Min*

Main category: eess.SP

TL;DR: 论文提出了一种基于Wasserstein距离的自适应混合分布方案（WS-DC），用于设计高效的任务导向语义通信（ToSC）码本，以提升频谱效率和信道容量利用率。


<details>
  <summary>Details</summary>
Motivation: 现有ToSC方法依赖稀疏激活的码本，导致频谱效率低和信道容量未充分利用，亟需设计既能支持任务推理又能接近信道容量理论极限的码本。

Method: 构建了一个频谱效率感知的码本设计框架，将码本激活概率纳入优化过程，并引入Wasserstein距离作为正则化指标，最小化学习分布与最优信道输入分布的差距。

Result: 实验表明，WS-DC在推理准确性和码本效率上均优于现有方法，为接近信道容量的语义通信系统提供了新方向。

Conclusion: WS-DC框架通过结合任务驱动和信道感知的潜在表示，显著提升了ToSC系统的性能，具有实际应用潜力。
...
### [170] [Less Signals, More Understanding: Channel-Capacity Codebook Design for Digital Task-Oriented Semantic Communication](https://arxiv.org/abs/2508.04291)
*Anbang Zhang,Shuaishuai Guo,Chenyuan Feng,Hongyang Du,Haojin Li,Chen Sun,Haijun Zhang*

Main category: eess.SP

TL;DR: 论文提出了一种针对低功耗边缘网络设计的信道感知离散语义编码框架，通过Wasserstein正则化目标优化语义保真度和任务准确性。


<details>
  <summary>Details</summary>
Motivation: 当前任务导向语义通信（ToSC）框架在离散映射中常忽略信道特性和任务需求，导致性能不佳和泛化能力有限。
...
### [171] [Physics-Informed Neural Network for Elastic Wave-Mode Separation](https://arxiv.org/abs/2508.04600)
*E. A. B. Alves,P. D. S. de Lima,D. H. G. Duarte,M. S. Ferreira,J. M. de Araújo,C. G. Bezerra*

Main category: physics.comp-ph

TL;DR: 论文研究了物理信息神经网络（PINN）在非均匀弹性介质中分离P和S波的能力，通过标量泊松方程降低计算成本，并在均质和非均质模型中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 非均匀弹性介质中的模式转换导致物理性质解释困难，正确分解这些模式对多学科研究至关重要。

Method: 使用物理信息神经网络（PINN）和标量泊松方程分离P和S波，与传统向量方法相比降低了计算成本。

Result: 在均质和非均质模型中，分离的波模式与传统数值方法结果接近，且横向波泄漏减少。

Conclusion: PINN方法在模式分离中表现出高效性和准确性，为复杂介质中的波场分析提供了新工具。
...
### [172] [OS Agents: A Survey on MLLM-based Agents for General Computing Devices Use](https://arxiv.org/abs/2508.04482)
...
<details>
  <summary>Details</summary>
Motivation: 实现像《钢铁侠》中J.A.R.V.I.S.一样多功能的AI助手，推动了基于操作系统环境的自动化任务代理的研究。

Method: 通过分析OS Agents的环境、观察空间、动作空间等关键组件，以及构建方法（如领域特定基础模型和代理框架），并评估其在多样化任务中的表现。

Result: 总结了OS Agents的研究现状，提出了评估协议和基准，并指出了当前挑战和未来方向（如安全性和个性化）。

Conclusion: 本文为OS Agents的研究提供了全面的综述，旨在指导学术和工业界的发展，并维护了一个开源资源库以促进创新。
...
### [173] [SEAgent: Self-Evolving Computer Use Agent with Autonomous Learning from Experience](https://arxiv.org/abs/2508.04700)
*Zeyi Sun,Ziyu Liu,Yuhang Zang,Yuhang Cao,Xiaoyi Dong,Tong Wu,Dahua Lin,Jiaqi Wang*

Main category: cs.AI

TL;DR: SEAgent是一种自进化框架，通过自主学习和任务生成，使计算机使用代理（CUA）能够适应新软件环境，性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型（LVLM）在缺乏标注数据的新软件环境中表现不佳，需要一种自主进化的解决方案。

Method: SEAgent通过世界状态模型和课程生成器实现自主学习和任务生成，结合对抗模仿和GRPO优化策略，采用专家到通用训练策略。

Result: 在OS-World的五个新软件环境中，SEAgent的成功率从11.3%提升至34.5%，显著优于UI-TARS。

Conclusion: SEAgent通过自主学习和任务生成，显著提升了CUA在新软件环境中的适应能力，性能优于现有方法。
...
### [174] [ZARA: Zero-shot Motion Time-Series Analysis via Knowledge and Retrieval Driven LLM Agents](https://arxiv.org/abs/2508.04038)
*Zechen Li,Baiyu Chen,Hao Xue,Flora D. Salim*

Main category: cs.CL

TL;DR: ZARA是一个基于代理的零样本、可解释的人类活动识别框架，直接从原始运动时间序列中工作，无需微调或特定任务分类器，性能优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要针对固定活动集训练且难以适应新行为或传感器设置，而基于大语言模型的方法准确率低且缺乏可解释性。

Method: ZARA结合自动生成的特征知识库、多传感器检索模块和分层代理流程，指导大语言模型迭代选择特征并生成预测和解释。

Result: 在8个HAR基准测试中，ZARA实现了零样本SOTA性能，F1分数比最强基线高2.53倍。
...
Method: MedCheck将基准测试开发分解为五个阶段，提供46项医学定制标准，并对53个医疗LLM基准测试进行实证评估。

Result: 分析揭示了系统性问题，如与临床实践脱节、数据完整性危机和安全性评估缺失。
...
TL;DR: 论文提出Chain of Questions (CoQ)框架，通过动态生成问题引导多模态语言模型选择性地激活相关感官模态，提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如思维链）在单模态中表现良好，但在多模态环境中未能充分应用，模型需主动决定使用哪些感官模态。

Method: 引入CoQ框架，鼓励模型生成针对环境的问题，选择性激活相关模态以收集关键信息。

Result: 在整合的多模态基准数据集上，CoQ方法提升了基础模型识别和整合感官信息的能力，提高了准确性和可解释性。

Conclusion: CoQ框架有效改进了多模态任务的推理过程，增强了模型的适应性和性能。
...
### [177] [On the meaning of the dynamo radius in giant planets with stable layers](https://arxiv.org/abs/2508.03861)
*Paula N. Wulff,Hao Cao,Jonathan M. Aurnou*

Main category: astro-ph.EP

TL;DR: 论文探讨了木星内部稳定分层对磁场生成的影响，发现稳定层的存在和深度会影响磁场的生成半径。


<details>
  <summary>Details</summary>
Motivation: 研究木星内部稳定分层（由氢和氦不混溶导致）对磁场生成机制和半径的影响，以解释Juno观测到的磁场数据。

Method: 使用三维数值发电机模拟，结合木星内部结构和电导率分布，分析稳定层对磁场生成的影响。

Result: 浅稳定层下，磁场生成仅发生在稳定层底部；深稳定层下，磁场生成半径可达0.87R_J。结果表明木星内部存在从0.8R_J延伸至0.9R_J的稳定层。

Conclusion: 木星内部存在一个从0.8R_J延伸至0.9R_J的稳定层，其物理起源与氦雨现象的关系尚需进一步研究。

Abstract: Current structure models of Jupiter and Saturn suggest that helium becomes
immiscible in hydrogen in the outer part of the planets' electrically
conducting regions. This likely leads to a layer in which overturning
convection is inhibited due to a stabilizing compositional gradient. The
presence of such a stably stratified layer impacts the location and mechanism
of convectively-driven dynamo action. Juno's measurements of Jupiter's magnetic
field enabled an estimate of its dynamo radius based on the magnetic Lowes
spectrum. A depth of ~0.8R_J is obtained, where 1R_J is Jupiter's radius. This
is rather deep, considering that the electrical conductivity inside Jupiter is
expected to reach significant values at ~0.9R_J. Here we use 3-dimensional
numerical dynamo simulations to explore the effects of the existence and
location of a stably stratified helium rain layer on both the inferred Lowes
radius and location of the radial extent of dynamo action. We focus on a
Jupiter-like internal structure and electrical conductivity profile. We find
that for shallower stable layers, there is no magnetic field generation
occurring above the stable layer and the effective dynamo radius and the
inferred Lowes radius is at the base of the layer. For deeper stable layers,
Lowes radii of ~0.87R_J are inferred as a shallow secondary dynamo operates
above the stable layer. Our results strongly suggest the existence of a stable
layer extending from ~0.8R_J up to at least ~0.9R_J inside Jupiter. The
physical origin of this extended stable layer and its connection to helium rain
...
### [178] [TDSNNs: Competitive Topographic Deep Spiking Neural Networks for Visual Cortex Modeling](https://arxiv.org/abs/2508.04270)
*Deming Zhou,Yuetong Fang,Zhaorui Wang,Renjing Xu*

Main category: cs.NE

TL;DR: 论文提出了一种基于脉冲神经网络（SNN）的时空约束损失函数（STC），用于构建地形深度脉冲神经网络（TDSNN），成功模拟了灵长类视觉皮层的功能组织，并在性能和生物逼真度上优于传统人工神经网络（ANN）。


<details>
  <summary>Details</summary>
Motivation: 传统深度人工神经网络（ANN）在模拟视觉皮层的功能组织时忽略了时间动态性，导致性能下降和生物逼真度不足。为了解决这一问题，作者利用脉冲神经网络（SNN）的时空特性，提出了一种新的方法。

Method: 作者提出了时空约束损失函数（STC），用于训练地形深度脉冲神经网络（TDSNN），以模拟视觉皮层的空间功能组织。该方法结合了脉冲神经网络的时空动态性。

Result: 实验表明，TDSNN在ImageNet任务中性能几乎无下降（与TopoNet相比下降3%），并且在生物逼真度上优于传统ANN。此外，地形组织通过脉冲机制促进了高效稳定的时间信息处理，增强了模型鲁棒性。

Conclusion: TDSNN在计算性能和生物逼真度之间取得了平衡，为神经科学现象的解释和高效深度学习模型的设计提供了新思路。
...
### [179] [Audio Does Matter: Importance-Aware Multi-Granularity Fusion for Video Moment Retrieval](https://arxiv.org/abs/2508.04273)
*Junan Lin,Daizong Liu,Xianke Chen,Xiaoye Qu,Xun Yang,Jixiang Zhu,Sanyuan Zhang,Jianfeng Dong*

Main category: cs.IR

TL;DR: 论文提出了一种重要性感知的多粒度融合模型（IMG），用于视频时刻检索（VMR），动态选择性地整合音频、视觉和文本模态，以解决现有方法忽视音频重要性或简单嵌入的问题。


<details>
  <summary>Details</summary>
Motivation: 现有VMR方法多忽略音频模态或简单处理，而实际中音频可能对检索无意义。因此，需要动态选择有用音频并精细融合多模态信息。

Method: 设计音频重要性预测器（伪标签监督）和多粒度音频融合模块（局部、事件、全局级），并引入跨模态知识蒸馏以处理推理时音频缺失问题。

Result: 在新建数据集Charades-AudioMatter上验证，模型在音频-视频融合的VMR方法中达到最优性能。

Conclusion: IMG模型通过动态音频选择和精细融合，显著提升了VMR任务性能，证明了音频模态的重要性。
...
### [180] [Fast Magnetic Resonance Simulation Using Combined Update with Grouped Isochromats](https://arxiv.org/abs/2508.03960)
*Hidenori Takeshima*

Main category: physics.med-ph

TL;DR: 提出了一种通过分组等色体来减少MR模拟计算时间的新方法，显著提升了模拟速度。


<details>
  <summary>Details</summary>
Motivation: 克服传统MR模拟器中需要单独模拟每个等色体的假设，以减少计算时间。

Method: 将多个等色体分组，共享模拟过程中的部分计算，适用于特定梯度类型的序列。

Result: 新方法比传统方法快3到72倍，例如在2700万等色体下，FSE和EPI序列的模拟时间分别从208.4秒和66.4秒降至38.1秒和7.1秒。

Conclusion: 分组等色体的方法显著提高了MR模拟的效率，适用于特定梯度类型的序列。
...
### [181] [A comparative study of the high-pressure structural stability of zirconolite materials for nuclear waste immobilisation](https://arxiv.org/abs/2508.03786)
...
TL;DR: 对核废料固定材料zirconolite-2M、-4M、-3O和-3T的高压行为进行了比较研究，发现zirconolite-2M在高压下存在新的三斜结构（P-1），并在14.7 GPa时转变为单斜结构（C2/c）。其他三种结构未发现相变。


<details>
  <summary>Details</summary>
Motivation: 研究核废料固定材料在高压下的行为，以改进其性能和应用。

Method: 使用同步辐射粉末X射线衍射和密度泛函理论计算。

Result: 发现zirconolite-2M的新三斜结构（2TR）及其高压相变，其他三种结构无相变。

Conclusion: 研究为核废料固定材料的高压行为提供了新见解，尤其是zirconolite-2M的结构变化。
...
### [182] [PLA: Prompt Learning Attack against Text-to-Image Generative Models](https://arxiv.org/abs/2508.03696)
*Xinqi Lyu,Yihao Liu,Yanjie Li,Bin Xiao*

Main category: cs.CR

TL;DR: 本文提出了一种新的提示学习攻击框架（PLA），用于在黑盒设置下绕过文本到图像（T2I）模型的安全机制，通过多模态相似性设计梯度训练方法，实验表明其攻击成功率高于现有方法。


<details>
  <summary>Details</summary>
Motivation: 尽管T2I模型广泛应用，但其可能被滥用以生成不安全内容（NSFW）。现有方法在搜索对抗提示时性能有限，黑盒设置下梯度训练面临挑战。

Method: 提出PLA框架，利用多模态相似性设计梯度训练方法，适用于黑盒T2I模型。

Result: 实验表明PLA能有效攻击黑盒T2I模型的安全机制（如提示过滤器和后验安全检查器），成功率高于现有方法。

Conclusion: PLA在黑盒设置下显著提升了对抗攻击的效果，揭示了T2I模型安全机制的潜在漏洞。
...
### [183] [Tell Me Without Telling Me: Two-Way Prediction of Visualization Literacy and Visual Attention](https://arxiv.org/abs/2508.03713)
*Minsuk Chang,Yao Wang,Huichen Will Wang,Yuanhong Zhou,Andreas Bulling,Cindy Xiong Bearfield*

Main category: cs.HC

TL;DR: 研究提出Lit2Sal和Sal2Lit模型，利用视觉素养差异改进可视化设计和评估，Lit2Sal在预测注意力方面优于现有模型，Sal2Lit能以86%准确率预测视觉素养。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽视了视觉素养对注意力行为的影响，希望通过个体差异提升可视化设计的有效性。

Method: 基于235名参与者的三项测试数据，分析注意力模式与视觉素养的关系，提出Lit2Sal和Sal2Lit模型。

Result: Lit2Sal优于现有显著性模型，Sal2Lit预测视觉素养准确率达86%。

Conclusion: 结合个体差异的显著性模型和注意力分析为个性化可视化数据通信提供了新方向。
...
### [184] [DRIVE-T: A Methodology for Discriminative and Representative Data Viz Item Selection for Literacy Construct and Assessment](https://arxiv.org/abs/2508.04160)
*Angela Locoro,Silvia Golia,Davide Falessi*

Main category: cs.HC

TL;DR: DRIVE-T方法通过任务项区分性和代表性构建数据可视化素养评估，分为标注、难度评分和Rasch模型分析三步，验证了其有效性。
...
Method: DRIVE-T方法包括标注任务项、独立评分难度、通过Many-Facet Rasch模型分析评分数据。

Result: 成功构建了基于任务项区分性和代表性的难度级别测量模型，并通过试点研究验证。

Conclusion: DRIVE-T为数据可视化素养测量提供了一种有效的归纳式方法，支持难度级别的动态构建。
