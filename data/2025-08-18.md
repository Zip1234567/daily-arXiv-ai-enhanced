...
Method: ViPE通过高效算法估计相机内参、相机运动和密集深度图，支持多种相机模型和动态场景。

Result: 在TUM/KITTI序列上性能提升18%/50%，处理速度为3-5FPS，并标注了大规模视频数据集。
...
<details>
  <summary>Details</summary>
Motivation: 解决视线数据在隐私保护与实用性之间的权衡问题，确保数据在保护隐私的同时仍可用于良性任务。

Method: 使用潜在噪声自编码器对视线信号进行处理，减少生物识别的可识别性，同时保留生理上合理的视线模式。

Result: 显著降低了生物识别的可识别性，且对数据实用性影响极小，优于现有方法。

Conclusion: 该机制为视线数据提供了有效的隐私保护方案，同时保持了数据的可用性，推动了视线系统中隐私保护的发展。
...
Conclusion: VTG-MLLMs在性能和泛化方面表现优异，但仍需进一步研究以解决现有局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如CFG、NASA、NAG）在负提示引导方面存在不足，VSF旨在提供更高效的解决方案。

Method: VSF通过翻转负提示的注意力值符号动态抑制内容，计算开销小，兼容多种架构（如MMDiT、Wan）。

Result: 实验表明，VSF在少步模型中显著提升负提示遵循能力，图像质量保持竞争力。

Conclusion: VSF在负提示引导方面表现优异，适用于静态图像和视频生成任务。
...
Method: 扩展PAE到RPR任务，提出一种新的重定位方案，利用PAE-based RPR优化APR预测，无需额外数据存储。通过对比等效架构的图像RPR模型验证其有效性。

Result: 在室内基准测试中，PAE-based RPR显著提升了APR定位精度，且仅需30%的训练数据即可达到竞争性能，大幅降低了数据收集负担。

Conclusion: 该方法在减少数据需求的同时，显著提升了相机定位精度，为零售场景的部署提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统封闭式3D检测无法满足开放世界应用需求，现有开放词汇方法虽语义准确但几何质量不足。
...
Method: 通过共享和融合中间高斯基元，实现邻居跨代理融合、几何与语义联合编码及稀疏对象中心消息传递。

Result: 在mIoU和IoU指标上分别优于单代理感知和基线协作方法，通信量减少时仍保持性能。
...
Conclusion: IDFSR通过身份解耦和拟合，有效解决了极端退化下的身份恢复问题，具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统木材分类方法耗时且依赖专家知识，亟需自动化解决方案以支持生态监测和森林管理。

Method: 构建自定义图像数据集，评估五种卷积神经网络架构（ResNet50、EfficientNet等），选择最优模型。

Result: ShuffleNetV2表现最佳，平均准确率99.29%，F1分数99.35%。

Conclusion: 轻量级深度学习模型适用于资源有限环境中的实时木材物种识别，推动生态信息学发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统池化方法在特征表达和鲁棒性上存在不足，NIRMAL Pooling旨在通过动态调整参数和引入非线性激活提升性能。

Method: NIRMAL Pooling通过动态调整池化参数并应用ReLU激活，结合自适应最大池化，增强特征表达。

Result: 在MNIST Digits、MNIST Fashion和CIFAR-10数据集上，NIRMAL Pooling的准确率分别为99.25%、91.59%和70.49%，均优于标准最大池化。

Conclusion: NIRMAL Pooling为图像识别任务提供了一种灵活可靠的池化替代方案，显著提升了CNN性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着智能手机和VR/AR技术的普及，环境中可能出现更多与虚拟元素连接的装饰性物体，检测这些物体是触发后续交互的第一步。

Method: 提出了一种新的特征描述符（形状方向直方图），用于描述Artcode的拓扑结构，并构建了检测系统。

Result: 实验结果表明，所提出的特征描述符能有效表示拓扑结构，检测系统表现良好。

Conclusion: 该研究为拓扑物体检测提供了初步解决方案，有望开启新的交互机会和应用场景。
...
Method: 利用自标注数据集和YOLO模型进行实时坑洼检测，结合OCR模块提取时间戳并与GPS数据同步，通过OpenStreetMap可视化结果。
...
Method: 提出增量补丁生成（IPG）方法，通过实验和消融研究（如YOLO特征分布可视化和对抗训练结果）验证其高效性和攻击性能。

Result: IPG生成的补丁能广泛覆盖模型漏洞，效率提升11.1倍，同时可作为构建鲁棒模型的知识基础。
...
Method: MedAtlas通过多轮对话、多模态图像交互和多任务集成，模拟真实诊断流程，并引入新的评估指标。

Result: 现有模型在多阶段临床推理中表现不佳，MedAtlas为开发稳健的医疗AI提供了挑战性评估平台。

Conclusion: MedAtlas填补了现有基准的不足，推动了医疗AI的发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决临床MRI数据中单壳低角分辨率采集导致的FOD估计不可靠问题，并推动深度学习在扩散MRI增强中的临床应用。

Method: 开发了FastFOD-Net，一种加速的端到端深度学习框架，优化了FOD的估计和增强。

Result: FastFOD-Net在健康对照和六种神经系统疾病中表现出色，分析速度快60倍，显著降低了样本量需求。

Conclusion: FastFOD-Net有望加速临床神经科学研究，提升疾病鉴别能力，并增强深度学习在扩散MRI中的临床信任。
...
<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在多模态任务中表现优异，但数据质量、复杂任务表现和评估协议不足限制了其可靠性和广泛应用。受人类利用外部工具启发，本文探讨如何通过工具增强MLLMs。

Method: 从四个维度分析外部工具的作用：1）高质量多模态数据的获取与标注；2）提升复杂下游任务性能；3）全面准确的模型评估；4）当前局限与未来方向。

Result: 外部工具在提升MLLMs能力方面具有变革潜力，尤其在数据、任务和评估方面表现显著。

Conclusion: 本文为工具增强MLLMs的发展和应用提供了前瞻性视角，强调其推动MLLMs进步的潜力。
...
Method: 设计了ORBIT基准测试，包含360张图像和1,080个计数问题，涵盖三种图像类型、三个推理级别和四个对象属性维度。

Result: 12个最先进的VLMs在零样本设置下表现不佳，最佳模型准确率仅为40%，尤其在真实图像和复杂推理任务上表现较差。

Conclusion: ORBIT揭示了VLMs在对象属性推理上的不足，呼吁开发更高效的基准测试方法和更强的推理模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶感知系统中因视觉模糊性（如同色异谱现象）导致的弱势道路使用者识别难题。

Method: 结合信息论技术和图像质量指标，提出波段选择策略，从高光谱数据中筛选最具信息量的波段（497 nm、607 nm、895 nm）。

Result: 实验表明，所选波段显著提升了VRU与背景的区分度，各项指标（如欧氏距离、SAM等）均有显著改善。
...
<details>
  <summary>Details</summary>
Motivation: 现有ControlNet在视频生成中因冗余计算导致延迟高，需解决效率问题。

Method: 提出时空双缓存策略：空间上分区计算，时间上跳过冗余去噪步骤。
...
<details>
  <summary>Details</summary>
Motivation: 探索VLM在无障碍领域的应用潜力，特别是模拟低视力人群的视觉感知能力。

Method: 通过调查40名低视力参与者，收集其视觉信息和图像感知数据，构建VLM提示以模拟参与者，并评估模拟结果与原始数据的匹配度。

Result: 仅提供少量提示时，VLM模拟准确性较低（0.59），结合视力信息和示例图像响应后显著提高（0.70）。

Conclusion: VLM在模拟低视力人群视觉感知时需结合具体信息和示例，单一样本即可显著提升性能，但额外样本效果有限。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集规模小且监督性强，限制了VLMs在建筑安全检查中的泛化能力。

Method: 构建包含图像标注、安全违规视觉问答和建筑元素视觉定位的多任务数据集。

Result: 评估显示VLMs在零样本和少样本设置下具有泛化能力，但需进一步训练以适应实际工地。
...
<details>
  <summary>Details</summary>
Motivation: 通用医学图像分割模型（如SAM）在多样化任务中表现出强大的泛化能力，但专用模型（如MedSAM）因数据有限和异质性等问题泛化能力不足。

Method: 提出一种零阶优化方法，自动发现最优层融合方案，并通过单任务和多目标优化满足不同临床需求。

Result: 在25个医学分割任务中，MedSAMix显著提升了性能，专用任务和多任务评估分别提高了6.67%和4.37%。

Conclusion: MedSAMix通过融合通用与专用模型的优势，有效解决了医学图像分割中的泛化和领域适应问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D VL数据集缺乏多视角推理和丰富的上下文对齐，限制了3D场景理解的深度发展。

Method: 提出MV-ScanQA数据集测试多视角推理，TripAlign数据集提供2D-3D-语言预训练语料，并开发LEGO方法进行知识迁移。

Result: LEGO在MV-ScanQA和现有3D密集描述与问答基准上均达到最佳性能。
...
<details>
  <summary>Details</summary>
Motivation: 尽管BMI是2型糖尿病的已知风险因素，但肥胖和非肥胖人群中的疾病分布差异提示腹部详细体成分可能揭示新的表型。

Method: 通过AI从临床CT中提取腹部特征，使用随机森林分类和SHAP分析，识别与糖尿病相关的体成分模式。

Result: 随机森林模型的AUC为0.72-0.74，发现肌肉脂肪化、内脏脂肪增多等是跨体重类别的共同特征。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将3D对象视为整体，忽略了语义部分层次结构，且高分辨率建模计算成本高，而真实物体具有稀疏和分层特性。
...
<details>
  <summary>Details</summary>
Motivation: 长时间使用电脑时的不正确坐姿已成为公共健康问题，传统监测方法存在隐私和舒适性问题。
...
Conclusion: RBDM成功实现了雾图和去雾图之间的高效双向转换，性能优越。
...
<details>
  <summary>Details</summary>
Motivation: 解决STEM教育中抽象科学概念可视化不足的问题，通过布局和叙事结构提升学习效果。

Method: 利用BookCover数据集，采用布局条件生成、对比视觉语义训练和提示调制。

Result: 生成连贯的视觉序列，支持Bloom分类法的推理，减少认知负荷。

Conclusion: LEARN为教育生成AI提供了新方向，支持未来研究和实际应用。
...
Method: 采用EM算法和双向布朗桥扩散模型（EM-B3DM），分两阶段学习：第一阶段用EM算法解耦配对图像联合分布，第二阶段利用预训练模型和大规模未配对数据优化性能。
...
<details>
  <summary>Details</summary>
Motivation: 在遥感场景中，源数据不可访问导致传统域适应方法受限，而源自由目标检测（SFOD）因伪标签噪声易导致训练崩溃。

Method: VG-DETR结合视觉基础模型（VFM），提出伪标签挖掘策略和双级对齐方法，利用少量标注目标数据提升伪标签质量和特征提取能力。
...
Method: IOVQA通过约束模型输出为整数范围[10,50]，并引入目标掩码策略，仅计算标签前两位整数的损失。
...
<details>
  <summary>Details</summary>
Motivation: CCD任务中缺乏新数据的类别数量和标签信息，同时需要避免灾难性遗忘，现有方法难以平衡新类别发现与分类，且容易累积错误。

Method: IDOD方法包括三个模块：独立多样性增强模块、联合新类别发现模块和正交性增量模块，分别解决特征多样性、单阶段新类别发现和低存储空间遗忘问题。

Result: 在细粒度数据集上，IDOD方法优于现有最先进方法。

Conclusion: IDOD通过独立多样性增强、联合新类别发现和正交性增量模块，有效解决了CCD中的关键问题，并在实验中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有适配器方法在视觉和文本模态对齐时，未能充分捕捉类别与图像样本之间的一对多关联，且难以处理未知类别的关联问题。

Method: LatHAdapter引入可学习的属性提示作为桥梁，并在双曲空间中投影类别、属性和图像，利用层次正则化学习潜在语义层次结构。

Result: 在四个少样本任务上的实验表明，LatHAdapter显著优于其他微调方法，尤其在已知类别适应和未知类别泛化方面表现突出。

Conclusion: LatHAdapter通过潜在语义层次结构的建模，有效解决了现有适配器方法的局限性，提升了少样本分类任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频标记化方法在空间和时间维度上存在局限性，GVT旨在通过生成式高斯分布和静态/动态分离策略解决这些问题。

Method: GVT采用Spatio-Temporal Gaussian Embedding（STGE）生成2D高斯分布，并通过Gaussian Set Partitioning（GSP）分离静态和动态内容。

Result: GVT在视频重建、动作识别和压缩任务中表现优异，优于基线方法MAGVIT-v2。

Conclusion: GVT通过生成式高斯分布和时空分离策略，显著提升了视频处理的性能和适应性。
...
Method: 通过数学证明和实验分析，提出CHARM3R方法，结合回归和基于地面的深度估计。

Result: CHARM3R在CARLA数据集上提升了45%的泛化性能，达到SOTA水平。

Conclusion: CHARM3R有效解决了相机高度变化对单目3D检测的影响，提升了模型鲁棒性。
...
Method: 利用大语言模型自动将单人教学视频转化为双人任务指导对话。

Result: 构建了包含507个对话、6636问答对和24小时视频的HowToDIV数据集。

Conclusion: 该方法为任务辅助对话研究提供了高效的数据集和基准。
...
Method: 提出UAV-VL-R1模型，采用监督微调（SFT）和多阶段强化学习（RL）的混合训练方法，结合GRPO算法提升推理的结构化和可解释性。

Result: UAV-VL-R1在零样本任务中比基线模型Qwen2-VL-2B-Instruct准确率高48.17%，甚至优于其36倍大的72B变体，且内存占用低，支持实时部署。

Conclusion: UAV-VL-R1通过混合训练方法有效解决了航拍图像推理的挑战，兼具高性能和轻量化，适用于资源受限的无人机平台。
...
Conclusion: 提出的框架有效提升了轻量级姿态估计的准确性和鲁棒性，适用于复杂场景。
...
<details>
  <summary>Details</summary>
Motivation: 解决行人重识别中因模态缺失或不确定带来的挑战，提升在资源受限环境下的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以满足多维度人类偏好（如动作自然性、唇同步准确性和视觉质量），且缺乏大规模高质量标注数据集。

Method: 引入Talking-Critic多模态奖励模型，构建Talking-NSQ数据集，并提出TLPO框架，通过专家模块分步优化多维度偏好。

Result: Talking-Critic在人类偏好评分上优于现有方法，TLPO在唇同步、动作自然性和视觉质量上均有显著提升。

Conclusion: 论文提出的方法在多维度偏好对齐上表现优越，为音频驱动肖像动画提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的对齐可能隐含并放大社会刻板印象，但标准准确性指标无法揭示这些偏见。

Method: 研究通过计算人脸图像与描述职业和活动的短语之间的余弦相似度差异，量化性别关联，并使用置信区间和零模型进行验证。

Result: 研究提供了一个详细的性别关联映射，揭示了视觉语言空间中的性别偏见。

Conclusion: 该研究为评估视觉语言模型中的性别偏见提供了一个稳健的框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在点云数据分布增强时忽略类别级分布和对齐，导致模型泛化能力不足。

Method: 提出类别级几何嵌入（CGE）感知几何特征，并引入几何一致性学习（GCL）对齐类别级几何嵌入。

Result: 实验证明该方法在领域泛化点云分割中具有竞争力。

Conclusion: 通过几何特征学习和一致性对齐，显著提升了3D语义分割的泛化能力。
...
Method: 通过多种视觉任务评估SAEs在三种视觉模型架构（视觉嵌入模型、多模态LMMs和扩散模型）中的表现。
...
TL;DR: 提出了一种统一框架，结合深度预测与时间感知优化，用于单目内窥镜组织重建，解决了深度模糊和组织变形等问题。


<details>
  <summary>Details</summary>
Motivation: 单目内窥镜手术中，组织重建和姿态估计面临深度模糊、组织变形等挑战，需提升导航和空间感知能力。

Method: 框架包含MAPIS-Depth模块（结合Depth Pro和Depth Anything）和WEMA-RTDL模块，通过RAFT计算像素对应关系，优化深度估计和姿态注册。
...
TL;DR: TimeMachine提出了一种基于扩散模型的框架，实现精确的年龄编辑同时保持身份特征不变，通过多交叉注意力模块注入年龄信息，并设计了Age Classifier Guidance模块提升编辑精度。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在面部图像编辑中取得了进展，但如何在保留个人身份的同时实现细粒度年龄编辑仍具挑战性。

Method: 提出TimeMachine框架，通过多交叉注意力模块分离年龄与身份特征，并引入Age Classifier Guidance模块在潜在空间预测年龄。构建了HFFA数据集以支持训练。

Result: 实验表明，TimeMachine在细粒度年龄编辑中实现了最先进的性能，同时保持了身份一致性。

Conclusion: TimeMachine通过创新的模块设计和高质量数据集，成功解决了细粒度年龄编辑中的身份保留问题。
...
TL;DR: 研究探讨了高光谱成像（HSI）在行人分割中的潜力，通过对比RGB和两种降维方法（PCA和CSNR-JMIM），发现CSNR-JMIM在性能上优于RGB。


<details>
  <summary>Details</summary>
Motivation: 解决RGB成像中行人与背景难以区分的安全问题。

Method: 使用H-City数据集，对比RGB与两种HSI降维方法（PCA和CSNR-JMIM），评估三种语义分割模型（U-Net、DeepLabV3+、SegFormer）。

Result: CSNR-JMIM在IoU和F1-score上分别平均提升1.44%和2.18%，有效减少误报。

Conclusion: 优化HSI波段选择能显著提升行人分割性能，具有安全关键应用潜力。
...
Method: 采用去噪-检索范式，包含文本条件去噪（TCD）和文本重构反馈（TRF）模块，动态识别噪声片段并净化表示。

Result: 在Charades-STA和QVHighlights数据集上超越现有方法，且范式可适配其他模型。

Conclusion: 去噪-检索范式显著提升性能，具有通用性和适应性。
...
TL;DR: LogicBench是一个用于诊断视觉语言模型（VLMs）逻辑理解能力的基准测试，LogicCLIP是一种提升VLM逻辑敏感性的训练框架。
...
Result: LogicCLIP在LogicBench上表现显著优于基线模型，同时在通用视觉语言任务上保持竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在拥挤环境或检测不准确时表现不佳，因忽略了物体间的几何关系。需要利用空间线索，但现有几何感知方法易受无关物体干扰。

Method: 设计了基于点对特征（PPF）的时空编码器，引入线索一致性变换模块对齐特征，并采用动态更新机制保持时空信息。

Result: 在nuScenes和Waymo数据集上验证了方法的有效性，nuScenes测试集上AMOTA达到70.3%。

Conclusion: DSC-Track通过线索一致性显著提升了3D多目标跟踪的性能，尤其在复杂场景中表现优越。
...
TL;DR: 论文提出了一种名为NoOp的噪声优化方法，用于解决扩散分类器（DC）中的噪声不稳定性问题，通过频率匹配和空间匹配原则学习“好噪声”，提升分类性能。
...
Method: 提出NoOp方法，通过频率匹配优化数据集特定噪声，并通过空间匹配训练Meta-Network生成图像特定噪声偏移，以替代随机噪声。

Result: 在各种数据集上的广泛实验证明了NoOp的有效性。

Conclusion: NoOp通过学习匹配噪声，显著提升了扩散分类器的稳定性和性能，同时避免了速度下降。
...
<details>
  <summary>Details</summary>
Motivation: 旨在开发一个可重复、可控制的框架，用于量化并减少人脸识别系统中的偏见。

Method: 结合StyleGAN3的身份保留生成和扩散模型的属性控制，生成10,000张平衡的人脸图像，并通过自动检测和人工验证评估真实性。

Result: AdaFace将组间TPR差异减少60%，光照因素占剩余偏见的42%，合成数据与真实数据的相关性达到0.85。

Conclusion: GANDiff FR为公平性审计提供了可重复的标准，尽管计算开销较高，但能生成更多属性条件变体，支持透明和可扩展的偏见评估。
...
Conclusion: IAQD是一种高效的蒸馏方法，显著提升了增量目标检测的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动分类方法需要大量代表性训练数据，人工成本高昂甚至难以负担。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击中的目标标签选择策略通常依赖随机性、模型预测或静态语义资源，导致可解释性、可重复性或灵活性不足。

Method: 提出了一种语义引导框架，利用BERT、TinyLLAMA和CLIP等预训练模型作为相似性来源，选择与真实标签最相关和最不相关的标签，构建最佳和最差对抗场景。

Result: 实验表明，这些模型能有效生成实用的对抗目标，尤其在远距离类别关系上优于静态语义数据库（如WordNet）。
...
TL;DR: RETFound首次被用于视盘分割任务，仅需少量任务特定样本训练即可超越现有最佳分割网络。
...
Method: 通过微调RETFound模型，仅训练一个轻量级头部，使用少量任务特定样本。

Result: 在多个公开和私有数据集上达到约96%的Dice分数，表现优于现有最佳分割网络。
...
TL;DR: 本文对Skeleton Recall Loss (SRL)进行了理论和实证分析，发现其在管状结构分割任务中并未超越传统基线模型，揭示了基于拓扑的损失函数的局限性。


<details>
  <summary>Details</summary>
Motivation: 图像分割是计算机视觉中的重要任务，针对管状结构的精确分割需要定制模型和损失函数。SRL声称在管状数据集上表现优异，但缺乏深入的理论和实证验证。

Method: 通过理论分析SRL损失的梯度，并在多个管状数据集上与传统基线模型进行性能比较。

Result: 实证结果显示，SRL并未超越传统基线模型，理论分析也揭示了其局限性。

Conclusion: 本文为研究人员提供了关于拓扑损失函数局限性的重要见解，有助于开发更有效的管状结构分割模型。
...
TL;DR: 提出了一种结合实例级嵌入蒸馏和关系级相似性蒸馏的统一知识蒸馏框架，显著提升了人脸识别模型在边缘设备上的性能。
...
Method: 提出两种新损失函数：实例级嵌入蒸馏（动态硬挖掘策略）和关系级相似性蒸馏（记忆库机制和样本挖掘策略）。
...
TL;DR: RMFAT是一种轻量级循环框架，用于高效恢复受大气湍流影响的视频，仅需两帧输入，显著降低计算负担，并在清晰度和速度上优于现有方法。
...
Method: 采用轻量级循环框架，结合多尺度特征编码和解码以及时间扭曲模块。
...
TL;DR: SelfAdapt是一种无需标签的自适应方法，通过学生-教师增强一致性训练、L2-SP正则化和无标签停止标准，提升预训练细胞分割模型的性能。


<details>
  <summary>Details</summary>
Motivation: 通用模型（如Cellpose）在跨领域数据上性能下降，而监督微调需要标注数据。SelfAdapt旨在解决这一问题，实现无标签的自适应。

Method: 采用学生-教师增强一致性训练，结合L2-SP正则化和无标签停止标准。

Result: 在LiveCell和TissueNet数据集上，AP0.5相对提升达29.64%，且能进一步提升监督微调模型。
...
TL;DR: 该论文研究了多模态生物特征模板的降维方法，通过融合不同模态的特征向量，在保持生物识别性能的同时显著减少模板大小，从而提升同态加密处理的效率。


<details>
  <summary>Details</summary>
Motivation: 生物特征识别广泛应用，但模板的隐私和安全性是关键问题。同态加密引入的计算负担较大，而多模态融合可以提升安全性。

Method: 利用深度神经网络提取特征，通过降维减少模板大小，实验基于虚拟多模态生物特征数据库。

Result: 多模态融合可将模板大小减少67%，同时保持或超越单模态的识别性能。

Conclusion: 降维方法在多模态生物特征识别中高效且安全，适用于同态加密场景。
...
Method: 提出ImagiDrive框架，结合VLM行为预测和DWM场景生成，引入早停机制和轨迹选择策略优化效率与准确性。
...
TL;DR: 论文提出了一种评估3D高斯泼溅中物体移除后语义残留的新基准和框架，并发布了Remove360数据集，揭示了当前方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 研究物体移除后语义残留对隐私保护3D重建和可编辑场景表示的重要性。

Method: 通过实验评估不同室内外场景中物体移除后的语义残留，并使用Remove360数据集进行验证。

Result: 发现当前方法在视觉几何缺失时仍能保留语义信息，但存在显著局限性。

Conclusion: 强调需要更鲁棒的解决方案以应对真实世界的复杂性。
...
TL;DR: MM-R1框架通过跨模态思维链推理策略，提升统一多模态大语言模型在个性化图像生成中的能力，无需针对每个新主题进行数据密集型微调。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要针对每个新主题进行数据密集型微调，限制了其扩展性。MM-R1旨在解决这一问题，释放统一MLLMs在个性化图像生成中的潜力。

Method: MM-R1采用跨模态思维链（X-CoT）推理策略，将个性化分为视觉推理和生成过程，并使用分组奖励近端策略优化（GRPO）增强推理能力。
...
Method: 采用基于图的新路径生成方法，结合可解释的数据增强和课程学习，实现高效、自动化的数据收集与训练。
...
TL;DR: CoFi是一种从粗到精的少样本分割方法，用于电子显微镜图像中的肾小球基底膜分割，减少了标注和计算负担，同时保持高精度。
...
Method: CoFi先用三张标注图像训练轻量网络生成粗分割掩码，再通过形态学修剪生成高质量点提示，引导SAM进行精细分割。

Result: Dice系数74.54%，推理速度1.9 FPS，显著降低标注和计算负担，结果准确可靠。

Conclusion: CoFi适合研究和临床应用，公开可用，潜力巨大。
...
Method: 提出TACR-YOLO，引入坐标注意力模块、任务感知注意力模块和强化颈部网络，优化锚框大小并使用DIoU损失。
...
<details>
  <summary>Details</summary>
Motivation: 建筑行业依赖视觉数据支持AI和ML应用，但现有数据集在规模、质量和代表性上差异较大，缺乏系统性分析，限制了AI应用的进一步发展。

Method: 通过系统检索学术数据库和开放数据平台，收集了51个公开视觉数据集（2005-2024年），并采用结构化数据模式进行分类。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视频合成技术取得进展，但多镜头视频生成的研究仍处于起步阶段，现有模型生成的视频过渡效果不稳定且单一。

Method: 构建多镜头视频文本数据集Cine250K，利用扩散模型的注意力映射设计掩码控制机制，实现任意位置的过渡。

Result: CineTrans生成的视频在过渡控制、时间一致性和整体质量上显著优于现有基线。

Conclusion: CineTrans通过掩码机制和数据集微调，成功生成符合电影编辑风格的多镜头视频。
...
TL;DR: 利用GPT模型结合建筑数据，通过机器学习分类建筑文化遗产价值，支持节能改造。


<details>
  <summary>Details</summary>
Motivation: 量化建筑节能措施时需兼顾文化遗产保护，传统方法耗时耗力，AI工具可提高效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法采用感知-规划分离范式，限制了规划导向的优化潜力。

Method: 提出VeteranAD框架，结合多模式锚定轨迹作为规划先验，感知模块针对性收集轨迹相关交通元素，采用自回归策略逐步预测未来轨迹。
...
TL;DR: 提出了一种名为HGFE的框架，通过图推理增强CNN的结构感知和特征表示能力，实验验证其有效性。
...
Method: 构建层次化图结构（局部和全局图卷积），引入自适应频率调制模块平衡信号传播。

Result: 在多个数据集（分类、检测、分割任务）上验证了HGFE提升结构表示和识别性能。

Conclusion: HGFE轻量、可端到端训练，能无缝集成到标准CNN中，显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决历史手写文本识别中的稀缺转录、语言变异和多样化手写风格问题，以释放档案文件的文化和学术价值。

Method: 使用TrOCR模型，结合针对历史手写特点的图像预处理和四种新数据增强方法，并评估集成学习策略。

Result: 最佳单模型增强（Elastic）的字符错误率（CER）为1.86，而前5投票集成的CER为1.60，相对改进显著。

Conclusion: 领域特定的数据增强和集成策略对提升历史手稿的识别性能具有重要影响。
...
Result: 在多个数据集上验证，AIM显著提升了可解释性（EPG评分）和准确性。
...
TL;DR: 提出YOLOv11-KW-TA-FP模型，用于混凝土裂缝检测与分割，通过动态KernelWarehouse卷积、三重注意力机制和FP-IoU损失函数优化性能。


<details>
  <summary>Details</summary>
Motivation: 长江三角洲地区基础设施老化快，传统检测方法效率低，现有深度学习模型对小目标裂缝检测效果不佳。

Method: 基于YOLOv11n架构，集成动态KernelWarehouse卷积、三重注意力机制和FP-IoU损失函数。

Result: 模型性能显著提升，达到91.3%精度、76.6%召回率和86.4% mAP@50。

Conclusion: 该模型为自动化基础设施检测提供了高效解决方案，具有实际工程价值。
...
TL;DR: 提出了一种多状态跟踪器（MST），通过轻量级的状态特定增强（SSE）和跨状态交互（CSI）模块，显著提升了特征表示能力，同时保持低计算开销，在多个数据集上表现优于现有高效跟踪器。


<details>
  <summary>Details</summary>
Motivation: 高效跟踪器通常因降低计算复杂度和模型参数而牺牲特征表示能力，限制了其在复杂环境中的准确性。MST旨在通过多状态特征增强和交互解决这一问题。

Method: MST采用多状态生成（MSG）提取多阶段特征，通过SSE进行目标特定增强，并通过CSI模块实现状态间的信息交互和特征整合。SSE和CSI模块采用轻量级设计（HSA-SSD），计算开销极低。

Result: MST在多个数据集上表现优于所有先前的高效跟踪器，显著提升了跟踪准确性和鲁棒性，尤其在GOT-10K数据集上AO分数提高了4.5%。

Conclusion: MST通过轻量级的多状态特征增强和交互设计，实现了高效且鲁棒的跟踪性能，为高效跟踪器的发展提供了新思路。
...
Conclusion: 该方法为资源有限环境下的医学图像分类提供了高效可行的解决方案。
...
TL;DR: Veason-R1是一种专用于视频推理分割（VRS）的LVLM，通过GRPO和CoT初始化训练，显著提升性能和推理可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在推理可解释性和时空推理能力上表现不足，需要改进。

Method: 使用GRPO和CoT初始化训练Veason-R1，结合高质量CoT数据和整体奖励机制。

Result: 在多个基准测试中达到SOTA性能，显著优于现有方法（如ReVOS和ReasonVOS）。

Conclusion: Veason-R1通过结构化推理和优化训练方法，显著提升了VRS任务的性能和鲁棒性。
...
TL;DR: 提出了一种无需训练的异常生成框架AAG，基于Stable Diffusion生成高质量异常图像，并通过改进的注意力机制提升生成效果。
...
Result: 在MVTec AD和VisA数据集上验证了AAG的有效性，生成的异常图像能提升下游异常检测任务的性能。

Conclusion: AAG是一种无需训练的高效异常生成方法，能显著提升工业异常检测的数据支持。
...
Method: TrajSV框架包含数据预处理、Clip Representation Network (CRNet) 和 Video Representation Network (VRNet)，使用轨迹增强的Transformer模块和无监督的三重对比损失优化。

Result: 在足球、篮球和排球视频数据集上，TrajSV在视频检索中提升近70%，动作识别中9/17类别最优，视频字幕提升近20%。
...
TL;DR: 研究发现，视频语言模型中位置编码对时间理解影响有限，而帧顺序反转会导致性能显著下降。时间信息通过帧间注意力逐步合成，最终整合到查询令牌中。基于此，提出了两种效率优化策略。
...
Result: 实验验证了两种策略的有效性，揭示了时间理解的新机制。

Conclusion: 研究首次系统探讨视频时间理解，为未来模型改进提供了新思路。
...
TL;DR: CoreEditor提出了一种新的文本驱动3D编辑框架，通过引入对应约束注意力机制和语义相似性，解决了多视图一致性问题，并支持用户选择性编辑。
...
Method: CoreEditor采用对应约束注意力机制，结合几何对齐和语义相似性，优化多视图信息交换，并设计了选择性编辑流程。

Result: 实验表明，CoreEditor能生成高质量、3D一致的编辑结果，细节更清晰，显著优于现有方法。

Conclusion: CoreEditor通过创新的注意力机制和用户控制设计，有效提升了文本驱动3D编辑的性能和灵活性。
...
TL;DR: 本文提出了一种通过奖励引导解码的方法来适应多模态大型语言模型（MLLMs），以提升其视觉定位能力。
...
Method: 构建了两个独立的奖励模型，分别控制对象精度和召回率，并通过动态调整奖励函数和搜索广度来指导解码过程。

Result: 在标准对象幻觉基准测试中，该方法显著优于现有方法，并提供了对MLLM推理过程的灵活控制。

Conclusion: 该方法不仅提升了视觉定位能力，还实现了推理过程的动态可控性。
...
Method: 通过空间分块计算余弦相似度，构建空间感知权重矩阵，加权聚合LoRA输出，并结合无分类器引导。
...
<details>
  <summary>Details</summary>
Motivation: 评估通用大语言模型（如GPT-5和GPT-4o）在乳腺X光视觉问答任务中的潜力，以支持乳腺癌筛查。

Method: 在四个公开乳腺X光数据集上评估GPT-5和GPT-4o的性能，包括BI-RADS评估、异常检测和恶性分类任务。

Result: GPT-5在多个任务中表现最佳，但敏感性和特异性低于人类专家。

Conclusion: GPT-5在乳腺X光筛查任务中表现有潜力，但仍需领域优化才能用于高风险临床应用。
...
TL;DR: Thyme是一种新型多模态大语言模型范式，通过生成和执行代码实现图像处理和数学计算，显著提升感知和推理任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有开源模型在图像处理和逻辑推理能力上不如专有模型（如O3），因此提出Thyme以填补这一空白。

Method: 采用两阶段训练策略：先通过SFT训练代码生成能力，再通过RL优化决策，并提出GRPO-ATS算法平衡推理探索与代码执行精度。

Result: 在近20个基准测试中，Thyme表现显著优于现有方法，尤其在复杂推理和高分辨率感知任务中。

Conclusion: Thyme通过代码生成与执行，成功扩展了多模态模型的图像处理和推理能力，为未来研究提供了新方向。
...
### [95] [The Role of Radiographic Knee Alignment in Knee Replacement Outcomes and Opportunities for Artificial Intelligence-Driven Assessment](https://arxiv.org/abs/2508.10941)
*Zhisen Hu,David S. Johnson,Aleksei Tiulpin,Timothy F. Cootes,Claudia Lindner*

Main category: eess.IV

TL;DR: 本文综述了膝关节置换术（TKR）的预后评估与膝关节对齐生物标志物的关系，探讨了AI在膝关节X光片自动分析中的应用及未来方向。


<details>
  <summary>Details</summary>
Motivation: 膝关节骨关节炎（OA）的治疗主要依赖TKR，但其预后难以预测。膝关节对齐是影响TKR预后的关键因素之一，AI技术为自动分析膝关节对齐提供了新方法。

Method: 综述了TKR预后评估的现有评分协议、膝关节对齐生物标志物及其与预后的关联，并讨论了基于AI的膝关节对齐分析方法。

Result: 现有研究多集中于膝关节OA诊断和MRI图像分割，而非膝关节对齐生物标志物与TKR预后的关系。AI在膝关节对齐自动测量方面具有潜力。

Conclusion: 未来研究应进一步探索膝关节对齐生物标志物与TKR预后的关系，并优化AI技术在膝关节对齐评估和预后预测中的应用。
...
### [96] [Deep Learning-Based Automated Segmentation of Uterine Myomas](https://arxiv.org/abs/2508.11010)
*Tausifa Jan Saleem,Mohammad Yaqub*

Main category: eess.IV

TL;DR: 论文探讨了子宫肌瘤的自动化分割需求，利用公开数据集UMD建立基准，推动标准化评估。


<details>
  <summary>Details</summary>
Motivation: 子宫肌瘤是女性生殖系统常见良性肿瘤，诊断依赖MRI，但人工分割耗时且易受专家差异影响，亟需自动化方法。

Method: 研究利用公开数据集UMD，采用深度学习算法实现子宫肌瘤的自动化分割。

Result: 通过公开数据集建立基准，为未来研究提供标准化评估基础。

Conclusion: 自动化分割方法有望提升子宫肌瘤诊断效率，公开数据集促进研究可比性。
...
### [97] [HistoViT: Vision Transformer for Accurate and Scalable Histopathological Cancer Diagnosis](https://arxiv.org/abs/2508.11181)
*Faisal Ahmed*

Main category: eess.IV

TL;DR: 提出了一种基于Transformer的深度学习框架，用于病理图像的肿瘤多分类，显著提升了分类性能并减少了预处理需求。
...
### [98] [Efficient Image-to-Image Schrödinger Bridge for CT Field of View Extension](https://arxiv.org/abs/2508.11211)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统CT重建算法在视野受限时无法准确恢复解剖结构的问题，同时克服传统扩散模型计算量大、推理速度慢的缺点。

Method: 采用图像到图像的Schrödinger Bridge（I²SB）扩散模型，直接学习有限视野与扩展视野图像之间的随机映射，避免从纯高斯噪声合成图像。

Result: 在模拟和真实数据上分别达到49.8HU和152.0HU的RMSE，推理速度仅需0.19秒/切片，远超现有扩散模型。
...
### [99] [A Convergent Generalized Krylov Subspace Method for Compressed Sensing MRI Reconstruction with Gradient-Driven Denoisers](https://arxiv.org/abs/2508.11219)
*Tao Hong,Umberto Villa,Jeffrey A. Fessler*

Main category: eess.IV

TL;DR: 提出了一种广义Krylov子空间方法（GKSM），用于高效解决压缩感知MRI重建中的优化问题，并建立了严格的收敛保证。
...
Method: 提出广义Krylov子空间方法（GKSM），用于高效求解优化问题，并在非凸设置下建立收敛保证。

Result: 在螺旋和径向采集的压缩感知MRI重建实验中，验证了GKSM的计算效率和理论预测的准确性。
...
TL;DR: 提出了一种基于小波状态空间模型和频率掩码图的条带修复方法，有效抑制低码率压缩中的条带伪影。


<details>
  <summary>Details</summary>
Motivation: 低码率压缩常引入条带伪影（如天空区域），影响视觉质量，尤其在用户生成内容中因多次转码而常见。

Method: 使用小波状态空间模型和频率掩码图，保留高频细节，并对比开源条带修复方法。

Result: 在公开数据集上表现优于现有方法（DBI值0.082），同时保留图像纹理。

Conclusion: 该方法有效抑制条带伪影，视觉检查结果验证了其效果。
...
Conclusion: AnatoMaskGAN在重建精度和感知质量上优于现有方法，各模块均对性能有重要贡献。
...
TL;DR: 论文提出了一种纯卷积神经网络模型LKFMixer，通过大卷积核模拟自注意力的非局部特征捕捉能力，解决了Transformer计算量大的问题。


<details>
  <summary>Details</summary>
Motivation: 自注意力在Transformer中的成功表明非局部信息对图像超分辨率的重要性，但其高计算量难以实现轻量化模型。

Method: 使用31x31大卷积核扩大感受野，通过坐标分解减少参数和计算量；设计了空间特征调制块（SFMB）和特征选择块（FSB）增强特征聚焦。

Result: LKFMixer家族在超分辨率性能和重建质量上优于其他SOTA方法，例如在Manga109数据集上比SwinIR-light快5倍且PSNR提升0.6dB。

Conclusion: LKFMixer通过大卷积核和优化模块设计，高效实现了非局部特征捕捉，为轻量化超分辨率模型提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决CT扫描中皮层下分割标注数据不足的问题，利用现有MRI模型提升CT分割性能。

Method: 通过集成MRI模型的输出，构建一个鲁棒的集成管道，应用于未标注的MRI-CT配对数据。

Result: 在多个公共数据集上验证了框架的优越性，生成的CT数据集提升了相关分割任务的性能。

Conclusion: 开源了框架、数据集和模型，为CT皮层下分割研究提供了首个公开资源。
...
### [105] [LayoutRectifier: An Optimization-based Post-processing for Graphic Design Layout Generation](https://arxiv.org/abs/2508.11177)
*I-Chao Shen,Ariel Shamir,Takeo Igarashi*

Main category: cs.GR

TL;DR: 提出了一种基于优化的方法LayoutRectifier，用于修正自动生成的图形设计布局中的缺陷（如错位、重叠和不符合要求的包含），通过两阶段优化提升布局质量。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法生成的图形设计布局常存在缺陷（如错位、重叠等），影响设计质量，需要一种无需额外训练的方法来优化这些布局。

Method: 采用两阶段优化：1）利用网格系统通过离散搜索减少错位；2）设计新的盒子包含函数调整元素位置和大小，避免重叠并满足包含需求。

Result: 在内容无关和内容感知的布局生成任务中，LayoutRectifier生成了更高质量的布局，更适合下游图形设计任务。

Conclusion: LayoutRectifier有效弥补了基于学习的布局生成方法的不足，无需额外训练即可显著提升布局质量。
...
### [106] [StyleMM: Stylized 3D Morphable Face Model via Text-Driven Aligned Image Translation](https://arxiv.org/abs/2508.11203)
*Seungmi Lee,Kwan Yun,Junyong Noh*

Main category: cs.GR

TL;DR: StyleMM是一个基于文本描述构建风格化3D可变形模型（3DMM）的新框架，通过扩散模型和图像训练实现风格迁移，同时保留面部属性。
...
Conclusion: StyleMM为风格化3D人脸生成提供了一种高效且一致的方法，适用于动画和多样化应用。
...
### [107] [SPG: Style-Prompting Guidance for Style-Specific Content Creation](https://arxiv.org/abs/2508.11476)
...
### [108] [Failures to Surface Harmful Contents in Video Large Language Models](https://arxiv.org/abs/2508.10974)
*Yuxin Cao,Wei Song,Derui Wang,Jingling Xue,Jin Song Dong*

Main category: cs.MM

TL;DR: 当前视频大语言模型（VideoLLMs）在生成视频摘要时存在安全漏洞，容易忽略视频中的有害内容。研究发现三个设计缺陷：稀疏帧采样、空间信息丢失和编码器-解码器脱节。攻击实验显示，有害内容遗漏率超过90%。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在关键应用中被广泛使用，但其生成摘要时可能忽略有害内容，导致安全隐患。

Method: 通过分析设计缺陷（帧采样不足、空间信息丢失、编码器-解码器脱节），提出三种零查询黑盒攻击方法。

Result: 实验表明，主流VideoLLMs在90%以上的情况下忽略有害内容，即使内容明显可见。

Conclusion: 当前VideoLLMs设计存在根本性漏洞，需改进采样策略、令牌压缩和解码机制，以确保语义覆盖而非仅追求速度。
...
### [109] [Allen: Rethinking MAS Design through Step-Level Policy Autonomy](https://arxiv.org/abs/2508.11294)
*Qiangong Zhou,Zhiting Wang,Mingyou Yao,Zongyang Liu*

Main category: cs.MA

TL;DR: Allen是一个新型多智能体系统（MAS），旨在提升策略自主性并平衡协作效率、任务监督与人工监督。
...
Method: 重新定义MAS的基本执行单元，构建四层状态架构（任务、阶段、智能体、步骤），从任务导向和执行导向两方面约束系统行为。

Result: 实现了策略自主性的提升，并在协作结构的可控性上取得平衡。

Conclusion: Allen在策略自主性和协作可控性之间取得了有效平衡，代码已开源。
...
### [110] [Model Interpretability and Rationale Extraction by Input Mask Optimization](https://arxiv.org/abs/2508.11388)
...
### [111] [Full-Wave Modeling of Transcranial Ultrasound using Volume-Surface Integral Equations and CT-Derived Heterogeneous Skull Data](https://arxiv.org/abs/2508.11100)
*Alberto Almuna-Morales,Danilo Aballay,Pierre Gélat,Reza Haqshenas,Elwin van 't Wout*

Main category: physics.med-ph

TL;DR: 提出了一种基于体积-表面积分方程的高效数值方法，用于模拟超声波在异质性颅骨组织中的全波传播，解决了CT分辨率限制的问题。


<details>
  <summary>Details</summary>
Motivation: 颅骨的高衰减性和异质性导致超声波在传播过程中发生畸变和焦点偏移，传统CT分辨率限制了数值模型的准确性。

Method: 采用体积-表面积分方程方法，可直接利用CT体素数据，在相对粗糙的网格上实现高精度模拟。

Result: 验证表明该方法在高分辨率边界元模型上表现优异，CT颅骨模型模拟显示显著的波束畸变和毫米级焦点偏移。

Conclusion: 该方法为颅骨超声治疗提供了高效且精确的模拟工具，克服了传统CT分辨率的限制。
...
### [112] [LD-LAudio-V1: Video-to-Long-Form-Audio Generation Extension with Dual Lightweight Adapters](https://arxiv.org/abs/2508.11074)
*Haomin Zhang,Kristin Qi,Shuxin Yang,Zihao Chen,Chaofan Ding,Xinhan Di*

Main category: cs.SD

TL;DR: 论文提出LD-LAudio-V1模型，通过双轻量适配器实现长视频音频生成，并发布高质量数据集，显著减少拼接伪影和时间不一致问题。
...
Result: 模型在多项指标上显著提升，如FD、KL、IS等，减少能量差异和语义不一致。
...
### [113] [GenFlowRL: Shaping Rewards with Generative Object-Centric Flow in Visual Reinforcement Learning](https://arxiv.org/abs/2508.11049)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型依赖生成数据质量且缺乏环境反馈，难以处理细粒度操作，同时大规模机器人数据集收集困难。

Method: GenFlowRL利用生成的物体中心流提取低维特征，从多样化演示中学习策略。

Result: 在10个模拟和真实世界跨体现任务中，GenFlowRL表现优异。

Conclusion: GenFlowRL通过物体中心流特征有效提升策略的通用性和鲁棒性。
...
TL;DR: 论文提出了一种主动重规划框架，通过比较当前场景图与参考图，在子任务边界检测并修正潜在失败，提高机器人任务成功率。


<details>
  <summary>Details</summary>
Motivation: 人类能根据环境状态调整行为，而机器人常因缺乏适应性导致失败。现有方法多为被动响应，主动重规划依赖人工规则。本文旨在解决这一问题。

Method: 构建当前RGB-D观测的场景图，与成功演示的参考图对比。若发现不匹配，轻量推理模块诊断并调整计划。

Result: 在AI2-THOR模拟器中，该方法能提前检测语义和空间不匹配，显著提升任务成功率和鲁棒性。

Conclusion: 提出的主动重规划框架有效预防执行失败，为机器人自适应行为提供了新思路。
...
### [115] [Relative Position Matters: Trajectory Prediction and Planning with Polar Representation](https://arxiv.org/abs/2508.11492)
*Bozhou Zhang,Nan Song,Bingzhao Gao,Li Zhang*

Main category: cs.RO

TL;DR: 论文提出了一种基于极坐标的轨迹预测与规划方法Polaris，相比传统笛卡尔坐标系方法，能更直观地建模空间变化和相对关系，显著提升了性能。
...
Method: 采用极坐标系表示位置，提出Polaris方法，通过专用编码和优化模块显式建模距离和方向变化。

Result: 在Argoverse 2和nuPlan等基准测试中，Polaris实现了最先进的性能。

Conclusion: 极坐标表示能更有效地支持轨迹预测与规划，Polaris方法优于传统笛卡尔方法。
...
### [116] [Visual Perception Engine: Fast and Flexible Multi-Head Inference for Robotic Vision Tasks](https://arxiv.org/abs/2508.11584)
*Jakub Łucki,Jonathan Becktor,Georgios Georgakis,Robert Royce,Shehryar Khattak*

Main category: cs.RO

TL;DR: VPEngine是一个模块化框架，通过共享基础模型和多任务并行处理，优化GPU使用，提升视觉多任务处理效率。


<details>
  <summary>Details</summary>
Motivation: 解决资源受限机器人平台上多模型部署导致的冗余计算、内存占用大和集成复杂的问题。

Method: 采用共享基础模型提取图像表征，并行运行多个任务专用模型头，避免GPU-CPU内存传输，支持动态任务优先级调整。

Result: 使用DINOv2作为基础模型，实现深度、目标检测和语义分割多任务，速度提升3倍，内存占用恒定。

Conclusion: VPEngine在NVIDIA Jetson Orin AGX上实现实时性能（≥50 Hz），开源且支持ROS2，适用于多样化机器人平台。
...
### [117] [Fluid Dynamics and Domain Reconstruction from Noisy Flow Images Using Physics-Informed Neural Networks and Quasi-Conformal Mapping](https://arxiv.org/abs/2508.11216)
*Han Zhang,Xue-Cheng Tai,Jean-Michel Morel,Raymond H. Chan*

Main category: math.NA

TL;DR: 该论文提出了一种基于物理信息神经网络和几何优化的方法，用于去噪血流图像，通过交替求解流体和几何子问题，实现了高质量的图像重建。


<details>
  <summary>Details</summary>
Motivation: 血流成像在医学诊断和治疗规划中至关重要，但获取高质量图像仍具挑战性，尤其是存在噪声或伪影时。

Method: 将问题建模为优化问题，分解为流体子问题（使用物理信息神经网络重建速度场）和几何子问题（通过拟共形映射优化流区域），交替求解。

Result: 在合成和真实主动脉数据上的实验验证了方法的有效性和鲁棒性。

Conclusion: 该方法能够高质量重建血流图像，并通过消融研究评估了关键超参数的影响。
...
### [118] [Match & Choose: Model Selection Framework for Fine-tuning Text-to-Image Diffusion Models](https://arxiv.org/abs/2508.10993)
*Basile Lewandowski,Robert Birke,Lydia Y. Chen*

Main category: cs.LG

TL;DR: 本文提出了首个模型选择框架M&C，帮助用户从模型平台中高效选择预训练的文本到图像（T2I）模型，而无需在目标数据集上对所有模型进行微调。


<details>
  <summary>Details</summary>
Motivation: 预训练的T2I模型在公开平台上广泛共享，但用户面临如何根据目标数据域选择最佳模型的挑战。目前模型选择在分类任务中已有研究，但在T2I模型中尚不明确。

Method: M&C框架通过构建匹配图（包含模型和数据集节点，以及模型-数据和数据-数据边）和基于图嵌入特征的预测模型，预测微调后性能最佳的模型。

Result: 在10个T2I模型和32个数据集上的评估中，M&C成功预测了61.3%的最佳模型，其余情况下也选择了性能接近的模型。
...
### [119] [Boosting the Robustness-Accuracy Trade-off of SNNs by Robust Temporal Self-Ensemble](https://arxiv.org/abs/2508.11279)
*Jihang Wang,Dongcheng Zhao,Ruolin Chen,Qian Zhang,Yi Zeng*

Main category: cs.LG

TL;DR: 本文提出了一种名为RTE的训练框架，通过时间集成增强SNNs的对抗鲁棒性，解决了时间子网络的脆弱性和对抗漏洞的时间传递性问题。


<details>
  <summary>Details</summary>
Motivation: SNNs在能效和类脑计算方面具有潜力，但其对抗鲁棒性尚未被充分理解。

Method: 提出RTE框架，通过统一损失函数和随机采样策略优化时间子网络的鲁棒性，并减少对抗扰动的时间传递性。

Result: 实验表明RTE在多个基准测试中优于现有方法，改善了SNNs的内部鲁棒性景观。
...
### [120] [Robust Convolution Neural ODEs via Contractivity-promoting regularization](https://arxiv.org/abs/2508.11432)
*Muhammad Zakwan,Liang Xu,Giancarlo Ferrari-Trecate*

Main category: cs.LG

TL;DR: 论文提出了一种通过收缩理论提升卷积神经常微分方程（NODEs）鲁棒性的方法，并通过正则化项在训练中实现收缩性。
...
Method: 利用收缩理论设计正则化项，包括系统动态的雅可比矩阵和权重正则化，以减少计算负担。

Result: 在MNIST和FashionMNIST数据集上验证了方法对噪声和攻击的鲁棒性。

Conclusion: 提出的正则化方法有效提升了NODEs的鲁棒性，适用于对抗噪声和攻击的场景。
...
### [121] [Temporally-Similar Structure-Aware Spatiotemporal Fusion of Satellite Images](https://arxiv.org/abs/2508.11259)
*Ryosuke Isono,Shunsuke Ono*

Main category: eess.SP

TL;DR: 本文提出了一种名为TSSTF的新型时空融合框架，用于卫星图像，通过TGTV和TGEC机制解决噪声问题并保留空间结构细节。


<details>
  <summary>Details</summary>
Motivation: 卫星图像在现实场景中常因噪声导致质量下降，现有方法在噪声鲁棒性上表现不足且易丢失空间细节。

Method: TSSTF引入TGTV和TGEC机制，通过约束优化问题建模，并采用预条件原始对偶分裂算法求解。

Result: 实验表明，TSSTF在无噪声条件下与先进方法相当，在噪声条件下表现更优，并提供了推荐参数。

Conclusion: TSSTF在噪声鲁棒性和空间细节保留上优于现有方法，具有实用性和可重复性。
