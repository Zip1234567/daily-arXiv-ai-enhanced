...
Motivation: 现有基于坐标的方法在渲染显式表面时计算量大，因此需要一种更高效且泛化能力强的3D表面重建方法。

Method: 方法包括三个模块：射线特征提取器、射线距离场预测器和多射线混合器，用于提取几何特征、预测距离并聚合预测结果。

Result: 在多个公开数据集上表现优异，能够通过单次前向传播在未见数据集上重建3D表面。

Conclusion: RayletDF在3D表面重建中具有高效性和泛化能力，适用于点云或3D高斯的输入。
...
Method: 框架包含两个组件：Latent Panel Anchoring（保持角色一致性）和Reciprocal Attention Value Mixing（软混合视觉特征）。结合预训练扩散模型和语言模型生成故事板。

Result: 在Rich Storyboard Benchmark上，Story2Board在布局多样性、背景叙事和一致性方面表现优异，并通过用户研究验证了其动态性和叙事吸引力。

Conclusion: Story2Board能够生成更动态、连贯且叙事性强的故事板，优于现有基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体上针对女性的攻击性内容较多，现有方法难以有效检测，需要针对性解决方案。

Method: 框架包括多模态注意力模块（MANM）、基于图的特征重构模块（GFRM）和内容特定特征学习模块（CFLM），结合自适应门控和测试时增强。

Result: 在MAMI和MMHS150K数据集上，平均宏F1分别提高了10.17%和8.88%。

Conclusion: 提出的多模态框架能有效检测针对女性的攻击性内容，性能显著优于现有方法。
...
Method: 采用两阶段训练策略：PA-SFT阶段使用高质量Chain-of-Thought数据集增强感知能力；SC-GRPO阶段通过奖励函数实现从感知到解释的跃升。

Result: 在7种VLMs上显著提升，平均准确率最高提升43.3%，0.5B参数模型超越GPT-4.1等商业模型。
...
<details>
  <summary>Details</summary>
Motivation: 由于AR的普及，认知攻击通过操纵用户语义感知受到关注，现有方法在语义推理或可解释性上存在不足。
...
<details>
  <summary>Details</summary>
Motivation: 智能交通系统中AI摄像头的普及引发了隐私与数据需求之间的冲突，现有方法（如模糊或加密）无法兼顾隐私保护和数据效用。

Method: 结合Mixture-of-Experts（MoE）架构和强化学习（RL）代理，将视觉数据转化为隐私保护文本描述。

Result: RL-MoE在CFP-FP数据集上将重放攻击成功率降至9.4%，同时生成比基线方法更丰富的文本内容。
...
<details>
  <summary>Details</summary>
Motivation: 解决情感计算中高质量、多样化深度人脸数据集的缺乏问题，以识别细微情感表达。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有深度FER模型在细微表情和高主体间变异性上的性能不足，同时避免数据隐私和传输限制。

Method: 通过预训练翻译器在源域数据上转换主体特定风格特征，并在目标域中性数据上适应，避免图像合成。

Result: PFT在潜在空间中操作，避免了复杂的面部表情生成，减少了计算开销，并优化了分类嵌入。

Conclusion: PFT是一种高效且轻量化的SFDA方法，适用于仅含中性表情的目标数据场景。
...
Method: 提出MME-Emotion基准测试，包含6,000多个视频片段和任务特定的问答对，涵盖八种情感任务，并通过多代理系统框架进行综合评估。

Result: 评估20个先进MLLMs显示，最佳模型的情感知觉得分仅为39.3%，推理得分56.0%，表明当前模型情感智能不足。
...
<details>
  <summary>Details</summary>
Motivation: 当前对抗性提示的评估标准可能高估了攻击效果，且安全机制未能有效阻止有害输出。

Method: 引入四轴评估框架（输入主题性、输入OOD强度、输出危害性、输出拒绝率）和递归重写策略BSD。

Result: BSD在13个MLLMs中显著提高了攻击成功率（67%）和输出危害性（21%）。

Conclusion: 揭示了当前多模态安全系统的潜在弱点，并提供了更有效的攻击方法。
...
<details>
  <summary>Details</summary>
Motivation: HMER领域因数据稀缺（手动标注成本高）而发展受限，需要一种高效的数据生成方法。

Method: 通过开发可扩展的数据引擎，结合手写公式和LaTeX渲染公式，构建了Tex80M数据集，并训练了TexTeller模型。

Result: TexTeller在几乎所有基准测试中均达到SOTA性能。

Conclusion: 研究者将公开模型、数据集和代码，以推动HMER领域的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI代理评估框架过于关注粗粒度任务完成，忽视了细粒度控制能力的重要性，因此需要一种新的评估标准。

Method: 提出FineState-Bench框架，包含多平台任务基准和四阶段指标，并开发VDA工具进行视觉定位能力的定量分析。

Result: 最先进模型的细粒度交互准确率为32.8%，理想视觉定位能力可将Gemini-2.5-Flash的成功率提升14.9%。

Conclusion: 当前GUI代理的主要瓶颈是视觉定位能力，FineState-Bench为细粒度控制评估提供了开源工具。
...
<details>
  <summary>Details</summary>
Motivation: 针对视觉助手系统中盲人和低视力用户可能无意中捕获个人隐私信息的问题，现有方法因粗粒度屏蔽导致可用性降低。
...
Conclusion: 该方法有效提升了VLN模型的效率，适用于计算资源有限的实际场景。
...
Method: SegDAC结合SAM进行对象中心分解，YOLO-World通过文本提示语义标注，采用新型transformer架构动态选择关注片段。
...
Method: 提出Lung-DDPM+，一种基于结节语义布局的去噪扩散概率模型（DDPM），并通过肺部DPM-solver加速，优化采样效率与质量。
...
<details>
  <summary>Details</summary>
Motivation: 结肠镜筛查中准确识别癌前息肉对降低结直肠癌风险至关重要，但现有方法在分类和分层上仍有改进空间。

Method: 采用Ultralight Med-Vision Mamba模型，一种基于状态空间的模型（SSM），擅长处理长短程依赖和图像泛化，适用于全切片图像分析。

Result: 该模型在计算速度和可扩展性上表现优异，适合实时临床应用。

Conclusion: Ultralight Med-Vision Mamba为结肠镜筛查提供了一种高效、精准的工具，有望改善患者预后。
...
<details>
  <summary>Details</summary>
Motivation: 为严重运动障碍者提供一种低成本、易用的辅助沟通方式。
...
Result: 在意大利手语数据集上表现优异，准确率达99.44%。
...
<details>
  <summary>Details</summary>
Motivation: 连续手语识别（CSLR）面临说话者间差异大和对新句子结构泛化能力差的问题，传统方法难以高效解决。

Method: 提出Signer-Invariant Conformer处理SI问题，结合卷积和多头自注意力；设计Multi-Scale Fusion Transformer处理US问题，采用双路径时间编码器捕捉细粒度动态。

Result: 在Isharah-1000数据集上，SI任务的WER降至13.07%（提升13.53%），US任务的WER为47.78%，优于之前工作。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割中注释者的变异性（如边界模糊、注释者偏好等）影响结果准确性，尤其是恶性病变更易引发分歧。

Method: 构建IMA++数据集，研究注释者、恶性程度等因素对IAA的影响，并预测IAA用于多任务学习。

Result: 恶性程度与IAA显著相关（p<0.001），预测IAA的误差为0.108，多任务学习使平衡准确率提升4.2%。

Conclusion: IAA可作为临床特征提升分割模型性能，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖显式骨骼姿势和启发式跨身份调整，难以实现高保真、跨多样身份的运动转移。

Method: 采用自监督端到端框架，联合学习运动编码器和潜在表示，并结合DiT视频生成模型，通过2D/3D增强和辅助解码器优化运动标记。

Result: 实验表明，X-UniMotion在运动保真度和身份保持方面优于现有方法。

Conclusion: X-UniMotion提供了一种高效、表达力强的跨身份运动转移解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决SAR目标检测中相干噪声的普遍影响，传统方法多基于空间域特征分析或增强，而DenoDet V2探索了变换域的新视角。

Method: 设计了一种注意力架构，通过振幅与相位信息的互补性，利用波段互调制机制实现相位与振幅谱的相互增强。
...
<details>
  <summary>Details</summary>
Motivation: 无人机在复杂环境中难以通过传统传感器（如RGB相机、LiDAR）检测亚毫米级障碍物（如钢丝、风筝线），亟需高效解决方案。

Method: 利用事件流中障碍物的独特特征，采用轻量级U-Net架构和Dice-Contour正则化损失进行检测。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种低成本、隐私保护的自主鸟类监测系统，适用于城市花园，支持公民科学级别的生物多样性记录。

Method: 使用运动触发的IP摄像头上传视频片段，通过Detectron2定位鸟类，EfficientNet-B3模型分类，所有处理在本地完成。

Result: 分类器在验证集上达到99.5%的准确率，实际应用中达到88%的top-1准确率。

Conclusion: 该系统证明了在家中进行公民科学级生物多样性记录的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集依赖单目RGB视频帧估计3D姿态，存在遮挡和时间不连续性问题，导致运动数据质量低。

Method: 利用3D人体形状和运动先验，从LiDAR点云中提取高质量、时间连续的3D骨骼运动数据。
...
Method: 采用两阶段方法：第一阶段通过政府数据和自动标注生成21万张标注的街景图像；第二阶段基于生成的数据集训练改进的ConvNeXt V2模型。

Result: 生成的数据集达到94.0%精确率和92.5%召回率，检测模型AP值为0.9236，显著优于先前工作。

Conclusion: 该研究首次提供了大规模、高质量的路缘坡道检测数据集和模型，为城市无障碍环境提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖确定性模型，忽略了室内布局的不确定性，限制了泛化能力。GOAL旨在通过生成流框架和LLM增强的语义地图解决这一问题。

Method: GOAL利用LLM推断的空间先验，将其编码为二维高斯场并注入目标地图，通过生成流模型实现更通用的语义补全。

Result: 在MP3D和Gibson数据集上达到最优性能，并在HM3D上表现出强泛化能力。

Conclusion: GOAL通过生成流和LLM增强的语义地图，显著提升了物体导航任务的性能与泛化能力。
...
Method: 提出PaIR-Net框架，包含CPAM、PGCS和IIM三个模块，分别用于识别接触相关身体部位、像素级接触分割和全局关系整合。
...
<details>
  <summary>Details</summary>
Motivation: 微表情识别（MER）在情感计算领域具有广泛应用，但由于标注困难导致训练样本稀缺，现有大型预训练模型（LMs）难以捕捉微表情的短暂和细微运动特征。

Method: 提出Motion Prompt Tuning (MPT)方法，包括运动放大和高斯标记化的运动提示生成，以及插入组适配器以增强LMs在MER领域的表现。

Result: 在三个广泛使用的MER数据集上的实验表明，MPT方法显著优于现有最先进方法。

Conclusion: MPT方法通过运动提示生成和组适配器设计，成功提升了LMs在MER任务中的性能，为微表情识别提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于参考的超分辨率（RefSR）方法依赖手动配对的图像，限制了实际应用。本文旨在通过自动检索相关参考图像，提升RefSR的实用性和灵活性。

Method: 提出RASR框架，结合语义参考检索器和基于扩散的RefSR生成器。检索器根据语义相似性选择参考图像，生成器利用扩散模型增强语义条件生成。

Result: 在RASR-Flickr30数据集上，RASRNet相比SISR基线提升了0.38 dB PSNR和-0.0131 LPIPS，生成更真实的纹理。

Conclusion: 检索增强是连接学术研究与实际应用的有效方向，RASR展示了RefSR在开放世界中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在大规模无标签数据上预训练后，可直接用于多种下游任务，但高光谱遥感由于光谱差异和数据稀缺性，直接应用面临挑战。

Method: HyperKD采用基于特征的知识蒸馏策略，包括光谱范围通道对齐、空间特征引导掩码和针对高光谱图像的增强损失函数，从Prithvi基础模型向EnMAP高光谱图像学生模型转移知识。

Result: 实验表明，HyperKD显著提升了MAEs中的表示学习，改善了重建保真度，并在土地覆盖分类、作物类型识别和土壤有机碳预测等下游任务中表现更稳健。
...
Method: 提出 Animate-X++ 框架，引入 Pose Indicator 从驱动视频中捕获运动模式，并通过多任务训练策略联合训练动画和 TI2V 任务。

Result: 实验表明 Animate-X++ 在通用性和广泛适用性方面表现优越，能够生成更真实的视频。

Conclusion: Animate-X++ 解决了现有方法的局限性，为角色动画提供了更通用和高效的解决方案。
...
Result: 在InternVL-2.5-8B上ASR@0.5超过65%，且在Ferret-7B和LlaVA-1.5-7B上表现出色，对干净样本影响小。
...
Method: 采用灵活的局部单元和GLoRA机制，结合轻量级适配模块与现有Transformer架构（如ViT和SegFormer）集成，并设计了一个轻量级的基于查询的掩码解码器。

Result: 在多个基准测试中实现了最先进的定位性能，为可扩展和模态无关的VML设定了新基准。

Conclusion: RelayFormer通过其模块化设计和高效处理能力，为视觉篡改定位任务提供了强大的解决方案。
...
Method: 通过多模态扩散变换器提取身份-表情表示，并在推理时使用一致注意力机制共享信息。
...
<details>
  <summary>Details</summary>
Motivation: 能量效率在鲁棒拟合中未得到足够关注，而高能耗是AI应用中的关键问题。

Method: 设计了新型脉冲神经网络，结合事件驱动的模型估计方法和算法策略，以适应Loihi 2硬件的限制。

Result: 神经形态鲁棒拟合的能耗仅为传统CPU方法的15%，且精度相当。

Conclusion: 神经形态计算为高效能量鲁棒拟合提供了可行方案，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中，深度伪造检测模型对未知样本表现不佳，但这些样本仍可用于分析，因此应将其视为少样本任务。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在大角度面部下难以保持身份特征，主要因缺乏有效的身份特征整合机制和针对大角度面部的数据集。

Method: 引入MoFE动态结合三种专家（身份、语义、细节），并开发了包含Face Constraints和Identity Consistency的数据处理流程，构建了LFA数据集。

Result: 实验表明，基于LFA数据集的方法在面部相似性、FID和CLIP语义对齐上显著优于现有方法。

Conclusion: MoFE和LFA数据集有效提升了大角度面部视频生成的身份保留能力。
...
<details>
  <summary>Details</summary>
Motivation: AI生成图像质量接近自然图像，引发安全隐患，现有检测器对未见过的生成模型性能有限。

Method: 使用预训练CLIP编码器提取特征，设计类似归一化流的无监督模型，利用代理图像（如频谱修改的自然图像）训练。

Result: 实验证明该方法对多种图像生成器生成的AI图像有效。

Conclusion: 该方法通过异常检测和无监督学习，提高了对未见生成模型的检测能力。
...
<details>
  <summary>Details</summary>
Motivation: 放射科医生的视觉注意力模式包含细粒度和粗粒度疾病信息，这些信息对深度学习框架的自动化图像解释至关重要。同时，专家不仅关注主要疾病模式，还会注意到少数长尾类别，这些信息对分类有帮助。

Method: GazeLT通过整合和解构视觉搜索过程的时间特性，利用放射科医生的眼动数据，改进长尾疾病分类。

Result: 在NIH-CXR-LT和MIMIC-CXR-LT数据集上，GazeLT比最佳长尾损失方法提高了4.1%，比基于视觉注意力的基线方法提高了21.7%的平均准确率。

Conclusion: GazeLT通过结合人类视觉注意力，显著提升了长尾疾病分类的性能，证明了其在自动化图像解释中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在卫星图像上表现不佳，因与RPC模型不兼容且泛化能力有限。

Method: SkySplat结合RPC模型，利用RGB图像和相对高度监督，通过CSCM模块和多视图一致性策略优化重建。

Result: SkySplat比EOGS快86倍且更准确，在DFC19数据集上MAE从13.18米降至1.80米，并在MVS3D基准上表现优异。

Conclusion: SkySplat显著提升了稀疏卫星图像的三维重建效果，具有高效性和强泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将长视频简化为静态关键帧匹配，忽略了时空关系和场景连续性，导致信息冗余和准确率下降。

Method: Video-EM将关键帧建模为时序事件，结合链式思维（CoT）迭代筛选最小但信息丰富的子集。

Result: 在多个基准测试中，Video-EM性能提升4-9%，且使用更少帧数。

Conclusion: Video-EM通过时空建模和高效筛选，显著提升了长视频理解的准确性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型的快速发展引发了对潜在滥用的担忧，现有检测方法在面对OOD生成模型时性能显著下降，因为它们依赖模型特定的伪影。

Method: 通过测量图像与其基于标题的重建之间的语义差异，提出SARE表示，利用假图像与标题高度一致而真实图像语义变化较大的特性。

Result: 实验表明，SARE在GenImage和CommunityForensics等基准测试中表现优于现有基线，具有强泛化能力。

Conclusion: SARE是一种有效的跨模型假图像检测方法，能够通过量化语义差异提升检测鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法依赖图表示和语言模型编码器，忽略了关键的几何信息，导致结合位点定位不准确和构象不真实。
...
<details>
  <summary>Details</summary>
Motivation: 手语生成领域仍需探索，现有模型在平衡分辨率和细节方面存在不足。

Method: 结合ProGAN和SAGAN的GAN变体，生成特征丰富、高分辨率的类别条件手语图像。

Result: 改进的注意力模型在IS和FID上分别提升了3.2和30.12，并发布了印度手语数据集。

Conclusion: 提出的模型在手语生成中表现出色，为未来研究提供了高质量数据集。
...
TL;DR: 本文首次系统研究和量化了单目标跟踪（SOT）中被长期忽视的相似物体干扰（SOI），通过实验验证其是性能瓶颈，并提出基于自然语言的解决方案。
...
Result: 消除干扰源显著提升性能（AUC增益达4.35），但现有视觉语言跟踪（VLT）方法效果有限（AUC变化-0.26至+0.71），而VLM方法表现优异（AUC增益达0.93）。

Conclusion: SOIBench为标准化评估平台，VLM方法显著优于现有VLT方法，为跟踪研究提供新方向。
...
TL;DR: 论文提出了一种名为SDT的新型视觉Transformer，通过动态数据依赖的空间衰减机制改进传统ViT在空间结构任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 传统ViT的自注意力机制缺乏显式的空间归纳偏置，导致在空间结构任务中表现不佳。现有方法使用固定距离度量的空间衰减，无法适应多样化的视觉场景。

Method: 提出SDT，引入Context-Aware Gating (CAG)机制，动态生成数据依赖的空间衰减，结合内容相关性和空间邻近性调节注意力。
...
TL;DR: 本文提出了一种新的非对称Kronecker压缩感知（AKCS）模型和测量感知交叉注意力（MACA）机制，通过提高测量非相干性和学习隐式测量表示，显著提升了图像压缩感知任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在感知阶段的非相干压缩测量和重建阶段的隐式测量表示方面存在不足，限制了整体性能。本文旨在解决这两个问题。

Method: 提出了AKCS模型以提高测量非相干性，并引入MACA机制学习隐式测量表示。两者结合形成测量增强的展开网络（MEUNet）。

Result: MEUNet在重建精度和推理速度上均达到了最先进的性能。

Conclusion: AKCS和MACA的结合显著提升了压缩感知任务的性能，为未来研究提供了新思路。
...
Method: COXNet通过跨层融合模块、动态对齐与尺度细化模块以及基于GeoShape相似度的标签分配策略，优化了多模态特征的利用和目标定位。
...
<details>
  <summary>Details</summary>
Motivation: 随着非对称多相机系统（如长焦-广角相机）的普及，传统立体匹配算法基于对称视觉特性的假设不再适用，视觉不对称会干扰成本体积计算，导致匹配困难。

Method: 研究分析了两种成本体积构建方法在非对称立体中的匹配成本分布，发现各自存在信息失真。基于此，提出了双阶段迭代体积融合网络（IVF-AStereo），先通过聚合拼接体积优化相关体积，再将两者融合以增强细节。

Result: 实验表明，IVF-AStereo在非对称场景中表现优异，对显著的视觉不对称具有鲁棒性，且在分辨率和颜色退化的情况下仍有效。

Conclusion: IVF-AStereo通过综合两种成本体积构建方法，有效解决了非对称立体匹配问题，为相关应用提供了新思路。
...
TL;DR: GoViG任务通过原始视觉数据生成导航指令，分解为视觉预测和指令生成，结合多模态推理策略，在R2R-Goal数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖结构化输入（如语义标注或地图），GoViG仅用原始视觉数据，提升对未知和非结构化环境的适应性。

Method: 任务分解为视觉预测和指令生成，结合自回归多模态大语言模型及两种推理策略（单次和交替推理）。

Result: 在R2R-Goal数据集上，BLEU-4和CIDEr分数显著优于现有方法，并具备跨域泛化能力。

Conclusion: GoViG通过纯视觉输入生成高质量导航指令，为未知环境提供了适应性强的解决方案。
...
TL;DR: 论文探讨了在图像分类任务中，使用生成模型生成的封闭集合成数据增强分类性能的效果，并量化了合成数据与真实数据增强的等价性。


<details>
  <summary>Details</summary>
Motivation: 解决机器学习中的一个关键问题：如何利用生成模型在封闭集上生成的合成数据增强图像分类性能。

Method: 通过实验探索真实图像与封闭集合成图像的异同，并量化合成数据增强的等效规模。

Result: 实证确定了合成数据增强的等效规模，并展示了合成数据与真实数据增强的定量等价性。

Conclusion: 研究为合成数据增强提供了量化指导，并展示了其效果随训练集规模和合成数据量的变化。
...
Method: 将归一化虹膜图像划分为网格，计算每个子区域的Betti0、Betti1及其比率，形成特征矩阵，结合逻辑回归、KNN和SVM进行分类，并与CNN进行比较。

Result: 逻辑回归的准确率为97.78 +/- 0.82%，优于CNN（96.44 +/- 1.32%）和其他基于特征的模型，拓扑特征表现出高精度和低方差。

Conclusion: 这是首次将数字同调理论的拓扑不变量用于虹膜识别，提供了一种高效、可解释且适用于多种领域的方法。
...
TL;DR: WeatherPrompt通过多模态学习框架解决无人机视觉地理定位在天气扰动下的性能下降问题，提出训练免费天气推理机制和动态门控机制，显著提升召回率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在天气扰动下性能下降严重，且受限于有限的天气类别和特征解耦不足。

Method: 结合图像嵌入与文本上下文，通过训练免费天气推理机制和多模态动态门控机制优化特征解耦。

Result: 在多样天气条件下，Recall@1显著提升（夜间+13.37%，雾雪+18.69%）。

Conclusion: WeatherPrompt有效提升无人机地理定位在复杂天气下的鲁棒性和性能。
...
Method: 结合退化描述符的曝光一致性对齐模块（ECAM）和小波变换的光照-细节解耦特性设计曝光恢复与细节重建模块（EDRM）。

Result: 在多个公开数据集上的实验表明，该方法显著优于现有算法，验证了其有效性和实用性。

Conclusion: WEC-DG方法通过改进曝光一致性和细节恢复，提升了复杂成像条件下的图像校正效果。
...
TL;DR: 论文提出了一种名为诊断链（CoD）的框架，旨在解决放射学报告生成（RRG）中临床效果不佳和缺乏可解释性的问题。通过生成问答对和诊断基础模块，CoD提高了报告的准确性和可解释性，并通过实验验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有放射学报告生成模型在临床效果和可解释性方面表现不佳，尤其是对病变属性的描述缺乏准确性，且结果难以被放射科医生信任。

Method: 提出诊断链（CoD）框架，包括生成问答对提取关键发现、利用大型语言模型生成准确报告，并通过诊断基础模块和病变基础模块增强可解释性。采用全监督学习策略利用多种标注数据。

Result: CoD在两个RRG基准测试中表现优于专家和通用模型，能够准确地将生成的句子与问答诊断和图像关联，展示了良好的可解释性。

Conclusion: CoD框架通过结合诊断链和全监督学习，显著提升了放射学报告生成的临床准确性和可解释性，为放射科医生提供了更高效的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有可控T2I模型（如Ctrl-X和FreeControl）在保留空间结构和捕捉细粒度条件（如物体姿态和场景布局）方面表现不足。

Method: 通过外观反馈和生成反馈的双递归机制，优化中间潜在表示，以更好地反映外观信息和用户意图。

Result: 实验表明，DRF能够生成高质量、语义一致且结构一致的图像，支持类不变的结构-外观融合（如将人类动作转移到老虎形态）。

Conclusion: DRF系统有效解决了现有模型的局限性，无需额外训练即可实现精细控制。
...
TL;DR: SHALE是一个自动构建的评估基准，用于细粒度分析大型视觉语言模型（LVLM）的幻觉问题，包括忠实性和事实性幻觉，并模拟噪声场景。


<details>
  <summary>Details</summary>
Motivation: 现有研究对LVLM的幻觉问题评估不够细粒度，且依赖手动标注或公共数据集，存在扩展性和数据泄露问题。

Method: 提出自动化数据构建流程和分层幻觉诱导框架，生成多样化评估数据，并设计SHALE基准。

Result: SHALE包含30K+图像-指令对，覆盖12个视觉感知方面和6个知识领域，实验显示LVLM存在显著事实性幻觉和对语义扰动的高敏感性。

Conclusion: SHALE为LVLM幻觉问题提供了可扩展、可控的评估工具，揭示了模型在噪声环境中的脆弱性。
...
TL;DR: BAAS是一个基于贝叶斯跟踪和融合的雷达检测标注框架，用于自动驾驶中的扩展目标跟踪，提供精确的轨迹和形状估计，支持多级监督标注，并支持闭环持续改进。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶中雷达检测的扩展目标跟踪和标注问题，提供高精度轨迹和形状估计，同时支持多级监督和性能评估。

Method: 采用贝叶斯跟踪、平滑和融合方法，结合多模块独立或联合分析，实现闭环改进。

Result: 在复杂城市场景中验证了框架的跟踪性能和标注误差，适用于不同动态目标和类别。

Conclusion: BAAS框架在自动驾驶中有效实现了高精度雷达检测标注和跟踪，支持闭环改进。
...
TL;DR: Hi-SMGNN提出了一种基于结构性和形态性连接组的层次化框架，用于预测胶质瘤IDH突变状态，表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前预测方法受限于功能MRI的低可用性和噪声，而现有基于连接组的方法忽略了大脑的层次结构和多尺度交互。

Method: Hi-SMGNN整合了从区域到模块层次的结构性和形态性连接组，采用多模态交互模块、多尺度特征融合机制和个性化模块划分策略。

Result: 在UCSF-PDGM数据集上，Hi-SMGNN在IDH突变预测中表现出更高的鲁棒性和有效性。

Conclusion: Hi-SMGNN为IDH突变预测提供了一种更优的非侵入性方法，并增强了可解释性。
...
TL;DR: SVG-Head提出了一种混合表示方法，结合表面和体积高斯模型，实现高保真、可编辑的头像生成，支持实时外观编辑。


<details>
  <summary>Details</summary>
Motivation: 解决头像编辑中几何与全局外观纠缠建模的挑战，提升AR/VR应用中的实时编辑能力。

Method: 使用表面高斯和体积高斯分别建模外观和非朗伯区域，结合FLAME网格和纹理图像优化。

Result: 在NeRSemble数据集上实现高保真渲染，并首次支持高斯头像的显式纹理编辑。

Conclusion: SVG-Head在重建质量和编辑灵活性上均表现出色，为头像编辑提供了新思路。
...
TL;DR: FaME是一种无需训练、高效推理的方法，通过图像质量评估模型识别低质量生成样本，并利用其采样轨迹作为负引导，提升生成图像的感知质量。


<details>
  <summary>Details</summary>
Motivation: 尽管现有扩散模型在类到图像生成中表现出色，但FID评分忽略了单样本的感知质量，导致某些类别生成图像质量不佳。CFG技术虽能提升指标，但会引入分布偏移和视觉伪影。

Method: FaME利用图像质量评估模型识别低质量生成样本，存储其采样轨迹，并作为负引导以避免未来采样进入低质量区域。

Result: 在ImageNet上的实验表明，FaME在不影响FID的情况下，显著提升了视觉质量。

Conclusion: FaME是一种有效提升生成图像感知质量的方法，并具有扩展到文本到图像生成的潜力。
...
TL;DR: BridgeTA是一种通过教师助理网络（TA）缩小LiDAR-Camera融合与纯相机模型之间表示差距的轻量级知识蒸馏框架，保持学生模型架构和推理成本不变。
...
Method: 引入轻量级TA网络，结合教师和学生的BEV表示，创建共享潜在空间；使用Young不等式推导蒸馏损失，分解为教师-TA和TA-学生双路径。

Result: 在nuScenes数据集上，BridgeTA比纯相机基线提升4.2% mIoU，优于其他最先进知识蒸馏方法45%。
...
Method: 采用迭代去噪过程直接从稀疏视图输入优化CBCT体积，并利用大型伪CBCT数据集进行训练。

Result: 在CT-RATE测试集上PSNR增益达12.96 dB，辐射暴露减少8倍，且性能与3D U-Net相当。

Conclusion: MInDI-3D在临床评估中表现优异，适用于患者定位并保持肿瘤边界。
...
<details>
  <summary>Details</summary>
Motivation: RANSAC方法在复杂场景中因内点阈值模糊而易产生假阳性平面检测，需一种更客观的机制来确定真实平面数量。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在3D航空场景语义分割中难以处理尺度变化和结构遮挡导致的语义模糊，限制了分割的准确性和一致性。
...
TL;DR: 论文提出了一种轻量级的纹理模块，通过将像素观察嵌入UV纹理空间，利用纹理对齐作为监督信号，提升单目3D手部重建的准确性和真实感。


<details>
  <summary>Details</summary>
Motivation: 现有高性能模型在手部几何与图像外观的覆盖上仍不完美，纹理对齐可能是一个未被充分利用的监督信号。

Method: 提出一个轻量级纹理模块，嵌入像素观察到UV纹理空间，并引入密集对齐损失，结合可微分渲染和已知拓扑的3D手部网格模型。

Result: 该方法提升了HaMeR架构的准确性和真实感，验证了外观引导对齐在手部重建中的价值。
...
TL;DR: 论文提出了一种名为Preacher的论文到视频转换系统，通过分解、总结和重构论文内容，生成高质量的视频摘要，解决了现有视频生成模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在上下文窗口、视频时长、风格多样性和领域知识表示方面存在限制，无法满足论文到视频任务的需求。

Method: Preacher采用自上而下的方法分解和总结论文内容，再通过自下而上的视频生成技术，结合渐进式思维链（P-CoT）进行细粒度规划，生成多样化的视频片段。

Result: Preacher在五个研究领域成功生成了高质量的视频摘要，表现优于现有视频生成模型。

Conclusion: Preacher为论文到视频任务提供了一种有效的解决方案，展示了其在跨领域应用中的潜力。
...
TL;DR: 提出了一种多对比融合模块（MCFM），用于提升超声图像中胎儿躯干平面的识别性能，通过多对比注意力机制增强特征建模，实验表明其显著提高了分类准确性和临床可靠性。
...
Method: 设计了MCFM模块，专注于神经网络的底层处理，通过多对比注意力权重增强特征提取，同时保持参数开销最小。
...
TL;DR: PG-SAM结合专家诊断文本引导的SAM模型，通过自动生成提示信息和跨序列注意力模块，提升了腮腺病变分割的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决腮腺病变分割中因复杂边界和缺乏精确提示信息导致的挑战，同时结合医学专家知识。

Method: 提出专家诊断报告引导的提示生成模块和跨序列注意力模块，整合多模态信息。

Result: 在三个独立临床中心验证了PG-SAM的先进性能，证明了其临床适用性。

Conclusion: PG-SAM通过结合专家知识和跨序列信息，显著提升了腮腺病变分割的效果。
...
TL;DR: ReMIND2Reg 2025挑战赛旨在通过多模态图像配准解决脑肿瘤手术中脑移位导致的导航不准确问题，提供公开基准数据集。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤手术中，术前MRI导航因脑移位失去准确性，术后超声与术前MRI配准可恢复空间精度，但面临解剖变化和模态差异的挑战。

Method: 挑战赛提供99例训练数据、5例验证数据和10例测试数据，包含3D ceT1 MRI、T2 MRI和术后超声图像，通过手动标注的解剖标志评估性能。

Result: 评估指标包括目标配准误差（TRE）、最坏情况标志错位（TRE30）和运行时间，旨在推动稳健、可临床部署的多模态配准算法发展。

Conclusion: ReMIND2Reg挑战赛为图像引导神经外科手术中的多模态配准问题提供了标准化评估框架，促进技术进步。
...
TL;DR: 本文提出了一种参数化逆问题框架，用于3D成像重建中的大规模线性问题，结合噪声估计和神经算子实现快速重建，并通过全局与局部时空数据特征融合提升精度。


<details>
  <summary>Details</summary>
Motivation: 解决非视线（NLOS）成像中信号弱、噪声敏感的问题，实现准确且鲁棒的3D重建。

Method: 采用噪声估计模块和参数化神经算子，通过深度算法展开构建重建框架，并融合全局与局部时空数据特征。

Result: 在模拟和真实数据集上验证了方法的有效性，尤其在快速扫描和稀疏照明点数据中表现优异。

Conclusion: 该方法为复杂场景下的NLOS成像提供了可行的解决方案，兼具高效性和鲁棒性。
...
TL;DR: NegFaceDiff是一种新的采样方法，通过在身份条件扩散过程中引入负条件，提高了生成数据的身份一致性和可分离性，从而优化了人脸识别模型的性能。
...
Conclusion: NegFaceDiff通过负条件采样有效提升了身份条件扩散模型的性能，为人脸识别系统的训练提供了更高质量的数据。
...
TL;DR: GSFixer利用参考引导的视频修复模型提升稀疏视图3D高斯泼溅（3DGS）重建的质量，结合2D语义和3D几何特征，显著优于现有方法。
...
Method: 提出GSFixer框架，基于DiT的视频扩散模型，结合参考视图的2D语义和3D几何特征修复伪影。

Result: 实验表明GSFixer在3DGS伪影修复和稀疏视图3D重建中优于现有方法。

Conclusion: GSFixer通过多特征融合有效提升3DGS重建质量，并提供了新的评估基准DL3DV-Res。
...
Conclusion: 该方法在手术场景中表现出色，为长期点跟踪提供了高效解决方案。
...
TL;DR: PaCo-FR是一种无监督框架，结合掩码图像建模和像素对齐，解决了现有面部表示预训练方法的三个关键挑战，并在多个面部分析任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在捕捉面部特征、空间结构和利用有限标注数据方面存在不足，需要一种更高效的面部表示学习方法。

Method: PaCo-FR采用结构化掩码策略、基于补丁的码书和空间一致性约束，结合掩码图像建模与像素对齐。

Result: 仅用200万未标注图像预训练，PaCo-FR在多种面部分析任务中表现优异，尤其在姿态、遮挡和光照变化场景下。
...
Method: 提出SAFF方法，结合槽注意力与补丁嵌入，通过相似性矩阵量化过滤后的嵌入相关性。

Result: 实验表明SAFF优于其他注意力机制，在多个小样本学习基准数据集上表现优异。

Conclusion: SAFF通过有效过滤无关特征，显著提升小样本分类性能。
...
TL;DR: MangaDiT利用扩散变换器和分层注意力机制，显著提升了参考引导线稿上色的区域级颜色一致性。
...
Conclusion: MangaDiT通过内部注意力机制隐式发现语义对应，无需外部标注，实现了更优的参考引导线稿上色。
...
TL;DR: NEURAL是一种基于语义引导的多模态医学图像压缩框架，通过结构修剪和知识图谱融合，显著减少数据大小同时保持高诊断性能。
...
Method: 利用生成式视觉语言模型的交叉注意力分数修剪X光图像，结合知识图谱生成统一图表示。

Result: 在MIMIC-CXR和CheXpert Plus数据集上，数据大小减少93.4-97.7%，诊断AUC达0.88-0.95。
...
TL;DR: 提出了一种基于sheaf的框架，用于融合MRI和组织病理学数据，解决现有方法在保留跨模态共享结构信息和处理缺失数据方面的不足。
...
Result: 模型在基准方法中表现优异，且在数据缺失或不完整情况下仍具鲁棒性。
...
Method: 采用空间Soft Argmax算子进行概率关键点回归，设计校准的损失函数，并引入RAIM进行故障检测。

Result: 模型在跑道图像数据集上表现优于基线架构，提供亚像素精度的校准不确定性估计。

Conclusion: 该方法为安全关键航空应用的认证提供了可行方案。
...
TL;DR: M3-Agent是一个新型多模态代理框架，具备长期记忆能力，能处理实时视觉和听觉输入，并通过强化学习训练在多模态任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一种具备人类类似长期记忆的多模态代理，以提升其在复杂环境中的理解和推理能力。

Method: M3-Agent采用实体中心的多模态记忆组织方式，结合强化学习训练，并在M3-Bench基准上评估其性能。

Result: M3-Agent在M3-Bench-robot、M3-Bench-web和VideoMME-long基准上分别比基线模型高出6.7%、7.7%和5.3%的准确率。

Conclusion: M3-Agent在多模态代理中实现了更接近人类的长期记忆，并为实际应用提供了设计思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LDM的图像和谐化方法在细节保留和和谐化能力上存在不足，且合成数据集缺乏真实光照变化。

Method: 提出R2R模型，结合Clear-VAE和MACA机制，通过区域信息注入实现和谐化；提出Random Poisson Blending构建RPHarmony数据集。

Result: 实验表明R2R在定量指标和视觉和谐化上优于其他方法，且RPHarmony数据集提升了模型在真实场景中的表现。

Conclusion: R2R方法显著提升了图像和谐化的效果，新数据集为研究提供了更丰富的资源。
...
TL;DR: 提出了一种混合模态内外专家（MoIIE）的方法，用于提升大型视觉语言模型的效率与性能。


<details>
  <summary>Details</summary>
Motivation: 解决密集大型视觉语言模型计算成本高的问题，并探索稀疏混合专家架构在多模态任务中的应用。

Method: 引入MoIIE，通过模态引导的路由机制，分别处理模态内和模态间特征，并采用两阶段训练策略。

Result: MoIIE模型在5.5B和11.3B参数规模下，性能匹配或超越现有开源多模态模型。

Conclusion: MoIIE方法在效率、性能和通用性上表现优异，为多模态模型提供了新思路。
...
TL;DR: 提出了一种新的形状匹配方法“组合匹配”，用于几何形状装配中的互锁部件组合，通过显式建模“相同表面形状”和“相反体积占用”两个特性，显著减少匹配中的局部模糊性。


<details>
  <summary>Details</summary>
Motivation: 传统几何装配方法依赖对齐相同表面，而互锁形状需要同时考虑表面形状和体积占用特性，现有方法未能充分解决这一问题。

Method: 通过等变神经网络学习表面形状和体积占用的对应关系，并估计形状方向以对齐旋转区域。

Result: 在几何装配基准测试中表现优异，显著优于现有方法。

Conclusion: 组合匹配方法通过显式建模互锁形状特性，实现了更鲁棒的部件组合。
...
TL;DR: DSS-Prompt利用静态和动态提示改进预训练视觉Transformer，在少样本类增量学习中表现优异。


<details>
  <summary>Details</summary>
Motivation: 探索预训练模型在少样本类增量学习中的应用，解决新概念学习和旧知识遗忘的挑战。

Method: 结合静态提示（适应预训练与下游任务）和动态提示（捕捉实例语义），利用多模态模型生成输入相关提示。

Result: 在四个基准测试中表现优于现有方法，缓解灾难性遗忘问题。

Conclusion: DSS-Prompt是一种简单有效的少样本类增量学习方法，无需增量任务训练即可达到先进性能。
...
TL;DR: MeMoSORT是一种实时多目标跟踪方法，通过记忆辅助卡尔曼滤波器和运动自适应IoU解决传统方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统基于检测的跟踪方法因卡尔曼滤波器的运动模型不匹配和刚性IoU关联在遮挡下表现不佳。

Method: 提出记忆辅助卡尔曼滤波器（MeKF）补偿运动模型误差，运动自适应IoU（Mo-IoU）扩展匹配空间并引入高度相似性。

Result: 在DanceTrack和SportsMOT上分别达到67.9%和82.1%的HOTA分数，表现优异。
...
TL;DR: MUJICA是一种改进PBR材质超分辨率的方法，通过跨图注意力机制提升预训练SISR模型的性能，解决跨图不一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有SISR方法在PBR材质超分辨率中存在跨图不一致、模态特征建模不足和数据分布偏移导致的泛化能力差的问题。

Method: 提出MUJICA，一种基于跨图注意力的适配器，用于改进预训练的Swin-transformer SISR模型。

Result: MUJICA提升了PSNR、SSIM和LPIPS分数，同时保持跨图一致性，并在资源有限的情况下实现高效训练。

Conclusion: MUJICA在PBR材质超分辨率任务中表现优异，具有高效性和先进性能。
...
TL;DR: 该论文提出了一种基于U-Net架构的深度学习模型，用于自动化分割脑组织照片中的组织，解决了传统方法需要高成本手动干预的问题。


<details>
  <summary>Details</summary>
Motivation: 脑库和神经病理学实验室常规收集的冠状切片照片需要组织分割，目前依赖昂贵的手动操作，亟需自动化解决方案。

Method: 采用U-Net架构，结合1,414张手动标注图像和2,000张合成图像进行训练，以提高模型的泛化能力。

Result: 模型在未见过的测试集上表现优异，Dice分数中位数超过0.98，平均表面距离小于0.4毫米，接近人工标注的变异性水平。

Conclusion: 该工具公开可用，为脑组织照片的自动化分割提供了高效且接近人工精度的解决方案。
...
TL;DR: 提出TRACE框架，通过将3D点建模为刚性粒子，直接学习其平移旋转动力学系统，无需人工标签即可建模复杂动态3D场景的运动物理。
...
Method: 将3D点建模为刚性粒子，学习其平移旋转动力学系统，并显式估计物理参数。

Result: 在多个动态数据集上表现优异，尤其在未来帧外推任务中。

Conclusion: TRACE不仅能建模复杂物理运动，还能通过物理参数聚类轻松分割物体。
...
TL;DR: 研究发现CLIP模型在训练初期与人类低层次图像质量评估相关性高，随后逐渐下降，原因与形状-纹理偏置和噪声下的分类准确性有关。


<details>
  <summary>Details</summary>
Motivation: 探究CLIP模型训练中与人类低层次感知相关性变化的现象及其原因。

Method: 通过分析形状-纹理偏置和对噪声的分类准确性变化进行研究。

Result: CLIP初期学习低层次视觉特征，与人类感知对齐但易受噪声影响；后期转向抽象形状表示，提高鲁棒性但降低感知对齐。
...
TL;DR: ViMoNet结合视频和运动数据理解人类行为，提出新框架和数据集VIMOS，性能优于现有方法。
...
Method: 提出ViMoNet框架，通过联合训练策略利用精确的运动-文本数据和全面的视频-文本数据。

Result: ViMoNet在标题生成、运动理解和行为解释任务中表现优于现有方法。
...
TL;DR: 提出了一种物理自回归模型（PAR），利用视频预训练中的世界知识来理解物理动态，无需动作预训练，实现了准确的视频预测和一致的动作轨迹。
...
Method: 基于自回归视频生成模型，PAR结合帧和动作的物理令牌表示机器人和环境的联合演化，采用DiT-based去令牌化器建模连续令牌，并引入因果掩码、逆运动学等技术提升性能。
...
Method: 采用声明式方法，通过YAML配置文件定义训练、推理和评估工作流，支持高级策略如基于补丁的学习、测试时增强和模型集成。
...
TL;DR: 提出了一种新颖的深度反向卷积算子，用于有效反转深度卷积，并通过实验验证其作为基础模块的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统转置卷积并非卷积的真正逆运算，缺乏标准化的反向卷积算子，限制了神经网络架构的发展。

Method: 通过正则化最小二乘优化问题设计深度反向卷积算子，并结合层归一化、1×1卷积和GELU激活构建反向卷积块。

Result: 实验表明，提出的反向卷积算子能有效替代传统卷积和转置卷积，在图像恢复任务中表现优异。

Conclusion: 该工作为深度学习模型设计中的新算子开发提供了可能，并展示了其实际应用潜力。
...
TL;DR: 提出了一种基于图神经网络的OIQA框架，通过显式建模视口间的结构关系来提升对空间失真非均匀性的感知。


<details>
  <summary>Details</summary>
Motivation: 现有OIQA方法难以评估局部非均匀失真，因未能有效建模质量的空间变化及捕捉局部细节与全局上下文的特征表示。

Method: 采用斐波那契球采样生成具有良好拓扑结构的视口，每个视口作为图节点；多阶段特征提取网络生成高维节点表示；结合GAT和图变换器捕捉局部与长程质量依赖关系。

Result: 在两个大规模OIQA数据库上的实验表明，该方法显著优于现有方法，验证了其有效性和强泛化能力。

Conclusion: 所提框架通过建模视口间结构关系，成功解决了局部非均匀失真评估问题，具有显著性能优势。
...
TL;DR: 论文提出了在小规模CelebAMask-HQ数据集上评估扩散模型生成人脸的基准，比较了UNet和DiT架构的无条件生成，并探索了LoRA微调预训练Stable Diffusion模型。通过引入InfoNCE损失和SegFormer分割编码器，提升了语义对齐和可控性。


<details>
  <summary>Details</summary>
Motivation: 研究旨在改进小规模数据集下人脸生成的语义对齐和可控性，探索对比嵌入学习和高级分割编码的效果。

Method: 比较UNet和DiT的无条件生成，采用LoRA微调预训练模型，集成InfoNCE损失和SegFormer分割编码器。

Result: 实验表明对比嵌入学习和高级分割编码在小规模数据下显著提升了人脸生成的语义对齐和可控性。

Conclusion: 研究证明了InfoNCE损失和SegFormer分割编码器在小规模数据集下对人脸生成的有效性，为可控生成提供了新思路。
...
TL;DR: ARI3D是一款用于交互式分析X射线CT图像中区域的软件工具，旨在改进相位识别、考虑部分体积效应、提高检测限和准确性，并统一不同科学领域的定量3D分析。


<details>
  <summary>Details</summary>
Motivation: X射线CT成像技术存在固有成像伪影（如束硬化和部分体积效应），导致微观结构定量分析需要用户基于体素灰度值做出多步决策，ARI3D旨在简化这一过程。

Method: 开发ARI3D软件工具，通过交互式分析辅助用户遵循协议步骤，对3D图像中的区域进行分类和量化。

Result: ARI3D能够改进相位识别、处理部分体积效应、提高检测限和量化准确性，并适用于多科学领域。

Conclusion: ARI3D为X射线CT图像的定量分析提供了一种高效、统一的解决方案，有望推动多领域的3D微观结构研究。
...
TL;DR: 研究发现，Vision Transformers (ViTs) 在图像识别任务中表现优异，但与人类感知的对齐程度受模型大小、数据集多样性、数据增强和正则化影响。


<details>
  <summary>Details</summary>
Motivation: 探索ViTs在图像识别任务中与人类感知的对齐程度，以及不同因素对其影响。

Method: 在TID2013数据集上系统分析模型大小、数据集大小、数据增强和正则化对ViT感知对齐的影响。

Result: 大模型对齐度较低；数据集多样性影响小，但重复训练会降低对齐；强数据增强和正则化进一步减少对齐。
...
<details>
  <summary>Details</summary>
Motivation: 离散视频VAE存在训练不稳定、时间长和重建质量差的问题，而连续VAE训练更易且性能更优，因此希望通过结合两者提升离散VAE的性能。

Method: 利用FSQ保留连续VAE先验，提出多令牌量化机制和首帧重建优化，并设计联合离散-连续优化方案。

Result: 训练速度提升数倍，PSNR提高近1 dB，4x16x16离散VAE性能显著提升，首次在单一网络中实现连续和离散表示的竞争性表现。

Conclusion: OneVAE成功结合连续和离散VAE的优势，显著提升离散视频VAE的性能和效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在3D建模和细节保留上存在几何不一致和粗糙重建问题，且生成能力弱导致运动泛化和场景不协调。

Method: HumanGenesis整合几何和生成建模，包括Reconstructor、Critique Agent、Pose Guider和Video Harmonizer四个代理，分别负责3D重建、区域优化、运动生成和视频合成。

Result: HumanGenesis在文本引导合成、视频重演和新姿势泛化等任务上达到最先进水平，显著提升了表现力、几何保真度和场景整合。
...
TL;DR: 论文提出了一种名为COME的通用框架，用于解决超声图像分析中多异构数据集训练的挑战，通过双结构-语义共享专家与源特定专家的协作，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统单数据集训练在新数据分布下表现不佳，尤其是在超声图像分析中，因数据有限、声学阴影和斑点噪声等问题。构建一个适用于多异构数据集的通用框架是必要的。

Method: 提出COME框架，建立双结构-语义共享专家，形成通用表示空间，并与源特定专家协作提取判别性特征。
...
TL;DR: 提出了一种基于事件相机的动态高斯泼溅方法（E-4DGS），用于多视角事件流的新视角合成，解决了高速运动和低光场景的挑战。
...
Method: 提出事件驱动的初始化方案、事件自适应切片泼溅、强度重要性修剪和自适应对比度阈值优化，设计了一个六相机多视角事件流数据集。

Result: E-4DGS在事件单独和事件-RGB融合基准上表现优异，为多视角事件重建提供了新方法。

Conclusion: E-4DGS为高速场景捕捉提供了一种有效的事件驱动解决方案，推动了多视角事件重建的研究。
...
TL;DR: 该论文提出了一种利用音频与视觉语音元素协同作用的新方法，通过自监督掩码预测任务学习音频-视觉语音表示，并将其直接应用于伪造视频检测任务，显著提升了跨数据集泛化能力和鲁棒性。
...
Method: 通过自监督掩码预测任务学习音频-视觉语音表示，编码局部和全局语义信息，并将模型直接迁移至伪造检测任务。

Result: 实验表明，该方法在跨数据集泛化和鲁棒性上优于现有技术，且无需使用任何伪造视频进行训练。

Conclusion: 音频-视觉语音表示学习为伪造视频检测提供了有效解决方案，显著提升了性能。
...
Method: 基于一个包含108,722个核的泛癌数据集，训练HistoPLUS模型，覆盖13种细胞类型。

Result: 在4个独立队列的外部验证中，HistoPLUS在检测质量和分类F1分数上分别提升5.2%和23.7%，参数减少5倍。

Conclusion: HistoPLUS显著改善了罕见细胞类型的研究，并展示了强大的跨域泛化能力，为TME研究提供了新工具。
...
TL;DR: 论文探讨了手写文本生成（HTG）模型在低资源手写文本识别（HTR）中的有效性，比较了三种前沿HTG模型，并提供了选择最佳模型的量化指南。


<details>
  <summary>Details</summary>
Motivation: 解决历史手稿数字化中HTR系统因数据分布差异导致的性能问题，尤其是针对小规模、特定作者的手写集合。

Method: 系统比较了三种HTG模型（生成对抗、扩散和自回归范式），分析合成数据的视觉和语言特征对HTR微调的影响。

Result: 提供了HTG模型在低资源HTR中的有效性评估，并提出了选择最佳模型的量化标准。
...
TL;DR: AST-n框架通过从中间噪声水平启动反向扩散并结合高阶ODE求解器，显著加速低剂量CT去噪推理，25步内达到高质量结果。
...
Result: AST-25在25步内PSNR>38 dB，SSIM>0.95，推理时间从16秒降至1秒/切片。

Conclusion: AST-n结合高阶求解器可实现快速高质量LDCT重建，提升临床可行性。
...
TL;DR: 利用现成的Stable Diffusion模型实现视觉上下文学习（V-ICL），无需额外训练即可适应多种视觉任务，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 探索如何简化视觉上下文学习（V-ICL）的流程，避免复杂的训练和额外数据，提高通用性。

Method: 在Stable Diffusion的自注意力层中重新计算注意力，显式结合查询与示例提示的上下文。

Result: 在六种视觉任务中表现优异，例如在Pascal-5i数据集上，前景分割任务的mIoU比现有方法提升8.9%和3.2%。

Conclusion: 无需微调的Stable Diffusion模型能有效实现视觉上下文学习，并通过集成多提示进一步提升性能。
...
TL;DR: LIA-X是一种新型可解释的肖像动画器，通过稀疏运动字典实现面部动态的细粒度控制，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在面部动态转移中缺乏可解释性和细粒度控制，LIA-X旨在解决这一问题。

Method: 采用自编码器模型，通过稀疏运动字典将面部动态分解为可解释因素，支持“编辑-变形-渲染”策略。

Result: 在自重现和跨重现任务中表现优异，支持用户引导的细粒度编辑和3D感知操作。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动营养分析领域缺乏标准化评估方法和高质量数据集的瓶颈问题。

Method: 1. 发布JFB数据集（1000张食物图像）；2. 开发综合评估框架和专用模型（january/food-vision-v1）。

Result: 专用模型在综合评分上达到86.2，比通用模型提升12.1分。

Conclusion: 该研究为自动营养分析提供了新的数据集和评估框架，推动了领域发展。
...
TL;DR: PERSONA结合3D和扩散方法，从单张图像生成个性化3D人体化身，解决了身份保持和姿态驱动变形的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D方法需要大量姿态丰富的视频，而扩散方法难以保持身份一致性。PERSONA旨在结合两者优势，从单张图像生成高质量化身。

Method: PERSONA利用扩散模型生成姿态丰富的视频，并基于此优化3D化身。引入平衡采样和几何加权优化以提升渲染质量。

Result: PERSONA能够从单张图像生成具有姿态驱动变形的高质量3D化身，同时保持身份一致性。

Conclusion: PERSONA框架有效解决了身份保持和姿态驱动变形的挑战，为个性化3D化身生成提供了实用解决方案。
...
TL;DR: 3D高斯泼溅（3DGS）作为NeRF的替代方案，提供高保真实时渲染，并支持多种下游应用。本文综述了3DGS的最新进展，涵盖语义理解、分割、编辑、生成等任务，并总结了数据集、评估方法和资源库。


<details>
  <summary>Details</summary>
Motivation: 探索3DGS在3D场景表示中的潜力，尤其是其显式和紧凑特性如何支持几何和语义理解，以及其广泛的应用前景。

Method: 综述了3DGS的应用，包括2D基础模型、NeRF方法的借鉴，以及3DGS在分割、编辑、生成等任务中的具体方法。

Result: 总结了代表性方法、监督策略和学习范式，并分析了公共基准测试中的性能比较。

Conclusion: 3DGS在3D表示和下游任务中展现出强大潜力，未来研究可通过持续更新的资源库进一步推动其发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决大型视觉语言模型（VLMs）计算和内存需求过高的问题，现有压缩方法存在模块不可比、评估局限和孤立使用技术的不足。

Method: 提出LLMC+基准工具包，支持多种算法，系统研究令牌级和模型级压缩。

Result: 发现空间和时间冗余需不同策略，令牌减少方法在多轮对话中表现不佳，联合压缩可实现高效压缩。
...
TL;DR: 论文探讨了GPT-4o生成的合成图像数据在补充真实世界数据不足和提供更清晰监督信号方面的优势，并提出了Echo-4o-Image数据集和新的评估基准。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决真实世界图像数据在罕见场景和文本-图像对齐方面的局限性，探索合成数据的潜力。
...
Conclusion: 合成图像数据能有效补充真实世界数据的不足，提供更干净的监督信号，具有广泛的应用潜力。
...
### [123] [Generative Artificial Intelligence in Medical Imaging: Foundations, Progress, and Clinical Translation](https://arxiv.org/abs/2508.09177)
*Xuanru Zhou,Cheng Li,Shuqiang Wang,Ye Li,Tao Tan,Hairong Zheng,Shanshan Wang*

Main category: eess.IV

TL;DR: 综述探讨了生成式AI在医学影像中的广泛应用，包括数据合成、图像增强、模态转换等，并评估了GANs、VAEs、扩散模型等技术的临床潜力。
...
Method: 系统回顾生成式AI在影像工作流中的作用，提出三层次评估框架（像素级、特征级、任务级）。

Result: 生成式AI在医学影像中展现出显著潜力，但仍面临泛化性、幻觉风险、隐私和监管等障碍。

Conclusion: 生成式AI与大规模基础模型的结合可能推动下一代可扩展、可靠且临床集成的影像系统发展。
...
### [124] [HiFi-Mamba: Dual-Stream W-Laplacian Enhanced Mamba for High-Fidelity MRI Reconstruction](https://arxiv.org/abs/2508.09179)
*Hongli Chen,Pengcheng Fang,Yuxia Chen,Yingxuan Ren,Jing Hao,Fangfang Tang,Xiaohao Cai,Shanshan Shan,Feng Liu*

Main category: eess.IV

TL;DR: HiFi-Mamba是一种新型双流Mamba架构，用于MRI图像重建，解决了传统方法对高频细节不敏感和扫描冗余的问题，显著提升了重建精度。


<details>
  <summary>Details</summary>
Motivation: 传统Mamba方法在MRI重建中存在对高频解剖细节不敏感和依赖冗余多向扫描的局限性。

Method: 提出HiFi-Mamba架构，包含WL块和HiFi-Mamba块，分别处理低高频特征，并通过自适应状态空间调制选择性整合高频特征。

Result: 在标准MRI重建基准测试中，HiFi-Mamba在重建精度上优于现有CNN、Transformer和其他Mamba模型。

Conclusion: HiFi-Mamba通过双流设计和高效的单向遍历策略，实现了高保真MRI重建，同时保持模型紧凑和高效。
...
### [125] [MedPatch: Confidence-Guided Multi-Stage Fusion for Multimodal Clinical Data](https://arxiv.org/abs/2508.09182)
*Baraa Al Jorf,Farah Shamout*

Main category: eess.IV

TL;DR: MedPatch是一种多阶段多模态融合架构，通过置信度引导的补丁整合多种模态，解决了医疗数据的异质性和稀疏性问题，在临床预测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 医疗数据具有异质性、规模小和稀疏性（缺失模态）的特点，限制了模型在临床预测任务中的性能。

Method: MedPatch包含多阶段融合策略、缺失感知模块和联合融合模块，通过置信度引导的补丁整合多模态数据。

Result: 在MIMIC数据集上评估，MedPatch在院内死亡率预测和临床条件分类任务中达到最优性能。
...
### [126] [Hybrid(Transformer+CNN)-based Polyp Segmentation](https://arxiv.org/abs/2508.09189)
*Madan Baduwal*

Main category: eess.IV

TL;DR: 提出了一种结合Transformer和CNN的混合模型，用于提升结肠息肉分割的准确性和鲁棒性，特别是在处理边界模糊和内窥镜伪影时表现优异。


<details>
  <summary>Details</summary>
Motivation: 结肠息肉分割面临尺寸、形状、光照等多变性和边界模糊的挑战，现有方法难以应对。

Method: 设计了一种混合（Transformer + CNN）模型，利用边界感知注意力机制和鲁棒特征提取技术。

Result: 在分割准确性和伪影鲁棒性上显著优于现有方法（Recall提升1.76%，准确率提升0.07%）。
...
### [127] [impuTMAE: Multi-modal Transformer with Masked Pre-training for Missing Modalities Imputation in Cancer Survival Prediction](https://arxiv.org/abs/2508.09195)
*Maria Boyko,Aleksandra Beliaeva,Dmitriy Kornilov,Alexander Bernstein,Maxim Sharaev*

Main category: eess.IV

TL;DR: impuTMAE是一种基于Transformer的多模态预训练方法，通过重建掩码补丁同时学习模态间和模态内交互，解决了医学数据缺失问题，在胶质瘤生存预测中表现优异。


<details>
  <summary>Details</summary>
Motivation: 医学数据多模态性复杂且常缺失，需有效处理以提升预后模型性能并促进疾病机制理解。

Method: 提出impuTMAE，一种端到端的Transformer方法，通过预训练学习交互并填补缺失模态，再微调用于胶质瘤生存预测。

Result: 在TCGA-GBM/LGG和BraTS数据集上，impuTMAE超越现有方法，达到最佳性能。

Conclusion: impuTMAE通过处理缺失数据并高效利用资源，在多模态医学数据中表现出色，为预后模型提供了新思路。
...
### [128] [FIVA: Federated Inverse Variance Averaging for Universal CT Segmentation with Uncertainty Estimation](https://arxiv.org/abs/2508.09196)
*Asim Ukaye,Numan Saeed,Karthik Nandakumar*

Main category: eess.IV

TL;DR: 提出了一种基于联邦学习的方法，利用模型不确定性和预测不确定性，实现跨不同腹部CT数据集的通用分割。
...
Method: 采用联邦学习框架，利用随机小批量梯度下降的噪声估计模型权重分布，并通过贝叶斯逆方差聚合方案在服务器端聚合参数。
...
### [129] [Zero-shot self-supervised learning of single breath-hold magnetic resonance cholangiopancreatography (MRCP) reconstruction](https://arxiv.org/abs/2508.09200)
*Jinho Kim,Marcel Dominik Nickel,Florian Knoll*

Main category: eess.IV

TL;DR: 零样本自监督学习重建技术可显著减少MRCP的屏气时间，同时提升图像质量，浅层训练进一步缩短了计算时间。


<details>
  <summary>Details</summary>
Motivation: 研究旨在验证零样本自监督学习重建技术是否能在减少MRCP屏气时间的同时保持图像质量。

Method: 采用零样本重建技术对屏气MRCP图像进行处理，并与并行成像和压缩感知重建方法对比。通过浅层训练减少计算时间。

Result: 零样本重建显著提升了图像质量，接近呼吸触发MRCP的水平，浅层训练将训练时间从271分钟缩短至11分钟。

Conclusion: 零样本学习结合浅层训练为临床提供了高效且高质量的MRCP重建方案。
...
### [130] [From Explainable to Explained AI: Ideas for Falsifying and Quantifying Explanations](https://arxiv.org/abs/2508.09205)
...
### [131] [AMRG: Extend Vision Language Models for Automatic Mammography Report Generation](https://arxiv.org/abs/2508.09225)
...
<details>
  <summary>Details</summary>
Motivation: 乳腺X光检查报告生成在医疗AI中是一个重要但未充分探索的任务，面临多视图图像推理、高分辨率视觉线索和非结构化放射学语言等挑战。

Method: 基于MedGemma-4B-it模型，采用LoRA进行参数高效微调，训练和评估在公开数据集DMID上进行。

Result: AMRG在语言生成和临床指标上表现优异，ROUGE-L为0.5691，METEOR为0.6152，CIDEr为0.5818，BI-RADS准确率为0.5582。

Conclusion: AMRG为放射学报告生成提供了可扩展和适应性强的基础，推动了多模态医疗AI的未来研究。
...
### [132] [A Generative Imputation Method for Multimodal Alzheimer's Disease Diagnosis](https://arxiv.org/abs/2508.09271)
*Reihaneh Hassanzadeh,Anees Abrol,Hamid Reza Hassanzadeh,Vince D. Calhoun*

Main category: eess.IV

TL;DR: 提出了一种生成对抗网络方法，用于从现有模态中重建缺失的脑影像数据，提高了阿尔茨海默病分类准确性。


<details>
  <summary>Details</summary>
Motivation: 多模态数据分析能提高脑部疾病诊断准确性，但数据缺失是主要挑战，传统方法存在局限性。

Method: 使用生成对抗网络从T1加权结构MRI和功能网络连接中重建缺失模态。

Result: 与传统方法相比，生成填补方法使阿尔茨海默病分类准确性提高了9%。

Conclusion: 生成对抗网络是解决多模态数据缺失问题的有效方法。
...
TL;DR: SurLonFormer是一种基于Transformer的神经网络，整合纵向医学影像和结构化数据用于生存预测，解决了现有方法在利用截尾数据、时间相关性及可解释性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有方法在利用截尾数据、捕捉纵向影像相关性及可解释性方面存在不足，限制了生存分析的准确性和实用性。
...
Conclusion: SurLonFormer通过整合多模态数据，显著提升了生存预测的准确性和可解释性。
...
TL;DR: 提出了一种名为T-CACE的框架，用于从非对比MRI直接合成多相增强MRI，解决了传统MRI的局限性，并在实验中表现优异。
...
Method: T-CACE结合了条件令牌编码（CTE）、动态时间感知注意力掩码（DTAM）和时间分类一致性约束（TCC），实现了多相增强MRI的合成。
...
### [135] [TFZ: Topology-Preserving Compression of 2D Symmetric and Asymmetric Second-Order Tensor Fields](https://arxiv.org/abs/2508.09235)
*Nathaniel Gorski,Xin Liang,Hanqi Guo,Bei Wang*

Main category: cs.GR

TL;DR: TFZ是一种新颖的压缩框架，专注于在2D对称和非对称二阶张量场的压缩过程中保留其拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 张量场的拓扑结构对科学和工程至关重要，但传统的有损压缩可能破坏这些结构，影响后续分析和可视化。

Method: TFZ通过逐单元扫描并保留局部拓扑特征，确保全局拓扑特性得以保留，支持对称张量场的退化点和非对称张量场的特征向量图。

Result: TFZ成功提升了SZ3和SPERR等有损科学数据压缩器的性能。

Conclusion: TFZ框架在保留张量场拓扑结构的同时，有效提升了压缩效率，适用于多种应用场景。
...
### [136] [DualPhys-GS: Dual Physically-Guided 3D Gaussian Splatting for Underwater Scene Reconstruction](https://arxiv.org/abs/2508.09610)
*Jiachen Li,Guangzhi Han,Jin Wan,Yuan Gao,Delong Han*

Main category: cs.GR

TL;DR: DualPhys-GS框架通过双路径优化机制解决水下场景3D重建中的颜色失真和几何伪影问题，结合RGB和深度信息优化衰减模型，并利用多尺度深度感知散射模型处理散射效应。


<details>
  <summary>Details</summary>
Motivation: 传统大气光学模型无法有效处理水下介质特有的光波长选择性衰减和悬浮粒子散射效应，导致远距离场景中的颜色失真和几何伪影。

Method: 提出双特征引导的衰减-散射建模机制：RGB引导的衰减优化模型结合RGB和深度信息处理边缘和结构细节；多尺度深度感知散射模型通过特征金字塔网络和注意力机制捕捉不同尺度的散射效应。设计了多种特殊损失函数以确保物理一致性和结构清晰度。

Result: 实验表明，该方法在悬浮物密集区域和远距离场景中优于现有方法，重建质量显著提升。

Conclusion: DualPhys-GS框架通过动态调整参数和优化策略，有效解决了水下3D重建的挑战，适用于不同水体类型。
...
TL;DR: 提出了一种名为Perceptual Reality Transformer的框架，通过六种神经网络架构模拟八种神经感知障碍，帮助他人体验这些病症的视觉状态。
...
Method: 使用Vision Transformer架构，从自然图像学习到特定病症的感知状态映射，并通过ImageNet和CIFAR-10数据集进行系统评估。

Result: Vision Transformer表现最优，优于传统CNN和生成方法，建立了首个神经感知模拟的系统基准。

Conclusion: 该框架在医学教育、同理心训练和辅助技术开发中有直接应用，并推动了对神经网络模拟非典型人类感知的理解。
...
### [138] [Real-time deep learning phase imaging flow cytometer reveals blood cell aggregate biomarkers for haematology diagnostics](https://arxiv.org/abs/2508.09215)
...
TL;DR: RT-HAD是一种基于深度学习的端到端图像数据处理框架，用于实时检测血液细胞聚集体，解决了传统方法在数据存储和处理上的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统血液学检测方法难以识别细胞聚集体，且需要人工复核，而定量相位成像流式细胞术虽能捕获聚集体形态，但面临大数据存储和离线处理的限制。

Method: RT-HAD结合物理一致的全息重建和检测技术，将每个血细胞表示为图结构以识别聚集体，实时处理大量图像数据。

Result: RT-HAD能在1.5分钟内处理超过30GB的图像数据，血小板聚集体检测错误率为8.9%，符合临床标准。

Conclusion: RT-HAD为无标记功能诊断提供了高效解决方案，解决了即时诊断中的大数据挑战。
...
### [139] [Learning to Detect Unknown Jailbreak Attacks in Large Vision-Language Models: A Unified and Accurate Approach](https://arxiv.org/abs/2508.09201)
*Shuang Liang,Zhihao Xu,Jialing Tao,Hui Xue,Xiting Wang*

Main category: cs.CR

TL;DR: 论文提出了一种名为LoD的无监督框架，通过异常检测解决大型视觉语言模型（LVLM）的越狱攻击检测问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 尽管现有方法尝试通过内部表征检测越狱攻击，但大多依赖启发式规则，性能不佳。

Method: LoD框架包含多模态安全概念激活向量（MSCAV）和安全模式自动编码器，通过建模安全输入的分布来检测异常。

Result: 在三个LVLM和五个基准测试中，LoD的平均AUROC达到0.9951，性能提升最高达38.89%。

Conclusion: LoD通过无监督方法实现了高效且统一的越狱攻击检测，性能显著优于现有方法。
...
### [140] [Speed Always Wins: A Survey on Efficient Architectures for Large Language Models](https://arxiv.org/abs/2508.09834)
...
<details>
  <summary>Details</summary>
Motivation: 传统Transformer架构计算需求高，限制了LLM的大规模训练和实际部署，因此需要探索更高效的架构。
...
### [141] [VisCodex: Unified Multimodal Code Generation via Merging Vision and Coding Models](https://arxiv.org/abs/2508.09945)
*Lingjie Jiang,Shaohan Huang,Xun Wu,Yixia Li,Dongdong Zhang,Furu Wei*

Main category: cs.CL

TL;DR: VisCodex是一个多模态大语言模型框架，通过融合视觉和编码语言模型，提升了多模态代码生成能力，并引入了新的数据集和基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在从多模态输入生成代码方面能力有限，需要一种更强大的框架来整合视觉理解和编码技能。

Method: 采用任务向量模型融合技术，将先进的编码语言模型集成到视觉语言骨干中，同时引入Multimodal Coding Dataset (MCD)和InfiBench-V基准测试。

Result: VisCodex在开源多模态大语言模型中表现最优，接近GPT-4o等专有模型。

Conclusion: VisCodex通过模型融合和新数据集，显著提升了多模态代码生成能力。
...
### [142] [Robustness analysis of Deep Sky Objects detection models on HPC](https://arxiv.org/abs/2508.09831)
...
<details>
  <summary>Details</summary>
Motivation: 随着天文观测数据的爆炸式增长和业余天文学家的参与，需要开发准确且鲁棒的自动化处理方法来解决深空天体检测的挑战。

Method: 使用高性能计算（HPC）并行化计算，训练和比较了YOLO和RET-DETR等检测模型在智能望远镜图像上的表现。

Result: 通过HPC加速，模型在检测深空天体时表现出更高的效率和鲁棒性。

Conclusion: 计算机视觉和深度学习技术结合高性能计算，为深空天体的自动化检测提供了有效解决方案。
...
Method: 提出MoLAN框架，将模态特征分块并动态分配去噪强度，MoLAN+进一步优化该方法。

Result: 在五个模型和四个数据集上验证了MoLAN的有效性，MoLAN+达到最先进性能。

Conclusion: MoLAN框架在多模态情感分析中具有广泛适用性和高效性。
...
TL;DR: PatchECG框架通过掩码训练策略解决不同ECG布局导致的信号异步和部分缺失问题，提升心律失常识别的鲁棒性。
...
Method: 提出PatchECG框架，基于掩码训练策略自适应学习缺失表示，关注关键块和导联间协作依赖。

Result: 在PTB-XL数据集和真实ECG图像上表现优异，AUROC达0.835，外部验证中房颤诊断AUROC为0.778。

Conclusion: PatchECG在不同ECG布局下表现稳定且优于现有方法，显著提升了心律失常识别性能。
...
TL;DR: SVG-1M数据集和SVGen模型解决了从自然语言生成精确SVG代码的挑战，通过数据增强和强化学习优化，表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 将创意转化为精确的矢量图形耗时，需高效解决方案。
...
TL;DR: 论文提出了一种轻量级、无需训练的方法，利用检索增强生成（RAG）通过线性映射跨越模态间隙，显著提升多模态输入文本描述的质量。


<details>
  <summary>Details</summary>
Motivation: 预训练大型多模态模型（LMMs）存在模态间隙，即文本和视觉表示在共享嵌入空间中的不对齐问题。微调虽能缓解此问题，但成本高昂且不切实际。

Method: 采用检索增强生成（RAG）和线性映射，无需训练即可高效跨越模态间隙。通过语言模型生成合成描述，优化标准图像描述指标。
...
Method: MoQE结合了一个全精度模型的多个量化变体作为专门的量化专家，并基于输入数据的特性动态路由到最合适的专家。设计了轻量级、结构感知的路由器模型。

Result: 在ResNet、LLaMA和Qwen模型家族及多个基准数据集上的实验表明，MoQE性能与SOTA量化模型相当，且未显著增加推理延迟。

Conclusion: MoQE通过动态路由和专家模型结合，有效缓解了量化模型的性能下降问题，同时保持了高效的推理速度。
...
TL;DR: 提出了一种动态连接掩码（DCM）机制，用于增强MLPs和KANs对噪声标签的鲁棒性，通过自适应掩码不重要连接减少梯度误差，实验表明其优于现有方法。
...
Result: 在合成和真实数据集上实验，DCM方法优于现有技术，并首次验证KANs在噪声标签分类中的优越性。
...
<details>
  <summary>Details</summary>
Motivation: 测试时扩展方法虽能提升模型性能，但计算时间大幅增加，限制了实际应用。作者希望保留其优势，同时减少推理开销。

Method: 提出噪声超网络，通过学习奖励倾斜的噪声分布，替代传统的测试时噪声优化，从而降低计算成本。

Result: 该方法在显著降低计算成本的同时，恢复了大部分显式测试时优化的质量提升。

Conclusion: 噪声超网络是一种高效的方法，能够在保持模型性能的同时大幅减少计算开销。
...
### [150] [DAgger Diffusion Navigation: DAgger Boosted Diffusion Policy for Vision-Language Navigation](https://arxiv.org/abs/2508.09444)
*Haoxiang Shi,Xiang Deng,Zaijing Li,Gongwei Chen,Yaowei Wang,Liqiang Nie*

Main category: cs.RO

TL;DR: 提出了一种名为DifNav的端到端优化VLN-CE策略，通过统一传统两阶段框架（路径点生成与规划）为单一扩散策略，解决了现有方法的全局次优化和性能瓶颈问题。
...
Method: 提出DifNav，一种基于条件扩散策略的端到端优化方法，直接建模连续导航空间中的多模态动作分布，并采用DAgger进行在线训练和专家轨迹增强。
...
### [151] [Toward Human-Robot Teaming: Learning Handover Behaviors from 3D Scenes](https://arxiv.org/abs/2508.09855)
*Yuekun Wu,Yik Lung Pang,Andrea Cavallaro,Changjae Oh*

Main category: cs.RO

TL;DR: 提出了一种仅通过RGB图像训练人机协作策略的方法，无需真实机器人数据，专注于人机交接任务。
...
Method: 利用稀疏视图高斯泼溅重建人机交接场景，生成图像-动作对，直接模拟机器人夹爪姿态变化。
...
### [152] [Describe What You See with Multimodal Large Language Models to Enhance Video Recommendations](https://arxiv.org/abs/2508.09789)
*Marco De Nadai,Andreas Damianou,Mounia Lalmas*

Main category: cs.IR

TL;DR: 论文提出了一种利用多模态大语言模型（MLLM）生成视频的丰富自然语言描述，以提升推荐系统性能的零微调框架。


<details>
  <summary>Details</summary>
Motivation: 现有视频推荐系统依赖低层次特征或用户定义的元数据，无法捕捉更深层次的语义（如意图、幽默等），影响个性化推荐效果。

Method: 通过MLLM生成视频的自然语言描述，结合文本编码器，将结果输入多种推荐模型（协同过滤、基于内容、生成式）。

Result: 在MicroLens-100K数据集上，该方法显著优于传统视频、音频和元数据特征。

Conclusion: 利用MLLM作为动态知识提取器，可以构建更具意图感知的视频推荐系统。
